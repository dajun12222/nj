 ...
 * @LastEditTime: 2022-04-13 15:54:33
 * @FilePath: \yunshu6.0\packages\portal\src\views\workflow-center\my-workflow.vue
 * @Description:
-->
<template>
  <my-workflow :storeId="storeId" @openForm="openForm" />
</template>
...
export default class Workflow extends Vue {
  ...
  openForm(obj: any) {
    const url = `/form/detail?workitemId=${obj.id}&workflowInstanceId=${
      obj.instanceId
    }&return=${location.pathname + location.search}&workitemType=myWorkflow`;
    if (this.isDingTalk) {
      this.$router
        .push({
          path: url,
