...
export default class WechatLogin extends Vue {
  ...
  initWechatLoginQrcode() {
    function wechatLogin(c) {
      const d = document.createElement('iframe') as any;
      let e =
      ...
      d.width = '210px';
      d.height = '300px';
      const f = document.getElementById(c.id) as any;
      f.innerHTML = '';
      f.appendChild(d);
      d.onload = function () {
        if (d.contentWindow.postMessage && window.addEventListener) {
          window.addEventListener('message', function (b) {
            if (
              (document as any).data &&
              (document as any).origin.indexOf('work.weixin.qq.com') > -1
            ) {
              window.location.href = b.data;
            }
          });
          d.contentWindow.postMessage('ask_usePostMessage', origin);
        }
      };
    }
  ...
  insertQrcode() {
    ...
    const styleSheet =
      '';
    this.$nextTick(() => {
  ...
  generateUrls() {
    ...
    // 组装地址
    this.redirectUrl = `${oauthHost}/login/wx/scan_login?redirect_uri=${encodeURIComponent(
      `${oauthHost}/oauth/authorize?client_id=${client_id}&response_type=code&scope=${scope}&redirect_uri=${redirectHost}/oauth`,
    )}&login_fail_redirect_uri=${encodeURIComponent(failUrl)}`;
  }
