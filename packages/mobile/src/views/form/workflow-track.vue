...
<template>
  <div class="workflow-track" :class="{ external: isExternal }">
    <h3-tab v-model="index" :lineWidth="2" :customBarWidth="'24px'">
    ...
    <div v-if="devOpsVisible" class="devOps form-actions" @click="toDevOps">
      流程运维
    </div>
  </div>
</template>
...
import { H3Swiper, H3SwiperItem, H3Tab, H3TabItem } from 'h3-mobile-vue';
...
export default class WorkflowTrack extends Vue {
  ...
  async getLoadData() {
    ...
    if (res.data.forPermission && res.data.forPermission.actionPermission) {
      Object.keys(this.mobileActions).forEach((key) => {
        this.mobileActions[key].visible =
          res.data.forPermission.actionPermission[key];
      });
    }
  }
  ...
  loadAsync() {
    const workflowInstanceId = (this.workflowInstanceId =
    ...
    workflowApi
      ...
      .then((resp) => {
        ...
        if (
          resp.data &&
          resp.data.headerAction &&
          typeof resp.data.headerAction === 'object'
        ) {
