...
<template>
  ...
  <div class="devOps-wrapper">
    <div v-if="currentOptions.length" class="choice-node">
      ...
      <div v-if="activityCode" class="choice-node-inner">
        <label>当前处理人</label>
        <div>
          <p>
            {{ getShowText(participants.map((el) => el.name).join('、'))
              ...
              v-show="
                !showAll &&
                participants.map((el) => el.name).join('、').length > 40
              "
              class="font-ele"
            >
              ...
            </span>
            <span v-show="showAll" @click="showAll = false">收起</span>
          </p>
          <span
            v-show="
              !showAll &&
    ...
    >
      <h3-list-item :hasExtra="true">
        批量处理
        <h3-switch slot="extra" v-model="isMultiplexing" color="#2970ff" />
      </h3-list-item>
      ...
      <h3-list-item
        :hasExtra="true"
        class="to-batch"
        @click="toNoParticipantsList"
      >
        <div @click="toNoParticipantsList">
          批量处理流程 【{{ $route.params.workflowName }}】无参与者流程{{
            total
          }}条
        </div>
...
export default class devOps extends Vue {
  ...
  setOptions() {
    this.options = this.options.filter((el) =>
      this.workflowAdminManageScopes.includes(el.value),
    );
  }
