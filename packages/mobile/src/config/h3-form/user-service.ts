...
若有问题，请参考前端相关二开文档。
*/
import { renderer } from 'cloudpivot';
import { UserService } from 'cloudpivot/form/renderer';

import { userApi, organizationApi } from 'cloudpivot/api';

export class DefaultUserService implements UserService {
  async getUserWorkflowFormDepartments(
    userId: string,
    workflowCode: string,
    isMobile: boolean,
  ) {
    const res = await userApi.getUserWorkflowFormDepartments(
      userId,
      workflowCode,
      isMobile,
    );
  ...
  async getDepartmentsBy(
    ...
    const departments = data.departmentList
