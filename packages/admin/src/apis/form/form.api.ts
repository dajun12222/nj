...
export function errorHandle(res: any) {
  if (res.hasOwnProperty('errcode') && ![302024, 0].includes(res.errcode)) {
...
export function get(schemaCode: string, sheetCode: string, copyCode?: string,isNew?:boolean) {
  return axios
    .get(`${path}/get`, {
      params: {
        schemaCode,
