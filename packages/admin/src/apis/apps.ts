...
export default {
  ...
  getIndexableProperties(params: object): Promise<Common.Response> {
    // 查询可以建立索引的数据项类型列表
    return Axios.get('/api/app/bizproperty/index_config', { params });
  },
  ...
  getBizmodeltitle(
    ...
    return Axios.get('/api/app/folders/getByCode', { params });
  },
  getSummary(
    params: Apps.Datamodel.DataItemListParams,
  ): Promise<Common.Response> {
   ...
   */
  check_package_new(
    params: Apps.AppItem.CheckModelParams,
  ): Promise<Common.Data> {
  ...
  RecycleDelete(params: any): Promise<Common.Data> {
    return Axios.delete('/api/app/apppackage/physically_delete', {
      data: params,
    });
  },
  ...
  getAppgroupV1(params: any): Promise<Common.Data> {
   ...
   * 获取SQL关键字
   */
  getReservedWords(params?: any): Promise<Common.Data> {
    return Axios.get('/api/app/bizproperty/reservedWords', {
      params,
    });
  },
/*
  获取子管理员授权角色范围
    /api/public/ceec/role/getAdminRoleRangeById
  */
 getAdminRoleRangByid(params:any): Promise<Common.Res<void>>{
  return Axios.get("/api/public/ceec/role/getAdminRoleRangeById", { params });
 },
 /*
 保存打印模板
 */
 printSave(params:any): Promise<Common.Res<void>>{
  return Axios.post("/api/ceec/print/save", params );
 },
 /*
 获取打印模板
 */
 printGet(params:any): Promise<Common.Res<void>>{
  return Axios.get("/api/ceec/print/get", { params } );
 }
};
