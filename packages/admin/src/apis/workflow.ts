...
export default {
  ...
  createWorkflow(
    params: Apps.Workflow.CreateWorkflowParams,
  ): Promise<Common.Data> {
  ...
  validateFormula(
    params: Apps.Workflow.ValidateFormulaParams,
  ): Promise<Common.Data> {
  ...
  //获取某个版本且为发布状态的流程模板
  getHistoryWorkflow(
    params: Apps.WorkflowSetting.GetHistoryWorkflowParams,
  ): Promise<Common.Data> {
    return Axios.get('/api/workflow/get_published', { params });
  },
  //获取某个版本且为发布状态的流程模板
  getWorkflowItem(params: any): Promise<Common.Data> {
    return Axios.get('/api/workflow/getWorkflow', { params });
  },
  ...
  getMockTaskList(params: any) {
    return Axios.get(`/api/runtime/workflow/simulative/search_task`, {
      params,
    });
  },
  /*提交待办任务*/
  ...
  getRejectActivity(params: any) {
    return Axios.get(`/api/runtime/workflow/list_reject_activities`, {
      params,
    });
  },
/*根据流程实例id获取驳回页面的流程节点列表*/
  getFlowAccount(params: any){
    return Axios.get(`/api/ceec/flow/getFlowAccount`, {params});
  },
  /*
  /api/public/ceec/node/getNodeInfo
  /api/public/ceec/node/getNodeInfo?functionCode=  (取remark=
1
字段判断是否为账套）
  */
  getNodeInfo(params: any){
    return Axios.get(`/api/public/ceec/node/getNodeInfo`, {params});
  }
};
