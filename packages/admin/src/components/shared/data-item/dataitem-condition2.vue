    ...
    <div class="format-content">
      <a-row v-for="(cond, index) in conditions" :key="index" type="flex">
        ...
        <a-col :span="1">
          ...
          <a-icon type="delete" @click="remove(index, cond)" />
        </a-col>
      </a-row>

      <div v-if="setConList" class="actions">
...
export default class DataitemCondition extends Vue {
  ...
  get setConList() {
    if (this.conditions.length > 0) {
      const conditions: any = this.conditions
        .map((item: any) => item.propertyCode)
  ...
  onItemSelect(cond: DataitemConditionItem) {
    ...
    this.dataOptions.forEach((s: any) => {
      const select = this.conditions.find(
        (inner: any) => inner.propertyCode === s.value,
      );
      if (select) {
      } else {
        s.disabled = false;
