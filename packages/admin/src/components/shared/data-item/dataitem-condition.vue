...
<template>
  <div>
    <div class="row">
      满足以下
      <a-select v-model="type" class="ml8 mr8" @change="onChange">
        <a-select-option :value="1"> 全部 </a-select-option>
        <a-select-option :value="2"> 任一 </a-select-option> </a-select>条件
    </div>
    ...
    <a-row v-for="(cond, index) in conditions" :key="index" type="flex">
      <a-col :span="8">
        ...
        >
          <a-select-opt-group>
            ...
            <template v-for="(i, idx) in dataOptions">
      ...
      <a-col v-show="!operatorAboutNull(cond.operatorType)" :span="8">
        ...
        <a-select
          ...
          @change="onChange"
        >
          <a-select-option v-if="showSelf_P(cond)" value="SELF_P">
            本人
          </a-select-option>
          <a-select-option v-if="showSelf_D(cond)" value="SELF_D">
            本部门
          </a-select-option>
        </a-select>
        <a-input
...
export default class DataitemCondition extends Vue {
    ...
    switch (type) {
      ...
      case DataItemType.DropdownMulti:
        if (code === 'sequenceStatus') {
          ...
          const result = textDataItemOperators.filter(
  ...
  isStaffSelector(type: DataItemType) {
    return (
      type === DataItemType.StaffSingle || type === DataItemType.DeptSingle
    );
  }
