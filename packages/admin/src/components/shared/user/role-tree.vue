...
export default class RoleTree extends Vue {
  ...
  getTreeDataBySearch(tree: any, parentObject?: any) {
    ...
    const stringTreeData: any =
      Array.isArray(tree) && JSON.parse(JSON.stringify(tree));
    ...
    for (let i = 0, len = stringTreeData.length; i < len; i++) {
      ...
      if (parentObject && !parentObject.disabled) {
        object.disabled = false;
      }
  ...
  async getRoleList() {
    ...
    if (Array.isArray(res.data) && res.data.length) {
      this.tree = res.data
        .filter((x: any) => x.children && x.children.length)
...
.role-tree {
  ...
  // 样式hack太严重 故在此重新hack
  /deep/.ant-select-tree-child-tree
