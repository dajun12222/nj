...
<template>
  ...
  >
    <template #footer>
      <a-button key="back" :disabled="loading" @click="cancel">
        {{ $t('languages.Apps.Cancel') }}
      </a-button>
      <a-button
        key="submit"
        :disabled="showErrorTips"
        type="primary"
        :loading="loading"
        @click="submit"
      >
    ...
    >
      ...
      >
        <template slot="label">
          ...
          <a-tooltip placement="top">
            <template slot="title">
              <span>请选择同个组织下的部门或用户</span>
            </template>
            <span class="icon aufontAll tips">&#xe9ab;</span>
          </a-tooltip>
        </template>
        <staff-selector
          :class="showErrorTips ? 'has-error' : ''"
          :options="userOpts"
          :hasAccount="''"
          :flowAccount="roleData.isEditFlow ? 'user' : ''"
          :roleCode="roleData.code"
          :disabled="userType === 1 || userType === 2"
          :onlyForm="onlyForm"
          :adminUser="isShowUser ? 'adminUser' : ''"
          :visiblePart="true"
          :showDetail="true"
          @ok="setUserId"
        />
        <template slot="extra">
          <span v-if="showErrorTips" class="error-tips">请选择同个组织下的部门或用户</span>
        </template>
      </a-form-item>
      ...
      >
        <staff-selector
          ...
          :flowAccount="roleData.isEditFlow ? 'scope' : ''"
          :roleCode="roleData.code"
          :disabled="showErrorTips"
          :options="ouScopeOpts"
...
import { Form, Input, Modal, TreeSelect, Button, Tooltip } from '@h3/antd-vue';
...
export default class AddUser extends Vue {
  ...
  onChangeUserData(newVal:any){
  
  }

  @Watch('roleData',{ deep: true, immediate: true })
  onChangeRoleData(newVal:any){
    
  }

  get isShowUser(){
    return this.roleData.isEditFlow && this.roleData.remarks === "ACCOUNT";
  }
  ...
  get hasAccount(){
    return this.roleData.remarks === "ACCOUNT" ? true : false;
  }
  
  setUserId(data:any){
    const organization = data.map((item: any) => {
      const departmentNameList = item.parentDepartmentName.split('/');
      const length = departmentNameList.length;
      return departmentNameList[length - 1];
    });
    console.log(organization);
    for (let i = 0; i < organization.length; i++) {
      if (organization[i] !== organization[0]) {
  ...
  submit() {
    this.form.validateFields((err: any) => {
      if (!err) {
        ...
        // 角色新增是否关联账套，若关联账套，则管理范围必填
        if(this.roleData.remarks === "ACCOUNT"){
          if(ouScope.length < 1){
            this.$message.error('请选择角色管理范围');
            return;
          }
        ...
        if (this.userType === 1 || this.userType === 2) {
          ...
          this.loading = true;
          OrgApi.updateUserOuscope(params)
            .then((res: any) => {
              this.loading = false;
              if (res.errcode) {
    ...
    if (this.userType === 1) {
      ...
      this.$nextTick(() => {
        if (this.form.setFieldsValue) {
          const userId =
            userData.unitType === 1 ? userData.deptId : userData.id;
