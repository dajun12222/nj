  ...
  <div class="todo-task">
    <div>
      <div class="search-head">
        ...
        <div class="row">
          <div>
            <span class="title">{{
              $t('languages.TaskTransfer.WorkerFlow')
            }}</span>
            <!-- <workflow-selector
            ...
            /> -->
            <div style="width: 314px; margin-right: 30px">
              <flowTree
                v-model="workflowCode"
                :isClear="true"
          ...
          <div>
            <span class="title">{{
              $t('languages.TaskTransfer.Originator')
            }}</span>
            <staff-selector
              v-model="originator"
              :options="taffSelectorOpts"
      ...
      <div>
        ...
        >
          ...
          <span slot="instanceNameTitle" class="resize">{{
            $t('languages.ProcessName')
          }}</span>
          <span slot="activityNameTitle" class="resize">{{
            $t('languages.TaskNode')
          }}</span>
          <span slot="workflowNameTitle" class="resize">{{
            $t('languages.TaskTransfer.WorkerFlow')
          }}</span>
          <span slot="originatorNameTitle" class="resize">{{
            $t('languages.TaskTransfer.Originator')
          }}</span>
          <span slot="timeTitle" class="resize">{{
            $t('languages.ReceiveTime')
          }}</span>
          ...
          <template slot="activityName" slot-scope="text">
            <a-tooltip v-if="text" placement="top">
              <template slot="title">
                <span>{{ text }}</span>
              </template>
      ...
      <div v-if="totalElements > 0" class="todo-task__page clearfix">
        <div>
          <a-pagination
            size="small"
    ...
    <div class="sub-btn">
      <div>
        ...
        >
          {{ $t('languages.TaskTransfer.TransferSelected') }}
        </a-button>
        <a-button
          class="btn ant-btn-large"
        ...
        >
          {{ $t('languages.TaskTransfer.TransferAll', { num: totalElements }) }}
        </a-button>
      </div>
    </div>
    ...
    >
      <span slot="title">
        {{ modalTitle }}
        <small
          v-if="modalTitle !== '交接全部数据'"
          style="margin-left: 12px; color: rgba(17, 18, 24, 0.5)"
          >已选{{ hasSelected }}条任务</small>
      </span>
...
export default class TodoTask extends Vue {
  ...
  getTasks() {
    ...
    params.page -= 1;
    params.workflowCode = vm.workflowCode
      ? vm.workflowCode.replace('workflow-', '')
      : '';
    params.workFlowName = vm.workFlowName;
    ...
    OrgApi.taskTransfer(params).then((res: any) => {
      if (!res.errcode) {
        if (res.data && res.data.fails) {
          vm.$message.warning(
            `交接完成！有${res.data.fails}条待办交接失败，可能的原因有：1. 流程异常了  2. 当前接收人已经是该流程的处理人，不允许交接给他`,
            3,
          );
        } else {
...
.todo-task {
  ...
  /deep/.ant-table-body {
    max-height: calc(100vh - 387px - 57px) !important;
