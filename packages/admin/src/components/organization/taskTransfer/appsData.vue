  ...
  <div class="apps-data">
    <div>
      <div class="search-head">
        <div class="search-tips">
          说明：批量更改数据的拥有者，交接后数据拥有者对数据有查看、编辑权限
        </div>
        <div class="row first">
          <div>
            <label>
              ...
              <span class="title right">{{
                $t('languages.TaskTransfer.Delegatee')
              }}</span>
            </label>
            <staff-selector
              v-model="targetUser"
          ...
          <div class="transfer-org">
            <label>
              <span class="red">*</span>
              <span class="title right">{{
                $t('languages.TaskTransfer.DelegateeOrg')
              }}</span>
            </label>
            <a-select
        ...
        <div class="row">
          <div>
            <span class="red">*</span>
            <span class="title">{{
              $t('languages.TaskTransfer.BizModel')
            }}</span>
            <biz-models-selector
              v-model="schemaCode"
              class="biz-models"
          ...
          <div>
            <span class="title">{{
              $t('languages.TaskTransfer.SequenceStatus')
            }}</span>
            <a-select
      ...
      <div :class="{ 'no-bizmodel': noBizModel }">
        ...
        >
          ...
          >
            ...
            />
            <span v-show="!showSelectAllBox && !isSelectAll" class="text">{{
              $t('languages.NO')
            }}</span>
          </span>
          <span slot="nameTitle" class="resize">{{
            $t('languages.TaskTransfer.Name')
          }}</span>
          <span slot="ownerDeptTitle" class="resize">{{
            $t('languages.TaskTransfer.OwnerDept')
          }}</span>
          <span slot="createdTimeTitle" class="resize">{{
            $t('languages.TaskTransfer.CreatedTime')
          }}</span>
          <span slot="moduleNameTitle" class="resize">{{
            $t('languages.TaskTransfer.SchemaCode')
          }}</span>
          <span slot="sequenceStatusTitle" class="resize">{{
            $t('languages.TaskTransfer.SequenceStatus')
          }}</span>
          ...
          <template slot="ownerDept" slot-scope="text">
            <a-tooltip v-if="text" placement="top">
              <template slot="title">
                <span>{{ text }}</span>
              </template>
          ...
          <template slot="moduleName" slot-scope="text">
            <a-tooltip v-if="text" placement="top">
              <template slot="title">
                <span>{{ text }}</span>
              </template>
      ...
      <div v-if="totalElements > 0" class="apps-data__page clearfix">
        <div>
          <a-pagination
            size="small"
      ...
      <div v-if="selectAllModel" class="biz-model_tip">
        <span>交接全部数据量较大，系统将后台执行交接动作</span>
      </div>
      <div v-else-if="noBizModel && !myTask.length" class="biz-model_tip">
        <span>请先选择业务模型</span>
      </div>
      <!-- <div class="load-more">
    ...
    <div class="sub-btn">
      <div>
        ...
        <template v-else>
          <a-button
            class="btn ant-btn-large"
            :disabled="transferSelected"
            type="primary"
            :ghost="true"
            @click="toTransferTask('section')"
          >
            {{ $t('languages.TaskTransfer.TransferSelected2') }}
          </a-button>
          <a-button
            class="btn ant-btn-large"
          ...
          >
            {{
              $t('languages.TaskTransfer.TransferAll2', { num: totalElements })
            }}
          </a-button>
        </template>
      </div>
    ...
    >
      ...
      <p class="tips">
        <span class="icon aufontAll h-icon-all-Components_Warning"></span>交接任务后不能撤回，请谨慎操作
      </p>
...
<script lang="ts">
import OrgApi, * as OrganizationApi from '@/apis/organization';
...
export default class AppsData extends Vue {
  ...
  showTransferTips(type: string) {
    ...
    if (type === 'every') {
      ...
      confirmTitle = vm.$t('languages.TaskTransfer.TranferTips', {
        num: vm.totalElements,
      });
      // 交接完成后不可撤回，请谨慎操作
  ...
  onWorkflowChange(value: any) {
    ...
    if (value.key === 'authineAllModel') {
      this.selectAllModel = true;
      this.myTask = [];
      this.totalElements = 0;
      this.schemaCode = value.key;
      this.schemaName = value.title;
      this.noBizModel =
        this.schemaCode === '' || this.schemaCode === 'authineAllModel';
      return;
    }
