  ...
  <div class="apps-data" :style="heightStyle">
    <div>
      ...
      <div class="table-wrapper">
        <role-scope-item
          v-if="roleData.adminManagerFlag"
          v-model="adminManagerFlag"
          :syncUserFlag="true"
          :data="roleData"
          title="管理员"
        />
        <template v-if="roleData.syncUserFlag === false">
          <p
            class="tips"
            style="margin-left: 42px; margin-top: 24px; margin-bottom: 16px"
          >
        ...
        <role-scope-item
          v-if="roleData.deptManagerFlag"
          v-model="deptManagerFlag"
          :syncUserFlag="roleData.syncUserFlag"
          :data="roleData.deptList"
          title="部门主管"
        />
        <role-scope-item
          v-if="roleData.userRoleFlag"
          v-model="userRoleFlag"
          :syncUserFlag="roleData.syncUserFlag"
          :data="roleData.userRoleItemList"
          title="角色"
        />
      </div>
    </div>
      ...
      <div>
        <template>
          <a-button
            ...
            :disabled="
              targetUser.length === 0 ||
              (!adminManagerFlag && !deptManagerFlag && !userRoleFlag)
            "
...
import { InputNumber, Modal, Input, Button } from '@h3/antd-vue';
...
export default class Role extends Vue {
  ...
  handleOk() {
    ...
    if (this.modalTitle === '交接全部角色') {
      ...
      params.deptIdList =
        this.roleData.deptList && this.roleData.deptList.map((el) => el.id);
      params.userRoleItemList =
        this.roleData.userRoleItemList &&
        this.roleData.userRoleItemList.map((el) => {
          return {
            roleId: el.roleId,
      ...
      params.deptIdList =
        this.roleData.deptList &&
        this.roleData.deptList.filter((el) => el.checked).map((el) => el.id);
      params.userRoleItemList =
        this.roleData.userRoleItemList &&
        this.roleData.userRoleItemList
          .filter((el) => el.checked || el.indeterminate)
          .map((el) => {
            return {
              roleId: el.roleId,
              roleName: el.roleName,
