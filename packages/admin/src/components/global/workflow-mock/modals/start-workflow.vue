...
<template>
  ...
  >
    <div class="workflow-tip">
      <div class="icon aufontAll warning">&#xe99a;</div>
      <div class="status-txt">
        流程运行产生的模拟数据将在流程中心中展示，为保障业务信息不被干扰，请勿在生产环境使用此功能！
      </div>
    </div>
    <!-- 填写预设人 -->
    <template v-if="type !== 'presetData'">
      ...
      <a-row class="row-flex" type="flex">
        ...
        <a-col :span="20">
          <a-radio-group v-model="params.runMode" class="radio-wrap">
      ...
      <div class="header-tips">
        注：支持流程基础功能运行，暂不支持消息通知、事件、超时、表单校验、关联表单
      </div>
    </template>
    ...
    <template v-else>
      ...
      <div v-if="dataItem.length" class="scroll-wrap">
        <a-row
          ...
          type="flex"
        >
          <a-col class="row-items" :title="item.name" :span="5">
            {{ item.name }}
          </a-col>
          <a-col class="row-cols" :span="19">
            ...
            <!-- 输入框 -->
            <a-input v-else v-model="item.value" class="select-wrap" />
          </a-col>
        </a-row>
      </div>
    ...
    <template slot="footer">
      <div class="start-workflow__link" @click="goHistoryRunning">
        无需发起流程，点此可查看历史运行流程
      </div>
      ...
      <a-button v-if="type === 'presetData'" key="back" @click="back">
        上一步
      </a-button>
      <a-button
...
export default class StartWorkflow extends Vue {
      ...
      if (arr && arr.length && Array.isArray(arr)) {
        arr.forEach((a: any) => {
          ...
          if (
            ...
            cCode &&
            item.subSchema &&
            Array.isArray(item.subSchema.properties)
          ) {
            const cItem = item.subSchema.properties.find(
              (c: any) => c.code === cCode,
            );
            if (!cItem) {
              return;
  ...
  async getDepartmentList(id: string) {
    ...
    if (res.errcode === 0 && Array.isArray(res.data)) {
      if (!this.params.originatorDept) {
        this.params.originatorDept = res.data[0].deptId || '';
      }
  ...
  nextStep() {
    // 必填校验
    const originatorValid =
      !this.params.originator || !this.params.originator.length;
