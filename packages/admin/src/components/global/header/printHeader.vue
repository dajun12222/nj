...
<template>
  <div class="print-header flex-justify-between">
    ...
    <div v-if="$route.fullPath.includes('print-template')" class="header-right">
      <a-button type="default" icon="eye-o" @click="preView"> 预览 </a-button>
      <a-button type="primary" icon="save" @click="saveDrawerJson">
        保存
      </a-button>
    </div>
...
export default class PrintHeader extends Vue {
  ...
  async saveDrawerJson() {
    // 添加打印页面配置
    const tempData: object[] = this.pages[0] as [];
