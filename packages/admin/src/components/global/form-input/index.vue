  ...
  <div :class="['form-input', message && 'has-error']">
    ...
    <template v-else-if="textarea">
      <a-textarea
        ...
        @blur="onBlur"
      />
      <p v-if="message && !noMessage" class="form-input__message">
        {{ message }}
      </p>
    </template>
    <template v-else>
 ...
 */
import { DatePicker, Input, InputNumber, Select, Tooltip } from '@h3/antd-vue';
...
export default class FormInput extends Vue {
  ...
  validateValue(e: any) {
    ...
    if (!this.message) {
      this.setValue(val);
    } else if (this.tipPosition === 'top' && this.message) {
      if (!valid) {
        return;
      }
