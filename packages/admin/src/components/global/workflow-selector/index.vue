...
export default class workflowSelector extends Vue {
  ...
  onLoadData(treeNode: any) {
    ...
    return new Promise((resolve: any) => {
      ...
      if (type === 'app') {
        ...
        let temp = treeNode.dataRef;
        while (temp.dataRef) {
          temp = temp.dataRef;
        }
        const ids: string[] = temp.root.split('-');
  ...
  setLeafChild(
    values: string[],
    tree: Array<treeSelector.treeItem>,
    res: Array<treeSelector.treeItem>,
  ) {
  ...
  getAppList() {
    // 工作交接
    const getAppList: any =
      this.usePage === 'handover' ? appApi.getAllApps : appApi.getAppList;
  ...
  getBizModels(appCode: string) {
    return appApi
      ...
      .then((res: Common.Data) => {
        ...
        function formatTree(list, baseCode) {
          if (list && list.length > 0) {
            return list.map((childnode) => {
              return {
                ...
                selectable: false,
                children:
                  childnode.children && childnode.type === 'Folder'
                    ? formatTree(
                        childnode.children,
                        `${baseCode}-${childnode.code}`,
                      )
                    : [],
                dataRef: {
                  type: childnode.type,
                  code: childnode.code,
                  root: `${baseCode}-${childnode.code}`,
                },
              };
            });
          }
        }
        const child = formatTree(res.data, appCode);
  ...
  getWorkflows(schemaCode: string) {
    return workflowApi
      .getWorkflowList({ schemaCode, isPortal: true })
      .then((res: Common.Data) => {
        const flows: any[] = this.onlyPublished
          ? res.data.filter((item: any) => item.published)
          : res.data;
        const child: Array<treeSelector.treeItem> = flows.map(
          (workflow: any) => ({
