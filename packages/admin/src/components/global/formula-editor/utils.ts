...
export default {
  ...
  formulaToContent(formulaContent: string, variableMap: VariableMap): string {
    ...
    let content: string = formulaContent.replace(
      /(\{.+?\})/g,
      (code: string) => {
    ...
    if (/\[\{.*?\}\]/.test(content)) {
      ...
      content = content.replace(/\[\{.*?\}\]/g, (code: string) => {
        try {
          const matchArr = JSON.parse(code);
  ...
  contentToFormula(editorContent: string): string {
    ...
    let formulaCtx: string = editorContent.replace(
      /<span\stitle="(.+?)"\scontenteditable="false">.+?<\/span>/g,
      '$1',
    );
  ...
  cursorToEnd(editor: any) {
    ...
    if (selection) {
      ...
      selection.collapse(editor, selection.focusOffset - 1);
    }
    return selection;
  },
  ...
  recognizeExpression(selection: Selection, variableMap: VariableMap) {
    ...
    const { nodeType, data, wholeText, previousSibling, nextSibling } =
      anchorNode as any;

    if (nodeType === Node.TEXT_NODE) {
      const nodeContent: string = wholeText || '';
      const { code, index } = this.pickExpression(
        variableMap,
        nodeContent,
        true,
      ) as any;
  ...
  joinConnection(editorDom: Element, content: string, selection: Selection) {
    ...
    // 当选中的是非文本节点，即元素节点
      ...
      nextNode =
        currentOffset < childNodes.length ? childNodes[currentOffset] : null;
