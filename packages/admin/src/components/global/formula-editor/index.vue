...
export default class FormulaEditor extends Vue {
  ...
  recordRange() {
    ...
    if (selection && selection.rangeCount) {
      ...
      const isInEditor: boolean = editorUtils.isChildOf(
  ...
  insertContent(content: string, atTail?: boolean, name?: string) {
    ...
    if (atTail) {
      ...
      if (this.editorDom.innerHTML !== '<br>') {
        html = '+' + html;
      }
  ...
  setEditorHtml() {
    this.editorDom.innerHTML =
      editorUtils.formulaToContent(this.formulaContent, this.variableMap) +
      '<br>';
    this.editorContent = this.editorDom.innerHTML;
    setTimeout(() => {
  ...
  contentToFormula() {
    ...
    const formulaResult: string = editorUtils.contentToFormula(
      this.editorContent,
    );
