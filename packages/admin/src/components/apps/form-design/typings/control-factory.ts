...
export function buildControlOf(
  ...
  if ((item.type === 2 || item.type === 3) && (item as any).options) {
    try {
      options.format = JSON.parse((item as any).options).format;
      options.format1 = JSON.parse((item as any).options).format;
    } catch {}
  }
...
export function autoLayout(
  ...
  for (const item of items) {
    ...
    } else if (item.code === 'parentId') {
      map['parentId'] = buildControlOf(item);
    } else {
      //Else Empty block statement
    }
  }
