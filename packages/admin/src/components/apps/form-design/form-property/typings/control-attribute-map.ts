...
export const formAttributeMap: { [key: string]: ControlAttributeOptions } = {
  ...
  borderMode: {
    value: '',
    type: ControlAttributeType.Checkbox,
    label: '边框模式',
    options: {
      valueMap: {
        true: 'open',
  ...
  trackDataCodes: {
    value: '',
    type: ControlAttributeType.Modal,
    attrType: ModalAttributeType.FormMarking,
    options: {
      // exportModal,
      importModal: (attr: any, attrs: any) => {
        const sheetData: any = attrs.find((res: any) => {
      ...
      exportModal: (
        ...
        // 获取到modal提交的的data
        console.log(data);
        vm.$emit('change', 'trackDataCodes', data.trackDataCodes);
        return data; // 返回的值可以在formatter里拿到
      },
      formatter: (s: any, c: any) => {
        //

        if (s) {
          const count: number = s.split(';').length ? s.split(';').length : 0;
          return `留痕数据项${count > 0 ? count : ''}项`;
        } else {
          return '未设置';
        }
      },
...
const controlAttributeMap: {
  ...
  '1': {
    ...
    readonlyCondition: {
      value: '',
      type: ControlAttributeType.Modal,
      attrType: ModalAttributeType.ReadonlyCondition,
      label: '只读条件',
      options: {
        formatter: formatterValueToSetStatus,
      },
    ...
    dataLinkage: {
      ...
      label: '数据联动',
      type: ControlAttributeType.Modal,
      attrType: ModalAttributeType.dataLinkage,
      options: {
        formatter: formatterValueToSetStatus,
      },
    },
  ...
  '2': {
    ...
    textAreaType: {
      value: '',
      type: ControlAttributeType.RadioGroup,
      options: {
        list: [
          {
            value: 'longText',
  ...
  '100': {
    ...
    seqNoSetting: {
      ...
      label: '单据号配置',
      type: ControlAttributeType.Modal,
      attrType: ModalAttributeType.SequenceNoModal,
      options: {
        formatter: formatterValueToSetStatus,
      },
    },
