...
const modalTypeFilter = (type: string) => {
  ...
  switch (type) {
    ...
    case ModalAttributeType.dropdownMultiOption:
    case ModalAttributeType.dropdownOption:
      field = 'defaultValue';
      break;
...
export const importModal = (attr: any, attrs: any, extraAttrs: any = null) => {
  console.log(attr);
  console.log(attrs);
  ...
  if (!!field) {
    ...
    if (!defaultField && Array.isArray(extraAttrs)) {
      defaultField = extraAttrs.find((item: any) => {
        return item.field === field;
      });
    }
  }
  ...
  let val;
  if (attr[0]) {
    if (attr[0].dataItem.type === 12 || attr[0].dataItem.type === 13) {
      const data = JSON.parse(attr[0].value);
      if (data.orderType === 1 && data.labels.length > 0) {
        val = data.labels
          .map((item) => item.data[data.sheetDataItem])
          .join(';');
      }
    }
  }
