  ...
  >
    <div class="calculate-rule">
      ...
      <div class="btn-group">
        <ul class="clearfix">
          <li
            v-for="(symbol, i) in symbolList"
            :key="i"
            @click="symbolBtnClick(symbol)"
          >
      ...
      <div class="btn-group">
        <ul class="clearfix sum">
          <li
            v-for="(rule, i) in ruleList"
            :key="i"
            @click="ruleBtnClick(rule)"
          >
...
import { Input, Modal, Button, Select } from '@h3/antd-vue';
...
export default class CalculateRule extends Vue {
  ...
  created() {
     ...
     * 赋予默认值
     */
    const s = this.testRuleString(
      '(sum({Sheet1576827530223.Number1576827532565})+avg({Sheet1576827530223.Number1576827542393})-max({Sheet1576827530223.Number1576827550259}))*{Number1576827505238}/{Number1576827508702}+count({Sheet1576827530223.Number1576827532565})-6+min(3，2，5)',
    );
    console.log('测试:', s);
  }
  ...
  parseStringToNode(str) {
    ...
    for (const s of str.split('')) {
      if (s === '{') {
        ...
        if (tempType === 'img') {
          const d = this.dataItems.filter((item) => item.code === tempStr);
          if (d.length) {
            const dataItem = d[0];
  ...
  dataItemChange(val: any) {
    const item = this.dataItems.filter((v: any) => v.code === val)[0];
  ...
  pushDataItem() {
    if (this.selectDataItem) {
      const imgID = Math.ceil(Math.random() * 1000) + this.selectDataItem.code;
      const imgSrc = textBeComeImg(this.selectDataItem.name);
      if (!!this.savedRange.commonAncestorContainer) {
        ...
        ruleHTMLNode.focus();
        setTimeout(() => {
          const imgNode = `<img src="${imgSrc}" id="${imgID}" data-val='{${this.selectDataItem.code}}'>`;
          ruleHTMLNode.insertAdjacentHTML('beforeend', imgNode);
  ...
  ruleBtnClick(rule: string) {
    if (!!this.savedRange.commonAncestorContainer) {
      const textNode = document.createTextNode(`${rule.toLowerCase()}()`);
      const endOffset = this.savedRange.endOffset;
      this.savedRange.insertNode(textNode);
      setTimeout(() => {
        const afterEndOffset = this.savedRange.endOffset;
  ...
  backData() {
    const ruleString = this.formatterRuleHTML();
  ...
  formatterRuleHTML() {
    let str = '';
    for (const n of ruleHTMLNode.childNodes) {
      if (n.nodeName === 'IMG') {
        str += `${n.dataset.val}`;
  ...
  testRuleString(str: string) {
    ...
    for (let j = 0, len = arr.length; j < len; j++) {
      const strTwo: string = arr[j].replace(/^\{/, '').replace(/\}$/, '');
    ...
    // 校验输入的内容格式是否正确
    let sss = str.replace(/(sum)|(avg)|(max)|(min)|(count)/g, 'C'); // 将所有公式标记为 C
    ...
    for (let _i = 0, l = sss.length; _i < l; _i++) {
      ...
      if (c === '(') {
        ...
        if (stack_arr.length === 0) {
          ...
          const _len = stack_arr.length;
          const _pre = stack_arr[_len - 1]; // 这个时候 需要判断最后一位是不是符号
          if (
            ~['+', '-', '*', '/'].indexOf(_pre) &&
            stack_arr[_len - 2] === 'N'
          ) {
        ...
        const _len = stack_arr.length;
        const _pre = stack_arr[_len - 1];
        if (_pre === 'N' && stack_arr[_len - 2] === '(') {
          stack_arr.splice(_len - 2, 2, 'N');
        }
        while (stack_arr.length > 1) {
          const _l = stack_arr.length - 1;
          const _p = stack_arr[_l - 1];
