    ...
    <div v-show="!isQuantity" class="verify-item" style="margin-bottom: 20px">
      ...
      <div v-show="isDynamicDay" class="content">
        ...
        <div v-show="isRangeRule" class="content" style="align-items: center">
          ...
          />
          <div
            style="margin: 0 6px; color: rgba(17, 18, 24, 0.5); font-size: 14px"
          >
            ~
          </div>
          <a-input-number
    ...
    if (promptText.length === 0) {
      ...
      if (partterValue === 1) {
        ...
        if (this.isRangeRule) {
          ...
          const t = moment(this.date).format(this.dateFormat);
          v = `${this.currControlOptions.name as any}必须${
            VerifyRuleEnum[verifyRuleValue]
          }${t}`;
        }
        ...
        if (this.isDynamicDay) {
          //动态值 当天模式
          const v = `${this.currControlOptions.name as any}必须${
            VerifyRuleEnum[verifyRuleValue]
          }`;
          return { value: { ...rValue, defaultPrompt: v }, status: true };
        } else {
          ...
          if (this.isRangeRule) {
            // 介于 规则
            const [cname1] = this.dateControlOptions.filter(
              (val) => val.code === this.rangeSelectDataItem1,
            );
            const [cname2] = this.dateControlOptions.filter(
            ...
            // 非介于 规则
            const [cname] = this.dateControlOptions.filter(
