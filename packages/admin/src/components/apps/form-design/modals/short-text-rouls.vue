...
import { ConfigProvider, Modal, Select, Button } from '@h3/antd-vue';
...
export default class CalculateRule extends Vue {
    ...
    if (parentCode) {
      ...
      for (const key of allCtrlKeys) {
        ...
        if (this.types.includes(item.type)) {
          ...
          item.type === FormControlType.Sheet &&
          item.key === parentCode
        ) {
            ...
            for (const lt of loopItem.panels) {
              ...
              for (const pnCtrlKey of pnCtrlKeys) {
                ...
                if (this.types.includes(pnCtrl.type)) {
                  ...
                  pnCtrl.type === FormControlType.Sheet &&
                  pnCtrl.key === parentCode
                ) {
  ...
  get items() {
    ...
    const result = targetArr.filter((res: any) => {
      return (
  ...
  backData() {
    ...
    const msg = this.testExp(ruleString);
  ...
  testExp(exp: string) {
    ...
    if (key === 'substring') {
      ...
      if (testIsCode(segs[1])) {
        if (
          !this.dataItemIs(getDataItemKey(segs[1]), schema.DataItemType.Date)
        ) {
