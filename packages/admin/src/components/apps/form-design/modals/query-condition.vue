  ...
  >
    <div class="header" :class="{ header_title: type === 'onlyQuery' }">
      <div v-if="type === 'onlyQuery'">
        <template v-if="clientType === 'APP'"> mobile查询条件 </template>
        <template v-else> 查询条件 </template>
      </div>
      <a-radio-group v-else v-model="type" buttonStyle="solid">
