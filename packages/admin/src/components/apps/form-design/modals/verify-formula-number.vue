...
export default class VerifyFormulaNumber extends Vue {
  ...
  verifyNumInput(
    ...
    if (!this.isRangeRule) {
      ...
      if (('' + numInput1).length > 0 && ('' + numInput2).length === 0) {
        ...
        return {
          value: { ...rValue, lInput: numInput1, rInput: numInput2 },
          status,
        };
      } else {
        //Else Empty block statement
  ...
  verifyPrompt(
    ...
    if (promptText.length === 0) {
      ...
      if (partterValue === 1) {
        ...
        if (this.isRangeRule) {
          // 介于 规则
          const [cname1] = this.selectListDataItems.filter(
            (val) => val.code === this.rangeSelectDataItem1,
          );
          const [cname2] = this.selectListDataItems.filter(
          ...
          // 非介于 规则
          const [cname] = this.selectListDataItems.filter(
  ...
  backData() {
    ...
    if (this.partterValue === 1) {
      const { value, status } = this.verifyNumInput(
        this.numInput,
        this.numInput1,
        this.numInput2,
        rValue,
      ...
      const { value, status } = this.verifySelect(
        this.selectDataItem,
        ...
        while (loop.length) {
          ...
          for (const _panel of _tabs.panels) {
            for (const _ctrlKey of Object.keys(_panel.controls)) {
              ...
              if (
                _ctrl.type === FormControlType.Number &&
