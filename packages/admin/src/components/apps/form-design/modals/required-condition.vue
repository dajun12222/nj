...
<template>
  <a-modal
    ...
    @ok="backData"
  >
    <a-row v-if="isRequireConditon" class="required-type">
      <a-col :span="4">
        <span>控件必填</span>
      </a-col>
      <a-col :span="20">
...
import { Col, Modal, Radio, Row } from '@h3/antd-vue';
import DataitemCondition, {
  DataitemConditionValue,
} from 'cloudpivot/form/src/common/data-item/dataitem-condition2.vue';
import { Component, Prop, Vue, Watch } from 'vue-property-decorator';
import { DataItemState } from '../stores/data-items.store';
import * as dataitemStore from '../stores/data-items.store-helper';

/**
 * 未发现在何时使用到该组件，暂不修改2022.2.23
 */
...
export default class RequiredConditon extends Vue {
  ...
  backData() {
    const symbol = this.model.type === 1 ? '&&' : '||';
    ...
    let exp = this.model.conditions
      .map((cond) => {
        ...
        if (cond.propertyType === DataItemType.Number) {
          ...
          val = val === 'true' || val === 1;
          text = `${val}`;
        } else if (cond.propertyType === DataItemType.StaffSingle) {
