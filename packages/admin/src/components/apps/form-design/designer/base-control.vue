...
import { FormControl, FormControlType, FormHtmlControl } from '../typings';
...
export default class BaseControl extends Vue {
  ...
  render(h: Function, context: any) {
    ...
    if (
      control.type === FormControlType.Radio ||
      control.type === FormControlType.Checkbox
    ) {
      // const items = ['选项1','选项2','选项3'];
      opts.props.options = control.options.options
        ? control.options.options.split(';')
        : [];
    ...
    if ([5, 6, 7, 19].includes(control.type)) {
      ...
      try {
        const opt: any = JSON.parse(control.options.options);
        ...
        if (opt.labels.length > 0) {
          options = opt.labels
            .map((el) => el.data[opt.sheetDataItem])
            .join(';');
    ...
    //     " 点击或拖拽图片上传"
    //   ]);
    const createUpload = () => h('h-image', opts);

    const createImage = () => h('h-image', opts);
    ...
    const createLocation = () =>
      h('a-input', opts, [
        h('i', {
          ...
          slot: 'suffix',
          class: ['aufontAll', 'h-icon-all-position-o'],
        }),
      ]);
    const createAddress = () => {
      const child: any[] = [
      ...
      if (addressDetail === 'true') {
        ...
        _opts.props.placeholder = this.$t(
          'cloudpivot.form.renderer.pleaseInput',
        );
    ...
    const createStaffSelector = () =>
      h('a-input', opts, [
        h('i', {
          ...
          slot: 'suffix',
          class: ['aufontAll', 'h-icon-all-process-o'],
        }),
      ]);
    const designComponent = getDesignComponentByControlType(control.type);
    if (designComponent) {
      return h(formatterComponentType(control.key), {
        props: {
          control,
        },
      });
  ...
  created() {
    // 获取当前组件的 设计时的组件
    const designComponent = getDesignComponentByControlType(this.control.type);
    if (designComponent) {
