  ...
  <div style="display: inline-block">
    ...
    >
      ...
      >
        <template slot="content">
          <p @click="openEditModel(itemData)">
          ...
          <p @click.stop="popModel(itemData)">
      ...
      <a-tooltip v-if="showTootip(displayName)" placement="top">
    ...
    >
      ...
      <template v-if="itemData.type === 'BizModel'">
        <div v-if="!itemData.published" class="biz-model-unpublished">
          <a-tooltip placement="top">
            <template slot="title">
              <span>数据模型和列表设计未发布，请前往发布</span>
            </template>
            <img src="@/assets/images/unpublished.svg" alt="" />
          </a-tooltip>
        </div>
      </template>
    </router-link>
    ...
    <!-- 新建报表弹窗 -->
    <AddReport v-model="showAddReport" :model="model" />
  </div>
</template>
...
export default class BizModel extends Vue {
  ...
  get visiable() {
    console.log(
      !this.searchKey,
      this.itemData.name.indexOf(this.searchKey) !== -1,
    );
    return !this.searchKey || this.itemData.name.indexOf(this.searchKey) !== -1;
  }
  ...
  get to() {
    if (this.itemData) {
      ...
      if (type === 'BizModel') {
        ...
        return `/apps/${this.appCode}/report-design/${this.itemData.code}/isNew/false`;
      } else {
      ...
      if (record) {
        ...
        switch (type) {
          case ModelAction.ModalActionTypes.DataModel:
            data = [];
            isExist = true;
            href = `${hashPrefix}/apps/model/${this.appInfo.appCode}/${bizSchemaCode}/data/dataModel`;
            break;
        ...
        if (!isExist) {
          // 删除这条记录
          const newAction: Array<ModelAction.ModelActionItem> =
