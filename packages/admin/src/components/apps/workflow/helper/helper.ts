...
export const SWTreeGenerator = (
  ...
  dataObj.forEach((data: any, index: number) => {
    ...
    const _value =
      type === 'workflow' ? `workflow-${data.workflowCode}` : data.code; // 因为流程编码可能与应用编码或其他编码重复，故加前缀
...
export const releaseSubTableDataItem = (data: any) => {
  const subDataItemArr: Array<any> = [];
