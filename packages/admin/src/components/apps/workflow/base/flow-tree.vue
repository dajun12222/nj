...
<template>
  <div class="flow-tree">
    <a-tree-select
      ...
      @search="onSearch"
    />
    <div v-else slot="notFoundContent" class="select-empty-content">
      暂无数据
    </div>
  </div>
</template>
...
export default class FlowTree extends Vue {
  ...
  getPopupContainer(triggerNode: any) {
    while (
      triggerNode.parentNode.classList.contains('h3-sider-body') &&
      triggerNode.parentNode.tagName !== 'BODY'
    ) {
  ...
  private async initTree() {
    // 获取所有应用列表
    const appsResult: any = await appApi.getAllAppGroups();
    if (appsResult.errcode !== 0) {
    ...
    // 生成一级树
    const appsTree: Array<TreeNode> = this.treeDataGenerator(
      appsResult.data,
    ) as Array<TreeNode>;
  ...
  async markFlow(propValue: string) {
    ...
    const path: Array<string> = (await this.getFlowPath(
      propValue,
    )) as Array<string>;
    ...
    const groupItem: TreeNode | undefined = this.sourceData.find(
      (item: TreeNode, idx: number) => {
        if (item.id === gourpId) {
          appIndex = idx;
          return true;
        }
    ...
    const appList = await this.getAppListByGroupCode(
      this.splitGroupCode(groupItem.value),
    );
    groupItem.children = appList;
    const folderBizModelItem = await this.getFolderOrBizModelsByCode(appCode);
    const node = this.filterTableMater(schemaCode, folderBizModelItem, 'value');
    const treeData = await this.getWorkFlowTreeData(schemaCode);
  ...
  private async getFolderOrBizModelsByCode(appCode: string) {
    ...
    // 添加回源树中
    const node = this.filterTableMater(appCode, this.sourceData, 'value');
  ...
  private async getAppListNode(code: string, node) {
    if (!code) {
      throw new Error('分组code不能为空');
    }

    const tree: any = await this.getAppListByGroupCode(
      this.splitGroupCode(code),
    );

    // 添加回源树中
    const curAppTreeIndex: number = this.sourceData.findIndex(
      (d: TreeNode) => d.value === code,
    ) as number;

    if (curAppTreeIndex <= -1) {
      throw new Error('没有找到对应的应用');
    }

    const temTree: Array<TreeNode> = JSON.parse(
  ...
  async getWorkFlowTreeData(schemaCode: string) {
    ...
    this.labelName = this.labelName
      ? this.labelName
      : result.data[0] && result.data[0].name;
    const workflowList: Array<any> = result.data.filter(
  ...
  recursiveGetTreeNode(temTree, treeLevel: any) {
    const treePos = treeLevel[0]; // 默认取第一个
  ...
  private async onSelect(value: any, vNode: any) {
    ...
    if (res) {
      ...
      const schemaCode: any = path[pathLength - 1];
      this.$emit('select', { schemaCode, workFlowName: vNode.label });
    } else {
      console.error(res);
    }
  }
  ...
  translateWorkflowToTree(data: any) {
    ...
    // 将相同的appcode项合并
    const combineTree: Array<TreeNode> = [];
    tree.forEach((node: TreeNode) => {
      const existNode = combineTree.find(
        (nod: TreeNode) => nod.value === node.value,
      );
      if (existNode) {
  ...
  private treeDataGenerator(data: any) {
    ...
    data.forEach((item: any) => {
      ...
      const icon: string = item.icon
        ...
        : iconPrefix + 'h-icon-all-folder-o';
  ...
  /**
   * 获取分组编码中除末尾添加的_group其余的字符串
   */
  splitGroupCode(groupCodeMix) {
    if (typeof groupCodeMix === 'string' && groupCodeMix.endsWith('_group')) {
      return groupCodeMix.substring(0, groupCodeMix.length - 6);
    } else {
      return groupCodeMix;
    }
  }
