...
<template>
  <div class="div-height">
    <!-- <div class="input-modal" @click="modalShow">
      <span class="txt" :class="inputValue === '已设置' ? 'opacity' : ''">{{
        inputValue
      }}</span>
    </div> -->
    <template v-if="inputValue === '已设置'">
      <is-seted
        :value="inputValue"
        :showDel="false"
        :showEllipsis="true"
    ...
    >
      ...
      <a-collapse v-model="activeKey" class="filter-collapse" :bordered="false">
        <a-collapse-panel
        ...
        >
          ...
          <div class="centent">
            <a-row :gutter="[20, 20]">
              ...
              <a-col :span="20">
                <a-select
                  v-model="item.conditionJoinType"
                  showSearch
                  style="width: 100%"
                >
                  <a-select-option key="AND">
                    满足所有条件的数据
                  </a-select-option>
                  <a-select-option key="OR">
                    满足任一条件的数据
                  </a-select-option>
                </a-select>
              </a-col>
            ...
            <div class="filters">
              ...
              >
                ...
                <a-col class="filterw100" :span="8">
                  ...
                  >
                    <template v-for="(d, i) in row.conditionTypeList">
                      <a-select-option :key="i" :value="d.code">
                        {{ `${d.name}` }}
                      </a-select-option>
                    </template>
                  </a-select>
                ...
                <a-col class="filterw100" :span="8">
                  <template
                    v-if="
                      row.conditionType !== 'EP' &&
                      row.conditionType !== 'NEP' &&
                      row.propertyType !== '6'
                    "
                  >
...
<script lang="ts">
import { BizRuleDataCondition } from '@/views/app/biz-rule/typings/filter-condition';
import {
  Modal,
  Row,
  Col,
...
export default class FilterCondition extends Vue {
  ...
  watchValue(val) {
    ...
    Array.isArray(val.triggerConditionList) &&
      ...
      val.triggerConditionList.forEach((item2: any) => {
        count += item2.conditionItemList.length ? 1 : 0;
      });
    if (count) {
      this.inputValue = '已设置';
  ...
  watchFiltersList(val) {
    this.triggerConditionList.forEach((item: any) => {
      if (item.conditionItemList.length) {
        this.subFlowTriggerFlag = true;
        const currentDataItem = item.conditionItemList.map(
  ...
  filterDataItem(i: number) {
    if (
      !this.triggerConditionList[i].conditionItemList ||
      !Array.isArray(this.triggerConditionList[i].conditionItemList) ||
      this.triggerConditionList[i].conditionItemList.length === 0
    ) {
      return [{}];
    }
  ...
  modalOk() {
    ...
    const value = {
      ...
      triggerConditionList: JSON.parse(
        JSON.stringify(this.triggerConditionList),
      ),
    };
  ...
  onHandleData1(id: any, index: number, key: number) {
    ...
    const item = this.targetFieldList.filter((v) => v.code === id)[0];
    this.triggerConditionList[key].conditionItemList[index].conditionTypeList =
      BizRule.filterConditionOf3(item.propertyType);
    this.triggerConditionList[key].conditionItemList[index].propertyType =
      item.propertyType;
  }
  ...
  addTrigger() {
    ...
    this.triggerConditionList = [...this.triggerConditionList];
