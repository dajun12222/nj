...
<template>
  <div class="mapping">
    ...
    >
        ...
        <a-table :dataSource="tableData" :pagination="false">
          ...
          <a-table-column key="parentWorfklow" data-index="parentWorfklow">
            ...
            <template slot-scope="parentWorfklow">
              ...
              >
                <template v-for="(w, i) in parentWorfklow.list">
                  <a-select-option
                    v-if="isIN_OUT(tableData[parentWorfklow.index], w)"
                    :key="i"
                    :value="w.code"
                  ...
                  >
                    {{
                      w.name_i18n
                        ? `${JSON.parse(w.name_i18n)[$i18n.locale]}[${w.code}]`
                        : $i18n.locale === 'zh'
                        ? `${w.name}[${w.code}]`
                        : `${zhToEn[w.name] || w.name}[${w.code}]`
                    }}
                  </a-select-option>
                </template>
              </a-select>
          ...
          <a-table-column key="mappingWay" data-index="mappingWay">
            ...
            >
              ...
              <a-tooltip placement="top">
                <span slot="title">父流程和子流程互相映射：父流程传递到子流程，同时子流程结束后将数据回填到父流程(子流程多个时以追加方式传递给父流程，同一子表数据只能选择同一种映射方式)</span>
                <i class="icon aufontAll h-icon-all-question-circle-o"></i>
              </a-tooltip>
            </span>
            <template slot-scope="mappingWay">
              <a-select
                v-model="mappingWay.val"
                showSearch
                class="m-w-section"
                @change="onChangeMapping"
              >
                <template v-for="(m, i) in mappingWay.list">
                  <a-select-option
                    v-if="
                      isIN_OUT_APPEND(
                        tableData[mappingWay.index].parentWorfklow.val,
                        tableData[mappingWay.index].subWorkflow.val,
                        m.value,
                      )
                    "
                    :key="i"
          ...
          <a-table-column key="subWorkflow" data-index="subWorkflow">
            <span slot="title" v-resize.east="{ translateX: 16 }" class="resize">子流程</span>
      ...
      <div class="footer">
        <a-button type="primary" :disabled="isSave" @click="onConfirm">
          保存
        </a-button>
      </div>
      <a-alert
...
import { Drawer, Table, Select, Tooltip, Button, Alert } from '@h3/antd-vue';
...
export default class MappingModal extends Vue {
  ...
  async getChildDataItem() {
    ...
    if (
      ...
      _maps.forEach((map: any, i: number) => {
        ...
        const _pname = pDataItems ? pDataItems.name : map.parentDataName;

        const sItemsDatas = this.subWorkflowList.find(
          (wd: any) => wd.code === map.childDataName,
        );
        // if (!sItemsDatas) { return;}
        const _sname = sItemsDatas ? sItemsDatas.name : map.childDataName;
        ...
        if (
          pDataItems &&
          pDataItems.propertyType === 0 &&
          pDataItems.code === 'id'
        ) {
          // 选择业务对象id 另外一侧有关联表单和短文本
          const handler = (handlingList: any) => {
            const arr: any = [];
            const rCode: string = pDataItems.schemaCode;
            handlingList.forEach((innerList: any) => {
              if (innerList.propertyType === 9) {
                ...
                const listRcode: string = innerList.relativeCode;
                if (listRcode === rCode) {
                  arr.push(innerList);
                }
              } else {
        ...
        } else if (pDataItems && pDataItems.propertyType === 9) {
          // 如果是关联表单，另一侧是合法的关联表单和业务对象id
          const handler = (handlingList: any) => {
            const arr: any = [];
            const rCode: string = pDataItems.relativeCode;
            if (!handlingList) {
              return;
            }
            handlingList.forEach((innerList: any) => {
          ...
          pList = handler(temPList);
        } else if (pDataItems && pDataItems.propertyType === 11) {
          const handler = (handlingList: any) => {
            const arr: any = [];
            const rCode: string = pDataItems.relativeCode;
            if (!handlingList) {
          ...
          cLists = childWorkflowList.filter((item: any) =>
            [0, 12, 14].includes(item.propertyType)
              ? [0, 12, 14].includes(pDataItems && pDataItems.propertyType)
              : [13, 15].includes(pDataItems && pDataItems.propertyType) ||
                (item.propertyType ===
                  (pDataItems && pDataItems.propertyType) &&
                  !item.defaultProperty),
          );
          pList = parentWorkflowList.filter(
            (item: any) =>
              item.propertyType === (pDataItems && pDataItems.propertyType),
          );
        }

        const PMainData: boolean =
          (pDataItems && pDataItems.code.indexOf('.')) === -1;
        const CMainData: boolean =
          (sItemsDatas && sItemsDatas.code.indexOf('.')) === -1;
  ...
  filterMappingOnly() {
    ...
    this.tableData.forEach((item: any) => {
      if (item.parentWorfklow.val && item.subWorkflow.val) {
        ...
        const currentMappingVal: string = item.mappingWay.val;
        if (mappingCode) {
  ...
  handleData(
    ...
    if (isSubTable) {
      ...
      if (mappingWay === 'IN_OUT_APPEND') {
        ...
        const triggerCode: string =
          this.curActivityProps.commonSettings.triggerMappingObj.code;
  ...
  handleFilter(type: string, selectedItemCode: any, obj: any) {
    ...
    const { itemtype, index, workflowType, name, relativeCode } =
      obj.data.attrs;
    ...
    if (curRowData.propertyType === 0 && curRowData.code === 'id') {
      ...
      temList.forEach((list: any) => {
        if (list.propertyType === 9) {
          ...
          const listRcode: string = list.relativeCode;
          if (listRcode === rCode) {
            arr.push(list);
          }
        } else {
      ...
      // 2. 找出合法的关联表单和业务对象id
      const rCode: string = relativeCode;
      temList.forEach((list: any) => {
      ...
      arr = tableDataCopy[index][workflowObj].list.filter((item: any) => {
        if ([0, 12, 13, 14, 15].includes(item.propertyType)) {
          return [0, 12, 14].includes(item.propertyType)
            ? [0, 12, 14].includes(itemtype)
            : [13, 15].includes(itemtype);
        } else {
  ...
  onWorkflowChange(value: any, obj: any) {
    ...
    if (
      ...
      //单选，多选映射不清除父流程选项
      if (
        itemtype !== beChangedType &&
        ![0, 12, 13, 14, 15].includes(itemtype)
      ) {
    ...
    if (dataCopy[_index]['subWorkflow'].val) {
      ...
      if (!flag) {
        dataCopy[_index]['subWorkflow'].val = '';
      }
    }
    ...
    if (workflowType === 'parentWorfklow') {
      ...
      if (value && value.indexOf('.') > -1) {
        ...
        if (
          parentArr &&
          parentArr['mappingWay'].val !== dataCopy[_index]['mappingWay'].val
        ) {
  ...
  onChangeMapping(value: any, obj: any) {
    ...
    if (parentWorfklow && parentWorfklow.indexOf('.') > -1) {
      ...
      if (parentArr && parentArr['mappingWay'].val !== value) {
        if (!this.tableData[index]['parentWorfklow'].val.includes('.')) {
  ...
  deleteItem(record: any) {
    ...
    if (!parentWorkflow || !childWorkflow) {
      return;
    }
    ...
    this.tableData.forEach((td: any) => {
      ...
      if (td.parentWorfklow.val) {
        ...
        if (
          ...
          // 关联表单
          const deletedItemRcode: string =
            parentWorkflow.relativeCode as string; // 被删除行父流程关联表单所关联的模型
          if (
  ...
  /*
    如果选择了覆盖模式，将不能选择系统字段
    如果选择了系统字段，将不显示覆盖模式（isIN_OUT_APPEND方法中做了判断）
  */
  isIN_OUT(tableData, item) {
    if (tableData.mappingWay && tableData.mappingWay.val === 'IN_OUT') {
  ...
  isIN_OUT_APPEND(prve: string, next: string, type: string): boolean {
    ...
    if (type === 'IN_OUT') {
      ...
      console.log(prveValue);
      if (prveValue && prveValue.defaultProperty) {
        return false;
      }
  ...
  onTableDataChange(val: any) {
    ...
    const tem_arr = JSON.parse(JSON.stringify(WorkflowDataItem_orign));
    const sheetArr: object[] = [];

    // 有子表数据
    if (tem_arr && tem_arr.length) {
      tem_arr.forEach((d: any) => {
        const mappingWay = val.find(
          (t: any) => t['parentWorfklow'].val.indexOf(d.code) > -1,
        );
        if (mappingWay) {
          sheetArr.push(mappingWay);
        }
      });
    }
    ...
    val.forEach((d: any) => {
      sheetArr.forEach((s: any) => {
        const sheetName = s['parentWorfklow'].val.split('.')[0];
