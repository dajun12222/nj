...
<template>
  ...
  >
    ...
    <!-- 按部门主管查找 -->
    <a-radio v-if="managerOptions.length" :value="OUManager.formula">
      <span class="middle">查找</span>
      <a-select
    ...
    <a-radio :value="FindUserByRole.formula">
      ...
      <span style="width: 220px; display: inline-block"><role-select :defaultValue="FindUserByRole.role" @select="selectRole" /></span>
...
export default class Expression extends Vue {
  ...
  // 部门主管
  get managerOptions(): any[] {
    const UserData = this.WorkflowDataItem.filter(
      (item: Apps.Datamodel.ChildeDataItem) =>
        `${item.propertyType}` === '5' && item.published,
    ...
    const childSheets = this.WorkflowDataItem.filter(
      (item: Apps.Datamodel.ChildeDataItem) =>
  ...
  mounted() {
    ...
    if (
      ...
      !/^([a-zA-Z]+)\((.+?)\)$/.test(this.value.expression)
    ) {
  ...
  setExpress() {
    this.$nextTick(() => {
      ...
      let _express: string = _res ? `${_res.formula}()` : '';
      if (_res && _res.param) {
