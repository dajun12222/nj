      ...
      .forEach((item: any) => {
        if (item.type === typeF.FindDepHeadByField) {
          ...
          const _express: any =
            `{${value}}` +
            `,` +
            (item.checkedList[0] || null) +
            `,` +
            (item.checkedList[1] || null) +
            `,` +
            item.value;
          const findFun: any =
            `${typeFun.FindDepHeadByField}(` + _express + `)`;
          ...
          const _express: any = `{${item.initiator}},${item.level},${
            item.checkedList[0] || null
          },${item.checkedList[1] || null}`;
          const findFun: any =
            `${typeFun.FindUsersByDirectSupervisor}(` + _express + `)`;
          listExpress.push(findFun);
        } else {
          //Else Empty block statement
        }
