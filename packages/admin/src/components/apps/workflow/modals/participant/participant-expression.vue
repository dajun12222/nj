      ...
      <div v-for="(item, index) in dataList" :key="index">
        ...
        <div v-if="item.type === 0" class="group-item">
          ...
          >
            <a-radio :value="item.manager.value">
              ...
              >
                <a-select-option
                  v-for="(its, idx) in systemStaff"
                  :key="idx"
                  :value="its.value"
                >
                  {{ `${its.label}${its.code ? '{' + its.code + '}' : ''}` }}
                </a-select-option>
              </a-select>
            ...
            <a-radio :value="item.department.value">
              ...
              <a-select
                ...
                placeholder="请选择"
                @change="
                  selectHandleChange($event, item, item.department.value)
                "
              >
                <a-select-option
                  v-for="(its, idx) in filedStaff"
                  :key="idx"
                  :value="its.value"
                >
                  {{ `${its.label}${its.code ? '{' + its.code + '}' : ''}` }}
        ...
        <div v-else-if="item.type === 2" class="group-item">
          ...
          <div class="ant-radio-wrapper">
            ...
            >
              <a-select-option
                v-for="(its, idx) in options"
                :key="idx"
                :title="`${its.label}${its.code ? '{' + its.code + '}' : ''}`"
                :value="its.value"
              >
                {{ `${its.label}${its.code ? '{' + its.code + '}' : ''}` }}
              </a-select-option>
        ...
        <div v-else-if="item.type === 4" class="group-item">
          ...
          <div>
            <div class="ant-radio-wrapper">
              ...
              >
                <a-select-option
                  v-for="(its, idx) in supervisorOptions"
                  :key="idx"
                  :value="its.value"
                >
                  {{ `${its.label}${its.code ? '{' + its.code + '}' : ''}` }}
      ...
      <a-dropdown>
        <span class="ant-dropdown-link" @click="(e) => e.preventDefault()">
          ...
          <span class="add-item-text">{{
            isNoParticipantorExr ? '添加处理人' : '添加参与者'
          }}</span>
        </span>
        <a-menu slot="overlay">
          <a-menu-item v-for="(item, index) in findTypes" :key="index">
            <a href="javascript:;" @click="addFindType(item)">{{
              item.name
            }}</a>
          </a-menu-item>
        </a-menu>
...
export default class Expression extends Vue {
  ...
  // 直属主管
  get supervisorOptions(): any[] {
    console.log(this.filedStaff);
    return [{ label: '发起人', value: 'Originator' }];
  }
  ...
  get filedStaff(): any[] {
    ...
    const UserData = that.WorkflowDataItem
      ...
      .filter(
        (item: any) =>
          [5, 50, 51, 60, 61].includes(item.propertyType * 1) && item.published,
      )
    ...
    const childSheets = that.WorkflowDataItem.filter(
      (item: Apps.Datamodel.ChildeDataItem) =>
        `${item.propertyType}` === '8' && item.published,
    );
  ...
  addFindType(inner: any) {
    ...
    const newItem: any = staticData.find(
      (item: any) => item.type === inner.type,
    );
