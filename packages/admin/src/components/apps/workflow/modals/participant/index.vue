...
参数格式要求：数据项、系统参数用{}包裹，常量用""（英文）包裹，例如OUManager({LastParticipants}),OUManager("cloudpivotUser")`;
  ...
  Init() {
    ...
    if (this.popupType === LINE.popupTypes.expression) {
      ...
      /^(Manager|OUManager|FindUserByRole|FindDepHeadByField|FindDepHeadByDep|FindUsersByDirectSupervisor)\(([^()]+?)\)$/.test(
        this.data,
      ) ||
      /^\[.*\]$/.test(this.data)
    ) {
  ...
  handleOk() {
    ...
    WorkflowApi.validateFormula(params)
    ...
    for (const item in dataList) {
      if (dataList.hasOwnProperty(item)) {
        ...
        if (itemValue.type === typeF.FindOrg) {
          ...
          if (
            Array.isArray(itemValue.manager) &&
            itemValue.manager.length === 0
          ) {
