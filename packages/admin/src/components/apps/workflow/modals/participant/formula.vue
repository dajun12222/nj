...
<template>
  <div class="formula-wrap">
    <section class="formula-left">
      <formula-tree :syncList="list" @select="selectItem" />
...
export default class Formula extends Vue {
  ...
  // choose item
  selectItem(info: any) {
    const { isLeaf, expression, parentCode, code } = info;
    if (isLeaf) {
  ...
  setDescription(code: string) {
    const res: Apps.Workflow.FormulaDescription | undefined =
      this.descriptions.find(
        (item: Apps.Workflow.FormulaDescription) => item.code === code,
      );
    if (res) {
      // console.log(res);
