...
<template>
  ...
  >
    ...
    <a-radio :value="FindUserByRole.formula">
      ...
      <role-select
        :defaultValue="FindUserByRole.role"
        style="width: 190px; margin: -6px 5px 0; vertical-align: text-top"
        @select="selectRole"
      />
    ...
    <a-radio :value="FindUserByDataItem.formula">
      ...
      >
        <a-select-option
          ...
          :value="sheet.value"
        >
          {{ `${sheet.label}${sheet.code ? '{' + sheet.code + '}' : ''}` }}
        </a-select-option>
      </a-select>
    </a-radio>
  </a-radio-group>
...
export default class SubExpression extends Vue {
  ...
  get childSheetUserList(): any[] {
    const UserData: any = [];
    const triggerMapping =
      this.curActivityProps.commonSettings.triggerMappingObj;
  ...
  mounted() {
    ...
    if (
      ...
      !/^([a-zA-Z]+)\((.+?)\)$/.test(this.value.subExpression)
    ) {
      Bus.$emit('changeFormulaRole', '');
      return;
    }
