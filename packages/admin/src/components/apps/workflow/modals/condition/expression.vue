...
<template>
  <div class="condition-panel">
    ...
    <section class="condition-box">
      <!-- 编辑区 -->
      <div v-for="(item, idx) in form.ruleList" :key="idx" class="row clearfix">
        <!-- 数据项 -->
        <a-select
        ...
        <a-tooltip :visible="item && !!item.errorMsg">
          ...
          <div :class="!!item.errorMsg && 'has-error'" class="select-data">
            <a-date-picker
              v-if="
                isDate(item.valueType) &&
                !['IsNotEmpty', 'IsEmpty'].includes(item.rule)
              "
              v-model="item.text"
...
<script lang="ts">
import { Select, Tooltip, DatePicker, Input } from '@h3/antd-vue';
...
export default class Expression extends Vue {
  ...
  rules: any[] = [
    ...
    { name: '不等于', value: '!=', acceptTypes: [0, 1, 2, 3, 12, 13, 14, 15] },
    { name: '包含', value: 'Contains', acceptTypes: [0, 1, 12, 13, 14, 15] },
    {
      name: '不包含',
      value: 'NotContains',
      acceptTypes: [0, 1, 12, 13, 14, 15],
    },
    {
      name: '为空',
      value: 'IsEmpty',
      acceptTypes: [0, 1, 2, 3, 12, 13, 14, 15],
    },
    {
      name: '不为空',
      value: 'IsNotEmpty',
  ...
  subRules: any[] = [
    {
      name: '包含',
      value: 'Contains',
      acceptTypes: [0, 1, 2, 3, 4, 12, 13, 14, 15],
    },
    {
      name: '不包含',
      value: 'NotContains',
  ...
  Init(ctx: string) {
    ...
    //将输入框中的值提取出来，防止影响表达式分割
    const mapResult = this.findInputInfosMap(ctx);

    if (/&&/.test(mapResult.ctxAfterMapping)) {
      this.form.connection = '&&';
    } else if (/\|\|/.test(mapResult.ctxAfterMapping)) {
      this.form.connection = '||';
    } else {
      //Else Empty block statement
    }

    const _expressions = mapResult.ctxAfterMapping.split(this.form.connection);

    //填充提取出来的输入值
    this.mapInputInfos(_expressions, mapResult.mapArr);
        ...
        if (_patternTxt.test(exp)) {
          ...
          if (rule === '==') {
            ...
            if (Array.isArray(this.dataItems) && this.dataItems.length > 0) {
              const it: Apps.Datamodel.ChildeDataItem = this.dataItems.find(
                (item: Apps.Datamodel.ChildeDataItem) =>
                  item.code === dataItemCode,
              );
              valueType = it ? `${it.propertyType}` : '1';
            } else {
              valueType = '0';
            }
          } else {
          ...
          let valueType = '0';
          if (dataItemCode) {
            let it: Apps.Datamodel.ChildeDataItem = this.dataItems.find(
              (item: Apps.Datamodel.ChildeDataItem) =>
                item.code === dataItemCode,
            );
            if (!it) {
              it = this.subSheetData[0].info.find(
                (item: Apps.Datamodel.ChildeDataItem) =>
                  item.code === dataItemCode,
              );
            }
  ...
  filteRuleList(item: any) {
    if (item && item.valueType) {
      ...
      if (item.isSub) {
        // 子表没有等于
        return this.subRules.filter((r: any) =>
          r.acceptTypes.includes(+item.valueType),
        );
      } else {
  ...
  filteDataList(item: Apps.Workflow.LineConditionItem, type): any[] {
    if (type === 1) {
  ...
  onDataItemChange(code: any, item: Apps.Workflow.LineConditionItem) {
    const _data: Apps.Datamodel.ChildeDataItem =
      this.dataItems.find(
        (d: Apps.Datamodel.ChildeDataItem) => d.code === code,
      ) ||
      this.subSheetData
        .reduce((arr, item) => {
          arr.push(...item.info);
        ...
        .find((d: Apps.Datamodel.ChildeDataItem) => d.code === code);
  ...
  validate(item: Apps.Workflow.LineConditionItem) {
    ...
    if (item.valueType === '2' && !/^\d+(\.\d+)?$/.test(item.text)) {
      err = '请输入数值';
    }
    item.errorMsg = err;
  ...
  clearValidate() {
    this.form.ruleList.forEach((item: Apps.Workflow.LineConditionItem) => {
      item.errorMsg = '';
    });
  }
  ...
  get dataItems() {
    ...
    return this.WorkflowDataItem.filter(
      (item: Apps.Datamodel.ChildeDataItem) =>
        ['0', '1', '2', '3', '4', '12', '13', '14', '15'].includes(
          `${item.propertyType}`,
        ) && item.published,
    );
  ...
  get subSheetData() {
    const res: any = [];
    const workflowDataItemCopy: any[] = cloneDeep(this.WorkflowDataItem);
  ...
  getExpression() {
    return new Promise((resolve, reject) => {
      ...
      const hasError = this.form.ruleList.some(
        (item: Apps.Workflow.LineConditionItem) => {
          // @ts-ignore
      ...
      const _results = this.form.ruleList.map(
        (item: Apps.Workflow.LineConditionItem) => {
          if (item.rule === 'Contains' || item.rule === 'NotContains') {
            ...
            return `{${item.dataItemCode}}${item.rule}${item.text.replace(
              /"/g,
              '',
            )}`;
          } else {
  ...
  /**
   * 将表达式里面的输入框抽取出来
   * 分隔后再填入，防止读取输入框中的字符当作分隔符
   */
  findInputInfosMap(ctx) {
    const divide = ctx.split('"');
    let ctxAfterMapping = '';
    const mapArr: any[] = [];
    divide.forEach((item: any, index: any) => {
      if (index % 2 !== 0) {
        mapArr.push(item);
        divide[index] = '**signForCover**';
      }
    });
    ctxAfterMapping = divide.join('"');
    return {
      ctxAfterMapping,
      mapArr,
    };
   ...
   * 往分隔后的表达式中填充输入框值
   */
  mapInputInfos(expressions, mapping) {
    let count = 0;
    expressions.forEach((item, index) => {
      if (item.includes('**signForCover**')) {
        expressions[index] = item.replace('**signForCover**', mapping[count]);
        count++;
      }
    });
  }
