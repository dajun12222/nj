...
<template>
  <div class="data-mapping-panel">
    <property-widget label="映射">
      ...
      <template v-if="mappingStr" slot="right">
        <is-seted
          value="已设置"
          :showDel="true"
          :showEllipsis="false"
          @click="showMappingModal"
          @delete="onMappingDelete"
        />
      </template>
    </property-widget>

    <mapping-modal v-model="isShowModal" @confirm="dataMapping" />
  </div>
</template>
<script lang="ts">
...
export default class DataMapping extends Vue {
  ...
  init() {
    if (this.curActivityProps.commonSettings.workflowCode) {
      ...
      if (this.curActivityProps.commonSettings.workflowCode.indexOf('-') > -1) {
        [, _workflowCode] =
          this.curActivityProps.commonSettings.workflowCode.split('-');
      }
      // 通过workflowCode获取schemaCode 这样可获取到子流程数据项
      const params: Apps.Workflow.WorkflowCode = {
      ...
      this.getWorkflowDetail(params).then(async (res: any) => {
        if (res.errcode === 0) {
          ...
          if (
            ...
            let workflowDataitemCopy = JSON.parse(
              JSON.stringify(this.WorkflowDataItem_all),
            );
            workflowDataitemCopy =
              Helper.releaseSubTableDataItem(workflowDataitemCopy);

            const { filterArr } = this;
            workflowDataitemCopy = workflowDataitemCopy.filter(
            ...
            const maps = this.compatibleOldData(
              this.curActivityProps.workflowDataMaps,
            );
            maps.forEach((map: any) => {
              const pDataItems = workflowDataitemCopy.find(
                (wd: any) => wd.code === map.parentDataName,
              );
              const pname = pDataItems ? pDataItems.name : map.parentDataName;
              const pcode = pDataItems ? pDataItems.code : map.parentDataName;
              const sDataItems = _list.find(
  ...
  compatibleOldData(maps: any) {
    const _maps: any = [];
    maps.forEach((map: any) => {
      const pDataItems = this.WorkflowDataItem_all.find(
        (wd: any) => wd.code === map.parentDataName && wd.propertyType === 8,
      );
      if (pDataItems) {
        const { subSchema, name, code } = pDataItems;
        if (subSchema && subSchema.properties.length > 0) {
          const temArr: Array<any> = subSchema.properties.filter(
            (dataItem: any) => !dataItem.defaultProperty,
          );
          const cDataItems = this.subInstanceData_orign.find(
            (wd: any) => wd.code === map.childDataName && wd.propertyType === 8,
          );
          temArr.forEach((subItem: any) => {
            const cTemArr =
              cDataItems.subSchema && cDataItems.subSchema.properties.length
                ? cDataItems.subSchema.properties.find(
                    (cd: any) =>
                      cd.code === subItem.code &&
                      cd.propertyType === subItem.propertyType,
                  )
                : false;
            if (cTemArr) {
              const res = {
