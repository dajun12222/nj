  ...
  <div class="data-item">
    <div class="data-item-wrapper">
      ...
      <div>
        ...
        <!-- 主表数据项 -->
        <template v-for="(item, key) in curActivityProps.dataItem">
...
export default class DataItem extends Vue {
  ...
  get canCheck() {
    if (
      ...
      this.curActivityProps.dataItem &&
      this.curActivityProps.dataItem.length
    ) {
      return true;
    }
    return false;
  ...
  dataMounted() {
    ...
    if (
      this.WorkflowDataItem.length &&
      !Object.keys(this.curActivityProps.dataItem).length
    ) {
      const value: any = [];
      const dataItem = this.WorkflowDataItem.filter(
        (item: any) =>
          !item.defaultProperty && item.published && item.propertyType !== 7,
      );
      dataItem.forEach((item: any) => {
        if (item) {
            ...
            item.subSchema.properties.forEach((sub: any) => {
              if (!sub.defaultProperty && sub.published) {
                subItem.push({
                  ...
                  formPropertyType: sub.formPropertyType,
          ...
          value.push({
            ...
            formPropertyType: item.formPropertyType,
      ...
      const sortValue: any = this.WorkflowDataItem.map((item: any) => {
        const dataItem: any = this.curActivityProps.dataItem.find(
          (data: any) =>
