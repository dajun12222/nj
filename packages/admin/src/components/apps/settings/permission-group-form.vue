...
<template>
  <div class="permission-group-form">
    ...
    <a-row type="flex">
      <a-col :span="4">
        {{ $t('languages.appSettings.GroupName') }}
      </a-col>
      <a-col :span="20" :class="{ error: error.name }">
        <a-input v-model="name" :maxLength="50" />
        <div v-show="error.name" class="message">
          {{ $t('languages.Apps.NameRule') }}
        </div>
      </a-col>
    </a-row>
    ...
    <a-row type="flex">
      <a-col :span="4">
        {{ $t('languages.appSettings.SetType') }}
      </a-col>
    ...
    <a-row v-if="visibleType.isExternalUser" type="flex">
      <a-col :span="4">
        {{ $t('languages.appSettings.IncludeOrg') }}
      </a-col>
      <a-col :span="20" :class="{ error: error.staffs }">
        ...
        />
        <div v-show="error.staffs" class="message">
          {{ $t('languages.appSettings.IncludeRuleOrg') }}
        </div>
      </a-col>
    </a-row>

    <a-row v-if="visibleType.roles" type="flex">
      <a-col :span="4">
        {{ $t('languages.appSettings.IncludeRole') }}
      </a-col>
      <a-col :span="20" :class="{ error: error.roles }">
        ...
        <role-select
          ...
          @select="selectRole"
        />
        <div v-show="error.roles" class="message">
          {{ $t('languages.appSettings.IncludeRule') }}
        </div>
      </a-col>
    </a-row>
    ...
    <a-row v-if="visibleType.isExternalUser" type="flex">
      <a-col :span="4">
        ...
        <a-tooltip
          :title="$t('languages.appSettings.externalUserTips')"
      ...
      >
        <a-checkbox v-model="isExternalUser" />
        <div v-show="error.isExternalUser" class="message">
          {{ $t('languages.appSettings.IncludeRuleOrg') }}
        </div>
      </a-col>
    </a-row>
    ...
    <template>
      ...
      >
        ...
        >
              ...
              <div class="item-title">
                ...
                <span>
                  <a-checkbox
                    ...
                    @change="
                      (e) => setField(childItem, 'visible', e.target.checked)
                    "
                  >
		  ...
		  <li
                      :class="childItem.dataPermissionType === 6 ? 'active' : ''"
                      v-if="visibleType.rolesRange"
                      @click="dataPermissionSelect(childItem,6)"
                  >
                    账套管理范围
                  </li>
                  <li
                    ...
                    @click="
                      dataPermissionSelect(childItem, 4, index, childIndex)
                    "
                  >
            ...
            >
              <div class="item-list">
                ...
                <ul>
                  <li
                    :class="
                      childItem.actionPermission.creatable ? 'active' : ''
                    "
                    @click="
                  ...
                  <li
                    :class="
                      childItem.actionPermission.deletable ? 'active' : ''
                    "
                    @click="
                      actionPermissionSelect(
                        childItem.actionPermission,
                  ...
                  <li
                    :class="
                      childItem.actionPermission.importable ? 'active' : ''
                    "
                    @click="
                      actionPermissionSelect(
                        childItem.actionPermission,
                  ...
                  <li
                    :class="
                      childItem.actionPermission.exportable ? 'active' : ''
                    "
                    @click="
                      actionPermissionSelect(
                        childItem.actionPermission,
                  ...
                  <li
                    :class="
                      childItem.actionPermission.printAble ? 'active' : ''
                    "
                    @click="
                      actionPermissionSelect(
                        childItem.actionPermission,
                  ...
                  <li
                    :class="
                      childItem.actionPermission.editOwnerAble ? 'active' : ''
                    "
                    @click="
                      actionPermissionSelect(
                        childItem.actionPermission,
                  ...
                  <li
                    :class="
                      childItem.actionPermission.batchPrintAble ? 'active' : ''
                    "
                    @click="
                      actionPermissionSelect(
                        childItem.actionPermission,
                  ...
                  <li
                    :class="
                      childItem.actionPermission.batchUpdateAble ? 'active' : ''
                    "
                    @click="
                      actionPermissionSelect(
                        childItem.actionPermission,
    ...
    <div class="ant-drawer-btn">
      <a-button class="cancel-btn" @click="cancel">
        {{ $t('languages.appSettings.cancel') }}
      </a-button>
      <a-button type="primary" :disabled="canDoubleClick" @click="save">
...
export default class PermissionGroupForm extends Vue {
  ...
  onConditionModalOk(val: any) {
    ...
    // 确定时要更新当前数据模型列表数据
    const curPerssion: any =
      this.list[this.curAppIndex] &&
      this.list[this.curAppIndex].children &&
      this.list[this.curAppIndex].children[this.curPerssionIndex];
  ...
  onConditionModalCancel() {
    ...
    if (this.beforeIndex !== undefined && this.beforeChildIndex !== undefined) {
      this.list[this.beforeIndex].children[
        this.beforeChildIndex
      ].dataPermissionType = this.beforeDataPermissionType;
    }
  }
    ...
    for (const key in this.changeDatas) {
      if (this.changeDatas.hasOwnProperty(key)) {
        ...
        const params = {
          ...
          departments:
            this.setType === 1 ? JSON.stringify(this.roles) : undefined,
  ...
  async setKeepDeptIds() {
    ...
    const bothDeptAndStaff: boolean = types.some(
      (i: number) => i === 3 || i === 1,
    );
  ...
  save() {
    ...
    if (this.setType === 0) {
      if (this.staffs && this.staffs.length) {
        params.departments = JSON.stringify(
          this.staffs.map((s: any) => ({
            id: s.id,
            name: s.name,
            unitType: s.unitType,
          })),
    ...
    if (
      ...
      params.dataPermissionGroups.map((item: any) => {
        if (item.dataPermissionType === 4 && !item.conditions) {
  ...
  rolesChange(val: any) {
    ...
    if (val.conditions && val.conditions.length > 0) {
      const condition = val.conditions;
      for (let i = 0; i < condition.length; i++) {
        if (
          condition[i].operatorType === 11 ||
          condition[i].operatorType === 12
        ) {
