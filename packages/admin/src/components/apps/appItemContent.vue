...
<template>
  <div class="item-content">
    ...
    >
      ...
      <template v-for="item in allGroup">
        <BizModelGroup
          :id="`anchor${item.id}`"
          v-if="item.type === 'Folder'"
          :key="item.code"
          :class="{ 'cur-folder': curFolderId === `anchor${item.id}` }"
          :itemData="item"
          :searchKey="searchKey"
    ...
    <!-- 新建报表弹窗 -->
    <AddReport v-model="showAddReport" :mode="'add'" />
...
import { Icon, Dropdown, Button, Menu, Input } from '@h3/antd-vue';
...
export default class AppItemContent extends Vue {
  ...
  onImportSuccess(v: boolean) {
    const fileName: string = (this.$refs.dataImport as any).fileName;
    if (fileName && !v) {
