        ...
        <collapse style="margin-top: 24px" :title="item.relativeName">
          ...
          <div class="modal-wrapper" :class="{ disable: isSeted(item) }">
            ...
            <div class="modal-set">
              ...
              <a-popover
                v-else
                overlayClassName="modal-set-popover"
              ...
              >
                ...
                <template slot="content">
                  <setCurrentDataItem
                    ...
                    wrapperWidth="580px"
                    :data="item.dataItems"
                    :checkedDataItemList="item.checkedList"
                    @checkedListChange="(list) => checkedListChange(list, item)"
                  />
                </template>
              </a-popover>
            </div>
...
export default class setRelevanceFormsDataItems extends Vue {
  ...
  onModalOptionsChange(options) {
    this.RelevanceForms = options.map((el, index) => {
      const item =
        this.relevanceFormsDataItems.find((item) => item.code === el.code) ||
        {};
  ...
  onRelevanceFormsDataItemsChange(val: any[]) {
    ...
    if (val.length === 0) {
      ...
      this.RelevanceForms.forEach((el) => {
        const item = val.find((item) => item.code === el.code);
        if (item) {
  ...
  isSeted(item) {
    const theSame = this.RelevanceForms.filter(
      (el) => el.options.schemaCode === item.options.schemaCode && el !== item,
    );
    if (theSame.length > 0) {
  ...
  getDataItems(options: any, index) {
    // 获取关联目标模型的可配置项
    this.getAsycDataItems(options.options.schemaCode).then((res) => {
      if (res.errcode === 0) {
        this.$set(
          this.RelevanceForms[index],
          'dataItems',
          this.filterData(
            res.data,
            this.RelevanceForms[index].isMainTable,
            options.options.schemaCode,
          ),
        );
      }
    });
