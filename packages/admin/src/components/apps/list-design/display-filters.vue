        ...
        <template slot="content">
          <div class="popover-box">
            ...
            <ul v-if="dialogList.length">
              <!-- 全选 -->
              <li class="section-bar" style @click="handleAllCheck">
...
<script lang="ts">
import { Checkbox, Popover, Button } from '@h3/antd-vue';
...
export default class DisplayFilters extends Vue {
  ...
  comfirmLevel1Popover() {
    const flag: boolean = this.dialogList.every((d: any) => !d.checked);
    const list: any[] = flag ? [] : this.dialogList;
