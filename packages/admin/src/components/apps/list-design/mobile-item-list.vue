  ...
  <div class="mobile-item">
    <div class="item-default">
      ...
      <div class="item-footer">
        <div v-if="dataItem.creater" class="footer-left">
          <i class="icon aufontAll h-icon-all-normal_smile"></i>
          <span>{{ dataItem.creater }}</span>
        </div>
        <div v-if="dataItem.createdTime" class="footer-right">
  ...
  dataMounted(v: any) {
    ...
    const data: any[] = v.map((pl: any, index: number) => {
      ...
      if (pl.isSystem) {
        ...
        const format = DateType.find((s: any) => s.type === pl.displayFormat);
        const propertyType: any = showFieldDefaultVal[pl.type];
        if (isNaN(Number(propertyType))) {
          value = propertyType;
        } else {
          value =
            propertyType && format
              ? moment(propertyType).format(format.format || 'YYYY-MM-DD HH:mm')
              : '- -';
        }
      }
    ...
    this.dataList = data.filter(
      (d: any) => !this.dataItemArray.includes(d.code),
    );
  }
