    ...
    <div class="permission-setting__row">
      <div>
        <a-radio-group v-model="visibleType" @change="onVisibleTypeChange">
          <a-radio value="ALL_VISIABLE">
            {{ $t('cloudpivot.report.AllAdminVisible') }}
          </a-radio>
          <a-radio value="PART_VISIABLE">
            {{ $t('cloudpivot.report.AccessByGroup') }}
          </a-radio>
        </a-radio-group>
      </div>
    </div>
...
import { Drawer, Popconfirm, Icon, Radio, Table } from '@h3/antd-vue';
...
export default class PermissionSetting extends Vue {
  ...
  formatDataSource(records: any) {
    if (this.dataSteamList.length === 0) {
      return records;
    }
    const result = records
      .split(',')
      .reduce((stat: any, item: any) => {
        this.dataSteamList.forEach((sub: any) => {
          if (sub.id === item) {
            stat.push(sub.name);
          }
        });
  ...
  formatUser(records: any) {
    ...
    const result = records
      .split(',')
      .reduce((stat: any, item: any) => {
        this.subManagers.forEach((sub: any) => {
          if (sub.id === item) {
            stat.push(sub.name);
          }
        });
