      ...
      if (record) {
        ...
        switch (type) {
          case ModelAction.ModalActionTypes.DataModel:
            data = [];
            isExist = true;
            href = `${hashPrefix}/apps/model/${appCode}/${bizSchemaCode}/data/dataModel`;
            break;
          case ModelAction.ModalActionTypes.Form:
            data = await this.getFormList(bizSchemaCode);
            isExist = data.some((item: any) => item.code === code);
            href = `${hashPrefix}/apps/model/${appCode}/${bizSchemaCode}/form-design/${
              isExist ? code : data[0].code
            }`;
            break;
          case ModelAction.ModalActionTypes.Workflow:
            ...
            if (!isExist && !data.length) {
              ...
              href = `${hashPrefix}/apps/model/${appCode}/${bizSchemaCode}/workflowDesign/${
          ...
          case ModelAction.ModalActionTypes.List:
            ...
            isExist = data.some((item: any) => item.code === code);
            href = `${hashPrefix}/apps/model/${appCode}/${bizSchemaCode}/list-design/${
              isExist ? code : data[0].code
            }`;
            break;
          default:
