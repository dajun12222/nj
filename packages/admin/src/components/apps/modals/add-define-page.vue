  ...
  >
    <template slot="title">
      <div class="custom-title">
        {{ title }}
        <small v-if="!code">可在应用中添加外部链接或编写的页面，添加后在应用内访问该链接</small>
      </div>
    </template>
    ...
    <div class="a-from">
      ...
      <a-row>
        ...
        <a-col :span="12">
          ...
          <div class="form-wrap">
            <a-radio-group
              v-model="getItem('openMode').value"
              name="radioGroup"
            >
              <a-radio
          ...
          <p class="roleMangeTip">
            角色管理：选择路由模型，地址配置为[ /application-role/role ]
          </p>
        </a-col>
      </a-row>
  ...
  createPage(params: Apps.AppCenter.DefinePageParams) {
    ...
    appsApi.createDefinePage(params).then((res: any) => {
      if (res.errcode === 0) {
        ...
        const url = `/apps/model/${this.$route.params.appCode}/${params.code}/customPage`;
