    ...
    >
      ...
      >
        <span slot="label">
          所属分组
          <!-- <a-tooltip>
            <template slot="title"> 不选择上级分组，则默认为根分组 </template>
...
import { Modal, Tooltip, Form, Input, TreeSelect } from '@h3/antd-vue';
...
export default class AddFolderModal extends Vue {
    ...
    return this.floders.map((item: any) => {
      if (item.children && item.children.length) {
        ...
        return {
          ...
          title: item.name,
          children: null,
          disabled: item.id === this.folderModel.id,
        };
      }
    });
  }
  ...
  convert(x: any, isLeaf: boolean) {
    ...
    return {
      ...
      title: x.name,
      children,
      disabled: x.id === this.folderModel.id,
    };
  }
    ...
    this.form.validateFields((err: any) => {
      ...
      if (!err) {
        ...
        if (!vm.folderModel.id) {
          ...
          vm.addFolders(folderParams).then((res) => {
            if (res.errcode === 0) {
          ...
          vm.updateFolders(folderParams).then((res) => {
            if (res.errcode === 0) {
