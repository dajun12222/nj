...
const actions = {
  ...
  async getAppList({ commit, state }: any, params: any) {
    ...
    appGroupList = params.isLoad
      ? await AppCenterApi.getAppgroup({})
      : {
          data: state.appGroupListAll,
          errcode: 0,
        };
    const apps: any = await AppCenterApi.getGroupAppByKey({
      groupCode: params.groupCode,
    });
    const currentGroup = appGroupList.data.find(
      (el) => el.code === params.groupCode,
    );
    currentGroup.children = apps.data.children;
    const res = appGroupList;
    ...
    if (res.errcode === 0) {
      ...
      const data: any = res.data.find(
        (d: Apps.AppCenter.AppGroup) => d.code === params.groupCode,
      );
      ...
      commit(
        'setAppGroupList',
        gourpListAll.filter((item) => item.code !== 'all'),
      );
      res.data.forEach((v) => {
   ...
   * @param params
   */
  async checkIfHasModel(
    { dispatch }: any,
    params: Apps.AppCenter.AppInfoParams,
  ) {
  ...
  async updateAppSort({ dispatch }: any, params: Apps.AppCenter.AppSortParams) {
    ...
    return await AppCenterApi.updateAppSort(params);
    // if (res.errcode === 0) {
    //   await dispatch('getAppList');
    // }
  },
