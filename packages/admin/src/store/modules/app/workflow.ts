...
const state = {
  ...
  historyListVisible: false, //历史流程列表展示标记
};
...
const mutations = {
  ...
  setHistoryListVisible(state: any, visible: boolean) {
    state.historyListVisible = visible;
  },
  ...
  clearWorkflowData(state: any) {
    ...
    state.beforeMovedActivities = [];
    state.subWorkflowTplsTree = [];
    state.WorkflowDataItem = [];
    state.WorkflowDataItem_origin = [];
    state.WorkflowDataItem_list = [];
    state.WorkflowDataItem_all = [];
    state.childWorkflowSchemaCode = '';
    state.taskList = '';
    state.isSaveProp = true;
    ...
    state.bizRuleList = [];
    state.workflowMenus = [];
    state.preConditionList = [
      { code: 'ALL', name: '全部', rule: {} },
      { code: 'ANY', name: '任一', rule: {} },
    ];
    //历史流程列表可见性清空时不做处理，路由跳转和创建时手动处理
    //state.historyListVisible = false;
  },
  ...
  addActivity(state: any, Activity: WorkflowNamespace.Activity) {
    ...
    if (
      _acticity.activityType === 'PARTICIPANT' ||
      _acticity.activityType === 'CONNECTION'
    ) {
      _acticity.preCondition = { type: 'ALL', rule: {} };
    }
    if(_acticity.activityType === "workflow"){
      _acticity.staffs = [];
    }
  ...
  setDefaultWorkflowData(state: any, data: any) {
    if (!data.name_i18n) {
      data = compatibleOldData({ data: data, type: 'workflow' });
    } else {
      data.name_i18n = JSON.parse(data.name_i18n);
    ...
    state.defaultActivities = data.activities;
    ...
    state.defaultWorkflowData = _workflowData;
  },
  ...
  dataToProps(state: any, type?: string) {
    ...
    const _object: any = {
      ...
      btnConfigList: _curActivity.btnConfigList,
      activitySenior: {
    ...
    //如果是系统活动节点，删除掉participant中的popupType属性，避免属性覆盖
    if (_curActivity.activityType === 'SYSTEM_ACTIVITY') {
      delete _object.participant.popupType;
      _object.commonSettings.popupType = _curActivity.popupType;
    }
  ...
  propsToData(state: any) {
    ...
    if (_index !== -1) {
      ...
      const btnConfigList = state.curActivityProps.btnConfigList;
  ...
  // 设置表单上的消息通知
  setTodoDataItems(state: any, value) {
    state.workflowData.todoDataItems = value;
  },
  ...
  setSmsPropValue(state: any, value) {
    ...
    state.curActivityProps.todoDataItems = [
      ...state.curActivityProps.todoDataItems,
    ];
  },
...
const actions = {
  ...
  async getWorkflowDraft(
    ...
    const defaultRes = await WorkflowApi.getWorkflowDraft(param);
    if (defaultRes.errcode === 0) {
      commit('setDefaultWorkflowData', defaultRes.data);
    }
    ...
    if (res.errcode === 0) {
      ...
      await res.data.activities.forEach((activity: any) => {
        ...
        if (
          ...
          activity.activityType === 'CIRCULATE'
        ) {
          ...
          if (
            activity.todoDataItem &&
            activity.todoDataItem.summary &&
            activity.todoDataItem.summary.length > 0 &&
            activity.todoDataItem.summary[0].hasOwnProperty('isDataItem') &&
            activity.todoDataItem.summary[0]['isDataItem'] === 1
          ) {
            keys.push('summary');
          }
          keys.forEach((key: string) => {
  ...
  async getHistoryWorkflow(
    ...
    const defaultRes = await WorkflowApi.getHistoryWorkflow(param);
    if (defaultRes.errcode === 0) {
      commit('setDefaultWorkflowData', defaultRes.data);
    }
