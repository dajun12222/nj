...
function userInfoChangeData(list: any, roleTitle: any) {
  if (list[0].group && list[0].children && list[0].children.length > 0) {
    ...
    roleTitle.groupName = list[0].groupName;
...
const mutations = {
    ...
    if (!state.treeParams.groupId) {
      ...
      for (let i = 0; i < childNodeLength; i += 1) {
        state.dataRef.children.push({
          ...
          parentId: state.dataRef.id,
          remarks: childNode[i].remarks,
          isEditFlow: childNode[i].isEditFlow,
          createdBy: childNode[i].createdBy
        });
      }
      // 角色搜索时，赋值给orgTreeBySearch
      if (state.isRoleSearchResult) {
  ...
  async setOrgRoleList(
    ...
    await list.forEach((res: any, index: number) => {
      ...
      //     .join(";");
      // }
      res.departmentName = res.parentDepartmentName;
...
const actions = {
  async getOrgRoleNode({ commit, state }: any,appCode?:string) {
    const res = await OrganizationApi.getRoleLeveOneInfoNew(false,appCode);
