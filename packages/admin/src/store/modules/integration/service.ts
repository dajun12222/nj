...
const mutations = {
  ...
  filterServices(state: any, payload: string) {
    ...
    if (payload) {
      ...
      state.list = state.services.filter(
...
const actions = {
  ...
  getServiceList({ commit }: any, categoryId: string) {
    ...
    serviceApi
      .listServicesById({ categoryId })
      .then((res: Common.Res<BizService.Service.Item[]>) => {
        if (!res.errcode) {
  ...
  deleteService(
    ...
    serviceApi.deleteService(params).then((res: any) => {
      if (!res.errcode) {
        const list: BizService.Service.Item[] = state.services.filter(
          (item: BizService.Service.Item) => item.id !== params.id,
        );
