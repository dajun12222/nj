...
class FormItemValidate {
  ...
  init() {
    ...
    const reservedWord = window.localStorage.getItem('reservedWords');
    if (reservedWord) {
      this.reservedWords = reservedWord.split(',');
    }
    ...
    if (maxLength) {
      this.el.onkeydown = (evt: KeyboardEvent) => {
        ...
        if (
          input.value &&
          maxLength.len &&
          input.value.length >= maxLength.len
        ) {
  ...
  async validateItem(bind?: VNodeDirective) {
      ...
      if (!value || value === '') {
        ...
        this.errMsg =
          typeof rule.required === 'string'
            ? rule.required
            : defaultRuleOptions.errMsg().required;
      }
    }
    if (!!value) {
      // 校验元素是否正确
      ...
      validate &&
        rule.maxLength &&
        (validate = this.lenCheck(rule.maxLength, 'max'));
    ...
    !validate &&
      this.error &&
      this.error(this.value, this.errMsg, this.el, this.vnode);
  ...
  regexpCheck(regexps: FormRegexp[] | FormRegexp): boolean {
    ...
    const res: FormRegexp | undefined = regexps.find((regexp: FormRegexp) => {
      ...
      if (typeof regexp.regexp === 'string') {
        if (this.reservedWords && regexp.regexp === 'regexpSQL') {
          const vas = this.value.toUpperCase();
          this.reservedWords.indexOf(vas);
          if (this.reservedWords.indexOf(vas) > -1) {
            validate = false;
          }
        } else {
          const defRegexp: RegExp =
            defaultRuleOptions.formRegexp[regexp.regexp];
          if (!defRegexp) {
  ...
  lenCheck(formLength: FormLength, type = 'min'): boolean {
    const cn =
      typeof formLength.cnChart === 'boolean'
        ? formLength.cnChart
        : defaultRuleOptions.cnChart;
    const strLen: number = getStrLen(this.value, cn);

    const res =
      type === 'min'
        ? strLen >= (formLength.len || 0)
        : strLen <= (formLength.len || 0);
  ...
  getParent(element: HTMLElement): any {
    if (
      ...
      !(element.parentElement as any)._formValidate
    ) {
...
export const formDirective: DirectiveOptions = {
  bind(el: HTMLElement, bind: VNodeDirective, vnode: any) {
    const temp = new FormValidate(el, bind, vnode);
  },
};
