  ...
  <div>
    <div class="integration__head">
      <section class="integration__title">
        {{ folderName }}
        <span v-if="plainOptions.length > 0" class="integration__subtitle">已选择<span class="num">{{ plainOptions.length }}</span>个集成服务</span>
      </section>
      <section>
        <span class="help-word" @click="toHelpWord">
          <em class="icon aufontAll h-icon-all-bulb-o"></em>
          <span>查看使用帮助</span>
        </span>
        <InputSearch
          v-model="searchKey"
    ...
    <div class="integration__table">
      ...
      >
        ...
        <span slot="codeTitle" class="text-ellipsis resize">服务编码</span>
        <span slot="nameTitle" class="text-ellipsis resize">显示名称</span>
        <span slot="adapterTitle" class="text-ellipsis resize">适配器</span>
        ...
        >
          <template v-if="isOperation(record)">
            <i @click="showAddDrawer(record)">查看</i>
          </template>
          <template v-else>
            <!-- 编辑 -->
            <i @click="showAddDrawer(record)">编辑</i>
    ...
    >
      <div class="refer-modal">
        <div class="refer-modal_tip success">
          <i class="icon aufontAll h-icon-all-check-circle-o"></i>
          <article>
            <header>保存成功！</header>
...
export default class IntegrationContent extends Vue {
  ...
  // 跳转帮助文档
  toHelpWord() {
    window.open('https://www.yuque.com/skwme4/hyk0u0/sttdcq', '_blank');
  }
   ...
   */
  async exportService() {
    const loading: any = this.$message.loading('集成服务导出中，请稍后...', 0);

    const serviceCodes = this.plainOptions.map((item) => {
      return item.code;
    });
    const res: any = await serviceApi.exportService({
    ...
    if (res.errcode && res.errcode !== 0) {
      ...
      const fileName =
        this.plainOptions.length > 1
          ? `${this.plainOptions[0].code}等.zip`
          : `${this.plainOptions[0].code}.zip`;
   ...
   * 关闭添加服务抽屉
   */
  closeServerDrawer(edit?: boolean) {
    if (edit) {
      this.plainOptions = [];
    }
