  ...
  <div class="add-method">
    <section class="add-method__block">
      <div class="add-method__form">
        ...
        <div v-if="form.shared" class="add-method__item">
          <span>共享人员：</span>
          <p>
            <a-select
        ...
        <div class="add-method__item add-method__item--full">
          ...
          <p>
            <!-- 
              TODO: 添加返回值类型字段
            -->
            <a-select v-model="dataBaseConnectId" showSearch>
              <a-select-option
      ...
      <div class="add-method__form">
        ...
        <div class="add-method__item">
          ...
          >
            ...
            <span slot="codeTitle" class="text-ellipsis resize">参数名称</span>
            <span slot="nameTitle" class="text-ellipsis resize">显示名称</span>
            <span slot="operatorTitle" class="text-ellipsis resize">
              逻辑关系
...
export default class AddMethod extends Vue {
  ...
  @DSMethodsModule.Action('createDatasourceMethods')
  createDatasourceMethods: any;

  @DSMethodsModule.Action('updateDatasourceMethods')
  updateDatasourceMethods: any;

  @DSMethodsModule.Action('getParamTypes') getParamTypes: any;

  @Prop() datasourceCategoryId?: string;

  @Prop() poolList!: Array<any>;
  ...
  dataBaseConnectId: string =
    this.poolList.length > 0 ? this.poolList[0].id : '';
  ...
  save(callback?: Function) {
    ...
    this.validateForm()
      .then(async () => {
        ...
        const operatorNull = this.inputParamConfigList.some(
          (record: any) => !record.operator,
        );
        if (operatorNull) {
        ...
        const recordNull = this.inputParamConfigList.some(
          (record: any) =>
        ...
        const dateNull = this.inputParamConfigList.some(
        ...
        this.form.inputParamConfig = JSON.stringify(
          this.inputParamConfigList.map((item: any) => {
            let value: any = [];
            if (item.type === 'DATE') {
  ...
  updateParamListByCodes(codes: Array<string>, dataKey: string) {
    ...
    const paramsList = codes.map((code: string, idx: number) => {
      const existItem = targetList.find(
        (param: BizService.Method.sqlParam) => param.code === code,
      );
      const index = idx + 1;
      if (existItem) {
        return {
  ...
  async getAPPManagers() {
    const selectUserIds: string = this.method.userIdList
      ? this.method.userIdList.join(',')
      : '';
