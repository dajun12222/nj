    ...
    >
      ...
      >
        <div>
          ...
          <span v-if="headerFooter.header.date" class="pl-10">{{
            headerFooter.date
          }}</span>
          <span v-if="headerFooter.header.time" class="pl-10">{{
            headerFooter.time
          }}</span>
        </div>
      </div>
      ...
      <article>
        ...
        <div
          :style="{
            backgroundImage: paperSize.isPrintBgImg
              ? 'url(' + bgImgUrl + ')'
              : '',
          }"
          class="main-body"
        >
          <div>
            ...
            >
              <table
                :style="`height: ${item.heightValue}px;padding: 0; border-spacing: 0;border-collapse: collapse;display: block;`"
              >
                <tr
              ...
              >
                <img
                  ...
                  :src="item.picUrl || imgObj.qr_code"
                  alt
                />
              </div>
              <div
              ...
              >
                <img
                  style="width: 100%; height: 100%"
                  :src="item.picUrl || imgObj.bar_code"
                  alt
                />
              </div>
              <div
      ...
      >
        <div>
          ...
          <span v-if="headerFooter.footer.date" class="pl-10">{{
            headerFooter.date
          }}</span>
          <span v-if="headerFooter.footer.time" class="pl-10">{{
            headerFooter.time
          }}</span>
        </div>
      </div>
...
<script lang="ts">
import { PrintNodeSettings } from '@/config/print/dataStructure';
...
export default class PreView extends Vue {
  ...
  created() {
    ...
    const settingData: object[] = this.pages[0] as [];
  ...
  getTrData(col: any, type) {
    if (
      col.rowSettings.bindSource &&
      col.rowSettings.bindSource.code === 'sequence'
    ) {
      return type;
    } else if (!col.rowSettings.bindSource) {
      return '';
    } else if (
      col.rowSettings.bindSource &&
      (col.rowSettings.bindSource.code === 'receiveTime' ||
        col.rowSettings.bindSource.code === 'finishTime')
    ) {
      return this.getNowFormatDate();
    } else {
      return col.rowSettings.bindSource.name + type;
    }
  }
...
.both-div {
  .item-div {
    div {
      ...
      height: 20px;
      &.top-left {
        border-bottom: 1px solid rgba(17, 18, 24, 0.25);
        border-right: 1px solid rgba(17, 18, 24, 0.25);
      }
      &.top-right {
        border-bottom: 1px solid rgba(17, 18, 24, 0.25);
        border-left: 1px solid rgba(17, 18, 24, 0.25);
      }
      &.bottom-left {
        border-top: 1px solid rgba(17, 18, 24, 0.25);
        border-right: 1px solid rgba(17, 18, 24, 0.25);
      }
      &.bottom-right {
        border-top: 1px solid rgba(17, 18, 24, 0.25);
