...
<template>
  <div
    ...
    :style="{
      ...
      height: pageSettingData._papersize.heightTopx + 'px',
    }"
  ...
  >
    <div
      ...
      :style="{
        top: pageSettingData._headerfooter.header.marginTopTopx + 'px',
      }"
    >
      <div>
    ...
    <div
      ...
      :style="{
        top: -pageSettingData._headerfooter.footer.marginTopTopx - 20 + 'px',
      }"
    >
      <div>
...
import { getOffsetLeft, guid } from '@/utils/print-util';
...
export default class PrintPage extends Vue {
  ...
  drawDividers() {
    ...
    const { left, top, bottom, right } = this.getItemRect(this.itemAttrs);
    ...
    if (this.dragItem.propertyType === schema.DataItemType.Sheet) {
      ...
      if (this.dragItem.owner.includes('left')) {
        ...
        if (this.dragItem.kind.includes('flowLog')) {
          const columns = processLog.map((item: any) => {
            const col = new SheetColumn('leftProcessLog', item.name, 50);
            col.rowSettings.bindSource = {
              code: item.code,
              name: item.name,
            };
          ...
          const widthValue = columns.length * 70;
          const heightValue = 30 * 3 + 1;
          ...
          console.log(tempObj, 'temsssssspObj');
  ...
  onMouseup(evt: MouseEvent, offset: Point) {
    ...
    if (this.rectPoint) {
      ...
      const rect = this.drawRect(evt, offset);
  ...
  drawRect(evt: MouseEvent, offset: Point) {
    ...
    const width = Math.abs(right - this.rectPoint.x);
    const height = Math.abs(bottom - this.rectPoint.y);
