      ...
      <div class="modal-h" :style="Init">
          ...
          <div class="ant-form-inline">
            <a-form-item label="宽" class="label-w">
              <a-input-number
                ...
                @change="
                  inputNumberChange($event, 1, 1500, '_papersize.widthTomm')
                "
              />
            </a-form-item>
            <a-form-item label="高" class="label-w">
              <a-input-number
                ...
                :parser="(value) => value.replace('mm', '')"
                @change="
                  inputNumberChange($event, 1, 1500, '_papersize.heightTomm')
                "
              />
            </a-form-item>
          </div>
              ...
              <a-input-number
                ...
                @change="
                  inputNumberChange(
                    $event,
                    1,
                    activeData._papersize.heightTomm,
                    '_pagemargin.UpTomm',
                  )
                "
              />
              ...
              <a-input-number
                ...
                @change="
                  inputNumberChange(
                    $event,
                    1,
                    activeData._papersize.widthTomm,
                    '_pagemargin.LeftTomm',
                  )
                "
              />
            </a-form-item>
          ...
          <a-tabs defaultActiveKey="1" @change="headerSwitch($event)">
            <a-tab-pane key="1" tab="页眉">
              ...
              <div class="mt-20 div-input">
                ...
                >
                  <span
                    ...
                    >{{
                      activeData._headerfooter.header.items.value.join(' ')
                    }}</span>
                </a-card>
              </div>
              ...
              <div class="pt-25 clear">
                ...
                <a-button class="ml-15" @click="Inser(1, 'Total')">
                  页数
                </a-button>
                <a-button class="ml-15" @click="Inser(1, 'Date')">
                  日期
                </a-button>
                <a-button class="ml-15" @click="Inser(1, 'Time')">
                  时间
                </a-button>
              </div>
            </a-tab-pane>
            <a-tab-pane key="2" tab="页脚" forceRender>
              <a-form-item
                label="页脚到底部的距离"
                :labelCol="{ span: 6 }"
                :wrapperCol="{ span: 6 }"
              ...
              <div class="mt-20 div-input">
                ...
                >
                  <span
                    ...
                    >{{
                      activeData._headerfooter.footer.items.value.join(' ')
                    }}</span>
                </a-card>
              </div>
              <div class="mt-20 div-input ml-15">
              ...
              <div class="pt-25 clear">
                ...
                <a-button class="ml-15" @click="Inser(2, 'Total')">
                  页数
                </a-button>
                <a-button class="ml-15" @click="Inser(2, 'Date')">
                  日期
                </a-button>
                <a-button class="ml-15" @click="Inser(2, 'Time')">
                  时间
                </a-button>
              </div>
            </a-tab-pane>
        ...
        <div v-if="tabPosition === 'bgImg'">
          ...
          <div class="imgTips">
            ...
            <a-checkbox
              :defaultChecked="activeData.isPrintBgImg"
              @change="onBgImgChange"
            >
              打印背景
            </a-checkbox>
          </div>
        </div>
        <div v-if="tabPosition !== 'bgImg'" class="item-tool">
...
export default class SettingModal extends Vue {
  ...
  InitData(): void {
    ...
    if (this.pageSettings._papersize) {
      this.activeData = this.pageSettings;
    }
  }
  ...
  paperSizeChange(e): void {
    ...
    const selectItem: settingHelper.PaperSpecSize[] = this.paperSpecItem.filter(
  ...
  save(): void {
    ...
    this.updatePageSettingData(
      Object.assign(this.activeData, {
        code: this.$route.params.bizSchemaCode,
      }),
    );
  ...
  rest(): void {
    ...
    if (this.tabPosition === 'page') {
      ...
      const active: string = this.activeDiv.split('_')[0];
      active === 'header'
        ? (this.activeDiv = 'header_Center')
        : (this.activeDiv = 'footer_Center');
      this.activeData._headerfooter.header =
        settingHelper.getDefaultHeaderFooter();
      this.activeData._headerfooter.footer =
        settingHelper.getDefaultHeaderFooter();
    } else {
  ...
  headerSwitch(e): void {
    const data: settingHelper.HeaderFooter = this.activeData._headerfooter;
    // 获取历史操作状态
    switch (e) {
      case '1':
  ...
  selectCard(value): void {
    ...
    const active: string[] = value.split('_');
  ...
  fillData(value: number, key: string, model: settingHelper.areaInsert): void {
    const align: string = this.activeDiv.split('_')[1];
    model.align = align;
    // 判断是否添加了内容
    const tempContent: string[] = model.items.key.filter(
    ...
    pageData.forEach((item: any, index) => {
      if (item.eleType === 'pageSettings') {
        ...
        if (this.activeData.bgImg.uid !== '') {
          ...
          this.activeData.bgImg.url =
            renderer.UploadControl.service.getDownloadUrl(file);
