    ...
    <template v-if="drawerElements.length > 0">
      ...
      >
        <div
          ...
          class="two-text-div"
          :class="{
            activeChoose: item.leftKey.active || item.rightValue.active,
          }"
        ...
        >
          <h3-size-rectangle
            ...
            class="box"
            @resize="
              (e) => onResize(e, item, 'leftKey', item.rightValue.widthValue)
            "
          >
            <div
          ...
          <h3-size-rectangle
            ...
            class="box"
            @resize="
              (e) => onResize(e, item, 'rightValue', item.leftKey.widthValue)
            "
          >
            <div
              class="two-text-div-one"
            ...
            `"
              :class="[
                item.rightValue.fontTCBNeat,
                item.rightValue.fontLCRNeat,
              ]"
...
export default class MainDrawer extends Vue {
  ...
  clearProcessLogSelected() {
    const processLog = this.$refs.processLog as any[];
    if (processLog) {
      processLog.forEach((s) => s.clearSelected());
    }
  }
  ...
  onResize(
    ...
    if (item.eleType === 'sheet' || item.eleType === 'processLog') {
      console.log(part.widthValue, 'part.widthValue');
      // part.widthValue = part.widthValue + 2;
      if (part.widthValue !== widthCopy) {
        item.columns.forEach((col: any) => {
          col.widthValue = Math.floor(
            (part.widthValue + 2) / item.columns.length,
          );
  ...
  getDrawerSize() {
    ...
    this.maxWidth =
      paperSize.widthTopx - 2 - margin.LeftTopx - margin.RightTopx - 30;
    this.maxHeight =
