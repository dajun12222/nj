  ...
  <div class="right-attr">
    <a-tabs :activeKey="activeLeft" @change="changeKey">
      <a-tab-pane key="1" tab="控件属性">
        ...
        <sheet-settings
          v-else-if="itemAttrs.eleType === 'sheet' || itemAttrs.parentId"
        />

        <sheet-settings
          v-else-if="itemAttrs.eleType === 'processLog'"
          :obj="processObj"
        />
        ...
        <template v-else-if="itemAttrs.eleType">
          <columnAttribute
            v-if="
              Object.keys(itemAttrs).length &&
              itemAttrs.eleType.includes('column')
            "
          />
          <textAttribute
            v-if="
              Object.keys(itemAttrs).length &&
              itemAttrs.eleType.includes('text')
            "
          />
          <picAttribute
            v-if="
              Object.keys(itemAttrs).length && itemAttrs.eleType.includes('Pic')
            "
          />
          <RectLine
            v-if="
              Object.keys(itemAttrs).length &&
              itemAttrs.eleType.includes('Drawer')
            "
          />
        </template>
...
export default class RightAttr extends Vue {
  ...
  onPropChange(p: schema.ControlProperty) {
    ...
    if (p.type === schema.ControlPropertyType.TextAlign) {
      ...
      p.type === schema.ControlPropertyType.Color ||
      ...
      const items = this.selecteds.filter(
        (x) => x.eleType.indexOf('Drawer') === -1,
      );
      ...
      this.setItemsValue(
        items.filter((x) => x.lineDirection === 'x'),
        'heightValue',
        p.value,
      );
      this.setItemsValue(
        items.filter((x) => x.lineDirection === 'y'),
        'widthValue',
        p.value,
      );
    } else {
