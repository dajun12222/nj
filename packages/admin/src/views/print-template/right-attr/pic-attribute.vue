...
<template>
  <div class="left-attr">
    <div
      v-show="itemAttrs.eleType.includes('leftQrcodePic')"
      class="left-item qr-code"
    >
      <p>二维码（QR）</p>
      <div class="select-input">
        <a-select v-model="itemAttrs.qrCreateOrShow" showSearch>
    ...
    <div
      v-show="itemAttrs.eleType.includes('leftBarcodePic')"
      class="left-item"
    >
      <p>条形码（code 128）</p>
      <div class="select-input">
        <a-select
          v-model="itemAttrs.barCodeKey"
          showSearch
          @change="barCodeKeyChange(itemAttrs.barCodeKey)"
        >
          <a-select-option value="formColumn">
            根据表单字段生成条形码
          </a-select-option>
          <a-select-option value="otherContent">
            根据其他内容生成条形码
          </a-select-option>
        </a-select>
      </div>
    ...
    <div v-show="itemAttrs.eleType.includes('topPic')" class="left-item">
      <p>图片</p>
      <div class="pic-div">
...
import { Button, Input, Select } from '@h3/antd-vue';
...
export default class LeftAttribute extends Vue {
  ...
  get barcodeItems() {
    if (this.bizSysData) {
      return this.bizSysData.filter(
        (x) => x.code === 'sequenceNo' || x.propertyType === 2,
      );
    }
    return [];
  }
  ...
  upLoadFile(e: File) {
    ...
    reader.onload = (readerObject: any) => {
      ...
      // @ts-ignore
      _this.setAttrsPicUrl({
        picUrl: readerObject.target.result,
        id: _this.itemAttrs.id,
      });
  ...
  barCodeKeyChange(value: string) {
    this.itemAttrs.barCodeValue =
      value === 'otherContent' ? '' : this.itemAttrs;
  }
}
</script>
