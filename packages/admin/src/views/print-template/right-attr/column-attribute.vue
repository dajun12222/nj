  ...
  <div class="left-attr">
    <div v-show="leftOrRight.includes('left')">
      <div class="left-item">
        ...
        <div v-show="whichRadio === 1" class="input-text">
          <a-input
            v-model="itemAttrs.leftKey.innerTxt"
            @change="txtChange($event, 'left')"
        ...
        >
          <a-select
            v-model="itemAttrs.leftKey.code"
            showSearch
            @change="
              (item) => {
                itemAttrs.leftKey.innerTxt = `\${ ${item.split('#_#')[2]} }`;
              }
            "
          >
            <a-select-option
      ...
      <div class="left-item">
        ...
        <div class="select-input">
          <line-size-dropdown
            :value="itemAttrs.borderWidthValue"
            @change="selectBorder($event, 'borderWidthValue')"
          />
      ...
      <div class="left-item">
        ...
        <div class="font-setting">
          <size-dropdown
            :value="itemAttrs.leftKey.fontSizeValue"
            @change="
              (e) => {
                itemAttrs.leftKey.fontSizeValue = e;
              }
            "
          />
    ...
    <div v-show="leftOrRight.includes('right')">
      <div class="left-item">
        ...
        <div v-show="whichRadio === 1" class="input-text">
          <a-input
            v-model="itemAttrs.rightValue.innerTxt"
            @change="txtChange($event, 'right')"
          />
        </div>
        <div v-show="whichRadio === 2" class="input-select">
          <a-select showSearch :value="rightCode" @change="onRightChange">
            <a-select-option
              v-for="item in bizSysData"
              :key="item.id"
              :value="item.code"
      ...
      <div class="left-item">
        ...
        <div class="font-setting">
          <size-dropdown
            :value="itemAttrs.rightValue.fontSizeValue"
            @change="
              (e) => {
                itemAttrs.rightValue.fontSizeValue = e;
              }
            "
          />
...
import { Radio, Input, Select } from '@h3/antd-vue';
...
export default class LeftAttribute extends Vue {
  ...
  txtChange(e: any, pos: string) {
    // @ts-ignore
    pos.includes('left')
      ? (this.itemAttrs.leftKey.code = '')
      : (this.itemAttrs.rightValue.code = '');
    const str: string = e.target.value;
    // @ts-ignore
    if (str.length > 500) {
      // @ts-ignore
      pos.includes('left')
        ? (this.itemAttrs.leftKey.innerTxt = str.substr(0, 500))
        : (this.itemAttrs.rightValue.innerTxt = str.substr(0, 500));
      this.$message.warning('长度不能超过500个字符！');
    }
    // @ts-ignore
    const height: number = document
      .querySelector('.two-text-div.activeChoose')
  ...
  selectBorder(e: any, border: string) {
    const part =
      this.itemAttrs[this.leftOrRight === 'left' ? 'leftKey' : 'rightValue'];
  ...
  onChange(item) {
    this.itemAttrs.leftKey.innerTxt = `\${ ${item.split('#_#')[2]} }`;
  }
}
</script>
