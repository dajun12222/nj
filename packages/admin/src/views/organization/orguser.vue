...
<template>
  <div class="org-user">
    <div class="sub-name">组织机构</div>
    <a-tabs defaultActiveKey="1" class="user-tabs" @change="tabChange">
      <a-tab-pane key="1" tab="在职人员">
        <div v-resize.east class="user-tree">
          <export-dept
          ...
          />
          <div v-if="hasCloudPivot" class="role-operation">
          ...
          <div class="user-title search__input">
            <h2 v-show="!isSearchTitle">
              ...
              <template v-else-if="curNodeState.enabled === false">
                <span style="color: rgba(17, 18, 24, 0.5)">{{
                  '(已禁用)'
                }}</span>
              </template>
            </h2>
            <div v-if="!isSearchTitle" class="add-operation">
              <!-- 20200426 ui改变 -->
              <template v-if="curNodeState.enabled">
                <a-dropdown v-show="personImportExport || deptPersonSync">
                  ...
                  <template slot="overlay">
                    <div class="btns-group">
                      <template v-if="personImportExport">
                        <div
                      ...
                      <template v-if="btnsDisplayPermission.batchOperation">
                      ...
                      <template v-if="deptPersonSync">
                        <div
                          v-show="btnsDisplayPermission.syncDepartment"
                          class="like-btn"
                          @click="() => (syncDepartVisible = true)"
                        >
                          同步部门
                        </div>
                        <div
		  ...
		  新建 
                  ...
                  <template slot="overlay">
                    <div class="btns-group">
                      <template>
                        <div
                          v-show="btnsDisplayPermission.createDepartment && curNodeState.extend1 !== 'ACCOUNT' "
                          class="like-btn"
                        ...
                        <div
                          v-show="btnsDisplayPermission.createUser && curNodeState.extend1 !== 'ACCOUNT' "
                          class="like-btn"
                          @click="showUser(0)"
                        >
                          新建用户
			...
			<div
                          v-show="btnsDisplayPermission.createUser && ( curNodeState.extend1 === 'ACCOUNT' || curNodeState.extend1 === 'ACCOUNT_DEPT')"
                          class="like-btn"
                          @click="showAccount(0)"
                        >
                          新建账套
		...
		<i class="icon aufontAll h-icon-all-plus-o op-icon"></i> 新建
          ...
          <div v-if="isSearchTitle" class="search-title">
            {{ searchTitle }}
          </div>
          <div v-show="noUser && !orgUserList.length" class="empty-box">
          ...
          >
            ...
            >
              ...
              >
                <a-checkbox
                  v-show="
                    showSelectAllBox || rowSelection.selectedRowKeys.length
                  "
                  v-model="isSelectAll"
              ...
              :trigger="['hover']"
                ...
                <i
                  ...
                  @click.self.stop="
                    curNodeState.enabled
                      ? showUser(1, record, 'PUSH')
                      : () => {}
                  "
                  >编辑</i>
                ...
                <i
                  v-else-if="
                    record.relatedSyncType === 'PULL' && record.deptNum > 1
                  "
                  :class="{
                    'disable-cursor': !curNodeState.enabled,
                  }"
                  @click.self.stop="
                    curNodeState.enabled
                      ? showUser(1, record, 'PULL')
                      : () => {}
                  "
                  >编辑</i>

                <!-- #40164 与测试人员沟通，与编辑按钮显示条件保持一致 原为 v-if="isCloudPivot" -->
                <!-- #62701 组织机构—管理员可重置钉钉/企微员工的密码 -->
                <!-- v-if="record.relatedSyncType === 'PUSH'" -->
                <!-- v-if="isCloudPivot" 罗立说这玩意没用了，删除 -->
                <template v-if="curNodeState.enabled">
                  <a-dropdown class="cursor" :trigger="['hover']">
                    <i
                      class="aufontAll h-icon-all-caret-down table-operation"
                      @click.stop="() => {}"
                      >更多</i>
                    <a-menu slot="overlay">
                ...
                <template v-else>
                  <i
                    class="aufontAll h-icon-all-caret-down table-operation disable-cursor"
                    @click.stop="() => {}"
                    >更多</i>
                </template>
              </div>
          ...
          <div v-show="!noUser" class="load-more">
            ...
            <span v-if="loadCompleted" class="text">{{
...
<!--新增/编辑账套-->
    ...
    <!-- 重置密码弹窗 -->
    <ResetPassword v-model="pwdVisible" :userData="pwdData" />
...
import AddAccount from "../../components/organization/accountSet/account.vue";
...
export default class OrgUser extends mixins(OrgMixins) {
  ...
  get filterDept() {
    const { relatedOrgType, corpId } = this.curNodeState;
  ...
  async exportPerson() {
    ...
    if ((res.errcode && res.errcode !== 0) || res.byteLength < 100) {
      ...
      const fileName = `人员导出${timer}.xlsx`;
  ...
  showDepartment(type: number, node?: any) {
    ...
    if (node) {
      switch(node['extend1']){
        case 'ACCOUNT':
          this.accountData = node;
          this.accountData.parentId = this.curDeptParentId;
  ...
  deleteDepartment() {
    this.isShowPop = false;
    const vm: any = this;
    const message = this.curNodeState.extend1 === 'ACCOUNT' ? '账套' : '部门';
    const tip = this.curNodeState.extend1 === 'ACCOUNT' ? `检测当前${message}下包含子${message}，请先删除子${message}` : `检测当前${message}下包含子${message}和用户，请先删除子${message}和用户` ;
    vm.$confirm({
      title: `确定删除此${message}吗？`,
      okText: vm.$t("languages.Apps.Ok").toString(),
      cancelText: vm.$t("languages.Apps.Cancel").toString(),
      onOk() {
        const params = {
        ...
        OrgApi.deleteDepartment(params).then((res: any) => {
          ...
          vm.$message.success(`删除${message}成功！`);
  ...
  showAccount(type: number, node: any, opType: string){
    this.userType = type;
    if (node) {
      this.userData = node;
    }
    this.accountVisible = true;
    console.log(this.accountVisible);
    // 新建用户
    if(type === 0) {
      // 新建用户取消禁用，非自维护组织不会有新建按钮
      this.bannedSwitch = false
  ...
  async syncPeople() {
    ...
    const { errcode, errmsg } = await orgApi.pushToDingTalk(users);
    if (errcode === 0) {
  ...
  async syncDepart() {
    ...
    const { errcode, errmsg } = await orgApi.pushToDingTalk(syncDepartSelected);
    if (errcode === 0) {
