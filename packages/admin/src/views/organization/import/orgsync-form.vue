    ...
    >
      <a-form class="orgsync-form" :form="form">
        ...
        >
          ...
          <div v-if="departmentError" class="ant-form-explain">
            同步部门不能为空
          </div>
        </a-form-item>
        <a-form-item
          v-if="
            (isSyncRoleScopeCheckboxShow && form.syncMethod !== 2) || shwoDept
          "
          label="其他同步项"
          class="other"
          :labelCol="formItemLayout.labelCol"
        ...
        >
          <a-checkbox
            ...
            v-show="form.syncMethod !== 2"
            v-model="form.isSyncRoleScope"
          >
            同步角色管理范围
          </a-checkbox>
          <a-checkbox v-if="shwoDept" v-model="form.isSyncDeptSetting">
            部门查看权限
          </a-checkbox>
          <a-checkbox v-model="form.isSyncDirector">
            员工直属主管
            <a-tooltip>
              <template slot="title">
                <span>开启后可同步员<br /></span>
                <span>工的直属主管<br /></span>
              </template>
...
export default class orgsyncForm extends Vue {
  ...
  orgSelectOptions: Array<any> = []; // 同步组织下拉选项
  ...
  toggleSyncMethod(e) {
    if (this.form.syncMethod === 1) {
      ...
      this.orgSelectOptions = this.orgSelectOptions.filter(
        (i) => i.relatedType !== 'CUSTOMIZE',
      );
    }
    this.resetDepartment();
  }
  ...
  getRelatedId() {
    ...
    for (let i = 0, len = this.orgSelectOptions.length; i < len; i++) {
      if (this.orgSelectOptions[i].corpId !== this.form.corpId) {
        continue;
      }
  ...
  getPartSyncParams() {
    ...
    for (let i = 0, len = departments.length; i < len; i++) {
      ...
      const item: any = {
        ...
        isSyncDirectManager: this.form.isSyncDirector ? 1 : 0,
      };

      if (i === 0) {
        item.corpId = value.corpId;
      }
  ...
  async getOrgsyncList() {
    ...
    if (res.data && res.data.length !== 0) {
      ...
      if (!this.form.corpId) {
        ...
        this.shwoDept =
          this.orgSelectOptions[0].relatedType === 'DINGTALK' ? true : false;
      } else {
        const orgSelectOptionsItem = this.orgSelectOptions.find(
          (o: any) => o.corpId === this.form.corpId,
        );
        this.shwoDept =
          orgSelectOptionsItem[0].relatedType === 'DINGTALK' ? true : false;
      }
    }
  }
  ...
  async submit() {
    ...
    if (this.form.syncMethod === 2) {
      ...
      params.isSyncDirectManager = this.form.isSyncDirector ? 1 : 0;
...
.other {
  padding-top: 16px;
  border-top: thin dashed rgba(17, 18, 24, 0.5);
