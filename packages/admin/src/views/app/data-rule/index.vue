    ...
    <div v-else>
      <template v-if="ruleLists.length > 0">
        <div class="add-rule">
          ...
          <a-button type="primary" icon="plus" @click="addDataRule">
        ...
        <div class="table-box">
          ...
          >
            ...
            <span slot="nameTitle" class="text-ellipsis resize">规则名称</span>
            <span slot="triggerActionTypeTitle" class="text-ellipsis resize">动作类型</span>
            <span slot="targetModelTitle" class="text-ellipsis resize">目标模型</span>
            <span slot="enabledTitle" class="text-ellipsis resize">是否启用</span>
      ...
      <div v-else class="on-data-wrap">
        ...
        <no-data tip="暂无数据规则，请点击新建按钮新建" @click="addDataRule" />
      </div>
    </div>
...
import { Button, Table, Switch, Spin, Drawer } from '@h3/antd-vue';
  ...
  async getList() {
    ...
    if (res.errcode === 0) {
      ...
      data.forEach((item: any, index: number) => {
        item.key = index;
        item.index = index + 1;
        item.targetModel = `${item.targetSchemaName || '待提供'}[${
          item.targetSchemaCode
        }]`;
        item.triggerActionType = this.actionTypeTranslater(
          item.triggerActionType,
        );
      });
      this.ruleLists = data;
      console.log('--------------------');
