...
export default class DataRuleHandler extends Vue {
  ...
  triggerObjLists: Array<any> = []; // 触发对象列表，模型以及模型下的子表集合

  upDateObjLists: Array<any> = []; // 更新对象列表
  ...
  abledDataItemTypes: Array<number> = [
  ...
  get isNextAble() {
    const {
      ...
      isShowErrRuleName,
      dataConditionData,
      dataConditionJoinType,
    } = this;
    let isDataOk: boolean = false;
    if (dataConditionData && dataConditionData.length > 0) {
      isDataOk = dataConditionData.every((item: any) => {
        if (
          item.filterCondition.val ===
            DataRuleType.DataConditionFilterType.EP ||
          item.filterCondition.val === DataRuleType.DataConditionFilterType.NEP
        ) {
          return (
            item.dataItem.val && item.filterCondition.val && !item.value.errMsg
          );
        } else {
          return (
            item.dataItem.val &&
  ...
  setCurFieldsList(propertyType: number, code: string) {
    ...
    if (propertyType === -1) {
      ...
      const subSchemaTem = dataItemsCopy.find(
        (item: any) => item.code === sheetCode,
      );
      const subSchemaMain = dataItemsCopy.filter(
        (res) => res.propertyType !== 8 && res.published,
      );
      if (subSchemaTem.subSchema) {
        list = subSchemaTem.subSchema.properties
      ...
      if (subSchemaTem.subSchema) {
        list = subSchemaTem.subSchema.properties.filter(
  ...
  selectTriggerAction(index: number) {
    ...
    DataCache.setCacheData(cacheKey, {
      filterData,
      dataActionData,
      ruleScopeChild,
      ruleActionMainScope,
  ...
  targetTableChange(val: any) {
    const selectData: any = this.upDateObjLists.find(
      (res: any) => res.code === val,
    );
  ...
  deepCopyRowData() {
    ...
    const dataItems_tem = this.curModelDataItems
      .filter((item: any) => disableTypes.indexOf(item.propertyType) < 0)
    ...
    const dataFilterConditionListCopy = Helper.jsonDeepCopy(
      this.dataFilterConditionList,
    );
  ...
  addDataCondition() {
    ...
    if (this.dataConditionData.length > 0) {
      const isExistIllegleTime: boolean = this.dataConditionData.some(
        (item: any) => item.value.errMsg,
      );
    ...
    const { dataItemsCopy, dataFilterConditionListCopy, currentSheetList } =
  ...
  validateSheetScope() {
    const list = this.filterData.filter((item: any) => {
      return (
        (item.currentField.val && item.targetField.val) ||
        (item.targetField.val && item.value.val === 3)
      );
    });
    if (list.length > 0) {
      return true;
  ...
  addDataAction(isSetDefault: boolean) {
    if (
      !isSetDefault &&
      this.UpDateObjIsSheet &&
      this.isShowActionSysDataItems &&
      !this.validateMainScope()
    ) {
  ...
  setCurrentDataItems(
    ...
    code: string,
    rowData: any,
    Datatype: DataRuleType.DataType,
  ) {
    // ;
    // 清空目前模型的值
    ...
    if (
      ...
      if (ctargetCode && ctargetCode.relativeCode === bizSchemaCode) {
        ...
        const idData = list.filter((item: any) =>
          filterCode.includes(item.code),
        );
        arr.push(...idData);
      }
    }
      ...
      arr = list.filter((item: any) => {
        ...
        return (
          (item.propertyType === type ||
            ((item.propertyType === DataItemType.RelevanceForm ||
              item.propertyType === DataItemType.RelevanceFormEx) &&
              item.relativeCode === curModel.code)) &&
    ...
    const filterIdAndParentId = [
      `${this.triggerVal}.parentId`,
      `${this.triggerVal}.id`,
      'id',
    ];
  ...
  setValue(
    ...
    code: string,
    rowData: any,
    operator?: DataRuleType.ConditionType,
  ) {
    // ;
  ...
  handleOperatorChange(params: DataRuleType.OperatorChangeParams) {
    ...
    const data: any =
      curDataType === DataRuleType.DataType.Filter
        ? this.filterData
        : this.dataActionData;
    const operatorEqualType: number =
      curDataType === DataRuleType.DataType.Filter
        ? DataRuleType.ConditionType.Equal
        : DataRuleType.ActionType.Equal;

    if (value === operatorEqualType) {
      // 操作符等于的时候
      // 值类型为为空的选项可点击
      ...
      data[index].targetField.list = targetFieldsCopy.filter(
        (item: any) => item.propertyType === DataItemType.Number,
      );
      const curTargetFieldVal: string = data[index].targetField.val;
      if (curTargetFieldVal) {
        const tarFieldProperty = targetFieldsCopy.find(
          (item: any) => item.code === curTargetFieldVal,
        ).propertyType;
      ...
      data[index].currentField.list = currentFieldCopy.filter(
        (item: any) => item.propertyType === DataItemType.Number,
      );
      const curFieldVal: string = data[index].currentField.val;
      if (curFieldVal) {
        const curFieldProperty = currentFieldCopy.find(
          (item: any) => item.code === curFieldVal,
        ).propertyType;
      ...
      if (data[index].value.val === DataRuleType.ValueType.FixedValue) {
        data[index].targetField.list = data[index].targetField.list.filter(
          (item: any) =>
            this.abledDataItemTypes.indexOf(item.propertyType) > -1,
        );
        data[index].currentField.list = data[index].currentField.list.filter(
          (item: any) =>
            this.abledDataItemTypes.indexOf(item.propertyType) > -1,
        );
      }
  ...
  ruleJsonFormater() {
    ...
    const ruleScopeChildJson: any = [];
    // 主表过滤条件
    const ruleActionMainScopeJson: any = [];
    ...
    this.filterData.forEach((item: any) => {
      if (item.targetField.val && item.currentField.val) {
        ...
        item.value.val === DataRuleType.ValueType.ToBeNull &&
    ...
    this.dataActionData.forEach((item: any) => {
      if (item.targetField.val && item.currentField.val) {
        ruleActionJson.push({
          ...
          actionType:
            this.triggerAction === DataRuleType.TriggerActionType.Add
              ? DataRuleType.TriggerActionType.Add
              : item.operator.val,
          valueType: item.value.val,
          triggerPropertyCode: item.currentField.val,
        });
    ...
    this.dataConditionData.forEach((item: any) => {
      const currentData = item.dataItem.list.find(
        (res) => res.code === item.dataItem.val,
      );
  ...
  validateData(type: string): boolean {
    ...
    const data: any =
      type === DataRuleType.DataType.Filter
        ? this.filterData
        : this.dataActionData;
    for (let i = 0; i < data.length; i += 1) {
      const item = data[i];
      if (
        (item.targetField.val === '' || item.currentField.val === '') &&
        item.value.val !== DataRuleType.ValueType.ToBeNull
      ) {
        isDataOk = false;
        break;
  ...
  submit() {
    ...
    if (triggerAction === DataRuleType.TriggerActionType.Update) {
      ...
      const isFilterDataOk: boolean = this.validateData(
        DataRuleType.DataType.Filter,
      );
      if (!isFilterDataOk) {
        this.$message.warning('目标模型数据字段或当前模型数据字段不能为空');
        return false;
      ...
      const isActionDataOk: boolean = this.validateData(
        DataRuleType.DataType.Action,
      );
      if (!isActionDataOk) {
        this.$message.warning('目标模型数据字段或当前模型数据字段不能为空');
        return false;
  ...
  async postData(type: DataRuleType.OperationMode) {
    const isAdd = type === DataRuleType.OperationMode.Add;
  ...
  async dataBackWriting() {
    if (this.mode === DataRuleType.OperationMode.Edit) {
      ...
      const {
        ...
        ruleScopeChildJson,
        ruleActionMainScopeJson,
        insertConditionJoinType,
        triggerSchemaCodeIsGroup,
      } = this.data;
      this.searchCondition = conditionJoinType;
      this.ruleName = name;
      this.targetTableCode = targetTableCode;

      this.insertConditionJoinType = insertConditionJoinType
        ? insertConditionJoinType
        : 1;
      //
      // 模型回写
      ...
      this.triggerObj = triggerSchemaCodeIsGroup
        ? `${sourceSchemaCode}&&${triggerSchemaCode}`
        : triggerSchemaCode;
      let ptype: number = 0;
      ...
      const { dataItemsCopy, dataFilterConditionListCopy } =
        this.deepCopyRowData();
      ...
      if (this.dataConditionJoinType !== 1) {
        ...
        dataConditionData.forEach((item: any, index) => {
          ...
          // 判断展示类型
          // 1. 判断当前字段是系��字段还是业务字段
          const curRowDataItem: any = this.currentFields.find(
            (child: any) => child.code === triggerPropertyCode,
          );

          const k: any = curRowDataItem.defaultProperty
            ? triggerPropertyCode
            : curRowDataItem.propertyType;
          ...
          const filterConditoinList: any = dataFilterConditionListCopy.filter(
            (filterCondition: any) =>
              filterMap.indexOf(filterCondition.index) > -1,
          );

          // 3. 获取展示类型
          const displayType: number = Helper.getDisplayType(
            k,
            dataConditionType,
          );
      ...
      await this.selectModel({
        code: targetSchemaCode,
        selectTrace: chooseAction,
        title: targetSchemaName || '',
      });
      ...
      // 主表过滤条件回写
      this.filterData = this.initRunTimeData(
        filterData,
        targetFieldsCopy,
        DataRuleType.DataType.MainCondition,
      );
      // 查找主表
      if (ruleActionMainScopeJson) {
        const ruleActionMainScope = JSON.parse(ruleActionMainScopeJson);
        this.ruleActionMainScope = this.initRunTimeData(
          ruleActionMainScope,
          targetFieldsCopy,
          DataRuleType.DataType.QueryMain,
        );
      }
      // ;
      ...
      if (ruleScopeChildJson) {
        ...
        this.ruleScopeChild = this.initRunTimeData(
          ruleScopeChild,
          targetItems,
          DataRuleType.DataType.SheetCondition,
        );
      }
      // 数据动作
      this.dataActionData = this.initRunTimeData(
        dataActionData,
        targetItems,
        DataRuleType.DataType.DataAcition,
      );
  ...
  getCurrentSubSchema() {
    const { targetFieldsCopy } = this.deepCopyRowData();
    const sheet: any = targetFieldsCopy.find(
      (innerItem: any) => innerItem.code === this.targetTableCode,
    );
  ...
  initRunTimeData(
    ...
    dataItem.forEach((item: any) => {
      ...
      if (type === DataRuleType.DataType.DataAcition) {
        list = list.filter(
          (innerItem: any) =>
            codeArr.includes(innerItem.code) || !innerItem.defaultProperty,
        );
      ...
      const curFieldsList = currentFieldCopy.filter(
        (innerItem: any) => innerItem.published,
      );
  ...
  getLogicType(targetList: any) {
    ...
    const item: any = targetList.list.find(
      (child: any) => child.code === targetList.val,
    );
    if (!item) {
      return;
    }
  ...
  onDataItemChange(rowData: any) {
    ...
    const isDefaultProperty = currentItem.defaultProperty;
    const propertyType = currentItem.propertyType;
    ...
    // 获取当前行过滤条件类型
    const curRowFilterCondition: any =
      this.dataConditionData[index].filterCondition;
    const curRowFilterVal: any = curRowFilterCondition.val;

    // 过滤过滤条件
    const list: any = dataFilterConditionListCopy.filter(
      (filterCondition: any) => filterMap.indexOf(filterCondition.index) > -1,
    );
    ...
    // 类型变化，值清空
    if (propertyType === DataItemType.StaffSingle) {
  ...
  onFilterConditionChange(value: any, vNode: any) {
    ...
    const curRowDataItemPtype: number = this.dataConditionData[
      index
    ].dataItem.list.find((item: any) => item.code === dataItemVal)
      .propertyType as number;
    const isDefault: boolean = this.dataConditionData[index].dataItem.list.find(
      (item: any) => item.code === dataItemVal,
    ).defaultProperty as boolean;
    const dataItemType: number | string = isDefault
      ? dataItemVal
      : curRowDataItemPtype;
  ...
  onIsShowDataRulerChange(v: boolean) {
    // 打开抽屉就加载最新的数据项
    if (v) {
