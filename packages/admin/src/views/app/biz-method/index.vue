  ...
  <div class="biz-method">
    ...
    <div v-else class="biz-method__main">
      <div class="biz-method__buttons">
        ...
        <a-button icon="plus" type="primary" @click="addMethod">
      ...
      <div class="biz-method__table">
        ...
        >
          ...
          <span slot="methodCountTitle" class="resize">{{
            $t('languages.Apps.BindService')
          }}</span>
          <span slot="descriptionTitle" class="resize">{{
            $t('languages.Apps.Description')
          }}</span>
          <span slot="actionTitle" class="resize">{{
            $t('languages.Apps.Action')
          }}</span>
          <!-- 表体 -->
          <span slot="description" slot-scope="text" class="text-ellipsis">{{
            text
          }}</span>
          <span
            slot="action"
...
import { Table, Spin, Button, Drawer } from '@h3/antd-vue';
    ...
    this.$confirm({
      ...
      onOk() {
        ...
        BizMethodApi.deleteBizMethod(params).then((res: any) => {
          if (!res.errcode) {
            vm.list = vm.list.filter(
              (method: BizMethod.MethodItem) => method.code !== record.code,
            );
          } else if (res.errcode === 500109) {
            vm.$message.error(
              '当前方法已被流程绑定使用，请先删除绑定使用！',
              2,
            );
          } else {
