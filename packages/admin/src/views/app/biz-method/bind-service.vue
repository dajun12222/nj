      ...
      <div class="bind-server__form">
        <div class="bind-server__item">
          ...
          <p>
            ...
            <a-select v-else v-model="serviceCode" @change="selectService">
              <a-select-option
        ...
        <div v-if="serviceCode" class="bind-server__item">
          <span>集成方法：</span>
          <p>
            <template v-if="isEdit">
              {{ service.serviceMethodName }}
            </template>
            <a-select v-else v-model="serviceMethodCode" @change="selectMethod">
              <a-select-option
...
<script lang="ts">
import { Select, Button } from '@h3/antd-vue';
...
export default class BindServer extends Vue {
  ...
  getOptions() {
    Promise.all([
      ...
      if (Array.isArray(dataItems.data)) {
        ...
        this.dataItemList = dataItems.data.filter(
          (item: any) =>
            ![6, 7].includes(item.propertyType) && !item.defaultProperty,
        );
        this.inputDataItemList = dataItems.data.filter(
          (item: any) => ![6, 7].includes(item.propertyType),
        );
      }
    });
  }
  ...
  combineBizPropertyType() {
    const currentMethod: any = this.methodList.find(
      (item: BizService.Method.Item) => item.code === this.serviceMethodCode,
    );
    if (!currentMethod) {
      return;
    }
    ...
    const setBizProperty = (
      mappingString: string,
      targetMapping: Array<any>,
    ) => {
          ...
          mappings.some((item: any) => {
            if (item.code === mapItem.serviceMethodParameterCode) {
              vm.$set(
                mapItem,
                'bizPropertyTypeName',
                getDataItemText(item.bizPropertyType),
              );
              return true;
            }
          });
