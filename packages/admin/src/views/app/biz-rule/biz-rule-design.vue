  ...
  <div class="workflow">
    <div class="workflow-head" @click="closeContextmenu">
      <div class="workflow-head-l">
        <span class="workflow-name">
          <i
            ...
            @click="backDataModel"
          ></i>{{ workflowData.name || '未命名规则' }}</span>
      </div>
      ...
      <div class="workflow-head-r">
        <a-tooltip placement="top">
          <template slot="title"> 检测 </template>
          <span
            class="workflow-btn"
            style="color: rgba(17, 18, 24, 0.5); background: #fff; border: none"
    ...
    >
      <div class="checkRuleBox">
        <p v-if="_checkResult.length === 0" class="tip_summary">
        ...
        <template v-else>
          ...
          <ol class="tip_detail">
            ...
            <li v-if="errList.functionNoData.length">
              {{
                errList.deadCycleRunMapList.length
                  ? errList.modelNoData.length
                    ? '3'
                    : '2'
                  : errList.modelNoData.length
                  ? '2'
    ...
    <div class="workflow-content">
      <!-- 节点仓库 -->
      <div class="workflow-content-left" @click="closeContextmenu">
        <h3-sider
          :options="leftPanel"
      ...
      <!-- 节点属性/流程属性 -->
      <div class="workflow-content-right" @click="closeContextmenu">
        <h3-sider
          :options="rightPanel"
          class="workflow-panel"
          @toggle="doAutoFit"
        >
          <biz-rule-property ref="bizRuleProperty" :dataItems="dataItems" />
...
import { Modal, Icon, Button, Tooltip } from '@h3/antd-vue';
...
export default class Workflow extends Vue {
  ...
  @BizRuleModule.Action('postNodeRunMaps') postNodeRunMaps: any;
  ...
  revoke() {
    ...
    /* (this.$refs.bizRuleContent as any).workflowSpaceDoDown(''); */
    Bus.$emit('autoFit');
  }
  ...
  _submitVerify(params) {
    if (!this._verifyCode(params.code)) {
      return false;
    } // 校验规则编码

    if (!this._verifyName(params.name)) {
      return false;
    }

    if (!this._verifyBizRule()) {
      return false;
    } //校验规则

    if (!this._verifyActivities()) {
      return false;
    } // 校验节点的合法性

    if (!this._verifyNode(params.node)) {
      return false;
    }

    if (!this._verifyLine(params)) {
      return false;
    }
  ...
  _verifyLine(params: any) {
    ...
    for (let i = 0; i < logicNode.length; i++) {
      ...
      const nodeInfo = `【${logicNode[i].nodeName}[${logicNode[i].nodeCode}]】`;
   ...
   */
  _verifyActivities() {
    for (const activeNode of this.activities) {
      ...
      const verifyCode = activeNode.nodeType.toUpperCase();
      const nodeInfo = `【${activeNode.nodeName}[${activeNode.nodeCode}]】`;
      switch (verifyCode) {
        case 'START':
        case 'END':
          break;
        case 'ASSIGN':
          {
            const { triggerObjectCode, filterCondition, dataActions } =
              activeNode;
        ...
        case 'DATA_CHECK':
          {
            ...
            if (
              ...
              (dataCondition.conditions &&
                dataCondition.conditions.length === 0)
            ) {
        ...
        case 'MESSAGE':
          {
            ...
            if (
              !(
                (receiver.departments && receiver.departments.length) ||
                (receiver.roles && receiver.roles.length) ||
                (receiver.selections && receiver.selections.length) ||
                (receiver.users && receiver.users.length)
              )
            ) {
              this.$message.error(`${nodeInfo} 接收人未设置`);
              return false;
        ...
        case 'LOGIC_VERIFY':
          {
            ...
            if (
              !judgeCondition.conditions ||
              judgeCondition.conditions.length === 0
            ) {
            ...
            if (
              ...
              verifyConditions.conditions.length === 0
            ) {
          ...
          break;
        case 'GET_LIST':
          const { dataSourceType, methodMapping } = activeNode;
  ...
  checkRule() {
    ...
    const checkLoading = this.$message.loading('检测中', 0);
    this.postNodeRunMaps(params).then((res) => {
      if (res.errcode === 0) {
  ...
  get _checkResult() {
    const errorRes = this.nodeRunMaps.filter((el) => {
      if (
      ...
      if (
        Array.isArray(el.deadCycleRunMapList) &&
        el.deadCycleRunMapList.length === 0
      ) {
        el.notExistPropertyObjectList = null;
      }
  ...
  get errMap() {
    ...
    this._checkResult.forEach((el: any) => {
      // @ts-ignore
      el.notExistPropertyObjectList &&
        errMap.notExistPropertyObjectList.push(el);
      // @ts-ignore
...
@import '~@/components/apps/form-design/style/form-design-scoped.less';
</style>
...
.checkRuleBox {
  ...
  & > div.board {
    width: 100%;
    height: ~'min(562px, calc(100vh - 240px))';
    overflow: scroll;
  }
}
</style>
