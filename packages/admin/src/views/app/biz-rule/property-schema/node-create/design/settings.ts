...
若有问题，请参考前端相关二开文档。
*/
import { ComponentType } from '@/typings/component-type';
import { ObjectPropertyInfo } from 'cloudpivot-designer/designer-core';
  ...
  subscription: {
    ...
    dataChange: function (
      ...
      if (key === 'triggerObjectCode' && value) {
        controller.children.childTriggerConditionType.display =
          value.indexOf('.') > 0;
      }

      if (key === 'targetSchema' && value.schemaCode) {
        controller.children.targetObjectCode.display = true;
      } else if (key === 'targetSchema' && !value.schemaCode) {
        controller.children.targetObjectCode.display = false;
      } else {
      ...
      //     controller.children.createDataAction.value = {};
      //     controller.children.dataCondition.value = {};
