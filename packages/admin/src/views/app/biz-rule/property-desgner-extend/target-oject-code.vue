...
export default class TargetOjectCode extends Mixins(PropertyComponent) {
  ...
  created() {
    ...
    this.controller.children.targetSchema.propertyChange.subscribe(
      (change: any) => {
  ...
  loadOptions() {
    const triggerObjectCode: string =
      this.controller.children.triggerObjectCode.value;
    const val = this.controller.children.targetSchema;
    ...
    if (this.optionsMap[triggerObjectCode + schemaCode]) {
      this.triggerObjLists = [
        ...this.optionsMap[triggerObjectCode + schemaCode],
      ];
      return;
    }
        ...
        AppsApi.getDataItemList({ schemaCode: schemaCode }).then((item) => {
          if (item.errcode === 0) {
            const sheetData = item.data
              .filter((dataItem) => dataItem.propertyType === 8)
              .map((i) => {
  ...
  initData() {
    if (
      this.triggerObjLists.length &&
      (!this.theVale || this.theVale === '-1')
    ) {
