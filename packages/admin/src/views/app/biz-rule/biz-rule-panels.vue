...
<template>
  <div class="acitivity-model-contain">
    <div class="acitivity-model-contain-title">动作节点</div>
    <template v-for="(item, index) in actionNode">
    ...
    <div class="acitivity-model-contain-title">逻辑节点</div>
    <template v-for="(item, index) in logicNode">
      <biz-rule-panels-item
        :key="index + 6"
        v-h3-drag
        :class="{ disabled: item.disabled }"
...
export default class BizRulePanels extends Vue {
  ...
  acitivityModel: Array<any> = [];

  get logicNode() {
    const actionNodeType = [
      NodeType.LogicVerify,
      NodeType.DataCheck,
      NodeType.ExistVerify,
    ];
    return this.acitivityModel.filter((item) => {
      return actionNodeType.indexOf(item.nodeType) > -1;
    });
  }
    ...
    if (val) {
      switch (val.code) {
        ...
        case 'Load':
          this.acitivityModel.forEach((m: any) => {
            if (
              m.nodeType === NodeType.Create ||
              m.nodeType === NodeType.Update ||
              m.nodeType === NodeType.Delete ||
              m.nodeType === NodeType.LogicVerify ||
              m.nodeType === NodeType.DataCheck ||
              m.nodeType === NodeType.ExistVerify ||
              m.nodeType === NodeType.Assign
            ) {
              m.disabled = true;
            }
          });
