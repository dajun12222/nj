      ...
      <div class="centent">
        ...
        <div class="filters">
          ...
          <a-row style="margin-top: 10px">
            ...
            <a-col :span="5" class="fieldlab" style="margin-left: 5px">
              对比函数
            </a-col>
            <a-col :span="4" class="fieldlab" style="margin-left: -5px">
              值类型
            </a-col>
          ...
          <a-row
            v-for="(row, index) in filtersList.main"
            :key="index"
          ...
          >
            ...
            <a-col :span="5">
              ...
              >
                <a-select-option
                  v-for="(type, i) in row.conditionTypeList"
                  :key="i"
                  :value="type.code"
                  :title="type.name"
                >
            ...
            <a-col :span="7">
              ...
              <template v-if="row.ruleValueType === 'FIXED' && row.visible">
                ...
                <div
                  v-else-if="[5, 50, 51, 60, 61].includes(row.propertyType)"
                  style="margin-left: -5px"
                >
                  <staff-selector
                    v-model="row.value"
                    :options="getStaffSelectorOpts(row.propertyType)"
                  />
                </div>
        ...
        <!--子表条件-->
        <div v-if="subSchemaCode" class="filters">
          <a-row style="margin-top: 15px">
...
export default class PropertyFilterCondition extends Mixins(PropertyComponent) {
  ...
  get subSchemaCode() {
    return this.controller.children.triggerObjectCode.value.split('.')[1];
  }
  ...
  tips: string = '满足以下条件的数据将执行数据赋值动作';
  ...
  initModal() {
    ...
    if (this.controller.value && this.controller.value.filterCondition) {
      ...
      if (Array.isArray(value.mainTableConditions)) {
        if (value.mainTableConditions.length === 0) {
          this.filtersList.main = [];
        } else {
          this.filtersList.main = BizRule.deepCopy(value.mainTableConditions);
        }
      ...
      if (Array.isArray(value.childTableConditions)) {
        ...
        if (this.filtersList.child.length > 0) {
          this.filtersList.child.forEach((res, index) => {
            const targetSchemaDataItem = res.targetSchemaDataItem.split('.');
            const currentSchemaDataItem = res.currentSchemaDataItem.split('.');
            res.targetSchemaDataItem =
              targetSchemaDataItem[targetSchemaDataItem.length - 1];
            res.currentSchemaDataItem =
              currentSchemaDataItem[currentSchemaDataItem.length - 1];
            this.targetDataItemChange(
              res.targetSchemaDataItem,
              index,
              'child',
  ...
  removeNotExistItem(type: string) {
    ...
    type === 'main' &&
      this.filtersList[type].length === 0 &&
      this.addRows(type);
  }
  ...
  getDataItems() {
    ...
    if (this.subSchemaCode) {
      ...
      this.targetFieldList.child.forEach((res: any) => {
        const code = res.code.split('.');
        res.code = code[code.length - 1];
      });
      const vals = this.trigerObject.trigerObj.split('.');
      this.currentFieldList.child = this.dataItems
        .find((d) => d.code === this.subSchemaCode)
        .subSchema.properties.filter(
          (val) =>
            val.propertyType !== DataItemType.Attachment &&
      ...
      if (childData && childData.subSchema && childData.subSchema.properties) {
        childData.subSchema.properties.map((c: any) => {
          c.code = !c.code.includes('.') ? `${vals[1]}.${c.code}` : c.code;
  ...
  modalOk() {
    ...
    if (Array.isArray(value.mainTableConditions)) {
      value.mainTableConditions.forEach((item) => {
        if (item.targetSchemaDataItem) {
          const currentItem: any = this.targetFieldList.main.find(
            (el) => el.code === item.targetSchemaDataItem,
          );
          if (currentItem) {
            item.targetSchemaDataItemName = currentItem.name;
          }
        }
        if (item.currentSchemaDataItem) {
          const currentItem: any = item.currentFieldList.find(
            (el) => el.code === item.currentSchemaDataItem,
          );
          if (currentItem) {
            item.currentSchemaDataItemName = currentItem.name;
          }
        }
      });
    }
    ...
    if (list.length > 0) {
      for (const item of list) {
        if (item.ruleValueType === '') {
          ...
          item.ruleConditionType === 'EP' ||
          item.ruleConditionType === 'NEP'
        ) {
          item.value = '';
          item.currentSchemaDataItem = '';
          item.error = true;
  ...
  setRuleValueTypeList(targetItem, index, type, isInit) {
    ...
    if (
      ...
      this.filtersList[type][index].ruleValueTypeList =
        this.ruleValueTypeList.filter((val) => val.id === 'DYNAMIC');
      return;
    } else if (
      [
  ...
  setCurrentFieldList(targetDataItem, index, type) {
    if (type === 'child') {
      if (targetDataItem.code === SystemDataItemCode.Id) {
        this.filtersList[type][index].currentFieldList = this.currentFieldList[
          type
        ].filter((val) => {
          return (
    ...
    switch (targetDataItem.propertyType) {
      ...
      case DataItemType.DropdownMulti:
        this.filtersList[type][index].currentFieldList = this.currentFieldList[
          ...
          ].includes(targetDataItem.propertyType)
            ? [
                DataItemType.ShortText,
                DataItemType.Radio,
                DataItemType.Dropdown,
              ].includes(val.propertyType)
      ...
      // if(arr.some((v) => v === targetDataItem.propertyType))
      //   (val.propertyType === DataItemType.ShortText || val.propertyType === DataItemType.Radio ||
      //     val.propertyType === DataItemType.Checkbox ||val.propertyType === DataItemType.Dropdown)
      // 关联表单需要映射 相同的数据模型
      case DataItemType.RelevanceForm:
        // 判断选中的关联表单是否关联的当前模型
        ...
        if (arry.length === 0) {
          // 主表触发主表
          this.filtersList[type][index].currentFieldList =
            this.currentFieldList[type].filter(
  ...
  operatorChange(val: any, index, type: string) {
    ...
    if (isVisible !== undefined) {
      console.log(isVisible.id === Eexpr.EP || isVisible.id === Eexpr.NEP);
  ...
  setValueCtrl(
    ...
    const currentRowData = this.filtersList[type][index];
    ...
    currentRowData.visible = !(
      currentRowData.ruleConditionType === 'EP' ||
      currentRowData.ruleConditionType === 'NEP'
    );
    // 根据Item值生成对应的控件
