  ...
  <div class="div-height">
    ...
    >
      ...
      <div class="centent">
          ...
          >
            ...
            <a-col :span="3">
              <a-select
                ...
                class="data-filter"
                placeholder="请选择"
                @change="operatorChange(row.conditionType, index, 'main')"
              >
                <a-select-option
            ...
            <a-col :span="7">
              <div v-if="row.valueType === 'DYNAMIC'">
                <data-item-select
                  v-model="row.value"
                  :onlyPublished="true"
              ...
              <template v-if="row.valueType === 'FIXED'">
                ...
                <div v-else-if="row.propertyType === 12">
                  <!-- <a-time-picker
                    ...
                    format="HH:mm:ss"
                    :defaultValue="
                      moment(row.value, 'HH:mm:ss', index, 'main', 1)
                    "
                    placeholder="请选择时间"
                  /> -->
                  <a-input
        ...
        <!--子表条件-->
        <div
          v-if="trigerObject.trigerType === 1 || trigerObject.trigerType === 3"
          class="filters"
        >
          <a-row style="margin-top: 15px">
          ...
          >
            ...
            <a-col :span="3">
              <a-select
                ...
                class="data-filter"
                placeholder="请选择"
                @change="operatorChange(row.conditionType, index, 'child')"
              >
                <a-select-option
                  v-for="(type, i) in row.conditionTypeList"
                  :key="i"
              ...
              <template v-if="row.valueType === 'FIXED'">
                ...
                <div v-else-if="row.propertyType === 12">
                  <a-date-picker
                    ...
                    format="HH:mm:ss"
                    :defaultValue="
                      moment(row.value, 'HH:mm:ss', index, 'child')
                    "
                    placeholder="请选择时间"
                ...
                <div v-else-if="row.propertyType === 4">
                  <a-select v-model="row.value" showSearch style="width: 220px">
                    <a-select-option key="1" value="true">
                      true
                    </a-select-option>
                    <a-select-option key="2" value="false">
...
interface IFilterCondition {
  ...
  /**主表条件 */
  itemList: Array<ITableCondition>;
  /**子表条件 */
...
export default class PropertyFilterCondition extends Mixins(PropertyComponent) {
  ...
  watchValue(val) {
    this.emitChange(val);
    if (this.value.itemList && this.value.itemList.length) {
      this.inputValue = '已设置';
    } else {
      this.inputValue = '未设置';
    }
  ...
  async initModal() {
    ...
    let schemaCode = this.selectedActivities[0].targetSchema.schemaCode;
    if (trigerObj && targetObj && trigerObj.value && targetObj.value) {
      ...
      // 主表触发主表
      if (!trigerBool && !targetBool) {
        this.trigerObject.trigerType = 0;
      }
      // 主表触发子表
      if (!trigerBool && targetBool) {
        this.trigerObject.trigerType = 1;
      }
      // 子表触发主表
      if (trigerBool && !targetBool) {
        this.trigerObject.trigerType = 2;
      }
      // 子表触发子表
      if (trigerBool && targetBool) {
        this.trigerObject.trigerType = 3;
      }
    }

    // 编辑时候
    schemaCode && (await this.getDataItems(schemaCode));
    if (this.controller.value && this.controller.value.existVerifyCondition) {
      const value = this.controller.value.existVerifyCondition;
      if (Array.isArray(value.itemList)) {
        value.itemList.length === 0
          ? (this.filtersList.main = [])
          : (this.filtersList.main = BizRule.deepCopy(value.itemList));
      ...
      if (this.trigerObject.trigerType === 0) {
        ...
        if (
          ...
          this.targetFieldList.child =
            sheetDataItem.subSchema.properties.filter(
              (val) =>
                val.propertyType !== DataItemType.Attachment &&
  ...
  modalOk() {
    ...
    const value = {
      ruleConditionJoinType: this.ruleConditionJoinType,
      itemList: BizRule.deepCopy(this.filtersList.main),
      childTableConditions: BizRule.deepCopy(this.filtersList.child),
    };
    if (Object.getOwnPropertyNames(value.itemList).length === 0) {
      value.itemList = [];
    }
    if (Object.getOwnPropertyNames(value.childTableConditions).length === 0) {
      value.childTableConditions = [];
    }
    ...
    if (Array.isArray(value.itemList)) {
      value.itemList.forEach((item: any) => {
        if (item.targetSchemaDataItem) {
          //
          const current = this.targetFieldList.main.find(
            (el) => el.code === item.targetSchemaDataItem,
          );
          if (current) {
            item.targetSchemaDataItemName = current.name;
          }
        }
        if (item.value) {
          const current = item.currentFieldList.find(
            (el) => el.code === item.value,
          );
          if (current) {
  ...
  setRuleValueTypeList(targetItem, index, type, isInit) {
    if (!isInit) {
      this.filtersList[type][index].valueType = 'DYNAMIC';
    }
    if (
    ...
    if (type === 'child') {
      if (targetDataItem.code === SystemDataItemCode.Id) {
        this.filtersList[type][index].currentFieldList =
          this.currentFieldList.main.filter((val) => {
            return (
      ...
      case DataItemType.RelevanceForm:
        ...
        if (arry.length === 0) {
          ...
          if (this.trigerObject.trigerType === 0) {
            ...
            // 子表触发主表
            this.filtersList[type][index].currentFieldList =
              this.currentFieldList.main.filter(
                (val) =>
                  (targetDataItem.relativeCode === val.relativeCode &&
                    val.propertyType === targetDataItem.propertyType) ||
                  val.code === SystemDataItemCode.Id ||
  ...
  setValueCtrl(
    ...
    currentRowData.conditionTypeList = BizRule.existVerifyConditionOf(
      item.propertyType,
      item.code,
    );
