    ...
    >
      <div class="content">
        ...
        <div class="filters">
                ...
                <div>
                  ...
                  >
                    ...
                    <a-select-option
                      ...
                      value="EXIST"
                      :disabled="
                        row.targetDataItemDefaultProperty ||
                        row.propertyType === 4
                      "
                    >
                      是否存在
                    ...
                    <a-select-option
                      key="TOTAL"
                      :disabled="
                        validModelType === 'main' || row.propertyType !== 2
                      "
                      value="TOTAL"
                    >
                      汇总比较
          ...
          <div class="add">
            <span style="cursor: pointer" @click="addRows('main')">
              <span>
...
export default class PropertyValidationCondition extends Mixins(
  ...
  modalHide() {
    ...
    // 只提交后台定义的属性
    let errorMsg: string = '';
    this.filtersList.forEach((m: any) => {
      const obj: any = {
      ...
      if (m.targetSchemaDataItem) {
        const currentItem: any = this.targetFieldList.find(
          (el) => el.code === m.targetSchemaDataItem,
        );
        if (currentItem) {
      ...
      if (m.value) {
        const currentItem: any = m.currentFieldList.find(
          (el) => el.code === m.value,
        );
        if (currentItem) {
        ...
        if (this.controller.value && this.controller.value.verifyConditions) {
          ...
          if (
            ...
            editData.conditions.map((c: any, index: number) => {
              ...
              editData.conditions[index].conditionTypeList =
                this.tiggerFilterOf(targetItem.propertyType, targetItem.code);
        ...
        if (sheetData && sheetData.subSchema.properties) {
          sheetData.subSchema.properties.map((p: any) => {
            if (p.propertyType !== DataItemType.Attachment) {
              p.code = !p.code.includes('.') ? `${vals[1]}.${p.code}` : p.code;
              sheetItem.push(p);
            }
          });
        }
  ...
  filterCurrentFieldList(current, index, editData?) {
    ...
    if (editData) {
      editData.currentFieldList = Arry;
    } else {
      this.filtersList[index].currentFieldList = Arry;
    }
  }
