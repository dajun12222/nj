    ...
    >
      ...
      <div class="content">
        ...
        >
          ...
          <a-row>
            <a-col :span="24">
              target:{{ filtersCondition[0].targetSchemaDataItem }}
            </a-col>
          </a-row>
          <a-row
  ...
  dataWrite() {
    ...
    if (this.value) {
      ...
      if (this.value[this.actionType].length > 0) {
        ...
        const currentDataItem = this.targetModelDataItems;
        createActions.forEach((res, index) => {
          ...
          const source: any = currentDataItem.find(
            (c: any) => c.code === res.currentDataItemValue,
          );
          const currentSchemaDataItem: string = res.currentDataItemValue;
          // if (!source) {
          //   currentSchemaDataItem = "";
          // }
          const row: any = {
  ...
  modalOk() {
    ...
    const action = this.assignDataList.map((res) => {
      ...
      if (res.targetSchemaDataItem) {
        const current = this.targetMainDataItems.find(
      ...
      if (res.currentSchemaDataItem) {
        let current = this.filterConditionCurrentDataItemList.find(
          (el) => el.code === res.currentSchemaDataItem,
        );
        if (!current) {
          current = this.targetModelDataItems.find(
            (el) => el.code === res.currentSchemaDataItem,
          );
        }
        if (current) {
