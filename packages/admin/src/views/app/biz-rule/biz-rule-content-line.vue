  ...
  <div class="lines-wrap">
    ...
    <template v-for="(line, index) in lines">
      <div
        v-if="
          line.startActivity &&
          (line.startActivity.nodeType === 'LOGIC_VERIFY' ||
            line.startActivity.nodeType === 'DATA_CHECK' ||
            line.startActivity.nodeType === 'EXIST_VERIFY')
        "
        :key="`text-${index}`"
...
export default class LinesInstance extends Vue {
  ...
  startDragLine(evt: DragEvent, point: LINE.handlerPoint, index: number) {
    ...
    if (this.operate.line) {
      const {
        line: { startActivity, endActivity, routeCondition },
      } = this.operate;
      ...
      if (
        endActivity.nodeType === NodeType.End &&
        routeCondition === 0 &&
  ...
  endDragLine() {
    ...
    const flag = this.lines.map(
      (item) => `${item.startActivity.ID}-${item.endActivity.ID}`,
    );
  ...
  get handlerPoints() {
    if (
      this.operate.line &&
      this.operate.line.handlerPoints &&
      !this.disabled
    ) {
