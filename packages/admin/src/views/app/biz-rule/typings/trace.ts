  ...
  constructor(_TraceType: number, _Target: any, _Source: any, vm: any) {
    ...
    switch (_TraceType) {
      case TraceType.Activity.Remove:
        {
          ...
          this.RelateLines = vm.lines.filter(
            (line: Line) =>
          ...
          _lines.forEach((m: any, j: string) => {
            if (
              m.startActivity.nodeCode === this.Activity.nodeCode ||
              m.endActivity.nodeCode === this.Activity.nodeCode
            ) {
          ...
          for (let i = 0; i < this.RelateLines.length; i++) {
            this.AtomicTraces.push(
              new AtomicTrace(
                ...
                this.RelateLines[i],
                TraceManager.GetWorkflowElementProperties(this.RelateLines[i]),
                vm,
              ),
            );
            // 删除线痕迹
      ...
      case TraceType.Activity.Add:
        {
          const AtomicTraceInstance = new AtomicTrace(
            _TraceType,
            _Target,
            _Source,
            vm,
        ...
        for (let a = 0; a < this.Targets.length; a += 1) {
          this.AtomicTraces.push(
            new AtomicTrace(
              _TraceType,
              this.Targets[a],
              this.SourceStates.filter((item) => {
                return item.ID === this.Targets[a].ID;
              })[0],
              vm,
            ),
          );
