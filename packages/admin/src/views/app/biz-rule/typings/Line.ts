...
class Line {
  ...
  static calcPoint(activity: Activity, offset: LINE.point) {
    const { left, top, width, height } = activity;
    return {
  ...
  static calcDirection(activity: Activity, point: LINE.point) {
    const { left, top, bottom, right } = activity;
    const { x, y } = point;
    let _arrowDirection;
   ...
   * @param _endPoint
   */
  static getEndDirection(
    _startPoint: LINE.point,
    _startDirection: LINE.ArrowDirection,
    _endPoint: LINE.point,
  ) {
  ...
  static calculatePoints(
    ...
    _startDirection: LINE.ArrowDirection,
    _endPoint: LINE.point,
    _endDirection: LINE.ArrowDirection,
  ) {
   ...
   * @param _newOffsetX
   */
  static joinToCrossOffsetX(
    _crossOffsetXs: Array<LINE.crossOffsetX>,
    _newOffsetX: any,
  ) {
    let inserted = false;
    ...
    if (_crossOffsetXs && _crossOffsetXs.length) {
      for (
        let _crossIndex = 0;
        _crossIndex < _crossOffsetXs.length;
        _crossIndex++
      ) {
        ...
        if (_newOffsetX + CrossRadius < curOffset.start) {
          ...
          _crossOffsetXs[_crossIndex].start = Math.min(
            curOffset.start,
            _newOffsetX - CrossRadius,
          );
          _crossOffsetXs[_crossIndex].end = Math.max(
            curOffset.end,
            _newOffsetX + CrossRadius,
          );
          // 判断合入下一个范围
          ...
          if (
            ...
            _crossOffsetXs[_crossIndex].end =
              _crossOffsetXs[_crossIndex + 1].end;
  ...
  init(lineConfig: LINE.config) {
    ...
    this.startDirection =
      lineConfig.startDirection || LINE.ArrowDirection.Unspecified;
    this.endActivity = lineConfig.endActivity;
    this.endDirection =
      lineConfig.endDirection || LINE.ArrowDirection.Unspecified;
  ...
  setProps(props: LINE.props) {
    ...
    const { startPoint, endPoint, startActivity, endActivity, points } = props;
  ...
  setOffsetToActivity() {
    if (this.startPoint && this.startActivity) {
      this.offsetToStartActivity = Line.offsetToActivity(
        this.startPoint,
        this.startActivity,
      );
    }
    if (this.endPoint && this.endActivity) {
      this.offsetToEndActivity = Line.offsetToActivity(
        this.endPoint,
        this.endActivity,
      );
    }
  ...
  setDirections() {
    if (!this.startDirection && this.startActivity && this.startPoint) {
      this.startDirection = Line.calcDirection(
        this.startActivity,
        this.startPoint,
      );
    }
  ...
  setPoints(_ResetStartAndEnd?: boolean) {
    ...
    if (_ResetStartAndEnd) {
      if (this.startActivity) {
        this.startPoint = Line.calcPoint(
          this.startActivity,
          this.offsetToStartActivity,
        );
      }
      if (this.endActivity) {
        this.endPoint = Line.calcPoint(
          this.endActivity,
          this.offsetToEndActivity,
        );
      }
    }
    ...
    if (
      this.startDirection === LINE.ArrowDirection.Unspecified &&
      this.endDirection === LINE.ArrowDirection.Unspecified
    ) {
      return;
    }

    if (this.endDirection === LINE.ArrowDirection.Unspecified) {
      this.endDirection = Line.getEndDirection(
        this.startPoint,
        this.startDirection,
        this.endPoint,
      );
    } else if (this.startDirection === LINE.ArrowDirection.Unspecified) {
      this.startDirection = Line.getEndDirection(
        this.endPoint,
        this.endDirection,
        this.startPoint,
      );
    ...
    this.points = Line.calculatePoints(
      this.startPoint,
      this.startDirection,
      this.endPoint,
      this.endDirection,
  ...
  setTextPosition(position?: LINE.point) {
    ...
    let _y: number = position ? position.y : 0;
    if (!_x && !_y) {
      switch (_len) {
        case 2:
    ...
    this.points.forEach((res) => {
      if (prePoint) {
        if (prePoint.x === res.x) {
          const n = prePoint.y - res.y;
          lineLenght += n > 0 ? n : -n;
        } else {
          const n = prePoint.x - res.x;
          lineLenght += n > 0 ? n : -n;
        }
      }
      prePoint = res;
    ...
    const lineLenght2 = lineLenght / 2;
    lineLenght = 0;
    prePoint = null;

    for (let i = 0; i < _len; i++) {
      const res = this.points[i];
      if (prePoint) {
        if (prePoint.x === res.x) {
          const n = prePoint.y - res.y;
          lineLenght += n > 0 ? n : -n;
  ...
  setHandlerPoints() {
    ...
    /* 堆栈起点 */
    const _firstHandler: LINE.handlerPoint = Line.calcHandlerPoint(
      this.points[0],
      this.points[1],
    );
    ...
    /* 堆栈终点 */
    const _lastHandler: LINE.handlerPoint = Line.calcHandlerPoint(
      this.points[_pointsCount - 2],
      this.points[_pointsCount - 1],
    );
    ...
    /* 至少要保留起点和终点 */
    if (_points.length < 2) {
      _points = [_firstHandler, _lastHandler];
    }
  ...
  removeCrossPointToLine(lineId: number) {
    if (this.crossPoints && this.crossPoints.length) {
      this.crossPoints = this.crossPoints.filter(
        (crossPoint: LINE.crossPoint) => crossPoint.verticalLineId !== lineId,
      );
      this.needRedraw = true;
    }
  }
  ...
  calcCrossPoints(lines: Array<Line>) {
    ...
    lines.forEach((curLine: Line) => {
      ...
      if (curLine.endPoint) {
        _arrowTailPoint[curLine.ID] = Line.calcArrowTailPoint(
          curLine.endPoint,
          curLine.endDirection,
        );
      }
    });
                ...
                const isCurrentLine = function () {
                  ...
                  return (
                    ...
                    [
                      ...
                      _otherSegementStartPoint.x -
                        Math.min(_segementStartPoint.x, _segementEndPoint.x),
                      // 当前线的右侧距离其他线的距离大于 线段圆角和交点圆角的总和
                      Math.max(_segementStartPoint.x, _segementEndPoint.x) -
                        _otherSegementStartPoint.x,
                      /* 当前线的y坐标在其他线的y方向范围内，且其他线两端与当前线的距离均大于交线+圆角半径和 */
                      _segementStartPoint.y -
                        Math.min(
                          _otherSegementStartPoint.y,
                          _otherSegementEndPoint.y,
                        ),
                      Math.max(
                        _otherSegementStartPoint.y,
                        _otherSegementEndPoint.y,
                      ) - _segementStartPoint.y,
                    ].every((el) => el > _minRoom)
                  );
                };

                const preLine = function () {
                  // 其他线是水平线
                  return (
                    ...
                    [
                      _segementStartPoint.x -
                        Math.min(
                          _otherSegementStartPoint.x,
                          _otherSegementEndPoint.x,
                        ),
                      Math.max(
                        _otherSegementStartPoint.x,
                        _otherSegementEndPoint.x,
                      ...
                      _otherSegementStartPoint.y -
                        Math.min(_segementStartPoint.y, _segementEndPoint.y),
                      Math.max(_segementStartPoint.y, _segementEndPoint.y) -
                        _otherSegementStartPoint.y,
                    ].every((el) => el > _minRoom)
                  );
                };
  ...
  draw(drawCrossPoints?: boolean) {
    ...
    const self = this;
    const { CornerRadius, CrossRadius, ArrowLength, ArrowWidth } =
      Line.settings;
    ...
    if (_PointsEndWithArrowTailPoint.length && self.endPoint) {
      _PointsEndWithArrowTailPoint[_PointsEndWithArrowTailPoint.length - 1] =
        Line.calcArrowTailPoint(self.endPoint, self.endDirection);
    }

    /* 画线 */
    _PointsEndWithArrowTailPoint.forEach(
      (curPoint: LINE.point, index: number) => {
        if (index === 0) {
        ...
        if (index < _PointsEndWithArrowTailPoint.length - 1) {
          ...
          if (
            ...
            self.crossPoints.forEach((crossPoint: LINE.point) => {
              if (
                ...
                crossPoint.x - Math.min(curPoint.x, next.x) >
                  CornerRadius + CrossRadius &&
                Math.max(curPoint.x, next.x) - crossPoint.x >
                  CornerRadius + CrossRadius
              ) {
                crossOffsetXs = Line.joinToCrossOffsetX(
                  crossOffsetXs,
                  crossPoint.x,
                );
              }
            ...
            if (crossOffsetXs && crossOffsetXs.length) {
              if (curPoint.x < next.x) {
                ...
                crossOffsetXs.forEach((cross: LINE.crossOffsetX) => {
                  pathDefine += ` H ${
                    cross.start
                  } a${CrossRadius} ${CrossRadius} 0 0 1 ${CrossRadius} -${CrossRadius} H${
                    cross.end - CrossRadius
                  } a${CrossRadius} ${CrossRadius} 0 0 1 ${CrossRadius} ${CrossRadius} `;
                });
              } else {
                // 添加拱点
                for (
                  let crossIndex = crossOffsetXs.length - 1;
                  crossIndex >= 0;
                  crossIndex--
                ) {
                  pathDefine += ` H ${
                    crossOffsetXs[crossIndex].end
                  } a${CrossRadius} ${CrossRadius} 0 0 0 -${CrossRadius} -${CrossRadius} H${
                    crossOffsetXs[crossIndex].start + CrossRadius
                  } a${CrossRadius} ${CrossRadius} 0 0 0 -${CrossRadius} ${CrossRadius} `;
                }
              }
            }
          }
          ...
          if (index === _PointsEndWithArrowTailPoint.length - 2) {
            ...
            if (
              Math.abs(curPoint.x - next.x) + Math.abs(curPoint.y - next.y) >
                CornerRadius * 2 &&
              Math.abs(next.x - nexter.x) + Math.abs(next.y - nexter.y) >
                CornerRadius * 2 &&
              nexter.x - curPoint.x !== 0 &&
              nexter.y - curPoint.y !== 0
            ) {
              const x =
                (CornerRadius * (nexter.x - curPoint.x)) /
                Math.abs(nexter.x - curPoint.x);
              const y =
                (CornerRadius * (nexter.y - curPoint.y)) /
                Math.abs(nexter.y - curPoint.y);

              const fn = function () {
                let res = false;
                if (curPoint.x > nexter.x) {
                  if (curPoint.x === next.x && curPoint.y < nexter.y) {
                    res = true;
                  }
                  if (curPoint.y === next.y && curPoint.y > nexter.y) {
                    res = true;
                  }
                } else if (curPoint.x < nexter.x) {
                  if (curPoint.x === next.x && curPoint.y > nexter.y) {
                    res = true;
                  }
                  if (curPoint.y === next.y && curPoint.y < nexter.y) {
    ...
    if (arrowPoint) {
      ...
      switch (this.endDirection) {
        case LINE.ArrowDirection.left:
          arrowDefine += `${arrowPoint.x - ArrowLength},${
            arrowPoint.y - ArrowWidth
          } ${arrowPoint.x - ArrowLength},${arrowPoint.y + ArrowWidth}`;
          break;
        case LINE.ArrowDirection.right:
          arrowDefine += `${arrowPoint.x + ArrowLength},${
            arrowPoint.y - ArrowWidth
          ...
          break;
        case LINE.ArrowDirection.down:
          arrowDefine += `${arrowPoint.x - ArrowWidth},${
            arrowPoint.y + ArrowLength
          } ${arrowPoint.x + ArrowWidth},${arrowPoint.y + ArrowLength}`;
          break;
        case LINE.ArrowDirection.up:
          arrowDefine += `${arrowPoint.x - ArrowWidth},${
            arrowPoint.y - ArrowLength
  ...
  autoFix() {
    ...
    if (this.startPoint && this.startActivity) {
      const fixedStart: LINE.point = Line.fixPoint(
        this.startPoint,
        this.startActivity,
      );
      if (
        fixedStart.x !== this.startPoint.x ||
        fixedStart.y !== this.startPoint.y
      ) {
    ...
    if (this.endPoint && this.endActivity) {
      const fixedEnd: LINE.point = Line.fixPoint(
        this.endPoint,
        this.endActivity,
      );
