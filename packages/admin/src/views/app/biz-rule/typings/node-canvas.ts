...
export default class NodeCanvas extends WorkflowBase {
  ...
  addLine(activities: any, rule: any) {
    ...
    const _startActivity = _activities.find(
      (activity: any) => activity.nodeCode === rule.preNode,
    );
    const _endActivity = _activities.find(
      (activity: any) => activity.nodeCode === rule.postNode,
    );
  ...
  setSameStyle(sameStyle: string, vm: any) {
    if (vm.selectedActivities && vm.selectedActivities.length > 1) {
      ...
      // workflow.autoFit();
      vm.traceManager.AddTrace(
        _TraceType,
        vm.selectedActivities,
        vm.beforeMovedActivities,
        vm,
      );
    }
  }
