...
export default class TraceManager {
  ...
  static GetWorkflowElementProperties(_WorkflowElement: any) {
    if (
      _WorkflowElement.WorkflowElementType ===
      WorkflowElementType.WorkflowElementType.Activity
    ) {
      ...
      _WorkflowElement.WorkflowElementType ===
      WorkflowElementType.WorkflowElementType.Line
    ) {
  ...
  static GetMultiObjAndStates(_Targets: any) {
    const _activities = _Targets.filter(
      (t: WorkflowNamespace.Activity) =>
        t.WorkflowElementType ===
        WorkflowElementType.WorkflowElementType.Activity,
    );
    const _Objects: any = [];
    const _States: any = [];
  ...
  AddTrace(_TraceType: number, _Target: any, _Source: any, vm: any) {
    ...
    if (
      ...
      this.LastTraceIndex < this.TraceStack.length - 1
    ) {
      this.TraceStack.splice(
        this.LastTraceIndex + 1,
        this.TraceStack.length - (this.LastTraceIndex + 1),
      );
    }
    // 新痕迹
