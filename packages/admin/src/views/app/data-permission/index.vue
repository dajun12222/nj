  ...
  <div class="data-permission">
    ...
    <div v-else-if="list.length" class="data-permission__main">
      <div class="data-permission__buttons">
        ...
        <a-button icon="plus" type="primary" @click="addPermission">新建</a-button>
      </div>
      <div class="data-permission__table">
        ...
        >
          ...
          <span slot="departments" slot-scope="text, record">{{
            record.departmentNames
          }}</span>
          <a-switch
            slot="enable"
          ...
          <span slot="action" slot-scope="text, record" class="actions">
            <i
              ...
              @click="editPerm(record)"
            ></i>
            <i
              class="icon aufontAll h-icon-all-delete-o"
              @click="deletePerm(record)"
            ></i>
          </span>
        </a-table>
      </div>
    ...
    <div v-else>
      ...
      <no-data tip="暂无数据权限，请点击新建按钮新建" @click="addPermission" />
    </div>
...
import { Spin, Table, Switch, Drawer, Button } from '@h3/antd-vue';
...
export default class DataPermission extends Vue {
  ...
  async getDataItems() {
    ...
    const sourceDataItems = this.dataItemList.filter(
