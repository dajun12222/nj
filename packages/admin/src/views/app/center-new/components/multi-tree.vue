...
export default class MultiTree extends Vue {
  ...
  convert(x: any, isLeaf: boolean) {
    ...
    return {
      ...
      title: this.isChinese
        ? x.name
        : x.name_i18n
        ? x.name_i18n.en || JSON.parse(x.name_i18n).en
        : x.name,
      name: this.isChinese
        ? x.name
        : x.name_i18n
  ...
  onDrop(info: any) {
    ...
    if (!info.dropToGap) {
      ...
      const dragKeyString = this.datas.filter((d: any) => d.key === dragKey);

      // 拖拽节点父级id
      const dragKeyParentId = dragKeyString[0] ? dragKeyString[0].parentId : '';
      /**
       * --------------------------------分割线---------------------------------------
       */
      // 获得目标节点的层级id数组 this.datas
      this.getItemAndParentsIdArr(data, dropKey);
      const dropKeyString = this.datas.filter((d: any) => d.key === dropKey);

      // 目标节点父级id
      const dropKeyParentId = dropKeyString[0] ? dropKeyString[0].parentId : '';
      /**
       * --------------------------------分割线---------------------------------------
       */
      // 只允许在第一级别父级id下进行拖动
...
.draggable-tree {
  ...
  /deep/.ant-tree-node-content-wrapper {
    color: rgba(17, 18, 24, 1) !important;
  }
}
</style>
