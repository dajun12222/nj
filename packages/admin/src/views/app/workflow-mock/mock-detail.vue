...
<template>
  <div class="mock-detail">
    <div class="mock-detail-header">
      <div class="back">
        <span v-if="showBack" @click="back">返回</span>
      </div>
      <div class="mock-detail-header_actions">
        <!-- <a-button class="mock-action" @click="loadData">撤回</a-button> -->
        <a-button v-if="showBtn" class="mock-action" @click="cancel">
          作废
        </a-button>
    ...
    >
      ...
      <template slot="footer">
        ...
        <a-button type="primary" @click="tipCancel"> 知道了 </a-button>
      </template>
    </a-modal>
  </div>
</template>
...
import PortalChartCard from 'cloudpivot/flow/src/components/pc/chart-card';
...
import { Button, Checkbox, Modal } from '@h3/antd-vue';
...
export default class MockDetail extends Vue {
  ...
  cancel() {
    ...
    vm.$confirm({
      ...
      onOk() {
        workflowApi
          .abortInstance(vm.workflowInstanceId, '', 0)
          .then((res: any) => {
            if (res.errcode === 0) {
  ...
  deleteForm() {
    ...
    vm.$confirm({
      ...
      onOk() {
        workflowApi
          .deleteInstance(vm.workflowInstanceId, '', 0)
          .then((res: any) => {
            if (res.errcode === 0) {
              vm.backWorkflowDesign();
  ...
  async clickActivity(activity: any, e: MouseEvent) {
    ...
    if (['START', 'END'].includes(activity.activityType)) {
      return;
    }
    ...
    const processAndExceptionNode = this.logs.filter((item: any) => {
      ...
      const condition = // 其他流程只有进行中才展示  ==> 进行中与已完成都显示（迭代46需求）
        item.activityCode === activity.activityCode &&
        (item.workItemStatus === 2 || item.workItemStatus === 3);
      if (subInstanceCondition) {
        return true;
      } else if (condition) {
        return true;
      } else {
    ...
    // 未激活节点显示预估处理人
    if (!processAndExceptionNode || !processAndExceptionNode.length) {
      ...
      if (!res.errcode) {
        PortalChartCard({
          ...
          estimate: true,
          i18n: this.i18n,
          statusFn: this.getWorkflowActionStatus,
        });
      }
      return;
    }
    ...
    if (activity.activityCode === startNextActivityCode) {
      // 流程开始节点弹框
      this.initialNodeDialog(activity, rect, processAndExceptionNode);
      ...
      // 传阅节点弹框
      this.circulateNodeDialog(activity, rect, processAndExceptionNode);
    } else if (
      processAndExceptionNode.some((item) => item.workItemStatus === 2)
    ) {
      // 流程进行中的弹框
      this.processNodeDialog(activity, rect, processAndExceptionNode);
    } else {
      // 流程已完成的弹窗
      this.finishedNodeDialog(processAndExceptionNode, rect);
    }
  }
  ...
  finishedNodeDialog(record: any, rect: DOMRect) {
    PortalChartCard({
  ...
  nodeProcess({ type, record }, cb?: any) {
    const _context = this as any; // reject eslint check
  ...
  getInstanceErrLog() {
    //查询指定流程实例的异常日志
    return workflowApi
      .getWorkflowInstanceErrInfo(this.workflowInstanceId)
      .then((res: any) => {
        if (res.errcode === 0) {
          return res.data;
        } else {
...
<style lang="less">
@import '~cloudpivot/flow/src/components/style.less';
.detail-delete-icon {
