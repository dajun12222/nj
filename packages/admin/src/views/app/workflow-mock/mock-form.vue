...
export default class mockForm extends pcForm.runtime.FormDetail {
  ...
  get mockActions() {
    ...
    return this.moveAgreeToFirst(
      this.actions.filter((ac: any) => ac.code !== runtime.FormAction.Print),
    );
  }

  moveAgreeToFirst(actions) {
    if (
      actions[0] &&
      actions[1] &&
      actions[0].code === runtime.FormAction.Agree
    ) {
      const temp = actions[1];
      actions[1] = actions[0];
      actions[0] = temp;
    }
    return actions;
...
.form-detail {
  min-width: 1100px;
  /deep/.my-comments-wrapper font,
  /deep/.up-file-content span {
    color: #2970ff;
  }
}
.textarea {
