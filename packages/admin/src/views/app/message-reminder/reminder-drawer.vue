...
<template>
  <div class="message-reminder-drawer">
    <a-row class="message-reminder-drawer-row">
      <a-col :span="24">
        <div class="message-reminder-drawer-title" @click="showDatas">
          提醒对象
        </div>
      </a-col>
    </a-row>
    ...
    <a-row class="message-reminder-drawer-row">
      ...
      <a-col :span="21">
        <role-tree
          v-model="remindDrawerValues.roleIds"
          placeholder="请选择角色"
        />
      </a-col>
    </a-row>
        ...
        <div>
          ...
          >
            <a-select-opt-group>
              ...
              <a-select-option v-for="i in object_bizSummaryList" :key="i.code">
                {{ i.name }}
              </a-select-option>
            </a-select-opt-group>
            <a-select-opt-group>
    ...
    <a-row class="message-reminder-drawer-row">
      <a-col :span="12">
        <a-row>
          ...
          <a-col :span="18">
            ...
            >
              <a-select-option v-for="item in dateControlEnum" :key="item.code">
                {{ item.name }}
              </a-select-option>
            </a-select>
          </a-col>
      ...
      <a-col :span="12">
        <a-row justify="space-around" align="middle" class="right-row">
          <a-col v-if="enumList && enumList.dateType" :span="5">
            <a-select
          ...
          <template v-if="remindDrawerValues.dateType !== 1">
            ...
            <a-col :span="4">
              ...
              >
                <a-select-option v-for="item in hours" :key="item">
                  {{ item }}
                </a-select-option>
              </a-select>
            </a-col>
            ...
            <a-col :span="4">
              ...
              >
                <a-select-option v-for="item in minutes" :key="item">
                  {{ item }}
                </a-select-option>
              </a-select>
            </a-col>
    ...
    >
      ...
      <a-col
        v-if="
          remindDrawerValues.conditionType !== 1 &&
          remindDrawerValues.conditionType !== null
        "
        :span="8"
        :offset="2"
      >
    ...
    <a-modal
      ...
      @cancel="onCancel"
    >
      <dataitem-condition v-model="conditionValue" :items="items" />
    </a-modal>
  </div>
</template>
...
import { OperatorService } from '@/components/shared/data-item/data-item';
...
export default class ReminderDrawer extends Vue {
  ...
  // 提醒对象数据项过滤后的系统数据项
  get object_defaultSummaryList(): Array<any> {
  ...
  // 提醒对象数据项过滤后的业务数据项
  get object_bizSummaryList(): Array<any> {
    return this.summaryList.filter(
      (data: any) => !data.defaultProperty && data.propertyType === 5,
    );
  }
  ...
  get contentDataList() {
    return this.summaryList.filter(
      (data: any) => data.propertyType !== 10 && data.published,
    );
  }
