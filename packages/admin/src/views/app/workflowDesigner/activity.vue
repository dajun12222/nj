...
<template>
  <div
    ...
    :class="{
      'activity-selected-first': firstActivityId === instance.ID,
  ...
  >
  <div v-if="isLock" class="model-mask">
    <i class="icon aufontAll h-icon-all-lock-o"></i>
  </div>
    <div class="activity activity-content">
      <div class="content-wrap">
        ...
        <span v-if="isChinese" class="activity">{{
          instance.activityName
        }}</span>
        <span v-else class="activity">{{
          instance.name_i18n[$i18n.locale]
        }}</span>
      </div>
    </div>
    ...
    <template v-if="instance.activityType !== 'END'">
      ...
      <activity-drag-bar
        ...
        :instance="instance"
        :container="dom"
        :showCircle="firstActivityId === instance.ID"
...
export default class Activity extends Vue implements H3AvtivityDraggable {
  ...
  @WorkflowModule.State(
    (state) =>
      (state.selectedActivities[0] && state.selectedActivities[0].ID) ||
      undefined,
  )
  firstActivityId: any;
  ...
  onDragstart(evt: DragEvent) {
    ...
    if (evt.dataTransfer) {
      ...
      this.$emit('activityDataTransfer', {
  ...
  activitySelect(e: any) {
    ...
    let preSelectedId = -1;
    console.log('--------isMac', isMac, this.instance);
    if (e.ctrlKey || (isMac && e.metaKey)) {
      ...
      preSelectedId =
        this.selectedActivities.length === 1
          ? this.selectedActivities[0].ID
          : -1;
      // 清空所有选中活动
      this.unSelect();
      this.select(this.instance);
    ...
    if ((e.button === 0 || e.button === 1) && e.type === 'mousedown') {
      const multiUnSelectFlag =
        !this.instance.isSelected && this.selectedActivities.length > 1;
      const flag =
        preSelectedId === -1
          ? this.curActivityID !== this.instance.ID
          : preSelectedId !== this.instance.ID;
      if (flag || (multiUnSelectFlag && this.instance.ID)) {
        const _ID =
          this.selectedActivities.length === 1
            ? this.selectedActivities[0].ID
            : this.instance.ID;
        const _activityType =
          this.selectedActivities.length === 1
            ? this.selectedActivities[0].activityType
            : this.instance.activityType;
        const _fixedOptions = this.selectedActivities.length === 1 ? this.selectedActivities[0].fixedOptions: this.instance.fixedOptions;
        const _activityCode = this.selectedActivities.length === 1 ? this.selectedActivities[0].activityCode: this.instance.activityCode;
        let propertyCompName: string | undefined = '';
        propertyCompName = getCompNameByActivityType(_activityType);
        // 切换成别的节点，先做赋值操作
        if (_activityType !== 'START' && _activityType !== 'END') {
          this.transformData(_ID);
        }
      
        //一般节点  子管理员 fixedOptions = true
        //if(_activityCode === "Activity2" || (_activityType !== 'START' && _activityType !== 'END' && _fixedOptions && this.isAppMin)){
          if((_activityType !== 'START' && _activityType !== 'END' && _fixedOptions && this.isAppMin)){
          //this.$emit('hasPermission',false);
          this.$message.error('固定节点，暂无权限，不可更改');
          this.$emit('hasPermission',true);
  ...
  get isLock(){
    ...
    const _fixedOptions = this.instance.fixedOptions;
    const _activityCode = this.instance.activityCode;
    // console.log('start....._activityType........');
    // console.log(_activityType);
    // console.log(_fixedOptions);
    // console.log(this.isAppMin);
    // console.log(this.instance);
    // console.log('end....._activityType........');
    // if(this.instance.activityCode === "Activity2"){
    //   return true;
    // }
    //isMain
    // console.log('_activityType    '+_activityType);
    // console.log('_fixedOptions    '+_fixedOptions);
    // console.log('isAppMin    '+this.isAppMin);
    // console.log(this.instance);
     //if((this.isAppMin && _activityCode === 'Activity2') || (_activityType !== 'START' && _activityType !== 'END' && _fixedOptions && this.isAppMin)){
   if(_activityType !== 'START' && _activityType !== 'END' && _fixedOptions && this.isAppMin){  
      status = true;
    }
    return status;
  ...
  activityDragEnd(e: any) {
    ...
    this.$nextTick(() => {
      const activity = this.activities.find(
        (item: WorkflowNamespace.Activity) =>
          item.activityCode === this.instance.activityCode,
      );
