...
<template>
  ...
  >
	...
	:dom="$refs.designer"
draggable="true"
          @activityDataTransfer="activityDataTransfer"
...
export default class WorkflowDesigner extends Vue implements H3Droppable {
  ...
  // refactor start 把vuex 中的三个变量全部下放到视图中
  itemData: any;

  originOffsets: any;

  workflowElementType: any;
  ...
  async mounted() {
    ...
    if (this.$route.query.version) {
      ...
      this.getHistoryWorkflow({
        workflowCode,
        workflowVersion: this.$route.query.version,
      });
    } else {
      this.disable = false;
      this.$emit('buttonDisable', this.disable);
      const res = await this.getWorkflowDraft({ workflowCode });
      let isMain = res && res.data && res.data.isMain || false;
      let isHide = res && res.data && res.data.isHide || false;
      this.isMain = isMain;
      this.$emit('getIsMain',isMain);
      this.$emit('getIsHide',isHide);
      if (res.errcode && res.errcode !== 999999) {
  ...
  pasteActivity() {
    ...
    const pasteData = cloneDeep(this.activityCopyArray);
    /* {
  ...
  activityDataTransfer({ itemData, originOffsets, workflowElementType }) {
    // data from child component activity
    this.itemData = itemData;
    this.originOffsets = originOffsets;
    this.workflowElementType = workflowElementType;
  }
  ...
  workflowSpaceMouseUp(e: any) {
    ...
    // 框选时展示第一个节点的属性面板
    if (this.selectedActivities.length) {
      const _activityType = this.selectedActivities[0].activityType;
      const _ID = this.selectedActivities[0].ID;
      ...
      this.$nextTick(() => {
        const activity = this.activities.find(
          (item: WorkflowNamespace.Activity) =>
            item.activityCode === itemData.activityCode,
        );
  ...
  autoFit(params?: any) {
    ...
    if (_type === 'mouseCtrl') {
      activityRightEdage =
        this.parsePxToNumber(_activity.dom.style.left) + _width; // 节点的最右边
      activityBottomEdage =
        this.parsePxToNumber(_activity.dom.style.top) + _height; // 节点的最下边
    } else {
    ...
    // 设置画布的宽度
    if (designerDom.clientWidth <= activityRightEdage) {
      // 节点已经超出画布
    ...
    // 设置画布的高度
    if (designerDom.clientHeight <= activityBottomEdage) {
      // 节点已超出画布
