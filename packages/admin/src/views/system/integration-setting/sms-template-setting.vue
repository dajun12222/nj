...
<template>
  <div class="sms-template-setting">
    <a-button class="sms-add" type="primary" @click="showPop('add')">
      新建
    </a-button>
    ...
    >
      <div class="content">
        <div class="list">
          <label>
            <span><span v-show="popType !== 'edit'" class="font-ele">*</span></span>
            模板类型
          </label>
          <div class="right">
            <a-select
            ...
            >
              ...
              <a-select-option v-if="popType === 'edit'" value="2">
                催办
              </a-select-option>
            </a-select>
          </div>
        ...
        <div class="list">
          <label>
            <span><span class="font-ele">*</span></span>
            模板编码
          </label>
          <div class="right" :class="{ error: errorObj.code.value }">
            <a-input
              v-model="currentParams.code"
              placeholder="请输入模板编码"
              @change="inputChange(currentParams, 'code', 20)"
            />
            <div v-if="errorObj.code.value" class="err-tips">
              {{ errorObj.code.info }}
            </div>
          </div>
        </div>
        ...
        <div class="list">
          ...
          <div class="right" :class="{ error: errorObj.name.value }">
            <a-input
              ...
              :disabled="popType === 'edit'"
              placeholder="请输入"
              @change="inputChange(currentParams, 'name', 40)"
            />
            <div v-if="errorObj.name.value" class="err-tips">
              {{ errorObj.name.info }}
            </div>
          </div>
        </div>
        ...
        <div class="list">
          ...
          <div class="right" :class="{ error: errorObj.content.value }">
            <a-textarea
              v-model="currentParams.content"
              :autosize="{ minRows: 4, maxRows: 4 }"
              @change="inputChange(currentParams, 'content', 200)"
              @focus="textareaFocus = true"
              @blur="textareaBlur"
            />
            <div
              v-if="currentParams.content.length === 0 && !textareaFocus"
              class="placehoder"
            >
              <p>变量格式：${name}</p>
              <p>示例：您有新的流程待处理，流程标题：${name},请及时处理</p>
            </div>
            <div v-if="errorObj.content.value" class="err-tips">
            ...
            >
              ...
              <div class="list-info">
                <a-input
                  v-model="item.value"
                  placeholder="请输入"
                  @change="inputChange(item, 'value', 40)"
                />
              </div>
            </div>

            <div v-if="paramsList.length === 0" class="empty">
...
import { Button, Input, Modal, Select, Table, Tooltip } from '@h3/antd-vue';
...
export default class smsTemplateSetting extends Vue {
  ...
  getParamsList() {
    const text: string = this.currentParams.content;
    if (text === '') {
      return;
    }
    const Arr: any[] = text.match(/(?<=(\$\{))\w+(?=\})/g) || [];
  ...
  handleOk() {
    ...
    const keys: string[] = this.paramsList.map((el: any) => el.key as string);
    const keyLen = keys.length;
    const lastLen = [...new Set(keys)].length;
    if (keyLen !== lastLen) {
      this.errorObj.content = {
        value: true,
        info: '变量名称不能重复',
