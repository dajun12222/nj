...
<template>
  <div class="notify-config">
    ...
    <a-row class="notify-config__item">
      <a-col :span="layout.left" class="notify-config__left">
        <a-checkbox v-show="isEditConfig" v-model="dingtalkSwitch">
          钉钉消息通知
        </a-checkbox>
        <span v-show="!isEditConfig"> 钉钉消息通知 </span>
      </a-col>
      <a-col :span="layout.right" class="notify-config__right">
        <div
          class="showOrg"
          :class="{ isEditing: isEditConfig, isCloseNotify: !dingtalkSwitch }"
          @click="toSetOrg('dingtalk')"
        >
          <!-- 编辑状态 -->
          <template v-if="isEditConfig">
            ...
            <template v-else>
              <span style="border: none; color: rgba(17, 18, 24, 0.5)">未设置组织或未开启</span>
            </template>
          </template>
          ...
          <template v-else>
            <template v-if="dingtalkSwitch && dingTalkSelected.length">
              ...
              <a-tooltip
                placement="bottom"
                overlayClassName="customTooltipStyle"
              >
                <template slot="title">
                  {{ getTitle('dingtalk') }}
                </template>
                <span v-if="dingTalkSelected.length > 5" class="count">+{{ dingTalkSelected.length - 5 }}</span>
              </a-tooltip>

              <!-- <span class="count" :title="getTitle('dingtalk')" v-if="dingTalkSelected.length > 5">+{{dingTalkSelected.length - 5}}</span> -->
            </template>
            <span v-else style="border: none; color: rgba(17, 18, 24, 0.5)">未设置组织或未开启</span>
          </template>

          <span class="del-org font-ele" @click.stop="delOrg('dingtalk')">
    ...
    <a-row class="notify-config__item">
      <a-col :span="layout.left" class="notify-config__left">
        <a-checkbox v-show="isEditConfig" v-model="wechatSwitch">
          企业微信消息通知
        </a-checkbox>
        <span v-show="!isEditConfig"> 企业微信消息通知 </span>
      </a-col>

      <a-col :span="layout.right" class="notify-config__right">
        <div
          class="showOrg"
          :class="{ isEditing: isEditConfig, isCloseNotify: !wechatSwitch }"
          @click="toSetOrg('wechat')"
        >
          <!-- 编辑状态 -->
          <template v-if="isEditConfig">
          ...
          <template v-else>
            <template v-if="wechatSwitch && wechatSelected.length">
              ...
              >
                ...
                <span v-if="wechatSelected.length > 5" class="count">+{{ wechatSelected.length - 5 }}</span>
              </a-tooltip>
          ...
          <span class="del-org" @click.stop="delOrg('wechat')">
            <i class="icon aufontAll h-icon-all-close"></i>
          </span>
        </div>
      </a-col>
    ...
    <a-row class="notify-config__item">
      <a-col :span="layout.left" class="notify-config__left">
        <a-checkbox v-show="isEditConfig" v-model="emailSwitch">
          邮件消息通知
        </a-checkbox>
        <span v-show="!isEditConfig"> 邮件消息通知 </span>
      </a-col>
      <a-col :span="layout.right" class="notify-config__right">
        <div
          class="showOrg"
          :class="{ isEditing: isEditConfig, isCloseNotify: !emailSwitch }"
          @click="toSetOrg('email')"
        >
          <!-- 编辑状态 -->
          <template v-if="isEditConfig">
          ...
          <template v-else>
            <template v-if="emailSwitch && emailSelected.length">
              ...
              >
                ...
                <span v-if="emailSelected.length > 5" class="count">+{{ emailSelected.length - 5 }}</span>
              </a-tooltip>
          ...
          <span class="del-org" @click.stop="delOrg('email')">
            <i class="icon aufontAll h-icon-all-close"></i>
          </span>
        </div>
      </a-col>
    ...
    <a-row class="notify-config__item" style="margin-top: 30px">
      ...
      <a-col :span="layout.right" class="notify-config__left">
        ...
        <a-button v-else type="primary" :disabled="!rootAdmin" @click="save">
          保存
        </a-button>
      </a-col>
    </a-row>
...
import { Row, Col, Checkbox, Tooltip, Button } from '@h3/antd-vue';
...
export default class NotifyConfig extends Vue {
  ...
  setOrgScopeOver(selected: string[]) {
    // 设置组织范围结束
  ...
  getName(type, count = 0) {
    ...
    let maxCount = 0;
    if (type === 'dingtalk') {
      firstItem = this.dingtalkList.find(
        (el: any) => el.corpId === this.dingTalkSelected[count],
      );
      maxCount = this.dingTalkSelected.length;
    } else if (type === 'wechat') {
      firstItem = this.wechatList.find(
        (el: any) => el.corpId === this.wechatSelected[count],
      );
      maxCount = this.wechatSelected.length;
    } else if (type === 'email') {
      firstItem = this.emailList.find(
        (el: any) => el.corpId === this.emailSelected[count],
      );
  ...
  async getNotifyConfig() {
    ...
    this.dingtalkList = res.data.filter((item: any) => {
      return (
        item.enabled &&
        ((item.syncType === 'PUSH' &&
          item.relatedType === 'DINGTALK' &&
          item.appkey !== '') ||
          (item.syncType === 'PULL' && item.relatedType === 'DINGTALK'))
      );
    });
    ...
    this.dingTalkSelected = info.data.dingtalkSettingList.map((item) => {
      return item.corpId;
    });
    this.wechatSelected = info.data.wechatSettingList.map((item) => {
      return item.corpId;
    });
    this.emailSelected = info.data.emailSettingList.map((item) => {
      return item.corpId;
    });
  }
  ...
  save() {
    this.dingtalkSettingList = this.dingtalkList
      .filter((item) => {
    ...
    this.wechatSettingList = this.wechatList
      .filter((item) => {
    ...
    this.emailSettingList = this.emailList
      .filter((item) => {
    ...
    OrgApi.saveNotifySetting(params)
      .then((res) => {
        if (res.errcode === 0) {
