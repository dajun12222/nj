          ...
          <a-col :span="layout.right" class="file-storage__right">
            <div v-if="item.type === 'input'">
              <span v-if="edit" class="check-dingtalk__right--edit">
                <template
                  v-if="['ftpPassword', 'keySecret'].includes(item.code)"
                >
    ...
    <a-row class="file-storage__item">
      ...
      <a-col :span="layout.right" class="file-storage__right">
        <div class="btn-group">
          ...
          <a-button :disabled="connectDisabled" @click="connect">
            {{ btnText[1] }}
          </a-button>
        </div>
      </a-col>
...
export default class FileStorage extends Vue {
  ...
  storageTypeOptios = [this.ossConfigVO, this.ftpConfigVO];

  handleValChange(item: any) {
    if (item.code !== 'ftpPort') {
      return;
    }
    ...
    systemApi.getOss().then((res: any) => {
      if (res.data) {
        if (res.data.ftpConfigVO || res.data.ossConfigVO) {
          vm.storageType = res.data.ftpConfigVO
            ? Number(res.data.ftpConfigVO.checked)
            : 0;
        }
