  ...
  <div class="apps-manager">
    ...
    <div class="apps-manager__content">
      <p class="content__tips">
        子管理员：对已配置的应用范围内的应用、授予数据管理权限的应用和组织管理范围内的组织拥有管理权限
      </p>
      <a-table
      ...
      >
        ...
        <p slot="departments" slot-scope="text" class="text-ellipsis">
          <span :title="text">{{ text }}</span>
        </p>
        ...
        <p slot="appManage" slot-scope="text" class="text-ellipsis">
          {{ text ? '开启' : '关闭' }}
        </p>

        <p slot="appPackages" slot-scope="text" class="text-ellipsis">
          <span :title="text">{{ text }}</span>
        </p>
        ...
        >
          <i @click="editManager(record.id)"> 编辑</i>
          <a-popconfirm
    ...
    systemApi.getListManager(this.manageParams).then((res: any) => {
      if (!res.errcode && res.data) {
        ...
        vm.dataSource.forEach((item: any, index: number) => {
          item.index = index + 1;
          item.appPackages = item.appPackages
            .map((apppackage: any) => apppackage.name)
            .join(', ');
          item.departments = item.departments
            .map((departments: any) => departments.name)
            .join(', ');
          if (!item.departments) {
            item.departments = '';
          }
...
.apps-manager {
  ...
  &__content {
    ...
    // }
    /deep/.ant-table-body {
      max-height: calc(100vh - 230px - 60px - 16px) !important;
      color: #111218;
    }
    .highlight {
      color: @primary-color;
