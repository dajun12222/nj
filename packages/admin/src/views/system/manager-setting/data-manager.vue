  ...
  <div class="data-manager">
    ...
    <div class="data-manager__content">
      <p class="content__tips">
        数据管理员：可对前台应用下的列表数据管理， 管理设置的应用管理范围下，
        组织管理范围内的人创建的数据
      </p>
      ...
      >
        ...
        <div slot="action" slot-scope="text, record" class="action right">
          <i
            class="icon aufontAll h-icon-all-edit-o edit"
            @click="editManager(record.id)"
          ></i>
          <a-popconfirm
...
export default class DataManager extends Vue {
  ...
  filterDataSource() {
    ...
    const reg = new RegExp(vm.keyWords.toLowerCase());
    vm.filterData = this.dataSource.filter(
      (res: any) =>
        reg.test(res.username.toLowerCase()) ||
