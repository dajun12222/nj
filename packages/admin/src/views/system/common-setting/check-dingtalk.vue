...
<template>
  <div class="check-dingtalk">
    ...
    <a-row v-if="isSuperAdmin" class="check-dingtalk__item">
      <a-col :span="layout.left" class="check-dingtalk__left" />
      <a-col :span="layout.right" class="check-dingtalk__right">
        <div class="btn-group">
          <!-- <a-button
...
import { Row, Col, Icon, Tooltip, Input, Button } from '@h3/antd-vue';
...
export default class CheckDingtalk extends Vue {
  ...
  initParams() {
    ...
    const { appkey, appSecret, agentId, pcServerUrl, corpId, synRedirectUri } =
      this.info;

    synRedirectUri
      ? (this.params.synRedirectUri = synRedirectUri)
      : (this.params.synRedirectUri = apiHost);
    synRedirectUri
      ? (this.params.redirectUri = synRedirectUri)
      : (this.params.redirectUri = oauthHost);
  ...
  // save
  saveParams() {
    const mobileServerUrl = `${this.params.pcServerUrl}/mobile`;
    // @ts-ignore
    const { appkey, appSecret, agentId, pcServerUrl } = this.params;
  ...
  createdDingtalk() {
    ...
    const params = { ...this.info, ...this.params };
