...
<template>
  <div class="organizetion-relate">
    <template direction="vertical" :current="-1" class="common-setting__step">
        ...
        <template
          v-if="
            (syncParams.relatedType === 'DINGTALK' ||
              syncParams.relatedType === 'OTHER') &&
            syncParams.syncType === 'PULL'
          "
        >
            ...
            <div slot="description">
              <div class="guide">
                ...
                <div class="organizetion-relate__form">
                  ...
                  <a-row
                    v-for="item in getEnableBook"
                    :key="item.key"
                    class="organizetion-relate__item"
                  >
                    <a-col
                      :span="layout.left"
                      class="organizetion-relate__left"
            ...
            <div slot="description">
              <div class="guide">
                ...
                <div class="organizetion-relate__form">
                  ...
                  <a-row
                    v-if="isSuperAdmin"
                    class="check-dingtalk__item btn-group-row"
                  >
    ...
    <template v-if="syncParams.relatedType === 'CUSTOMIZE'">
      <div>
        ...
        <a-row class="custom-row">
          ...
          <a-col :span="layout.right" class="color85">
            {{ syncParams.name }}
          </a-col>
        </a-row>
        <a-row class="custom-row">
          ...
          <a-col :span="layout.right" class="color85">
            {{ syncParams.customizeRelateType }}
          </a-col>
        </a-row>
        <a-row class="custom-row">
          <a-col :span="layout.left">
        ...
        <div
          v-if="Object.keys(JSON.parse(syncParams.syncConfig)).length"
          class="custom-params"
        >
          自定义集成参数
        </div>
        ...
        >
          <a-row class="custom-row">
            ...
            <a-col :span="layout.right">
              <span style="font-size: 12px">组织集成凭证值</span>
            </a-col>
          </a-row>
          <a-row
            v-for="(val, key, index) in JSON.parse(syncParams.syncConfig)"
            :key="index"
            class="custom-row"
          >
            <a-col :span="layout.left">
    ...
    <template v-else-if="syncParams.syncType === 'PUSH'">
      <div class="check-dingtalk">
        ...
        <a-row class="check-dingtalk__item">
          ...
          <a-col :span="layout.right" class="check-dingtalk__right">
            <span class="check-dingtalk__right--edit">{{
              (syncParams.parentDept && syncParams.parentDept.name) || '--'
            }}</span>
          </a-col>
        </a-row>
        ...
        <div class="check-dingtalk__header">
          <span>自维护向钉钉同步</span>
          <div class="header__tips">
            云枢与钉钉绑定所需参数<slot name="settingTips"></slot>
          </div>
        </div>
...
import { Row, Col, Tooltip, Icon, Button, Drawer } from '@h3/antd-vue';
...
export default class CheckDingtalk extends Vue {
  ...
  get getEnableBook(): any[] {
    this.syncParams.syncConfigModels = this.syncParams.syncConfigModels
      ? this.syncParams.syncConfigModels
      : [];
    return this.syncParams.syncConfigModels.filter(
  ...
  connect() {
    ...
    const params: any = { ...vm.params, ...vm.syncParams, ...otherParms };
  ...
  async deleteOrg(params) {
    // @ts-ignore
    const result: any = await OrgApi.deleteOrgan(params);
    if (result.errcode === 0) {
