...
<template>
  <div class="common-setting">
    <div ref="header" class="common-setting__header">
    ...
    <div class="common-setting__content">
      <div v-if="orgList[active].orgType === 'MAIN'" class="set-support">
        <p>主组织维护方式</p>
        <a-select
      ...
      <template v-if="active === 0">
        ...
        <template v-if="isCloudPivot === 0">
          ...
          >
            <a-step id="app-list1">
              <div slot="description" class="common-setting__dingtalk">
                <check-dingtalk
                  :info="orgList[active]"
                  :edit="edit"
          ...
          >
            <a-step id="app-list1">
              <div slot="description" class="common-setting__dingtalk">
                ...
                >
                  <span
                    class="settingTips setting-tips"
                    @click="ShowConfigRule"
                  >
                    <a href="javascript:void(0)">配置规则</a>
                  </span>
      ...
      <template v-else>
        ...
        >
          <span slot="settingTips" class="setting-tips" @click="ShowConfigRule">
            <a
              href="javascript:void(0)"
              style="display: inline-block; font-size: 12px; padding-left: 8px"
              >配置规则</a>
          </span>
        </organizetion-relate>
      </template>
...
import { Drawer, Icon, Input, Modal, Select, Steps, Tabs } from '@h3/antd-vue';
...
export default class CommonSetting extends Vue {
  ...
  get orgTypes() {
    ...
    return {
      Main: this.orgList.filter(
        (item) =>
          item.orgType === 'MAIN' && item.name.indexOf(this.searchText) > -1,
      ),
      SelfMaintenance: this.orgList.filter(
        (item) =>
          item.orgType !== 'MAIN' &&
          item.syncType === 'PUSH' &&
      ...
      Wechat: this.orgList.filter(
        (item) =>
          item.orgType !== 'MAIN' &&
          item.relatedType === 'WECHAT' &&
          item.name.indexOf(this.searchText) > -1,
      ),
      Other: this.orgList.filter(
        (item) =>
          item.orgType !== 'MAIN' &&
          item.syncType === 'PULL' &&
          item.relatedType === 'CUSTOMIZE' &&
          item.name.indexOf(this.searchText) > -1,
      ),
    };
  }
  ...
  handleChange(cloudPivotType) {
    ...
    systemApi.setCloudProvt({ cloudPivotType }).then((res) => {
      ...
      if (errcode === 0) {
        ...
        this.$message.error(errmsg as any);
      }
    });
  }
  ...
  async update(params) {
    // @ts-ignore
    const { errcode, errmsg } = await OrgApi.updateOrgan(params);
    if (errcode === 0) {
      this.$message.success(errmsg);
    } else {
      this.$message.error(errmsg);
  ...
  async getOrgList() {
    // @ts-ignore
    const res: any = await OrgApi.getOrgaList();
    if (res.errcode !== 0) {
    ...
    if (res.data.length) {
      // @ts-ignore
      const main = res.data.find((item: any) => item.orgType === 'MAIN');
      this.main = main;
      // 自维护初始化企业名称
      this.cropName = main && (main.name as string);

      // @ts-ignore
      const others = res.data.filter((item: any) => item.orgType !== 'MAIN');
