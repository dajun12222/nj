...
<template>
  <a-form class="add-organization">
    ...
    <a-row v-if="syncType === 0" class="item-wrap">
      <a-col :span="6" class="item-title"> 通讯录 </a-col>
      <a-col>
        <a-checkbox-group v-model="bookType">
          <a-checkbox
            v-for="item in options"
            :key="item.key"
            :value="item.key"
            @change="toggleCheckedBook(item)"
          >
            {{ item.name }}
          </a-checkbox>
        </a-checkbox-group>
    ...
    <a-row v-if="syncType === 3" class="item-wrap">
      <a-col :span="6" class="item-title">
        <span class="red">*</span>组织类型编码
        <a-tooltip>
          <template slot="title">
            必须与同步逻辑中该组织定义的通道类型保持一致，系统方可识别匹配、进行相关路由。如钉钉的通道类型为DINGTALK
          </template>
          <i
    ...
    <div v-show="syncType === 3" class="custom-params">
      <a-row>
        <a-col class="custom-item" :span="10">
          <span style="font-size: 12px">组织集成凭证项</span><a-tooltip>
            <template slot="title">
              可根据组织的数据源接口实现类定义相关参数，如钉钉同步所需的CorpId、SSOSecret、AppKey等
            </template>
            <i
              class="icon aufontAll h-icon-all-question-circle-o"
              style="margin-left: 5px"
      ...
      >
        <a-col class="custom-item" :span="10">
          <a-input v-model="row.customKey" placeholder="请输入" />
        </a-col>
        <a-col class="custom-item" :span="10">
      ...
      <div class="add">
        <span @click="addRows"><i class="icon aufontAll h-icon-all-plus-o"></i>新增</span>
      </div>
    </div>
    ...
    <div v-show="syncType !== 3">
      <h3 class="vice-title">
        <span v-show="syncType === 0 || syncType === 1">钉钉</span><span v-show="syncType === 2">企业微信</span>集成参数
      </h3>
      <div v-show="syncType === 0 || syncType === 1" class="vice-tips">
        云枢与钉钉绑定所需参数
      </div>
      <a-row v-if="syncType !== 1" class="item-wrap">
        <a-col :span="6" class="item-title">
      ...
      <a-row v-if="syncType !== 2" class="item-wrap">
        <a-col :span="6" class="item-title">
      ...
      <div v-if="syncType === 0">
        <a-row v-for="item in checkedBook" :key="item.key" class="item-wrap">
          <a-col :span="6" class="item-title">
    ...
    >
      <a-row class="book-list">
        <template>
          <draggable :list="options" :options="dragOptions" handle=".handles">
            ...
            >
              ...
              <a-col
                v-if="modulem.key !== 'edu' && modulem.key !== 'citizen'"
                :span="4"
        ...
        <a-row class="option-row">
          <a-col :span="24">
            <span class="addBtn" @click="addRow">
              <i class="icon aufontAll h-icon-all-plus-o"></i>
              <span>添加通讯录</span>
            </span>
          </a-col>
...
export default class ErrorComponent extends Vue {
  ...
  onBookTypeChange() {
    ...
    this.checkedBook = this.options
      .filter((item) => this.bookType.includes(item.key))
      .map((i) => {
        i.enable = true;
        return i;
      });
  ...
  created() {
    ...
    if (this.info) {
      ...
      this.formInfo.syncConfigModels = this.formInfo.syncConfigModels
        ? this.formInfo.syncConfigModels
        : [];
      this.checkedBook = this.formInfo.syncConfigModels.filter(
        (item) => item.enable === true,
      );
      this.options = this.formInfo.syncConfigModels;
      this.bookType = this.checkedBook.map((i) => {
        return i.key;
      });
      ...
      // @ts-ignore
      this.customParamList = Object.entries(
        JSON.parse(this.info.syncConfig),
      ).map((item) => {
  ...
  toggleCheckedBook(item) {
    switch (item.key) {
      case 'edu':
        const idx = this.bookType.findIndex((x) => x === 'citizen');
        if (idx > -1) {
          this.bookType.splice(idx, 1);
        }
        break;
      case 'citizen':
        const eduidx = this.bookType.findIndex((x) => x === 'edu');
        if (eduidx > -1) {
  ...
  confirm() {
    ...
    if (this.syncType !== 0) {
      ...
      const empty = this.formInfo.syncConfigModels.find(
        (item) => item.rootId === '',
      );
      if (empty) {
    ...
    const { parentId, name, syncType } = params;
    ...
    if (this.syncType === 3) {
      const obj: any = {};
      this.customParamList.forEach((item) => {
  ...
  async getDepart(deptIds: string) {
    // @ts-ignore
    const res: any = await OrgApi.getOrgDepartmentInfo({
      deptIds,
    });
  ...
  connect() {
    ...
    if (typeof vm.formInfo.syncConfig === 'object') {
      vm.formInfo.syncConfig = JSON.stringify(vm.formInfo.syncConfig);
      change = true;
    }
    ...
    systemApi.checkPortalSetting(vm.formInfo).then((res: any) => {
      ...
      if (change) {
        vm.formInfo.syncConfig = JSON.parse(vm.formInfo.syncConfig);
        change = false;
      }
    });
  }
...
.book-list {
  background-color: rgba(244, 245, 248, 0.65);
  border-radius: 4px;
  margin-top: 5px;
  .rowStyle {
    line-height: 32px;
