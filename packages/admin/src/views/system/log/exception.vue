  ...
  <div class="exception">
    ...
    <div class="clearfix exception__header">
      <smart-search @reset="reset" @search="query">
        <ul slot="search" class="exception__filter--left">
          <li class="exception-item">
            <span class="exception-item__label">流程模版：</span>
            <div class="exception-item__input" style="width: 220px">
              <flowTree
                v-model="params.workflowCode"
                :isClearFinish.sync="isClearFinish"
                :isClear="true"
          ...
          <li class="exception-item">
            ...
            <div class="exception-item__input">
              <a-input
                v-model="params.workflowInstanceName"
                placeholder="请输入"
              />
            </div>
          </li>
    ...
    <div class="exception__content">
      ...
      >
        ...
        <span slot="workflowName" slot-scope="text" class="text-ellipsis">{{
          text
        }}</span>
        <span
        ...
        <span slot="createdTime" slot-scope="text" class="text-ellipsis">{{
          text
        }}</span>
        <span slot="status" slot-scope="text">{{
          text === 1 ? '已修复' : '未修复'
        }}</span>
        <span slot="fixedTime" slot-scope="text" class="text-ellipsis">{{
          text
        }}</span>
        <p
          slot="action"
          slot-scope="text, record"
          class="exception__actions operation"
        >
          <a v-if="record.status === 0" @click="fix(record)">修复</a>
          <a @click="openDetail(record)"><i>打开</i></a>
          <a @click="openForm(record)"><i>表单详情</i></a>
        </p>
      </a-table>
    </div>
    <div v-show="totalPage" class="exception__footer">
      <div>
        <a-pagination
...
export default class Exception extends Vue {
   ...
   */
  getErrorLogList() {
    this.params.originator = this.originators
    ...
    const params: BPM.System.GetErrorLogParams = {
      ...this.params,
      workflowCode: this.params.workflowCode.replace('workflow-', ''),
      startTime: '',
      endTime: '',
    };
  ...
  fix(record: any) {
    ...
    const params: BPM.System.ExceptionItemParams = {
      exceptionLogId: record.id,
    };
  ...
  openDetail(record: BPM.System.ExceptionInfo) {
    ...
    systemApi
      ...
      .then((res: Common.Res<BPM.System.ExceptionInfo>) => {
        if (res.data) {
  ...
  setChildWorkflowSchemaNew(name: any) {
    this.params.workflowCode = name.schemaCode;
  }
  ...
  openForm(record: BPM.System.ExceptionInfo) {
    ...
    if (record.workItemId) {
      const path: string = `${env.portalHost}/form/detail?workitemId=${
        record.workItemId
      }&workflowInstanceId=${record.workflowInstanceId}&return=${
...
.exception-item__input .flow-tree /deep/ .ant-select {
  width: 100%;
}
.exception {
  &__header {
