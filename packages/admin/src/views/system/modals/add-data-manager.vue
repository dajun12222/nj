  ...
  >
    <div class="add-data-manager">
      ...
      <a-row class="add-data-manager__item">
        ...
        <a-col :span="layout.right" class="add-data-manager__right">
          <apps-select v-model="selectData" />
        </a-col>
      </a-row>

      <a-row v-show="false" class="add-data-manager__item">
        <a-col :span="layout.left" class="add-data-manager__left">
          <span>查看数据:</span>
        </a-col>
        <a-col :span="layout.right" class="add-data-manager__right">
...
import { Modal, Row, Col, Checkbox } from '@h3/antd-vue';
...
export default class AddDataManager extends Vue {
  ...
  get orgParams() {
    // @ts-ignore
    if (this.users.length === 0 || this.users[0].relatedOrgType === 'MAIN') {
      return {};
    }

    // @ts-ignore
    const { corpId = '' } = this.users[0];
    return { corpId };
  }
  ...
  get staffParams() {
    const params = { filterType: 'related' };
    // @ts-ignore
