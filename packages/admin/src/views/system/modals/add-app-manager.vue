        ...
        <a-tab-pane key="2" tab="后台权限">
          ...
          <a-row class="add-data-manager__item">
            ...
            <a-col :span="layout.right" class="add-data-manager__right">
              ...
              >
                <!-- 管理员并且无权限 -->
                  <a-radio :value="true"  :disabled="isOnlyAppAdmin && !onlyAppAdmin.appManage">开启</a-radio>
          ...
          <a-row class="add-data-manager__item">
            ...
            <a-col :span="layout.right" class="add-data-manager__right">
              <a-radio-group v-model="roleManage" name="radioGroup">
                  <a-radio :value="true"  :disabled="isOnlyAppAdmin && !onlyAppAdmin.roleManage">开启</a-radio>
                  <a-radio :value="false">关闭</a-radio>
                </a-radio-group>
            </a-col>
          </a-row>
          <a-row v-if="roleManage" class="add-data-manager__item">
            <a-col :span="layout.left" class="add-app-manager__left">
              <span>选择角色:</span>
            </a-col>
            <a-col :span="layout.right" class="add-data-manager__right">
              <AppsSelectRoleJurisdiction :selectData="selectData" :roleIdsDefault="roleIdsDefault" @change="changeSelectRole" />
            </a-col>
          </a-row>
          ...
          <a-row class="add-data-manager__item">
            ...
            <a-col :span="layout.right" class="add-data-manager__right">
              <a-radio-group v-model="isEdit" name="radioGroup">
                  <a-radio :value="true"  :disabled="isOnlyAppAdmin && !onlyAppAdmin.isEdit">是</a-radio>
          ...
          <a-row class="add-data-manager__item">
            ...
            <a-col :span="layout.right" class="add-data-manager__right">
              <a-radio-group v-model="dataDictionaryManage" name="radioGroup">
                  <a-radio :value="true" :disabled="isOnlyAppAdmin && !onlyAppAdmin.dataDictionaryManage">开启</a-radio>
                  <a-radio :value="false">关闭</a-radio>
                </a-radio-group>
            </a-col>
          </a-row>
...
import { Modal, Row, Col, Tabs, Radio } from '@h3/antd-vue';
...
import AppsSelectRoleJurisdiction from '../manager-setting/apps-select-role-jurisdiction.vue';
...
export default class AddAppManager extends Vue {
  ...
  onlyAppAdmin:onlyApp = {
    roleManage: false,
    dataDictionaryManage: false,
    appManage: false,
    isEdit: false,
  } ;
  ...
  get staffParams() {
    const params: any = { filterType: 'related' };

    // @ts-ignore
    const { users, curCorpId } = this;
    if (users.length > 0) {
      // @ts-ignore
  ...
  */
  ...
  async getManagerUser(){
    ...
    const res:any = await systemApi.getManagerUserId({ userId: this.loginedUserInfo.id });


    if(res.errcode === 0){
      let data = res.data;
      this.onlyAppAdmin = {
        roleManage: data.roleManage, //角色管理
        dataDictionaryManage: data.dataDictionaryManage, //数据字典
        appManage: data.appManage, //应用
        isEdit: data.isEdit, //是否可以编辑
      }
    }
  }
  ...
  backPermission() {
    ...
    const obj: any = {
      ...
      isEdit: this.isEdit,                        //是否有编辑权限
      roleIds: this.roleIds,                      //角色范围
      adminGroups: this.filterFrontPerMap,        // 前台权限
      dataDictionaryManage: this.dataDictionaryManage //数据字典权限
    };

    // 前台权限应用范围和组织范围必须都填写才能提交
    const emptyApp: boolean = obj.adminGroups.some(
      (item: any) => item.appPackages === undefined && !item.deleted,
    );
    const emptyDep: boolean = obj.adminGroups.some(
      (item: any) =>
        item.departments.length === 0 &&
        !item.externalLinkVisible &&
        !item.deleted,
    ); //当外链数据和组织范围都是空并且删除状态为false时无法提交.
    if (emptyApp) {
    ...
    // 提交时
    if (
      obj.appPackages.length === 0 &&
      obj.departments.length === 0 &&
      obj.adminGroups.length === 0
    ) {
  ...
  async getDetail() {
    const vm: any = this;
    // 应用权限的变量的值
    let appManage: boolean  = false;
    let roleManage: boolean | null = null;
    let isEdit: boolean  = false
    const frontMap: Array<any> = [];
      let roleIds: string[] = [];
    // 添加
    if (!vm.id) {
      vm.users = [];
  ...
  async setKeepDeptIds() {
    ...
    const targetIds: string[] = departments
      .map((staff: any) => {
        if (staff.unitType === 1) {
          return staff.unitId;
        }
