...
<template>
  <div class="add-data-dictionary">
    <group-title> 基本信息 </group-title>

    <list-item :label="'字典编码'" :showRequire="true">
      <a-input
        v-model="addParams.code"
        :disabled="!isNew"
        placeholder="请输入"
      />
    </list-item>
    ...
    <list-item :label="'字典分类'" :showRequire="false">
      <a-select
        v-model="addParams.classificationId"
        showSearch
    ...
    <list-item :label="'字典类型'" :showRequire="false">
      <a-radio-group
        v-model="addParams.dictionaryType"
        :disabled="!isNew"
        name="dictionaryType"
        :defaultValue="0"
        @change="onChange"
    ...
    <div v-if="addParams.dictionaryType === 0">
      <key-value
        ...
        @recordsDataChange="recordsDataChange"
      />
    </div>
...
import { Input, Select, Radio } from '@h3/antd-vue';
...
export default class addDataDictionary extends Vue {
  ...
  remarksChange() {
    if (this.addParams.remarks.length > 200) {
      ...
      this.$set(
        this.addParams,
        'remarks',
        this.addParams.remarks.slice(0, 200),
      );
    }
  }
  ...
  saveDictionary() {
    ...
    if (!/^[a-zA-Z][a-zA-Z0-9_]{0,27}$/.test(this.addParams.code)) {
      this.$message.error(
        '字典编码仅支持以字母开头不超过28个字符，仅支持字母、数字、下划线',
      );
      return;
    }
    ...
    this.addParams.name =
      this.addParams.name && this.addParams.name.replace(/\s/g, '');
    if (this.addParams.name === '') {
      this.$message.error('字典名称必填');
      return;
    }

    if (this.addParams.remarks && this.addParams.remarks.length > 200) {
      this.$message.error('说明不能超过200字');
      return;
    }
    ...
    this.recordsData.forEach((el, index) => {
      ...
      if (!/^[-_a-zA-Z0-9]+$/.test(el.code)) {
        ErrorTypes.push(
          '第' + (index + 1) + '行：代码仅支持英文、数字及英文符号”-“和”_“',
        );
      }
    ...
    const names = this.recordsData.map((el) => el.name);
    const codes = this.recordsData.map((el) => el.code);
    if (
      [...new Set(names)].length <
      this.recordsData.filter((el) => !el.deleted).length
    ) {
  ...
  onCurrentDataDictionaryChange(val) {
    ...
    if (this.classList.length && this.currentDataDictionary) {
      if (
        this.classList.every((el) => el.id !== this.addParams.classificationId)
      ) {
  ...
  async searchDictionaryRecord(options) {
    const res: any = await dataDictionaryApi.searchDictionaryRecord({
      id: options.id,
      searchWord: '',
      page: 1,
      size: 99999,
