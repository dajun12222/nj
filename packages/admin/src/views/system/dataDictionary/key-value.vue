...
<template>
  <div class="key-value">
    <div class="content-top">
      <div class="query-box">
        <a-input v-model="keyWords" :placeholder="'搜索中文名称或代码'">
          <a-icon
            v-if="keyWords"
            slot="suffix"
            type="close-circle"
...
<script lang="ts">
import { Input, Icon, Switch, Modal } from '@h3/antd-vue';
...
export default class keyValue extends Vue {
  ...
  getDictionaryList() {
    this.data = this.data.filter(
      (el) =>
        ~el.name.indexOf(this.keyWords) || ~el.code.indexOf(this.keyWords),
    );
  }
  ...
  addOne(options) {
    ...
    setTimeout(() => {
      // @ts-ignore
      document.querySelector('.add-data-dictionary').scrollTop =
        document.querySelector('.add-data-dictionary').scrollHeight + 400;
      const inputs = document.querySelectorAll('.key-value .list-item input');
      // @ts-ignore
  ...
  getIndex(code) {
    ...
    const currentIndex = result.findIndex((item) => {
