...
export const autoDrawLine = (
    ...
    case LINE.ArrowDirection.up:
      ...
      activities.forEach((target: Activity) => {
        ...
        if (target.bottom < startActivity.top) {
          ...
          const _curDistance =
            Math.abs(_percentX - startPoint.x) * VerticalPerceage +
            (startActivity.top - target.bottom);
          // 距离更小则收录
    ...
    case LINE.ArrowDirection.down:
      ...
      activities.forEach((target: Activity) => {
        ...
        if (target.top > startActivity.bottom) {
          ...
          const _curDistance =
            Math.abs(_percentX - startPoint.x) * VerticalPerceage +
            (target.top - startActivity.bottom);
          // 距离更小则收录
          if (_curDistance < distance) {
            distance = _curDistance;
            curLine.endActivity = target;
    ...
    case LINE.ArrowDirection.left:
      ...
      activities.forEach((target: Activity) => {
        ...
        if (target.right < startActivity.left) {
          ...
          const _curDistance =
            Math.abs(_percentY - startPoint.y) * VerticalPerceage +
            (startActivity.left - target.right);
          // 距离更小则收录
          if (_curDistance < distance) {
            distance = _curDistance;
            curLine.endActivity = target;
    ...
    case LINE.ArrowDirection.right:
      ...
      activities.forEach((target: Activity) => {
        ...
        if (target.left > startActivity.right) {
          ...
          const _curDistance =
            Math.abs(_percentY - startPoint.y) * VerticalPerceage +
            (target.left - startActivity.right);
          // 距离更小则收录
          if (_curDistance < distance) {
            distance = _curDistance;
            curLine.endActivity = target;
  ...
  if (curLine.endActivity.activityType === 'START') {
    ...
    const _existSimilar = lines.some((verLine: Line) => {
      // 起点和终点一致
      const isSame =
        (verLine.startActivity.ID === _curStartCode &&
          verLine.endActivity.ID === _curEndCode) ||
        // 起点和终点倒置过来后一致
        (verLine.endActivity.ID === _curStartCode &&
          verLine.startActivity.ID === _curEndCode);
 ...
 */
export const redrawOnActivityChange = (
  changedActivities: Array<Activity>,
  lines: Array<Line>,
) => {
  // ;
 ...
 */
export const calculateDockActivity = (
  evt: DragEvent,
  activities: Array<Activity>,
    ...
    activities.some((target: Activity) => {
      if (
        ...
        if (x < target.left || x > target.right) {
          // 取偏下的位置作为y轴
          dockPosition.y = Math.max(target.top, Math.min(y, target.bottom));
          if (x < target.left) {
 ...
 */
export const dragToDraw = (
  evt: DragEvent,
  currentLine: Line,
  activities: Array<Activity>,
) => {
  /* 搜索最近的节点：距离小于最小间距 */
  const dockPosition: LINE.DockPosition = calculateDockActivity(
    evt,
    activities,
  );
  ...
  if (dockPosition.x !== undefined && dockPosition.y !== undefined) {
    ...
    if (dockPosition.Activity) {
      ...
      if (
        (endPoint.x === left || endPoint.x === right) &&
        (endPoint.y === top || endPoint.y === bottom)
      ) {
        return currentLine;
      }
    }
...
export const moveEnd = (
  ...
  let endPoint: LINE.point | undefined;
  if (
    dockPosition.x !== undefined &&
    dockPosition.y !== undefined &&
    dockPosition.Activity
  ) {
    endPoint = {
      x: dockPosition.x,
      y: dockPosition.y,
...
export const moveStart = (
  ...
  if (
    ...
    // 起始点在停靠节点的四个拐角时停止绘制——无法判断连线方向
    if (
      (startPoint.x === left || startPoint.x === right) &&
...
export const moveText = (evt: DragEvent, currentLine: Line) => {
  ...
  for (let i = 0; i < maxIdx; i++) {
    ...
    _distance =
      (_cursor.y - _curNearestPoint.y) * (_cursor.y - _curNearestPoint.y) +
      (_cursor.x - _curNearestPoint.x) * (_cursor.x - _curNearestPoint.x);
