...
export default class Trace {
  ...
  operationSnapshot(_TraceType: number, _Target: any, _Source: any) {
    ...
    if (_TraceType === TraceType.Activity.Remove) {
      this.Activity = _Target;
      const _ActivityID = this.Activity.ID;
      this.RelateLines = workflowStoreData.lines.filter(
        (line: Line) =>
          (line.startActivity && line.startActivity.ID === _ActivityID) ||
          (line.endActivity && line.endActivity.ID === _ActivityID),
      );
      this.AtomicTraces = [];
      this.AtomicTraces.push(
        new AtomicTrace(TraceType.Activity.Remove, this.Activity, _Source),
      );
      // 先新连接线数组， 再记录完线条痕迹
      _lines.forEach((m: any, j: string) => {
        // multiple lines need to be removed
        if (
          m.startActivity.activityCode === this.Activity.activityCode ||
          m.endActivity.activityCode === this.Activity.activityCode
        ) {
          //vm.$store.commit(`${this.StorePath}/removeLine`, m);
      ...
      [
        TraceType.Multi.VerticalDistance,
        TraceType.Multi.HorizontalDistance,
        TraceType.Multi.SameTop,
        TraceType.Multi.SameRight,
        TraceType.Multi.SameBottom,
        TraceType.Multi.SameLeft,
      ...
      for (let a = 0; a < this.Targets.length; a += 1) {
        this.AtomicTraces.push(
          new AtomicTrace(_TraceType, this.Targets[a], this.SourceStates[a]),
        );
      }
    } else if (_TraceType === TraceType.Activity.Resize) {
      this.Targets = _Target;
      this.SourceStates = _Source;
      AtomicTraceInstance = new AtomicTrace(
        _TraceType,
        this.Targets,
        this.SourceStates,
      );
      ...
      _TraceType === TraceType.Line.Remove ||
      _TraceType === TraceType.Line.Add
    ) {
      this.Line = _Target;
      AtomicTraceInstance = new AtomicTrace(_TraceType, _Target, _Source);
    } else if (
      _TraceType === TraceType.Line.PointChange ||
