...
export default {
  ...
  properties: {
    ...
    noParticipant: {
      ...
      title: '无参与者',
      description:
        '转交管理员指的是找不到人时找对这条数据有管理权限的数据管理员，如果没有找到数据管理员就找系统管理员',
    },
    originator: {
      type: 'string',
      default: 'DEFAULT',
      enum: ['DEFAULT', 'APPROVE'],
      title: '是发起人',
    },
    perviousParticipate: {
      type: 'string',
      ...
      title: '前一活动处理',
      description:
        '与上一活动处理人相同，如A的下一节点是B,当B的参与者与A相同，自动通过',
    },
    participated: {
      type: 'string',
      default: 'DEFAULT',
      ...
      title: '处理过流程',
      description:
        '在之前活动参与过流程,注当某个用户驳回之后，下一次任务再到达时不算参与过流程，例如A提交给B，B提交给C，C驳回给A，当A提交，再次到C时，不会自动跳过C',
    },
    // 数据权限
    propertyPermissions: {
      type: 'array',
    ...
    timeoutStrategy: {
      ...
      enum: ['NOTIFY_HANDLER', 'APPROVE', 'CIRCULATE_MANAGER'],
      title: '超时后策略',
    },
    // 事件处理
    beforeActivate: {
