...
export default class AF {
  ...
  af(start: number, end: number) {
    ...
    while (this.theStack.length > 0) {
      const v = this.getAdjUnvisitedVertex(
        this.theStack[this.theStack.length - 1],
      );
      if (v === -1) {
        this.tempList = [];
        for (let i = 0; i < this.n; i++) {
        ...
        this.graph
          .getVertexList()
          [this.theStack[this.theStack.length - 1]].setAllVisitedList(
            this.tempList,
          );
      ...
      if (
        this.theStack.length > 0 &&
        end === this.theStack[this.theStack.length - 1]
      ) {
