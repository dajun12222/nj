  ...
  <div class="workitems" :class="{ searching: true, single: appCode }">
          ...
          <div class="a-dropdown">
            <a-dropdown>
              <p>
                {{
                  listType === 'all'
                    ? $t('cloudpivot.flowCenter.mobile.all')
                    : $t('cloudpivot.flowCenter.mobile.systemTransfer')
                }}
                <a-icon type="down" />
              </p>
              <a-menu slot="overlay">
                <a-menu-item @click="listTypeChange('all')">
                  <p class="a-dropdown-p">
                    {{ $t('cloudpivot.flowCenter.mobile.all') }}
                  </p>
                </a-menu-item>
        ...
        <div>
          <transition name="fade">
            <toptip v-show="showTip">
              ...
              }}{{ $t('cloudpivot.flowCenter.mobile.todoItems') }}
            </toptip>
          </transition>
    ...
    <template v-if="mode === 'batch'">
      ...
      <div class="bottom-control">
        <span class="num">
          {{ $t('cloudpivot.flowCenter.mobile.haveSelected') }}:
          <em>
            {{ taskIds.length }}
          </em>
        </span>
        <div>
          <span
            v-if="listType !== 'all'"
            class="btn-batch batch-transfer"
            @click="batchTransfer"
          >
            {{ $t('cloudpivot.flowCenter.mobile.batchTransfer') }}
          </span>
          <span class="btn-batch" @click="batchOpt">{{
            $t('cloudpivot.flowCenter.mobile.batchApprove')
          }}</span>
        </div>
      </div>
    </template>
...
import { homeApi, mobileHome } from '../../../../api';

import mobile from '../../../../form/src/renderer/components/mobile';

import UnfinishedWorkItem from './components/unfinished-workitem.vue';

import Toptip from '../../../../common/src/components/mobile/toptip.vue';
import Empty from '../../../../common/src/components/mobile/empty/empty.vue';
import Searchbar from '../../../../common/src/components/mobile/searchbar.vue';
import common from '../../../../common/mobile';
import * as pcUtils from '../../../../common/src/utils/pc/utils';
...
export default class MyUnfinishedWorkitems extends Vue {
  ...
  created() {
    ...
    const { pageMode } = this.$route.query as any;
    if (pageMode === 'batch') {
        ...
        .then((res) => {
          if (
            ...
            this.$h3.modal.show({
              type: 'alert',
              content: `${this.$t(
                'cloudpivot.flowCenter.mobile.noBatchWorkitemTip',
              )}`,
              actions: [
                {
...
@import '../../../../common/styles/mobile/index.less';
