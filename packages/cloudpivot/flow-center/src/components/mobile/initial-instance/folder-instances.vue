<template>
  <div class="folder-instances">
    <p slot="title" class="folder-instances__title" @click="openFolder">
      <span>{{ `${appName}-${getLangName(folder)}` }}</span>
      <!-- 控制图标 -->
      <span class="right">
        <span class="counts">{{ folder.size }}
          {{ $t('cloudpivot.flowCenter.mobile.flowItems') }}</span>
...
export default class FolderInstances extends Vue {
  ...
  openFolder() {
    if (this.opened) {
      ...
      initialInstanceApi
        .listMyWorkflowByFolderId(this.folder.id, true)
