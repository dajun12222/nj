<template>
  <div class="search-panel">
    <!-- 内容 -->
    <div v-show="showResult" class="search-panel__wrap">
    ...
    <!-- 历史 -->
    <div v-show="!showResult" class="search-panel__wrap search-panel__history">
      <p class="search-panel__title">
        {{ $t('cloudpivot.flowCenter.mobile.historySearch') }}
      </p>
      <ul>
        <li v-for="(key, i) in history" :key="i" @click="selectWord(key)">
          {{ key }}
        </li>
      </ul>
    </div>
...
export default class SearchPanel extends Vue {
  ...
  search(wd: string) {
    ...
    if (this.appSearch) {
      ...
      initialInstanceApi
        .listWorkflowsByName(searchParams)
        .then((res: initialInstanceParams.HttpResponse<any>) => {
          // console.log('搜索结果', res);
          if (res.data && Array.isArray(res.data.appList)) {
            this.total = res.data.size;
            const list = res.data.appList.reduce(
              (pre: any, next: any) => [...pre, ...next.dataList],
              [],
            );
            this.list = list;
          } else {
