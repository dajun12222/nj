<template>
  <div v-show="!loading" class="instance-list">
    <empty v-if="isEmpty" />
    <div v-else class="instance-list__container">
...
import { initialInstanceApi, initialInstanceParams } from '../../../../../api';
...
export default class InstanceList extends Vue {
  ...
  loadMore(page: any, done: any) {
    ...
    if (!vm.firstTimeLoad) {
      const isMobile = true;
      initialInstanceApi
        .listMyWorkflow(isMobile)
        .then((res: initialInstanceParams.HttpResponse<any>) => {
          if (Array.isArray(res.data) && res.data.length) {
            vm.sourceList = res.data.map((app: initialInstanceParams.AppData) =>
              vm.formatItem(app),
            );
    ...
    if (done) {
      const delay = setTimeout(() => {
        clearTimeout(delay);
        vm.appList = vm.appList.concat(
          vm.sourceList.slice(
            (vm.page - 1) * vm.pageSize,
            vm.page * vm.pageSize,
          ),
        );
        done(vm.pageSize, vm.sourceList.length);
  ...
  formatItem(item: initialInstanceParams.AppData) {
    const folders: Array<initialInstanceParams.InstanceItem> = item.dataList
      .filter(
        (ins: any) =>
          ins.type === initialInstanceParams.AppTreeItemTypes.Folder &&
          ins.size,
      )
      .map((ins: any) => {
        ins.children = [];
    ...
    const instances: Array<initialInstanceParams.InstanceItem> =
      item.dataList.filter(
        (ins: any) =>
          ins.type === initialInstanceParams.AppTreeItemTypes.BizModel,
      );
    let list: Array<
      | initialInstanceParams.InstanceItem
      | initialInstanceParams.InstanceRootItem
    > = [];
    if (instances.length) {
