<template>
  <div @click="goForm">
    ...
    >
      ...
      <span v-if="mode === 'batch'" slot="checkbox" class="batch-item-checkbox">
        <div class="work-item-checkbox" @click="checkBoxContainerClick">
          <checkbox-item
            :defaultChecked="workitem.isChecked"
...
export default class UnfinishedWorkItem extends Vue {
  ...
  setTimeType() {
    ...
    if (this.workitem.stayTime) {
      ...
      if (
        this.workitem.workItemTimeoutStatus === 'RED' ||
        this.workitem.workItemTimeoutStatus === 'ORANGE'
      ) {
    ...
    switch (this.workitem.workItemTimeoutStatus) {
      case 'TIMEOUT':
        this.stayTime = `${this.$t(
          'cloudpivot.flowCenter.mobile.timeOut',
        )} ${this.getTimerString(getTimerString)}`;
        // if (day > 0) {
      ...
      case 'RED':
      case 'ORANGE':
        this.stayTime = ` ${this.getTimerString(getTimerString)} ${this.$t(
          'cloudpivot.flowCenter.mobile.timeoutLater',
        )}`;
        // if (day > 0) {
      ...
      default:
        this.stayTime = ` ${this.$t(
          'cloudpivot.flowCenter.mobile.haveWaitFor',
        )} ${this.getTimerString(getTimerString)}`;
        // if (day > 0) {
