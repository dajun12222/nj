 ...
 * @LastEditTime: 2022-03-02 15:15:02
 * @FilePath: \yunshu6.0\modules\cloudpivot\flow-center\src\components\mobile\components\batch-fail-list.vue
 * @Description:
-->
<template>
  <div>
...
export default class BatchFailList extends Vue {
  ...
  created() {
    (document as any).title = this.$t(
      'cloudpivot.flowCenter.mobile.approveFail',
    );
  }
  ...
  summary(workitem) {
    if (this.$i18n) {
      ...
      return !(this.$i18n.locale && this.$i18n.locale === 'en')
        ...
        : workitem.sourceName_i18n[locale];
    }
    return '';
  }
}
