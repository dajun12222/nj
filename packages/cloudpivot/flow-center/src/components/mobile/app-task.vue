  ...
  <div class="app-task">
    <div class="app-task__nav">
      <div class="app-task__nav--left">
        <div
          :class="{ active: curType === taskType.ToDo }"
          @click="changeType(taskType.ToDo)"
        >
          {{ $t('languages.common.todo') }}
        </div>
        <div
          :class="{ active: curType === taskType.ToRead }"
          @click="changeType(taskType.ToRead)"
        >
          {{ $t('languages.common.toread') }}
        </div>
      </div>
      <div class="app-task__nav--right">
        <div v-if="total > 0" class="badge">
          {{ total }}
        </div>
    ...
    <div class="app-task__content">
      ...
      <div v-show="tasks.length === 0" class="app-task__content--nodata">
...
export default class AppTask extends Vue {
  ...
  linkMore() {
    // 查看更多，是否需要app过滤
    if (this.curType === this.taskType.ToDo) {
      this.$router.push('/home/workitems').catch((err: any) => {
        console.log(err);
      });
    } else {
      this.$router.push('/home/circulates').catch((err: any) => {
        console.log(err);
      });
    }
  }
  ...
  async getWorkCount() {
    ...
    if (res.errcode === 0) {
      if (this.curType === this.taskType.ToDo) {
        this.total = res.data.workItemCount;
      } else {
        this.total = res.data.circulateItemCount;
      }
    }
  }
