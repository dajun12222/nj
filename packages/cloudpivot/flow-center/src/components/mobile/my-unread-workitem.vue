  ...
  <div class="circulates" :class="{ searching: true, single: appCode }">
    <div class="circulates-topbar">
      ...
      <search-bar
        ...
        @cancel="cancel"
      >
        <span class="circulates-search" @click="toggleView">{{
          $t('cloudpivot.flowCenter.mobile.batch')
        }}</span>
      </search-bar>
      <!-- check-tool-bar -->
      <section v-show="showCheckbox">
        <div class="circulates-topbar-check">
          ...
          <p class="check-right" @click="toggleView">
            {{ $t('cloudpivot.flowCenter.mobile.completed') }}
          </p>
        </div>
        <div class="circulates-footbar">
          <p class="footbar-left">
            {{ $t('cloudpivot.flowCenter.mobile.haveSelected') }}:
            {{ checkAll ? totalItem.circulates : checkedIds.length }}
          </p>
          <p class="footbar-right">
            <span class="footbar-right-button" @click="markMultiRead">{{
      ...
      <div>
        <transition name="fade">
          <toptip v-show="showTip">
            ...
            }}{{ $t('cloudpivot.flowCenter.mobile.toreadItems') }}
          </toptip>
        </transition>
        <!-- <circulates-item
...
export default class Circulates extends Vue {
  ...
  markItemRead(circulate: any) {
    ...
    homeApi.updateCirculateReaded([circulate.id]).then(() => {
      ...
      const resultList: Array<mobileHome.Workitem> = this.circulates.filter(
        (item: mobileHome.Workitem) => item.id !== circulate.id,
      );
  ...
  markMultiRead() {
    ...
    if ((!this.checkedIds || !this.checkedIds.length) && !this.checkAll) {
      /* TODO: 此处的toast未成功调用 */
      this.$h3.toast.show({
        text: this.$t('cloudpivot.flowCenter.mobile.notSelectUnread'),
      });
      return;
    }
    const params: string[] = this.checkAll ? [] : this.checkedIds;
    homeApi
      .updateCirculateReaded(params)
      .then(() => {
        if (this.checkAll) {
          ...
          this.$store.commit('circulate/setTotalItem', {
            circulates: Math.max(
              0,
              this.totalItem.circulates - this.checkedIds.length,
            ),
          });
        }
