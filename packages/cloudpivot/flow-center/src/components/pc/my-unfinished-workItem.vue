<template>
  <div id="MyUnfinishedWorkItem" ref="workItem" class="workitem-box">
    <div class="content-top">
      <h2>
        ...
        <p>
          <span
            ...
            @click="listTypeChange('all')"
            >{{ $t('cloudpivot.flowCenter.pc.all') }}</span>
          <span
            :class="{ active: listType === 'transfer' }"
            @click="listTypeChange('transfer')"
            >{{ $t('cloudpivot.flowCenter.pc.systemTransfer') }}</span>
        </p>
      </h2>
      <div class="content-right">
        ...
        <a-button
          ...
          @click="startWorkflow"
        >
          {{ $t('cloudpivot.flowCenter.pc.startFlow') }}
        </a-button>
        <filterCard
        ...
        <div class="toggle-mode-icon" @click="toggleMode">
          ...
          <span v-if="mode === 'batch'" class="batch-tip-mask"></span>
        </div>
      </div>
    </div>
      ...
      <div ref="table" class="table">
        ...
        >
          ...
          <template slot="stayTime" slot-scope="{ text, record }">
            <span :class="record.isRead ? 'gray' : ''" :title="record.time">{{ record.time
              }}<span
                v-if="
            ...
            <span class="deadline-tip">
              <i v-if="record.urgeCount > 0" class="overtime beUrged">{{
        ...
        >
          ...
          <template slot="stayTime" slot-scope="{ text, record }">
            <span :class="record.isRead ? 'gray' : ''">{{ record.time
              }}<span
                v-if="
                  record.workItemTimeoutStatus === 'RED' ||
                  record.workItemTimeoutStatus === 'ORANGE'
        ...
        <div class="no-data">
          ...
          <div v-if="mode === 'batch' && isShowNoData" class="empty-data">
            {{
              $i18n.locale === 'en'
                ? 'The to-do list does not contain batch-capable tasks. please switch to'
                : '待办列表未包含可批量处理的任务，请切换至'
            }}<span class="standalone" @click="toggleMode">{{
              $i18n.locale === 'en'
    ...
    <batch-info v-model="showBatchInfoModal" @batchHandle="batchHandle" />
...
import customPop from '../../../../form/src/common/components/customPop.vue';
...
export default class MyUnfinishedWorkItem extends mixins(WorkItemMixin) {
  ...
  startWorkflow() {
    ...
    if (this.rootAdmin || isAdmin) {
      ...
      this.$message.warn(
        this.$t('cloudpivot.flowCenter.pc.adminNewWorkflowErr') as ConfigType,
      );
      return;
    }
  ...
  batchDialog(type?) {
    if (type === 'batchInfo') {
      this.showBatchInfoModal = true;
    }
    if (type === 'failList') {
      this.showBatchFailListModal = true;
    }
    if (type === 'batchSuccess') {
      this.showBatchSuccessModal = true;
    }
  }
