<template>
  <div id="myComments" ref="workItem" class="workitem-box">
      ...
      <div ref="table" class="table">
        ...
        >
          ...
          <template slot="contentName" slot-scope="{ text, record }">
            <span v-if="record.listhover" class="handles">
...
export default class myComments extends mixins(WorkItemMixin) {
  ...
  batchDel() {
    ...
    this.$confirm({
      ...
      cancelText: '取消',
      onOk: () => {
        const ids = _this.checkedItems.map((el) => el.id);
  ...
  onItemCheckboxChange() {
    ...
    this.indeterminate =
      ...
      this.checkedItems.length < this.tableData.length;
