...
export default class WorkflowItemMixin extends Vue {
  ...
  get isTrident() {
    return (window as any).navigator.userAgent.indexOf('Trident') !== -1;
  }
  ...
  commonHandler(res: any, type?: string) {
    ...
    if (res.errcode === 0) {
      ...
      if (res.data.content.length <= 0) {
        ...
        let calType: string = '';
        switch (name) {
          case WorkflowCenterRouterTypes.WorkflowCenterRouter
          ...
          case WorkflowCenterRouterTypes.WorkflowCenterRouter.MyWorkflow:
            calType =
              WorkflowCenterNS.CalculateMaxHeightTypes.HasTabsAndFilters;
            break;
  ...
  removeLoadAll() {
    this.$nextTick(() => {
      ...
      if (loadAllBox) {
        (
          document.querySelector('.common-table__tbody-wrap') as any
        ).removeChild(loadAllBox);
      }
    });
  }
  ...
  tableScrollerHandler() {
    this.$nextTick(() => {
      ...
      const table: any = document.querySelector(
        'div.common-table',
      ) as HTMLElement;
  ...
  setTableMaxHeight() {
    ...
    let diff: number = 0;
    this.$nextTick(() => {
      const thead: HTMLElement = document.querySelector(
        'thead.ant-table-thead',
      ) as HTMLElement;
      if (thead.clientHeight > 39) {
        // 说明表头已经两行了，需要对偏差值进行累加
        diff += 13;
      }
      ...
      if (isTrident) {
        ...
        (doc.querySelector('.ant-table-body') as any).style.maxHeight =
          doc.querySelector('.table').clientHeight + 'px';
        return;
      }
    });
    */
  ...
  dataTranslateToFilterCard(data: any, type?: string) {
    ...
    delete deepData.participantUnitType;
      ...
      if (emptyVals.indexOf(deepData[key]) <= -1) {
        if (key === 'instanceState') {
          ...
          vals.push({
            ...
            content:
              key === 'participantId'
                ? deepData[key].split('_$$_')[1]
                : deepData[key],
            type: -1,
          });
          if (key === 'participantId') {
            data.participantId = deepData[key].split('_$$_')[0];
          }
        }
      }
    });
  ...
  nameTranslator(key: string, type: string) {
    ...
    switch (key) {
      ...
      case 'time':
        return type === 'en'
          ? this.timeLabelHandler('en')
          : this.timeLabelHandler('zh');
      case 'originator':
