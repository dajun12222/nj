  ...
  <div class="start-workflow-wrap">
    <div class="start-workflow-head">
      <div class="start-workflow-head-left">
        <i
          class="goback icon aufontAll h-icon-all-circle-left"
          @click="goBack"
        ></i>
    ...
    <div id="start-workflow" ref="workflow" class="start-workflow-content">
      <div ref="workflowScroll" class="start-workflow-scroll">
        <div v-if="isShowSearchResult" class="search-result">
          {{ $t('cloudpivot.flowCenter.pc.searchRzt') }}
          <span class="search-num">{{ searchTotal }}</span>
          {{ $t('cloudpivot.flowCenter.pc.items') }}
        </div>
        <template v-if="!isShowSearchResult">
          ...
          />
          <div v-if="favoriteList.length" class="line"></div>
        ...
        <template v-else>
          ...
          <div v-show="!isShowSearchNoData" class="already-load">
            {{ $t('cloudpivot.flowCenter.pc.showAll') }}
          </div>
        </template>
        <div class="no-data">
          <PageNoData
  ...
  reloadMessage(event: any) {
    if (
      ...
      (typeof event.data === 'string' &&
        (event.data.indexOf('/workflow-center/start-workflow') !== -1 ||
          event.data.indexOf('%2Fworkflow-center%2Fstart-workflow') !== -1))
    ) {
