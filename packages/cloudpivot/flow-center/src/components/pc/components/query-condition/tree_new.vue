<template>
  <div class="workflow-tree" @click.stop="">
    <div v-if="!isShow" class="show-value" @click="showOption">
      <i v-if="showName" class="icon aufontAll h-icon-all-logic"></i>
      {{ showName }}
      <span v-if="showName === ''" class="placeholder-text">
        {{ $t('cloudpivot.list.pc.pleaseSelect') }}
      </span>
    ...
    <div
      ...
      class="tree-wrapper"
      :style="{
        top: hasTabs ? 412 - scrollTop + 'px' : 366 - scrollTop + 'px',
        maxHeight: hasTabs
          ? 'calc(100vh - ' + (395 - scrollTop) + 'px)'
          : 'calc(100vh - ' + (348 - scrollTop) + 'px)',
      }"
    >
      ...
      >
        ...
        >
          <template>
            <p>{{ $t('cloudpivot.list.pc.recentSearch') }}</p>
            <span
              v-for="(item, key) in historySearchList"
              :key="item + key"
              class="history-item"
              :class="'history-' + key"
              @click="onSearch(item)"
      ...
      <div v-if="currentSearchValue !== ''" class="search-resulte">
        <div class="resulte-item">
          <template v-for="item in searchResulte">
            <p
              v-for="el in item.dataList"
              :key="el.id || el.code"
              :title="item.appName + ' | ' + el.name"
              @click="checkedItem(el)"
            >
              <i
                class="icon aufontAll"
                :class="el.icon || 'h-icon-all-logic'"
              ></i>
              {{ item.appName }} |
              <!-- eslint-disable-next-line vue/no-v-html -->
              <span v-html="setHighlightText(el.name)"></span>
            </p>
          </template>
          <p
            ...
            class="no-resulte"
          >
            {{ $t('cloudpivot.list.pc.noMatchResults') }}
          </p>
        </div>
      </div>
      <a-tree
...
export default class WorkflowTree extends Vue {
  ...
  saveHistorySearch() {
    ...
    const index: number = this.historySearchList.indexOf(value);
    if (index !== -1) {
  ...
  delHistoryItem(item, obj) {
    const index: number = this.historySearchList.indexOf(item);
    if (index !== -1) {
      this.historySearchList.splice(index, 1);
      localStorage.setItem(
        'processTemplate',
  ...
  valueChange(_workflowCode, name, name_i18n) {
    // 值改变事件
    if (!_workflowCode) {
      return;
    }
  ...
  async handleGetWorkflowList(code: string) {
    ...
    if (_worfklowRes.errcode === 0) {
      ...
      // 流程树
      const workflowList: Array<any> = workflowData.filter(
