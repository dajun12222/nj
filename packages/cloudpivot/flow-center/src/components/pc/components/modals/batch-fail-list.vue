<template>
  ...
  >
    <div class="specification">
      <i class="icon aufontAll h-icon-all-exclamation-circle"></i>{{ listData.successNum }}条待办任务已批量处理完成，以下{{
        listData.failNum
      }}条任务未处理完成
    </div>
    <div ref="table" class="table dialog-table">
      <a-table
...
export default class BatchFailList extends Vue {
  ...
  calTable() {
    const failResults = this.listData.failResults || [];
    const curPage = this.curPage;
    const pageSize = this.pageSize;
    this.total = failResults.length;
    this.tableData = failResults.slice(
      (curPage - 1) * pageSize,
      curPage * pageSize,
    );
  }
