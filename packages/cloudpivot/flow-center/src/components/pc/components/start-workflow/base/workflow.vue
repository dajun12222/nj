<template>
  <div class="workflow-wrap" :class="{ pop: pop }" @click="onClick(data.code)">
    <div
      v-if="$i18n.locale === 'zh'"
    ...
    <a-tooltip placement="top">
      <template slot="title">
        <span v-if="!favoriteClass">{{
          $t('cloudpivot.flowCenter.pc.setCommon')
        }}</span>
        <span v-else>{{ $t('cloudpivot.flowCenter.pc.cancelCommon') }}</span>
      </template>
      <div class="flow-star" :class="{ pop: pop }" @click="setFavorite">
        <img
          class="square"
          :class="{ pop: pop }"
        ...
        />
        <i
          class="aufontAll h-icon-all-star"
          :class="{ favorite: favoriteClass, pop: pop }"
        ></i>
      </div>
    </a-tooltip>
  </div>
...
export default class Workflow extends Vue {
  ...
  onClick(code: string) {
    ...
    if (this.isDingTalk) {
      this.$router.push(route).catch((err: any) => {
        console.log(err);
      });
    } else {
