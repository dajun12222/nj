<template>
  <div ref="group" class="workflow-group-wrap" @click="showGroup">
    <div class="workflow-group-content">
      ...
      <h4
        :title="
          $i18n.locale === 'zh' ? data.name : data.name_i18n[$i18n.locale]
        "
      >
        {{ $i18n.locale === 'zh' ? data.name : data.name_i18n[$i18n.locale] }}
      </h4>
    ...
    >
      <template v-for="(data, index) in workflowData" slot="content">
        <workflow
          :key="index"
          :pop="true"
          :appCode="appCode"
...
export default class WorkflowGroup extends Vue {
  ...
  iconArray: Array<string> = [
    'h-icon-all-package',
    'h-icon-all-renamebox',
    'h-icon-all-star-circle',
    'h-icon-all-tag-heart',
    'h-icon-all-ic_favorite',
    'h-icon-all-ic_shopping_cart',
    'h-icon-all-ic_shopping_basket',
    'h-icon-all-ic_event_note',
    'h-icon-all-account-box',
    'h-icon-all-book',
    'h-icon-all-description',
    'h-icon-all-insert-chart',
    'h-icon-all-folder',
    'h-icon-all-restaurant-menu',
    'h-icon-all-business-center',
  ]; // 应用分组图标(可随机从数组中取任意图标作为分组图标)
  ...
  created() {
    this.curIconBg = this.workflowIconRandom(
      this.iconBgArray,
      this.groupIndex,
      this.penterIndex,
    );
    this.curIcon = this.workflowIconRandom(
      this.iconArray,
      this.groupIndex,
      this.penterIndex,
    );
    ...
    if (!res.errcode && res.data && res.data.length) {
      ...
      if (this.curNum <= this.workflowData.length) {
        ...
        if (left + width > workflowDom.clientWidth) {
          this.popRight = '0px';
          this.popArrowLeft = `${
            left - (workflowDom.clientWidth - width) + 105
          }px`;
          this.popLeft = '';
        } else {
