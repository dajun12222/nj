<template>
  <div ref="commonTable" class="common-table">
    <div
      ...
      :style="{
        ...
        minWidth: minTableWidth ? minTableWidth + 'px' : 0,
      }"
    >
      <!-- 表头 -->
        ...
        <div class="common-table__thead">
          <div class="common-table__row">
            ...
            >
              ...
              <slot v-else :record="col" :name="col.hSlot"></slot>
            </div>
          </div>
        </div>
      </div>
      ...
      <!-- 表体 -->
      <div class="common-table__tbody-wrap">
        <div class="common-table__tbody">
          ...
          >
            <div
              ...
              @mouseenter="$set(data, 'listhover', true)"
              @mouseleave="$set(data, 'listhover', false)"
            >
              <div
                ...
                :style="{ ...col.colStyles, width: col.width + 'px' }"
                :class="
                  colIdx === 0 ? 'fixed-left-column fixed-left-column-last' : ''
                "
              >
                <div class="table-wrapper">
                  <span v-if="col.bSlot" :title="setTitle(data[col.dataIndex])">
                    <slot
                      v-if="col.bSlot"
                      :name="col.bSlot"
                  ...
                  <span
                    v-else
                    class="text-ellipsis"
                    :title="setTitle(data[col.dataIndex])"
...
export default class CommonTable extends Vue {
   ...
   */
  onEnd(evt: any) {
    this.$emit('sortEnd', this.dataSource);
  }
  ...
  destroyed() {
    const theadWrapper = this.$el.querySelector(
   ...
   */
  initDomEvents() {
    this.tableContainerElement = this.$el.querySelector(
      '.common-table-contain',
    ) as any;

    if (this.resizable) {
      const theadSelector = '.common-table__thead-wrap';
      const theadWrapper = this.$el.querySelector(theadSelector);
      if (theadWrapper) {
  ...
  initColStyle(columns: any) {
    let tableClientWidth =
      ...
      (this.tableClientWidth = this.$el.clientWidth || 0);
    if (this.minTableWidth && tableClientWidth < this.minTableWidth) {
      tableClientWidth = this.minTableWidth;
    }

    const tableWidth = columns.reduce((s, c, i) => (s += c.width), 0);
    const widthPercent = (tableClientWidth - tableWidth) / tableWidth;
  ...
  // 初始化列调整尺寸功能
  initColumnsResizing() {
    const theadWrapper: any = this.$el.querySelector(
      '.common-table__thead-wrap',
    );
    if (!theadWrapper) {
      return console.error('tableHead not found!');
    }

    const ths = theadWrapper.querySelectorAll('.common-table__col');
  ...
  columnsResizing(ths: any) {
    ...
    [].forEach.call(ths, (th: any, i: any) => {
      const column = this.tableColumns[i];
      const isLast = i === thLen - 1;
      ...
      const minWidthOffset = isLast ? 5 : 0;
      const minWidth = offsetWidth
        ? offsetWidth + 31 + minWidthOffset
  ...
  resizeMoveHandler(e) {
    ...
    const { column, index, width, minWidth, isLastColumn } = this.resizeBar;
  ...
  resizeEndHandler(e) {
    ...
    const { width, parent, column } = this.resizeBar;
  ...
  computeWidth() {
    ...
    const scrollbarWidth = this.isChrome ? 5 : 15;
    const exsertedWidth = tableScrollWidth - tableClientWidth - scrollbarWidth;
    ...
    if (exsertedWidth === 0) {
      ...
      lastResizebar.minWidth =
        lastResizebar.minWidth - exsertedWidth > lastResizebar.backupMinWidth
          ? lastResizebar.minWidth - exsertedWidth
