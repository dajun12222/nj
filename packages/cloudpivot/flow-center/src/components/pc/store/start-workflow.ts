...
const mutations = {
  ...
  setSearchList(state: any, data: any) {
    const locale: string =
      (window.localStorage.getItem('locale') as string) || 'zh';
    ...
    if (state.searchList && state.searchList.length) {
      state.searchList.forEach((item: any) => {
        if (locale === 'zh') {
          ...
          item.name_i18n[locale] = utils.searchHighLight(
            state.wd,
        ...
        if (item.dataList && item.dataList.length) {
          item.dataList.forEach((list: any) => {
            if (locale === 'zh') {
              ...
              list.name_i18n[locale] = utils.searchHighLight(
                state.wd,
                list.name_i18n[locale],
              );
            }
          });
        }
