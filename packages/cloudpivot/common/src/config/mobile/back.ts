/*
 * @Author: Fu Zhuohang
 * @Date: 2022-03-07 15:57:51
 * @LastEditors: Fu Zhuohang
 * @LastEditTime: 2022-03-07 15:58:13
 * @FilePath: \yunshu6.0\modules\cloudpivot\common\src\config\mobile\back.ts
 * @Description:
 */
import Vue from 'vue';
  ...
  platform.service.setLeft({
    fn: () => {
      ...
      // }
        ...
        (window as any).Environment.historyLength - stackLength <= 1
      ) {
        ...
        if (!searchParams.get('pid') && !searchParams.get('gid')) {
          (window as any).Environment.historyLength -= 1;
          stackLength += 1;
        }
      }
    },
