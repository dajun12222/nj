...
import { formApi, listApi } from '../../../../../api';
...
export class DefaultFileService implements renderer.FileService {
  ...
  getListUploadUrl(): string {
    return `${this.env.apiHost}/api/runtime/query/upload_file2`;
  }
  ...
  downloadBatch(files: renderer.H3File[]): void {
    ...
    const refIds = files
      ...
      .join(',');
    let url = `${this.env.apiHost}/api/aliyun/download_multi?multiVO=${refIds}`;
  ...
  exportSheet(params: any, e?: any) {
    const name =
      params.name + utils.DateHandle.dateFormat(new Date(), 'YYYYMMDDHHmmss');
    //
