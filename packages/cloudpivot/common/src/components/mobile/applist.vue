<template>
  <div :class="['app', !recent && 'app_norecent']">
    <ul v-if="list && list.length" class="app-list">
      <li
      ...
      >
        ...
        ></span>
        <span v-else class="app-list__name">{{ getName(app) }}</span>
      </li>
    </ul>
    <div v-else class="no-data">
      {{ $t('cloudpivot.Common.mobile.empty') }}
    </div>
  </div>
</template>
<script lang="ts">
...
export default class AppsList extends Vue {
  ...
  splitKeyword(app: any) {
    // 提取当前应用的国际化语言对应显示名
    const str: string = utils.BusinessFunctions.getLangName(
      app,
      this.displayName,
    );
