<template>
  <ul class="grid-list">
    ...
    >
      <span
        :class="['grid-list__icon', `grid-list__icon--${randomColor(block)}`]"
      >
        <i :class="['icon', 'aufontAll', block.icon]"></i>
      </span>
      <span
        v-if="searchWord"
      ...
      ></span>
      <span v-else class="grid-list__name">{{ getName(block) }}</span>
    </li>
  </ul>
</template>
<script lang="ts">
...
export default class GridList extends Vue {
  ...
  randomColor(block: any) {
    if (this.colorKey && block[this.colorKey]) {
      const letter: string = block[this.colorKey].replace(
        /^([a-zA-Z]).+?$/,
        '$1',
      );
      const index: number =
