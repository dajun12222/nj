...
class AbstractRoleService {
  ...
  generateChilds(
    childs: Array<any>,
    groupName: string,
  ): Array<RoleSelectorTypings.RoleItem> {
  ...
  generateSearchRoles(
    ...
    return results.map((role: any) => {
      const displayName: string = wd
        ? searchHighLight(wd, role.roleName)
        : role.roleName;
      return {
        displayName,
        name: role.roleName,
   ...
   */
  async getGroupRoles(
    params: RoleSelectorTypings.GroupRolesParams,
    group: RoleSelectorTypings.RoleGroupItem,
  ) {
  ...
  async getRoleInfoByID(roleId: string) {
    ...
    let roleInfo: RoleSelectorTypings.RoleItem | null = null;
    if (res.errcode === 0 && res.data) {
      const { groupId, code, name, groupName } = res.data;
      roleInfo = {
        groupName: groupName,
        groupId: groupId,
        code: code,
