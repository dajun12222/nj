...
export function replaceValueData(
  ...
  if (str.includes('real')) {
    ...
    if (!Object.keys(realData).length || !Object.keys(dataType).length) {
      return;
    }
        ...
        if (item.eleType.includes('column')) {
          ...
          if (item.leftKey.code) {
            ...
            dataType.forEach((type: any, i: number) => {
              // @ts-ignore
              if (type.code === leftCode) {
                itemType = type.propertyType;
              }
            });
            realReplace(
              leftCode,
...
function realReplace(
  ...
  if (Array.isArray(list)) {
    const currentItem: any = list.find((x) => x.code === code);
    if (currentItem) {
      itemType = currentItem.propertyType;
    }
  }
  if (kind.includes('column')) {
    ...
    if (itemType === 10) {
      // @ts-ignore
      itemOne[str].innerTxt = `${codeObj[code].provinceName || ''}${
        codeObj[code].cityName || ''
      }${codeObj[code].districtName || ''}${codeObj[code].address || ''}`;
      return;
    }
    if (
      itemType === 12 ||
      ...
      } else if (Array.isArray(codeObj[code])) {
      ...
      if (codeObj[code].length && codeObj[code][0].name) {
        ...
        attaches.forEach((attache) => {
          if (attache.response && attache.response.data) {
            // @ts-ignore
            picUrls.push(formDeatil.getDownloadUrl(attache.response.data));
                  ...
                  try {
                    ...
                    if (typeof obj === 'object' && obj) {
                      ...
                      if (
                        ...
                        // @ts-ignore
                        itemOne[str].innerTxt = `${obj.provinceName || ''}${
                          obj.cityName || ''
  ...
  if (kind.includes('text')) {
    ...
    if (itemType === 10) {
      // @ts-ignore
      itemOne.innerTxt = `${codeObj[code].provinceName || ''}${
        codeObj[code].cityName || ''
      }${codeObj[code].districtName || ''}${codeObj[code].address || ''}`;
      return;
    }
