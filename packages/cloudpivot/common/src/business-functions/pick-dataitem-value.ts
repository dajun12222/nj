...
function getDataString(value: any, displayFormat: number) {
  ...
  if (value && value !== '--') {
    if (typeof value === 'string') {
      //修复当传入yy-mm格式的数据safari/ie下日期转换问题
      let pads: string[] = [];
      pads = value.split('-');
      if (pads.length < 3) {
    ...
    switch (displayFormat) {
      ...
      case 10:
        dataString = `${time} ${hours > 12 ? '下午' : '上午'} ${
          hours > 12 ? Number(hours) - 12 : hours
        }:${minutes}`;
        break;
      case 11:
  ...
  if (value === 0) {
    ...
    switch (displayFormat) {
      ...
      case 1:
        ...
        break;
      case 2:
        numberString = utils.addZero(
          Number(value).toFixed(2).slice(0, -1).toLocaleString(),
          1,
        );
        break;
      case 3:
        numberString = utils.addZero(
          Number(value.toFixed(3).slice(0, -1)).toLocaleString(),
          2,
        );
        break;
      case 4:
      ...
      case 11:
        numberString = utils.addZero(
          Number(value).toFixed(4).slice(0, -1).toLocaleString(),
          3,
        );
        break;
      case 12:
        numberString = utils.addZero(
          Number(value).toFixed(5).slice(0, -1).toLocaleString(),
          4,
        );
        break;
      case 13:
      ...
      case 23:
        numberString = `${utils.addZero(
          Number(value).toFixed(6).slice(0, -1).toLocaleString(),
          5,
        )}`;
        break;
      case 24:
        numberString = `${utils.addZero(
          Number(value).toFixed(7).slice(0, -1).toLocaleString(),
          6,
        )}`;
        break;
      case 25:
        numberString = `${utils.addZero(
          Number(value).toFixed(8).slice(0, -1).toLocaleString(),
          7,
        )}`;
        break;
      case 26:
        numberString = `${utils.addZero(
          Number(value).toFixed(9).slice(0, -1).toLocaleString(),
          8,
        )}`;
        break;
      case 27:
...
export function getRealValue(property: listParams.QueryColumns, value: any) {
  const { propertyType, propertyCode, displayFormat } = property;
