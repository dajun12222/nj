import mobileListTemplate from '../../../../list/src/components/listCustom/template/mobile/handler';
import pcListTemplate from '../../../../list/src/components/listCustom/template/pc/handler';
...
export class ListHtmlGenerator {
  ...
  private _presentationType: QueryPresentationTypes =
    QueryPresentationTypes.LIST;
  ...
  private validatePresentation() {
    this._presentationType =
      this.options.queryPresentationType || QueryPresentationTypes.LIST;
    this._presentation = {
      htmlJson: '',
      actionsJson: '',
      columnsJson: '',
    ...
    if (this.options.queryPresentation) {
      const { htmlJson, actionsJson, columnsJson } =
        this.options.queryPresentation;
      this._presentation.htmlJson = htmlJson
        ? ListElementGeneratorService.parseString(htmlJson)
        : '';
      this._presentation.actionsJson = actionsJson
        ? ListElementGeneratorService.parseString(actionsJson)
        : '';
      this._presentation.columnsJson = columnsJson
        ? ListElementGeneratorService.parseString(columnsJson)
        : '';
    }
  }
  ...
  private validateActions() {
    ...
    this._actions = this.options.queryActions.map((act: QueryAction) => {
      ...
      const attrs = actionAttrs.find(
        (item: ActionJsonItem) => item.actionCode === act.actionCode,
      );
     ...
     * 补充处理自定义操作按钮
     */
    const customActions = actionAttrs.filter(
      (item: ActionJsonItem) =>
        !this.options.queryActions.some(
          (act: QueryAction) => act.actionCode === item.actionCode,
        ),
    );
  ...
  private validateColumns() {
    ...
    this._columns = this.options.queryColumns.map((col: QueryColumn) => {
      ...
      const columnItem = columnCustoms.find(
        (item: ColumnJsonItem) => item.propertyCode === col.propertyCode,
      );

      if (
        columnItem &&
        columnItem.custom &&
        columnItem.custom[this._presentationType.toLowerCase()]
      ) {
        column.customHtml =
          columnItem.custom[this._presentationType.toLowerCase()];
      }
  ...
  private generateStyle() {
    ...
    let inner = '\n';
    if (
      this._presentation.htmlJson &&
      typeof this._presentation.htmlJson !== 'string'
    ) {
  ...
  private async generateTemplateAsync() {
    ...
    // 在编辑器内增加字段提醒
    const shownFieldsComment = `<!--\n\t! 当前展示字段:\n\t[\n\t\t${this.options.queryColumns
      .map((c) => `{ name:"${c.name}", propertyCode:"${c.propertyCode}" }`)
      .join(',\n\t\t')}\n\t]\n-->\n`;
  ...
  private generateScriptVersionLogs(version) {
    const handler =
      this.listEventHooksHandler ||
      (this.listEventHooksHandler =
        require('../../../../list/src/components/listCustom/eventHooks/handler').default);
    const versionClass = handler.loadSupportingVersionClass(version);

    return (this._scriptVersionLogs =
      `<!-- 自定义脚本: 配置/扩展/事件钩子 (version:${version}) -->` +
      '\n' +
  ...
  private generateScript() {
    ...
    // 如果有值, 优先输出原值
      ...
      try {
        ...
        scription = handler.parseScriptString(
          this._presentation.htmlJson.scriptJson,
        );
        // 如果获取到的数据不正确, 也不做处理了, 直接返回原值
        if (!scription.options || !scription.options.version) {
          throw new Error();
        }
        // 正常处理
  ...
  private combineContent() {
    ...
    this._htmlContent =
      ...
      '\n\t' +
      (isPC
        ? 'PC端共四个可编辑模块: style、actions、template、script'
        : 'Mobile端共三个可编辑模块: style、template、script') +
  ...
  static createElementString(attribute: CreateElementAtt) {
    ...
    if (attrs.length) {
      attrString =
        ...
        attrs
          .map((att: ElementAttr) =>
            att.name === 'sort' ? '' : `${att.name}="${att.value}"`,
          )
          .join(' ');
    }
