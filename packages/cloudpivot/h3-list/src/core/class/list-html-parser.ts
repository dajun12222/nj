import ListEventHooksHandler from '../../../../list/src/components/listCustom/eventHooks/handler';
import {
  ActionElement,
  ActionJsonItem,
  AssociationTypes,
...
export class ListHtmlParser {
  ...
  validateAttrs(
    ...
    if (must.requiredKeys.length) {
      const LostKeys: string[] = must.requiredKeys.filter(
        (key) => !el.attrs.some((att) => att.name === key),
      );
      if (LostKeys.length) {
        throw new Error(
          `<${el.tagName}> must has attributes: ${LostKeys.join('、')}!`,
        );
      }
    }
  }
  ...
  validateChildren(
    ...
    if (el.children.length) {
      const childNodes = el.children.filter(
        (e) => !must.optionalNames.includes(e.tagName),
      );
  ...
  findElementsByClass(className: string) {
    const els = this._elements.filter((e) =>
      e.attrs.some((attr) => attr.name === className),
    );
    return els;
  }
  ...
  private callParseHtml(html: string) {
    ...
    parseHTML(html, {
      start(
        ...
        const parent: ASTElement | null = stack.length
  ...
  private handlerActions() {
    ...
    // 检测预设按钮是否健全
    if (
      !this.sysActions.every(
        (sa) =>
          _actions.some((a) => a.code === sa) ||
          ((this.options.modelType === 'TREE' ||
            this.options.queryPresentationType === 'GANTT') &&
            sa === 'qr_code'),
      )
    ) {
      throw new Error(`请勿删除默认按钮或修改默认按钮code属性`);
    }
  ...
  private combineStyle() {
    ...
    if (styleElement && styleElement.innerHTML) {
      (this._queryPresentation.htmlJson as HtmlJson).styleJson =
  ...
  private combineScript() {
    ...
    if (scriptElement && scriptElement.innerHTML) {
      ...
      const scription = ListEventHooksHandler.parseScriptString(
        scriptElement.innerHTML,
      );
      // let versionHandler = ListEventHooksHandler.loadSupportingVersionHandler(scription.)
      (this._queryPresentation.htmlJson as HtmlJson).scriptJson =
        scriptElement.innerHTML;
    }
    if (elIdx > -1) {
      // 每次校验完将元素剔除，减少后续的元素搜寻时间
  ...
  private combineCustomTemplate() {
    ...
    if (
      ...
      if (compiled.errors.length) {
        ...
        throw new Error('自定义模板解析错误！详情请查看控制台！');
      }

      (this._queryPresentation.htmlJson as HtmlJson).templateJson =
        customTemplate.innerHTML;
    }
    if (elIdx > -1) {
      // 每次校验完将元素剔除，减少后续的元素搜寻时间
  ...
  private combineActions() {
    ...
    outputOptions.queryActions = (outputOptions.queryActions as []).filter(
      ...
      (ac1: ActionJsonItem) => {
        return (
          this.sysActions.includes(ac1.actionCode) ||
    ...
    this.actions.forEach((act: ActionElement) => {
      ...
      Object.keys(act).forEach((key: string) => {
        const extAttributes: string[] = this.sysActions.includes(act.code)
          ? ['code', 'text', 'sort']
          : ['code', 'sort'];
      ...
      const isSysAction: boolean = self._outputOptions.queryActions.some(
        (action: QueryAction) => {
    ...
    if (actionsJson.length) {
      self._queryPresentation.actionsJson = actionsJson.map((a1: any) => {
        const a2: any = self._outputOptions.queryActions.find(
          (a: any) => a.actionCode === a1.actionCode,
        );
        if (a2 && a2.queryActionType === QueryActionTypes.CUSTOM) {
          return { ...a1, ...a2 };
        } else {
          return a1;
        }
  ...
  setQueryColumn(col: {
    ...
    (this._outputOptions as ListOptions).queryColumns.some(
      (column: QueryColumn) => {
  ...
  sortColumns(sorts: string[]) {
    ...
    if (sortedColumns.length === queryColumns.length) {
      ...
      throw new Error(
        "Generated columns length is not equal to source columns' length",
      );
    }
  }
  ...
  private combinePresentation() {
    const { htmlJson, actionsJson, columnsJson } = this._queryPresentation;
...
export abstract class ListElementParserService {
  ...
  static convertAttrName(attrName: string) {
    const name = attrName.replace(/-([a-z]{1})/, ($0: string, $1: string) =>
      $1.toUpperCase(),
    );
    return name;
  }
