...
const shouldIgnoreFirstNewline = (tag: any, html: string) =>
...
export function parseHTML(html: string, options: any) {
  ...
  while (html) {
    ...
    if (!lastTag || !isPlainTextElement(lastTag)) {
      ...
      const rest = html.replace(reStackedTag, function (all, text, _endTag) {
        endTagLength = _endTag.length;
  ...
  function parseEndTag(tagName?: string, start?: any, end?: any) {
