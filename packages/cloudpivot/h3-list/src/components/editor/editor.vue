...
export default class ListEditor extends Vue {
  ...
  async onOptionsChange(options: ListOptions) {
    ...
    if (generator) {
      vm._content = vm.content =
        (generator as ListHtmlGenerator).htmlContent || '';
    }
  }
  ...
  parseToHtml(options: ListOptions) {
    ...
    const loadingModal = vm.$message.loading(
      vm.$t('languages.Apps.listDesignPage.htmlGenerating'),
      0,
    ) as any;

    return new Promise((resolve, reject) => {
      setTimeout(async () => {
        const generator: ListHtmlGenerator = await Converter.objectToHtml(
          options,
        );
        loadingModal();
        resolve(generator);
