...
class ListTableParser extends ListHtmlParser {
  ...
  validateStart(el: ASTElement, parent: ASTElement | null) {
    if (this.options.clientType === 'APP') {
      return;
    }
    ...
    switch (name) {
      case 'l-table':
        this.validateParent(
          { name: 'template', id: 'customTemplate' },
          el,
          parent,
        );
        break;
  ...
  async combineColumns() {
    ...
    if (
      ...
      this.options.queryColumns.some((col) => {
        return (
          !ths.some((thsItem) =>
            thsItem.attrs.find((a) => a.name === 'key' && a.value === col.code),
          ) ||
          !tds.some((tdsItem) =>
            tdsItem.attrs.find((a) => a.name === 'key' && a.value === col.code),
          )
        );
      })
    ) {
      throw new Error(
        `adding/deleting column is not allowed in table template`,
      );
    }
    ...
    await ths.forEach((th: ASTElement) => {
      ...
      const matchedTd = tds.find(
        (td: ASTElement) =>
          ListElementParserService.getElementAttr('key', td) === key,
      );

      if (!matchedTd) {
        throw new Error(
          'Unequal columns between the table head and table body!',
        );
      }
      ...
      if (matchedTd && matchedTd.children.length) {
        matchedTd.innerHTML = matchedTd.children
          .map((el: any) => {
