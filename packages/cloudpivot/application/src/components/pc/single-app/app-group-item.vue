<template>
  <div class="app-group-item" @click="openItem">
    ...
    <span class="app-group-item__name" :title="getLang(name, name_i18n)">{{
      getLang(name, name_i18n)
    }}</span>
  </div>
</template>
...
export default class AppGroupItem extends Vue {
  ...
  get appCode() {
    return (
      this.environment.appCode ||
      ((window as any).Environment && (window as any).Environment.appCode)
    );
  }
