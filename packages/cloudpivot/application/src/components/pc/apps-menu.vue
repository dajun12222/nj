  ...
  <div class="menu-content">
    ...
    >
      <template v-for="menu in appTrees">
        ...
        >
          <router-link v-if="menu.type === 'BizModel'" :to="''">
            <i class="icon aufontAll" :class="menu.icon"></i>
            <span :class="specStyle ? 'specStyles' : ''">{{
              isChinese ? menu.name : menu.name_i18n[$i18n.locale]
            }}</span>
          </router-link>
          <a v-else>
            <i class="icon aufontAll" :class="menu.icon"></i>
...
export default class AppsMenu extends Vue {
  ...
  async loadFirstMenu() {
    ...
    if (res.errcode === 0) {
      ...
      console.log(this.appTrees, 'this.appTrees');
  ...
  setTreeStatus(treeData: any) {
    // 之前使用index作为defaultOpenKeys,现在全部用id,需要重新计算
    const { query: queryData } = { ...this.$route };
    const parentId = queryData.parentId;
    const code = queryData.code;
    const defaultOpenKeys = [] as any;
    const _backtrack = (data, id) => {
      if (!data || !id) {
        return;
      }
      data.some((item: any) => {
    ...
    const _helper = (data, id) => {
      if (!data) {
        return;
      }
      data.some((item: any) => {
        // 这个是做深度遍历,这个是获取指定节点ID
        if (item.code === code && item.parentId === id) {
          //// 后面回溯时不再做这个处理
    ...
    if (treeData['0'].parentId !== parentId) {
      // 表明是自定义的分组
  ...
  async getFolder() {
    ...
    if (queryData && queryData.parentId && queryData.code) {
      ...
      if (parentIds === this.appTrees[0].parentId) {
        const index = this.appTrees.findIndex(
          (tree: any) => tree.code === codes,
        );
        if (index !== -1) {
          this.selectKeys = [this.appTrees[index].id];
          if (!queryData.openMode || queryData.openMode === 'undefined') {
            const params: any = {
            ...
            queryData.openMode === 'RECENT_PAGE_MODE' &&
            queryData.pcUrl
          ) {
            this.$router
              .push({
  ...
  initFirstMode() {
    ...
    } else if (this.appTrees[0].type === 'Page') {
      // 自定义页面
  ...
  setDefaultKeys(schemaCodes: any, indexKey?: number) {
    if (schemaCodes && indexKey) {
      const key = this.appTrees[indexKey - 1].children.findIndex(
        (tree: any) => tree.code === schemaCodes,
      );
      if (key === -1) {
        this.initFirstMode();
        return;
  ...
  updateFolderChildById(key: string, listData: any) {
    // 更新folder下对应的children,会改变appTree的值
    const _helper = (data) => {
      if (!data) {
        return;
      }
  ...
  getItemFromTreeById(key: any, isFolder?: any) {
    ...
    const _helper = (data) => {
      ...
      data.some((item: any) => {
        if (item.id === key && item.type !== 'Folder' && !isFolder) {
          // 查询item info
          selectItem = item;
          return true;
        } else if (item.id === key && item.type === 'Folder' && isFolder) {
          // 查询folder info
          selectItem = item;
          return true;
  ...
  getNameBySelectedKeys(keys: any) {
    ...
    //   if(!data)return;
    //   data.some((item: any)=>{
    //     if(item.id === _keys && item.type !== 'Folder'){
    //       selectItem = item;
    //       return true;
    //     }else if(item.type === 'Folder' && item.children){
    //       _helper(item.children);
  ...
  switchRouter(menu: any) {
    if (menu.type === 'BizModel') {
      ...
      this.reportGo(menu);
    } else if (menu.pcUrl && menu.pcUrl.includes('workflow-design')) {
      // 流程设计,则新开
  ...
  bizModelGo(menu: any) {
    if (menu.appCode === 'nfq007' && menu.code === 'liucheng0001') {
      this.$router
        .push({
