  ...
  <ul class="application-menu">
    <li @click="setSliderPos">
      <router-link to="/workflow-center">
        <span :title="$t('languages.common.workflowCenter')">{{
    ...
    <li
      ...
      :key="index"
      :class="
        temAppList.length > 6 && index === appList.length - 1 ? 'last-menu' : ''
      "
      @click="onClickAppRouter(app)"
    >
      <router-link :to="'/application/' + app.code">
        <span :title="isChinese ? app.name : app.name_i18n[$i18n.locale]">{{
          isChinese ? app.name : app.name_i18n[$i18n.locale]
        }}</span>
      </router-link>
    </li>
    <li v-if="isShowMoreApps" class="more">
      <a-dropdown
        v-model="visible"
        :trigger="trigger"
      ...
      >
        <i
          class="icon aufontAll h-icon-all-appstore"
          :class="isIconActive ? 'active' : ''"
        ></i>
        <div
          slot="overlay"
          class="apps-box"
          :style="{
            minHeight: appListExtra.length <= 0 ? `${minHeight}px` : 'unset',
          }"
        >
          ...
          <p v-if="appListExtra.length <= 0" class="no-result">
            {{ $t('cloudpivot.application.pc.noResult', { keyword: keyword }) }}
          </p>
    ...
    <div
      class="slider"
      :style="{
        width: `${colorBlock.width}px`,
        transform: `translate3d(${colorBlock.left}px, 0, 0)`,
        opacity: colorBlock.opacity,
      }"
    ></div>
  </ul>
</template>
...
export default class CommonHeader extends Vue {
  ...
  getDownloadUrl(refId: string) {
    let url = `${
      this.apiHost
    }/api/aliyun/download?refId=${getDownloadUrlNew.getRefId(
  ...
  async init() {
    ...
    // 初次加载是否显示更多按钮高亮
    const isExistInExtraApp: number = this.appListExtra.findIndex(
      (app: any) => app.code === appCode,
    ) as number;
  ...
  initSlider() {
    this.$nextTick(() => {
      ...
      const curRouteDom: HTMLElement = document.querySelector(
        '.application-menu li a.router-link-active',
      ) as HTMLElement;
      if (!curRouteDom) {
        return;
      }
    ...
    if (keyword === '') {
      ...
      this.appListExtra = this.appListExtraCopy.filter((list: any) => {
        const name: string = this.isChinese
          ? list.name
          : list.name_i18n[this.$i18n.locale];
  ...
  setSearchHighLight() {
    this.appListExtra.forEach((list: any) => {
      const key: string = this.isChinese
        ? list.name
        : list.name_i18n[this.$i18n.locale];
  ...
  async getAppList() {
    ...
    try {
      res = await listApi.list({ isMobile: false });
    } catch (e) {
      console.log(e, '----------------------E');
      res = await listApi.list({ isMobile: false });
    }
    ...
    if (Array.isArray(res.data)) {
      res.data.forEach((data: any) => {
        ...
        data.displayName = this.isChinese
          ? data.name
          : data.name_i18n[this.$i18n.locale];
      });
    }
    ...
    if (res.data.length <= 6) {
      ...
      const rows: number =
        this.appListExtra.length <= 4
          ? 1
          : Math.ceil(this.appListExtra.length / 4);
      const _h: number =
        rows * this.singleRowHeight + this.searchBoxHeight + this.boxPadding;
