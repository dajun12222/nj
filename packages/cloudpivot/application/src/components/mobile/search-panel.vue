  ...
  <div class="search-panel">
    ...
    <div v-show="!showResult" class="search-panel__wrap search-panel__history">
      <p class="search-panel__title">
        {{ $t('cloudpivot.application.mobile.HistorySearch') }}
      </p>
      <ul>
...
import { listApi, initialInstanceParams } from '../../../../api';
  ...
  search(wd: string) {
    ...
    if (this.appSearch) {
      ...
      this.appList.forEach((app: any) => {
        if (app.name.toLowerCase().indexOf(this.wd.toLowerCase()) !== -1) {
          this.list.push(app);
        }
      });
      ...
      listApi
        .searchBizModels({ searchKey: wd, appCode: this.appCode })
        .then((res: any) => {
