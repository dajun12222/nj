<template>
  <div class="app-list-tab">
    ...
    >
      <span class="item-text" :title="item.name" v-text="item.name"></span>
    </div>
    <div
...
export default class AppListTab extends Vue {
  ...
  get tabItemStyle() {
    const style: any = {};
    const tabLength =
      this.moreList.length > 0 ? this.showMaxSize + 1 : this.showMaxSize;

    const percent: any = 100 / tabLength + '%';
    style.width = percent;
    return style;
  }
  ...
  showMoreClick() {
    this.$nextTick(() => {
      const bodyHeight = (document as any).documentElement.clientHeight;
      const position: any = this.$el.getBoundingClientRect();
