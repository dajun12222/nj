...
export default class FormCommentBase extends Vue {
  ...
  get workflowInstanceId() {
    return this.formObj.bizObject
      ? this.formObj.bizObject.workflowInstanceId
      : '';
  }
  ...
  getDelBtnPermission(item: any) {
    ...
    const isAdmin: boolean =
      isAppAdmin || isSysAdmin || isRootAdmin || isDtaAdmin;
  ...
  handleInputBase(atwhoEditWrap: any, atPane: any) {
    ...
    if (strWithoutHtmlTag.indexOf('@') <= -1) {
      return;
    } // 没有输入@
    ...
    const strAfterAt: string = strWithoutHtmlTag
      .substr(strWithoutHtmlTag.lastIndexOf('@'))
      .split('@')[1]
      .trim();
  ...
  async getCommListNum() {
    ...
    if (!formObj || !bizObjectId || !schemaCode) {
      return;
    }
    const params: FormCommentIns.formCommentParams.CommentList = {
      bizObjectId,
  ...
  getWidth() {
    ...
    const el = document.querySelector(
      '.attachment-box.preview > ul > li:first-of-type > .attachment-info',
    );
    if (el) {
      size = el.clientWidth;
    }
