...
import mobile from '../../../../form/src/renderer/components/mobile';
import * as platform from '../../../../platform';
import dd from 'dingtalk-jsapi';
import { H3Upload } from 'h3-mobile-vue';
import { Component, Watch } from 'vue-property-decorator';
import * as Back from '../../../../common/src/config/mobile/back';
...
export default class FormComment extends FormCommentBase {
  ...
  showCommentPanel() {
    platform.service.setTitle(
      `${this.$t('cloudpivot.formComment.mobile.writeComment')}`,
    );
  ...
  handlePreview(file: any) {
    if (file.response) {
      ...
      if (UploadHelper.testDocument(ext)) {
        ...
        const msg = this.$t('文件类型不支持，请在PC查看').toString();
        this.$messager.error(msg);
      }
    }
  }
  ...
  // type 为空 初次加载
  init(type?: string) {
    Promise.all([this.getCommList(), this.getCommListNum()]).then(() => {
      // 设置钉钉title
      if (this.isShowCommentPanel) {
        // 评论面板
        platform.service.setTitle(
        ...
        // 评论列表面板
        platform.service.setTitle(
          `${this.$t('cloudpivot.formComment.mobile.comment')}（${this.num}）`,
        );
      }
        ...
        if (res.errcode === 0) {
          ...
          this.$nextTick(() => {
            function setCollspan(item) {
              ...
              const remStandard: number = parseFloat(
                (document as any)
                  .querySelector('html')
                  .style.fontSize.replace('px', ''),
              ) as number;
              const heightRem: number =
                (
                  (document as any).querySelector(selector) as any
                ).getBoundingClientRect().height / remStandard;
          ...
          this.$nextTick(() => {
            ...
            (document.querySelector('.mescroll') as any).style.bottom =
              maxHeight;
          });
          ...
          this.getCommListNum().then(() => {
            if (this.isShowCommentPanel) {
              ...
              // 评论列表面板
              platform.service.setTitle(
                `${this.$t('cloudpivot.formComment.mobile.comment')}（${
                  this.num
                }）`,
              );
            }
  ...
  beforeUploadMobile(files: any) {
    if (!super.checkFilesNum(files)) {
      (this as any).$messager.error(
        `${this.$t('cloudpivot.formComment.mobile.imageTips1')}`,
      );
      return false;
    }
    ...
    // 检查类型
    const isAllTypeOk: any = fileList.every((file: any) =>
      super.checkFileType(file),
    );
    ...
    if (!isAllTypeOk) {
      (this as any).$messager.error(
        `${this.$t('cloudpivot.formComment.mobile.imageTips2', {
          text: notAllowExt.join(','),
        })}`,
      );
      return false;
    }
    ...
    // 检查大小
    const isAllSizeOk = fileList.every((file: any) =>
      super.checkFileSize(file),
    );
    ...
    if (!isAllSizeOk) {
      (this as any).$messager.error(
        `${this.$t('cloudpivot.formComment.mobile.moreThan')}`,
      );
      return false;
    }
  ...
  async send(item: any, type?: string) {
    if (type === 'comment') {
      ...
      if (super.getInnerHtmlLen(comment) <= 0 && fileLists.length <= 0) {
        this.$messager.error(
          `${this.$t('cloudpivot.formComment.mobile.contentNoEmpty')}`,
        );
        return;
      }
    } else {
      if (super.getInnerHtmlLen(this.replayComment) <= 0) {
        this.$messager.error(
          `${this.$t('cloudpivot.formComment.mobile.replyNoEmpty')}`,
        );
        return;
      }
    }
    ...
    this.isInit = true;
    super
      .sendBase(item, this.replayComment, type)
      .then((res: any) => {
        if (res.errcode === 0) {
          if (type === 'comment') {
            this.comment = '';
  ...
  async handleDeleteComm(id: string) {
    ...
    super
      ...
      .then((res: any) => {
        if (res.errcode === 0) {
          (this as any).$messager.success(
            `${this.$t('cloudpivot.formComment.mobile.deleteCommentSuccess')}`,
          );
          setTimeout(() => {
            // this.getCommList();
            this.loadMoreS({ num: 1 }, null);
      ...
      .catch((errMsg: any) => {
        (this as any).$messager.error(
          `${this.$t('cloudpivot.formComment.mobile.deleteCommentFailed')}`,
        );
      });
  }
  ...
  ok(data: any) {
    ...
    if (this.atClickTrigger) {
      if (!isReplayMode) {
        ...
        data.forEach((d: any) => {
          this.replayComment =
            this.replayComment + `${this.embeddedItemTemplate(d)} `;
        });
      }
    }
