<template>
  <div v-show="visible" class="user-picker">
    <div class="user-picker-header">
      <div class="input-box">
        <search-bar :value="keyword" @cancel="cancelSearch" @search="search" />
      </div>
    </div>
      ...
      >
        <div class="member-list">
          <ul>
            ...
            >
              <div class="select-box" :class="item.selected ? 'selected' : ''">
                <i
                  v-if="item.selected"
    ...
    <div class="user-picker-footer">
      <div class="user-picker-footer-left">
        ...
        <span class="has-select">{{
          `${selectedNum}${$t('cloudpivot.formComment.mobile.person')}`
        }}</span>
      </div>

      <div>
        <button class="j-button" @click="submit">
          {{ $t('cloudpivot.formComment.mobile.sure') }}
        </button>
...
import { H3Button, H3SearchBar } from 'h3-mobile-vue';
