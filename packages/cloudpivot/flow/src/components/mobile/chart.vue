<template>
  <div class="flow-track-chart" @touchstar.stop>
    ...
    <!-- 操作引导 -->
    <div v-if="!hasGuide" class="chart-guide" @click="hideGuide">
...
export default class WorkflowChart extends Vue {
    ...
    workflowApi
      ...
      .then((resp: any) => {
        ...
        resp.data.activities.forEach((item: any) => {
          // 记录横向和纵向的最大值, 用以做缩放处理
          if ((value = item.x + item.width) > hMax) {
            hMax = value;
          }
          if ((value = item.y + item.height) > vMax) {
            vMax = value;
          }
          // 兼容处理(多语言)
  ...
  initChartLayerStyle(isFullScreen?: boolean) {
    ...
    (this.chart.rules || []).forEach((item: any) => {
      ...
      points.forEach((point: any) => {
        ...
        if ((tempVal = parseInt(str2Arr[0], 10)) > width) {
          width = tempVal;
        }
        if ((tempVal = parseInt(str2Arr[1], 10)) > height) {
          height = tempVal;
        }
      });
    });
    ...
    const wrapper =
      this.drawerWrapper ||
      (this.drawerWrapper = this.$el.querySelector(
        '.flow-track-chart__drawer',
      ) as any);
    const wrapperOffset = 35; // 增加侧边的偏移, 计算合适的偏移让初始缩放不碰到边
    const wrapperWidth = wrapper.clientWidth - wrapperOffset;
    const wrapperHeight = wrapper.clientHeight - 30;
    const baseOnWidth = wrapperWidth / wrapperHeight < width / height;
    let scale = baseOnWidth
      ? +(wrapperWidth / width).toFixed(2)
    ...
    // 3. 位置偏移
    const offsetLeft = -(width - wrapperWidth - wrapperOffset) / 2;
    const offsetTop = -(height - wrapperHeight - wrapperOffset) / 2;
    ...
    return {
      ...
      // 初始为了绝对居中要做的上下或左右偏移
      baseOnWidth,
      initCenterOffsetLeft: baseOnWidth
        ? 0
        : (wrapperWidth - scale * width) / 2,
      initCenterOffsetTop: baseOnWidth
        ? (wrapperHeight - scale * height) / 2
        : 0,
  ...
  get serializedChartStyle() {
    const { width, height, offsetLeft, offsetTop, scale, hidden } =
  ...
  touchmove(e) {
    ...
    const fingerLen = fingers.length;
    const touching = this.touching;

    if (fingerLen > 1) {
      if (!this.startMaxFingerDistance) {
        this.startMaxFingerDistance = this.preMaxFingerDistance =
          this.getMaxFingerDistance(fingers);
      }
  ...
  touchend(e) {
    this.isDragging = false;
    this.startMaxFingerDistance = this.preMaxFingerDistance = 0;
  ...
  dragmove(e, finger) {
    ...
    const touching = this.touching;
    const { prevX, prevY, powerX, powerY } = touching;
    const pageX = finger.pageX,
      pageY = finger.pageY;
    ...
    const { maxOffsetLeft, maxOffsetRight, maxOffsetTop, maxOffsetBottom } =
      layerStyle;
  ...
  naturalScrolling() {
    ...
    let {
      offsetLeft: posX,
      offsetTop: posY,
      maxOffsetLeft: minPosX,
      maxOffsetRight: maxPosX,
      maxOffsetTop: minPosY,
      maxOffsetBottom: maxPosY,
      width,
      height,
      scale,
    } = layerStyle;
    let { powerX, powerY, frictionX, frictionY, elasticX, elasticY, style } =
      touching;
    const { clientWidth, clientHeight } = this.drawerWrapper;
  ...
  stopNaturalScrolling() {
    const layer = this.chartLayerStyle;
    const touching = this.touching;
    const { posX, posY, slideTimer, style } = touching;
  ...
  zoom(e: any, fingers: any) {
    ...
    const layerX =
      ([] as any).reduce.call(
        fingers,
        (s: any, c: any, i: any) => s + c.pageX,
        0,
      ) / fingers.length;
    const layerY =
      ([] as any).reduce.call(
        fingers,
        (s: any, c: any, i: any) => s + c.pageY,
        0,
      ) /
        fingers.length -
      this.zoomWrapperPageOffsetTop;
    const layerCenterX =
      this.zoomWrapperCenterX ||
      (this.zoomWrapperCenterX =
        (
          this.zoomWrapper ||
          (this.zoomWrapper = this.$el.querySelector(
            '.flow-track-chart__drawer',
          ))
        ).clientWidth / 2);
    const layerCenterY =
    ...
    const mouseOffsetY = layerY - layerCenterY + movingTop;
    ...
    // 手指缩放的距离, 取最大移动
    const zoomDistance = this.getMaxFingerDistance(fingers);
    const zoomDistanceOffset = zoomDistance - this.preMaxFingerDistance;
    let zoomScaleInRealSize =
      (zoomDistanceOffset / Math.max(width, height)) * 10;
    ...
    // 缩放
    if (zoomDistanceOffset > 0) {
      if (scale + zoomScaleInRealSize > maxScale) {
        zoomScaleInRealSize = maxScale - scale;
      }
    } else if (zoomDistanceOffset < 0) {
      if (scale + zoomScaleInRealSize < minScale) {
        zoomScaleInRealSize = -(scale - minScale);
      }
    } else {
      //Else Empty block statement
    }
    ...
    // 实际需要偏移位置
    let scaleLeft =
        offsetLeft - ((width * zoomScaleInRealSize) / 2) * mouseOffsetXScale,
      scaleTop =
        offsetTop - ((height * zoomScaleInRealSize) / 2) * mouseOffsetYScale;
    ...
    const { maxOffsetLeft, maxOffsetRight, maxOffsetTop, maxOffsetBottom } =
      layerStyle;
    if (scaleLeft < maxOffsetLeft) {
      scaleLeft = maxOffsetLeft;
    } else if (scaleLeft > maxOffsetRight) {
      scaleLeft = maxOffsetRight;
    } else {
      //Else Empty block statement
    }
    if (scaleTop < maxOffsetTop) {
      scaleTop = maxOffsetTop;
    } else if (scaleTop > maxOffsetBottom) {
      scaleTop = maxOffsetBottom;
    } else {
      //Else Empty block statement
    }
  ...
  getMaxFingerDistance(fingers) {
    ...
    [].forEach.call(fingers, (t: any, i: any) => {
  ...
  setMaxOffset(scale) {
    ...
    let offsetHoriRange =
        (width * (scale - initScale)) / 2 - initCenterOffsetLeft,
      offsetVertRange =
        (height * (scale - initScale)) / 2 - initCenterOffsetTop;
    ...
    if (maxOffsetLeft > maxOffsetRight) {
      maxOffsetLeft = [maxOffsetRight, (maxOffsetRight = maxOffsetLeft)][0];
    }

    if (maxOffsetTop > maxOffsetBottom) {
      maxOffsetTop = [maxOffsetBottom, (maxOffsetBottom = maxOffsetTop)][0];
    }
