<template>
  <div class="form-approval">
    <!-- 已完成流程增加结束 -->
    <div v-if="completed || canceled" class="form-approval__item">
      ...
      <div class="form-approval__instances">
        ...
        <div class="form-approval__instance">
          <div class="form-approval__progress">
            <div class="form-approval__date end">
              {{ finishTime }}
            </div>
          </div>
        </div>
    ...
    >
      ...
      <div class="form-approval__instances">
        <div class="form-approval__title">
          <label>{{
            isChinese ? approval.activityName : approval.name_i18n[$i18n.locale]
          }}</label>
        </div>
          ...
          <a-collapse :bordered="false">
            <a-collapse-panel :showArrow="false" class="approval-item-collapse">
              <template v-slot:header="props">
                <a-icon
                  :type="props.isActive ? 'minus-circle-o' : 'plus-circle-o'"
                />
                <span v-show="!props.isActive">{{
                  approvalStats(approval.nodes)
                }}</span>
              </template>
...
import { workflow, workflowApi, user } from '../../../../../api';

import ApprovalItem from './approval-item.vue';

import { workflowActionStatus } from '../../../typings/workflow-enum';
...
export default class FormApproval extends Vue {
  ...
  download(refId: string) {
    ...
    // const url = `${env.apiHost}/api/aliyun/download?refId=${refId}&T=${token}`;
    // window.open(url);
    this.$emit('download', {
      refId,
    });
  ...
  approvalStats(nodes: Array<any>) {
    ...
    Object.entries(workflowActionStatus).forEach(([action, label]) => {
      const sameActionItems: Array<any> = nodes.filter(
        (node: any) => `${node.workActionType}` === action,
      );
    ...
    const statsMessage: string = stats
      .map((stat: any) => {
        let label = this.$t(
          `cloudpivot.flow.pc.WorkflowActionStatus.${stat.label}`,
        );
        if (btnConfigList) {
          for (const con of btnConfigList) {
            if (
              stat.label.toLowerCase() === con.code.toLowerCase() &&
              con.displayName
            ) {
              label = con.displayName;
            }
            if (
              stat.label === 'Circularize' &&
              con.code === 'circulate' &&
              con.displayName
            ) {
              //传阅特殊处理
        ...
        return stat.total + this.$t('cloudpivot.flow.pc.Person') + label;
      })
      .join('；');
...
.form-approval {
  ...
  overflow-y: scroll;
  padding-right: 12px;
  .form-approval__workflow {
    flex: 0;
    .form-approval__trail {
      margin: 0 8px;
    }
  }
  .form-approval__instances {
    flex: 1;
