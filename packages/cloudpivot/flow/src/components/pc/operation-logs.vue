  ...
  <div class="form-operation-logs">
    <div class="header form-header">
      <div class="back">
        ...
        <i
          class="back-icon icon aufontAll h-icon-all-arrow-left-o router-link-active"
          @click="back"
        ></i>
    ...
    <section class="main">
      ...
      >
        ...
        <!-- 平台 -->
        <span slot="clientTitle" class="resize">{{
...
export default class FormOperationLogs extends Vue {
  ...
  showDetail(data: any) {
    if (
      data.operationType === 26 ||
      data.operationType === 6 ||
      data.operationType === 8
    ) {
    ...
    const formOptions: ConfirmOptions = {
      ...
      content: `${
        data.detail ? data.detail : this.$t('cloudpivot.flow.pc.NoDetail')
      }`,
      buttons: ['ok'],
      align: 'center',
      buttonsText: [
