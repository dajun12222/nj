    ...
    >
      <template v-if="!estimate">
        ...
        >
          ...
          <div class="content">
            <div v-if="log.subInstanceActivity" class="user">
              ...
              />
              <label
                v-if="!!statusFn(log.subInstanceStatus, 'subInstance')"
                :class="`workflow-action ${getSubWorkflowActionStatusColorClass(
                  log.subInstanceStatus,
                )}`"
                ><span>{{
                  statusFn(log.subInstanceStatus, 'subInstance')
                }}</span></label>
            </div>
            <div v-else class="user">
              ...
              />
              <label
                v-if="!!statusFn(log.approvalActionStatus, 'workflow')"
                :class="`workflow-action ${getWorkflowActionStatusColorClass(
                  log.approvalActionStatus,
                )}`"
                ><span>{{
                  statusFn(log.approvalActionStatus, 'workflow')
                }}</span></label>
            </div>
            <div class="timers">
              <label class="timer">{{
        ...
        <div class="table_item">
          <div class="code center">序号</div>
          <div class="user">参与者</div>
          <div class="start_time center">接收时间</div>
          <div class="end_time center">完成时间</div>
          <div class="use_time center">耗时</div>
          <div class="option center">操作</div>
        </div>
        <div
          v-for="(log, i) in record"
          :key="i"
        ...
        >
          ...
          <div class="user">
            <a-avatar
              ...
              :src="getImageUrl(log.originator)"
            />
            <span>{{log.originator.name}}</span>
          </div>
          <div class="start_time center">{{ `${workItemLogTimeFormat(log.receiveTime, log, 'YYYY-MM-DD')}` }}</div>
          <div class="end_time center">{{ `${workItemLogTimeFormat(log.finishTime, log, 'YYYY-MM-DD')}` }}</div>
          <div class="use_time center">{{computerUseTime(log.usedTime)}}</div>
          <div class="option center">{{ statusFn(log.approvalActionStatus, 'workflow')}}</div>
        </div>
      </template>
      <template v-else>
        <label class="title">{{ predictor }}</label>
        <div v-for="(user, i) in record" :key="i" class="estimate-item">
          <Participants
            slot="originator"
            class="name"
            :participants="[user]"
            :i18nData="i18n"
            :trustor="user.trustor && user.trustor.id ? user.trustor : ''"
            :showTitle="false"
          />
        </div>
        <div v-if="!record || !record.length" class="estimate-item">
          {{ nothing }}
        </div>
      </template>
    </div>
...
<script lang="ts">
import { msesToTimeStr } from "../../../../../common/src/utils/date";
...
// 不兼容ie
// import { closest } from '../../../../../common/src/utils/dom';
...
export default class FormWorkflowTrackChartCard extends mixins(WorkflowMixin) {
  ...
  computerUseTime(val){
    return msesToTimeStr(val) ? msesToTimeStr(val) : "--"
  }
  destroy() {
    this.$emit('destroy');
  }
