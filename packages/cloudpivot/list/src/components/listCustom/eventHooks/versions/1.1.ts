...
export default class SubHandler_v_1_1 extends ListEventHooksHandler {
  ...
  static modes: string[] = ['combine', 'replace'];
  ...
  fetchApi(api: any, params) {
    ...
    if (!api || (typeof api !== 'object' && typeof api !== 'function')) {
      throw new Error('pageConfig.api should be a function or an object');
    }
    ...
    if (!mode) {
      ...
      throw new Error(`mode:"${mode}" is not allowed!`);
    } else {
      //Else Empty block statement
    }

    if (!request || typeof request !== 'function') {
      throw new Error(`api.request should be a function`);
    }
  ...
  //   return true;
  // }
  static isScriptObjectAvailabile(scription: any): boolean {
    // 调用父级校验
    ...
    if (typeof pageConfig === 'object') {
      ...
      if (apiConfig) {
        ...
        if (typeof apiConfig !== 'object') {
          throw new Error(
            `ListEventHooksHandler Error: pageConfig should be an object`,
          );
        }
        ...
        apiKeys.some((key: string) => {
          ...
          if (!api || (typeof api !== 'object' && typeof api !== 'function')) {
            throw new Error(
              `ListEventHooksHandler Error: pageConfig api should be a function or an object`,
            );
          }
          ...
          if (typeof api === 'object') {
            ...
            if (api.mode && !modes.includes(api.mode)) {
              throw new Error(
                `ListEventHooksHandler Error: pageConfig api.mode supports only [${modes}]`,
              );
            }
            ...
            if (!api.request) {
              throw new Error(
                `ListEventHooksHandler Error: pageConfig api.request missing`,
              );
            }

            if (typeof api.request !== 'function') {
              throw new Error(
                `ListEventHooksHandler Error: pageConfig api.request should be a function`,
              );
            }
          }
        });
