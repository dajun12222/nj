...
class QueryConditions {
  ...
  getValue(
    type: number,
    queryConditions: Array<QueryCondition.QueryCondition> = [],
    filterData: Array<QueryCondition.FilterData> = [],
  ) {
  ...
  getValueFromDefault(
    queryConditions: Array<QueryCondition.QueryCondition>,
  ): Array<QueryCondition.QueryCondition> {
  ...
  getValueFromFilterData(
    ...
    queryConditions = queryConditions.filter(
      (qc: QueryCondition.QueryCondition) => qc.visible,
    );
    ...
    if (vals.length <= 0 || queryConditions.length <= 0) {
      return [];
    }
    const arr: Array<any> = [];
    vals.forEach((val: QueryCondition.FilterData) => {
      const cd: QueryCondition.QueryCondition = queryConditions.find(
        (qc: QueryCondition.QueryCondition) =>
          qc.propertyCode === val.propertyCode,
      ) as QueryCondition.QueryCondition;
      if (cd) {
        if (cd.propertyCode === 'sequenceStatus') {
          ...
          cd.propertyType === DataItemType.RelevanceForm ||
          cd.propertyType === DataItemType.RelevanceFormEx
        ) {
          arr.push({
            name: cd.name,
            name_i18n: cd.name_i18n,
