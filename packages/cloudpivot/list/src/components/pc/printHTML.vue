        ...
        >
          ...
          <div
            ...
            :style="{
              width: pageStyle.width + 'px',
              height: `${
                (options.fontSize >= 12 ? options.fontSize : 12) * 1.5
              }px`,
              fontSize: `${translatePx2Mm(
                options.fontSize >= 12 ? options.fontSize : 12,
              )}px`,
              textAlign: 'center',
              overflow: 'hidden',
              whiteSpace: 'nowrap',
...
export default class PrintHtml extends Vue {
  @Prop({ default: { width: 300, height: 700, fontSize: 12, qrCodes: [] } })
  options!: Options;
  ...
  getMT(item: any) {
    ...
    const txtH: number = item.text
      ? (this.options.fontSize >= 12 ? this.options.fontSize : 12) * 1.5
      : 0;
