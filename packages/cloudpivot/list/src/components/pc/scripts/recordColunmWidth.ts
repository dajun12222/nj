...
export default {
   ...
   * @param width
   */
  handleColumResizeEnd(
    colum: AllTypes.ColumnOption,
    recordKey: string,
    columnsOptsKey: string,
  ) {
    const { width, column, parentColumn } = colum;
    const colKey: string = parentColumn
      ? `${parentColumn.id}.${column.id}`
      : column.id;
    ...
    if (records) {
      ...
      const item: AllTypes.Record = recordJson.value.find(
        (t: AllTypes.Record) => Object.keys(t)[0] === colKey,
      ) as AllTypes.Record;
      if (item) {
        // update
    ...
    // 更新列信息记录中的宽度
    const columnOpts: string = window.localStorage.getItem(
      columnsOptsKey,
    ) as string;
    if (columnOpts) {
      const cols: Array<AllTypes.DisplayedColumn> = JSON.parse(
        columnOpts,
      ) as Array<AllTypes.DisplayedColumn>;
      let childItem: any = null;
      const colItem: AllTypes.DisplayedColumn = cols.find(
        (col: AllTypes.DisplayedColumn) => {
          if (
            parentColumn &&
            parentColumn.id === col.id &&
            Array.isArray(col.childColumns)
          ) {
            childItem = col.childColumns.find(
              (c: any) => `${parentColumn.id}.${c.id}` === colKey,
            );
            return false;
          } else {
