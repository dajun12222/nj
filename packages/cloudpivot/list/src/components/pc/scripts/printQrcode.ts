...
export default {
  ...
  async createPrintQrCodeData(vm: any, data: any) {
    const customDisplayColumns = data.textData
      .filter((d: any) => d.isDataItem === 1)
      .map((c: any) => {
        return c.code;
      });
    ...
    if (res.errcode === 0 && res.data) {
      ...
      dataArr = res.data.content
        .filter((d: any) => objectIds.includes(d.id))
        .map((item: any) => {
          let str = '';
      ...
      // 调用接口批量生成短码
      const genShortCodesRes: any = await listApi.genShortCodes(
        genShortCodesParams,
      );
      if (genShortCodesRes.errcode === 0 && genShortCodesRes.data) {
        ...
        dataArr.forEach((item: any, index: number) => {
          ...
          const url: string = `${
            config.apiHost
          }/api/qrcode/generate_qrcode?text=${encodeURIComponent(text)}`;
          item.url = url;
        });
      }
    }
  ...
  printQrCode(vm: any, result: any) {
    ...
    const closeLoading = vm.$message.loading(
