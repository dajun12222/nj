<template>
  <a-modal
    ...
    @ok="sureImport"
  >
    <div v-if="isHalfSuccess" class="data-import-status-half">
      ...
      <p class="data-import-status-half__error">
        ...
        <span v-if="failNum && failNum > 0">{{ `${failNum}条数据导入失败，请` }}
          <a href="javascript:;" @click="exportErrorResult">下载错误数据</a>，修改后再导入
        </span>
    ...
    <div class="title-flex">
      ...
      <div class="flex">
        ...
        <a-popover
          :visible="showBacthUpdate"
          overlayClassName="bacth-update-popover"
                ...
                <div class="filters">
                  ...
                  >
                          ...
                          <a-select-option
                            ...
                            :title="
                              Array.isArray(type)
                                ? type
                                    .map((i) =>
                                      getNameAndErrorText(i.name, i.type),
                                    )
                                    .join('、')
                                : type
                            "
                    ...
                    >
                      <span
                        class="delete"
                        @click="delRows(index, 'filterList')"
                      >
                  ...
                  <a-row>
                    ...
                    <div class="add">
                      <span
                        :style="
                          filterList.length === 0 ? 'margin-left: -86px' : ''
                        "
                        @click="addRowsFilter"
                      >
                        <span>
    ...
    <sheet :columns="columns" :dataSource="dataSource" />
    ...
    <template slot="footer">
      ...
      <a-button :disabled="hasErrorStatus" type="primary" @click="sureImport">
...
export default class ImportErrModal extends Vue {
        ...
        if (Array.isArray(inner)) {
          inner.forEach((i) => {
            if (Object.prototype.toString.call(i) === '[object Object]') {
              if (!i.id && i.excelType) {
                i.propertyType = this.importData.headColumns.find(
                  (el) => el.propertyCode === key,
                ).propertyType;
                list.push(i);
              }
                ...
                if (Array.isArray(k)) {
                  k.forEach((l) => {
                    if (
                      ...
                      const _iItem = this.importData.headColumns.find(
                        (el) => el.propertyCode === iItem,
                      );
  ...
  changeData(list, arr, arr2, item?: any) {
    if (list) {
      ...
      const status = list.filter((i) => i.excelType);
  ...
  handleOnBatchConfirm() {
    ...
    if (this.filterList.length > 0) {
      this.filterList.forEach((i) => {
        const rowArr: any = i.data && i.data.split('、');
        let parentSheet: any = null;
        ...
        this.importData.secondImportData.map((v) => {
          if (parentSheet && v[parentSheet]) {
            const child = v[parentSheet].filter((o) => {
              if (o[rowArr[0]]) {
                if (Array.isArray(o[rowArr[0]])) {
...
<style lang="less">
.import-err-modal.ant-modal-wrap {
  .ant-modal-content {
    background: #ffffff;
