<template>
  <span v-if="source.length === 1" class="filter-card mr16">
    <template v-if="sourceSingle.type === 4">
      <span
        ...
        :title="
          isChinese
            ? sourceSingle.name
            : JSON.parse(sourceSingle.name_i18n)[$i18n.locale]
        "
        >{{
          isChinese
            ? sourceSingle.name
      ...
      <img
        ...
        alt
      />
      <img v-else class="logic-img" src="./images/off.png" alt />
      <i
    ...
    <template v-else>
      <span
        class="item-title-all"
        :title="
          (isChinese
            ? sourceSingle.name
            : JSON.parse(sourceSingle.name_i18n)[$i18n.locale]) +
          ': ' +
          contentHandler(sourceSingle.type)
        "
        >{{
          isChinese
            ? sourceSingle.name
            : JSON.parse(sourceSingle.name_i18n)[$i18n.locale]
        }}: {{ contentHandler(sourceSingle.type) }}</span>
  ...
  <span v-else-if="source.length > 1" class="filter-card">
  ...
  contentHandler(type: number) {
    if (type === -2) {
      ...
      return this.isChinese
        ? (this.sourceSingle.content as any).name
        : (this.sourceSingle.content as any).name_i18n[this.$i18n.locale];
    } else {
  ...
  getContent(content: string, type: number) {
    switch (type) {
      ...
      case DataItemType.RelevanceForm:
        try {
          const _address: any = JSON.parse(content);
          if (
            ...
            // 确保是关联的是地址
            return `${_address.provinceName}${_address.cityName}${_address.districtName}`;
          }
          return content;
        } catch (err) {
          return content;
