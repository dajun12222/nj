<template>
  <div class="new-query-form-list">
    <div class="query-form">
      <!-- <div class="query-title-top">
        <h3>筛选</h3>
        <i
          class="icon aufontAll h-icon-all-close"
          @click.stop="closeQueryItem"
        ></i>
      </div> -->
...
export default class QueryForm extends Vue {
  ...
  setFields(fields: any[]) {
    setTimeout(() => {
      if (localStorage.getItem('form-temp')) {
        const tempArr = JSON.parse(localStorage.getItem('form-temp') || '[]');
        tempArr.forEach((item) => {
          if (
            (item && item.id) ===
            (this.$store.state.System.System.loginedUserInfo &&
              this.$store.state.System.System.loginedUserInfo.id)
          ) {
            this.controls = item.data;
          }
        });
      } else if (localStorage.getItem('form-daily')) {
        const dailyArr = JSON.parse(localStorage.getItem('form-daily') || '[]');
  ...
  query() {
    this.$nextTick(() => {
      ...
      if (this.daily) {
        const dailyArr = localStorage.getItem('form-daily')
          ? JSON.parse(localStorage.getItem('form-daily') || '[]')
          : [];
        const index = dailyArr.findIndex(
          (item) =>
            item.id ===
            (this.$store.state.System.System.loginedUserInfo &&
              this.$store.state.System.System.loginedUserInfo.id),
        ...
        const tempArr = localStorage.getItem('form-temp')
          ? JSON.parse(localStorage.getItem('form-temp') || '[]')
          : [];
  ...
  toFormControl(field: any) {
    ...
    if (
      type === 1 &&
      field.options &&
      field.options.includes('dictionaryData')
    ) {
