<template>
  ...
  >
    <div class="form-marking-modal-content">
      <a-timeline v-if="!isDetail" class="form-marking-line">
        <a-timeline-item
          v-for="table in content"
          :key="table.id"
          color="#32B683"
        >
              ...
              <template v-slot:before="before">
                <div>
                  <div v-if="before.type === 6">
                    <template
                      v-if="before.beforeValue && before.beforeValue !== 'null'"
                    >
                      <div
                        v-for="(item, index) in attachmentFormate(
                          before.beforeValue,
                        )"
                        :key="index"
                      ...
                      >
                        ...
                        <!-- 预览 -->
                        <div class="no-wrap">
                          <a @click="filePreview(item.refId, item)">预览</a>
                        </div>
                      </div>
                    </template>
                  </div>
              ...
              <template v-slot:after="after">
                ...
                <span v-if="after.type === 2">
                  <span>
                    ...
                    <a-icon
                      v-if="
                        isBigger(after.beforeValue, after.afterValue) ===
                        'bigger'
                      "
                      type="arrow-up"
                      class="arrow-up"
      ...
      <a-timeline v-else class="form-marking-line">
        <a-timeline-item
          v-for="table in content"
          :key="table.id"
          color="#32B683"
      ...
      if (res.errcode === 0) {
        ...
        this.content = content.map((item) => {
          item.list.map((subItem, index) => {
            // 合并标题项目
            index === 0
              ? (subItem.rowSpan = item.list.length)
              : (subItem.rowSpan = 0);
