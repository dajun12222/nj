...
import H3SizeSlider from '../../../../../../common/src/components/pc/size-slider.vue';
...
export default class H3Table extends Vue {
  ...
  getFreezeCells(row: any[]) {
    return this.frozenKeys.map((k: string) =>
      row.find((cell) => cell.key === k),
    );
  }
  ...
  onDataSourceChange() {
    ...
    if (!this.dataSource || this.dataSource.length === 0) {
      mapList = this.unFreezeColumns.map((_, i: number) => {
        const row: any = {};
        row.id = i;
        this.unFreezeColumns.forEach((inner: any) => {
          row[inner.key] = '';
        });
        return row;
      });
