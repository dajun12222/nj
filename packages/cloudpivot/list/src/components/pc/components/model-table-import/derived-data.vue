  ...
  <div>
    ...
    >
              ...
              <div v-else :key="item.code" class="export-content-subsheet">
                <div class="export-content-subsheet--left">
                  <a-checkbox
                    v-model="item.checked"
                    @change="
                      (el) => subCheckChange(item.code, el.target.checked)
                    "
                  />
                </div>
      ...
      <div v-else class="export-inner">
        ...
        <div class="building-text">文件正在生成中</div>
        <div class="down-text">
          请前往<span class="export-text" @click="downFiles">导出任务列表</span>下载文件
        </div>
        <a-button @click="handleCancel"> 知道了 </a-button>
      </div>
    </a-modal>
  </div>
</template>
...
<script lang="ts">
import { Button, Checkbox, Divider, Modal, Row } from '@h3/antd-vue';
...
export default class Import extends Vue {
  ...
  created() {
    const { domainCode, bizGroupCode, bizModuleCode, schemaCode } =
      this.bindingModel;
    if (!schemaCode) {
      return;
    }
  ...
  handleOk() {
    this.disabled = true;
    const { appCode, bizGroupCode, bizModuleCode, schemaCode, domainCode } =
      this;
