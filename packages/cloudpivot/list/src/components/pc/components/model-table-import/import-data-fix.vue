  ...
  <div class="import-data-modal">
    ...
    >
      <div class="import-data-fix--header clearfix">
        ...
        <div>
          <a-button v-show="!isBatch" @click="BatchEdit"> 批量修正 </a-button>
          <a-button v-show="isBatch" @click="CancelBatchEdit">
            取消批量修正
          </a-button>
        </div>
      </div>
      ...
      >
        <!-- 头部 -->
        <template v-for="col in mergeColumns" :slot="col.slotTitle">
          <div :key="col.slotTitle">
            ...
            >
              <!-- 人员单选-->
              <span
                v-if="col.valueType === 'RADIO_PERSON'"
                class="icon-gap icon aufontAll h-icon-all-user-o"
                @click="showStaffAll(col)"
              ></span>
              <!-- 人员多选-->
              <span
                v-else-if="col.valueType === 'MULTI_SELECT_PERSON'"
                class="icon-gap icon aufontAll h-icon-all-team-o"
                @click="showStaffAll(col)"
              ></span>

              <span
                v-else-if="col.valueType === 'RADIO_DEPARTMENT'"
                class="icon-gap icon aufontAll h-icon-all-department-single-se"
                @click="showStaffAll(col)"
              ></span>

              <span
                v-else-if="col.valueType === 'MULTI_SELECT_DEPARTMENT'"
              ...
              <span
                v-else
                class="icon-gap icon aufontAll h-icon-all-user-list-o"
                @click="showStaffAll(col)"
              ></span>
            </template>
          </div>
              ...
              >
                  ...
                  <a-tooltip>
                    <template slot="title">
                      <div class="import-error-tips">
                        <div class="import-error-tips--left">不存在</div>
                ...
                >
                  <a-tooltip>
                    <template slot="title">
                      <div class="import-error-tips">
                        <div class="import-error-tips--left">重复</div>
                        <div class="import-error-tips--right">
                          <span
                ...
                >
                  <!--人员单选-->
                  <span
                    v-if="value.valueType === 'RADIO_PERSON'"
                    class="icon-gap icon aufontAll h-icon-all-user-o"
                    @click="showStaff(value, row[col.key])"
                  ></span>

                  <!--人员多选-->
                  <span
                    v-if="value.valueType === 'MULTI_SELECT_PERSON'"
                    class="icon-gap icon aufontAll h-icon-all-team-o"
                    @click="showStaff(value, row[col.key])"
                  ></span>

                  <span
                    v-if="value.valueType === 'RADIO_DEPARTMENT'"
                    class="icon-gap icon aufontAll h-icon-all-department-single-se"
                    @click="showStaff(value, row[col.key])"
                  ></span>

                  <span
                    v-if="value.valueType === 'MULTI_SELECT_DEPARTMENT'"
                    class="icon-gap icon aufontAll h-icon-all-department-multiple-"
                    @click="showStaff(value, row[col.key])"
                  ></span>
...
export default class ImportDataFix extends Vue {
  ...
  handleImportFixed() {
    ...
    const list: any = [];
    this.dataList.forEach((inner: any) => {
  ...
  showStaffAll(col: any) {
    ...
    if (this.dataSource.length > 0) {
      ...
      if (
        currentValue[col.key].notExists &&
        currentValue[col.key].notExists.length > 0
      ) {
        this.currentSelect = null; // 选中的值
      } else {
  ...
  get columns() {
    ...
    const cols = this.children.map((node: any) => {
      ...
      const tableCol = {
        ...
        node,
        input,
        children: node.childColumns
          ? this.initColumns(node.childColumns, col.id)
          : [],
      };
    ...
    if (childIds.length > 1) {
      return childIds.map((col: any) => {
        ...
        const tableCol: any = {
          ...
          node: col,
          input,
          children: col.childColumns
            ? this.initColumns(col.childColumns, col.id)
            : [],
        };
        return tableCol;
      });
