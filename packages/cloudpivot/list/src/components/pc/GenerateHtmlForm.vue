              ...
              <tr :style="` min-height: ${pxToMM(item.heightValue)}mm;`">
                ...
                >
                  ...
                  >
                    ...
                    >
                      <img
                        v-for="(picUrl, key) in item.picUrls"
                        :key="key"
                        :style="`height: ${pxToMM(
                          item.heightValue,
...
export default class GenerateHtml extends Vue {
  ...
  pageStyle() {
    ...
    const that = this;
    this.pages.forEach((i: any) => {
      i.forEach((item: any, n: number) => {
  ...
  getImgUrl(arrItem: any) {
    ...
    const url = UploadControl.service.getDownloadUrlByRefId(data.refId);
    return url;
  }
}
</script>
