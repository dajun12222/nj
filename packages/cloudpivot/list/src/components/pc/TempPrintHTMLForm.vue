                ...
                >
                  ...
                  >
                    <img
                      ...
                      :src="
                        printEle.picUrl || printEle.response.data.base64ImageStr
                      "
                      alt
...
export default class TempPrintHtml extends Vue {
  ...
  sheetColOnload(uid, index, colIndex, arrItemIndex) {
    ++this.pageImgLoad;
    this.$nextTick(() => {
      const len = Object.keys(this.pageImgCount).length;
  ...
  get pageStyle() {
    ...
    const widthPx = this.setting.width - this.setting.paddingX * 2;
    const heightPx = this.setting.height - this.setting.paddingY * 2;
  ...
  // DOM渲染完成, 抛出计算结果
  castCalcResult() {
    const upTopx = getPxFromMm(this.pageSettings._pagemargin.UpTomm / 2);
    const downTopx = getPxFromMm(this.pageSettings._pagemargin.DownTomm / 2);
    // 页面内容展示区域实际高度
    // let pageContentHeight = this.PageHeightPx-this.pageSettings._pagemargin.UpTopx-this.pageSettings._pagemargin.DownTopx
    const pageContentHeight = this.PageHeightPx - upTopx - downTopx;
    // 元素内容展示高度
    const eleContentHeight = (this.$refs['tempPrintHTMLEleRef'] as any)
      .scrollHeight;
    // 元素默认内容高度
    const eleDefaultHeight = this.printEle.heightValue;
    // 元素距离页面顶部的高度
    const eleTopHeight = this.printEle.top;
  ...
  getImgUrl(arrItem: any) {
    const picurl = arrItem.url;
    const data = arrItem.response.data;
    const base64 = data.base64ImageStr;
    const refIdUrl = UploadControl.service.getDownloadUrlByRefId(data.refId);
