<template>
  <div class="search">
    ...
    <form-list-sort :querySorts="querySorts" @sort="handleSort" />

    <div v-show="currentWay" class="search-content">
      <div v-show="currentWay === 'status'" class="search-content-status">
        <ul>
          <li
...
import FormListSort from './list-sort.vue';
...
export default class ListFormSearch extends Vue {
  ...
  switchWay(item: any) {
    if (
      item.type === 'filter' &&
      Array.isArray(this.queryConditions) &&
      !this.queryConditions.length
    ) {
      this.currentWay = '';
      this.$h3.toast.show({
  ...
  setFilterStatus() {
    this.queryConditions.forEach((con: any) => {
      if (con.propertyCode === 'sequenceStatus') {
        (this.$refs.queryCondition as any).setFilterValue(
          'sequenceStatus',
          this.currentStatusName,
        );
      } else {
        (this.$refs.queryCondition as any).setFilterValue(con.propertyCode, '');
      }
    });
