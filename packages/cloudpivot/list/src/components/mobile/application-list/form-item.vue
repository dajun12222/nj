<template>
  <div class="work-item" @click="goForm">
    <div class="card-item" @click="$emit('click')">
      <section class="item-content">
        ...
        >
          <p v-for="(col, idx) in columns" :key="idx">
            {{ col.name }}：{{ col.value }}
          </p>
        </div>
      </section>
    ...
    >
      <img
        v-show="showCreater"
        :src="getDownloadUrlNew(avatar) || defaultAvatar"
        class="content-user-avatar"
      />
      <span v-show="showCreater" class="content-user-name">{{ username }}</span>
      <span v-show="showTime" class="content-user-time">{{
...
export default class formItem extends Vue {
  ...
  get imgUrl() {
    return this.status
      ? require(`../../../../../common/assets/mobile/images/${this.status}${
          this.lang === 'zh' ? '' : '-en'
        }.png`)
      : '';
  }
