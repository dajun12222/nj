<template>
  <div id="mobile-formlist-container" ref="application" class="form-list">
    <div v-show="!showFilter" class="form-list-search">
      <form-list-search
        ref="listSearch"
    ...
    <div v-show="showFilter" class="form-list-batch">
      <div>
        <checkbox-item
    ...
    <div v-if="loadedConditions" class="form-list-item">
      <h3-scroll
        v-show="isNoEmpty"
        ref="scroll"
        :loadMore="loadMore"
    ...
    <div v-show="showFilter" class="form-list-batch-bottom">
      <div>已选：{{ selectNum }}</div>
      <div class="form-list-batch-delete" @click="batchDelete">批量删除</div>
    </div>
...
import { QueryActions } from '../../../../../api/src/application';
import common from '../../../../../common/mobile';
import form from '../../../../../form/src/renderer/components/mobile';
import { dateFormatter } from '../../../../../form/src/renderer/utils';
import * as platform from '../../../../../platform';
// 生命周期钩子
import listEventHooksHandler from '../../listCustom/eventHooks/handler';
...
export default class FormList extends Vue {
  ...
  beforeCreate() {
    const stroage = sessionStorage.getItem('theQueryList3');
    if (stroage) {
      sessionStorage.setItem('theQueryList', stroage);
    }
  ...
  beforeDestroy() {
    ...
    const stroage = sessionStorage.getItem('theQueryList');
    if (stroage) {
      sessionStorage.setItem('theQueryList2', stroage);
    }
    sessionStorage.removeItem('theQueryList');
  }
  ...
  async mounted() {
    ...
    this.params.queryCode = this.queryCode
      ? (this.queryCode as string)
      : this.schemaCode;
    const vm: any = this;
    const params = {
    ...
    if(res.errcode === 0 && res.data.options){
      this.pagingSetting = (JSON.parse(res.data.options) as any).pagingSetting;
    }
  
    // 注册钩子, 并触发 onPreLoad
    this.listConfig = res.data;
    this.params.queryCode = this.listConfig.code;
    ...
    if (
      ...
      const theQueryList = sessionStorage.getItem('theQueryList');
      let queryList: any = {};
      if (theQueryList) {
        queryList = JSON.parse(theQueryList);
        if (queryList.schemaCode === this.schemaCode) {
          vm.queryConditions = queryList;
        } else {
  ...
  batchDelete() {
    ...
    const ids = this.selectItems;
    const param = { schemaCode: this.schemaCode, ids };
    listApi.checkDeleteItems(param).then((res) => {
      if (res.errcode === 0) {
        const dataCopy: any = [];
        res.data.forEach((item) => {
          const target = dataCopy.find(
            (el) => el.resultCode === item.resultCode,
          );
          if (target) {
            target.objectIds.push(item.objectId);
          } else {
            dataCopy.push({
  ...
  async goForm() {
    ...
    if (!action) {
      return console.error('action "add" not found!');
    }
    if (
    ...
    const queryActionType: any = action.queryActionType;
    //判断是否有权限发起流程表单
