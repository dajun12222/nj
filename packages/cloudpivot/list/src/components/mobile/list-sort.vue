    ...
    <div class="select-wrap">
      <div class="list-wrap">
        ...
        <ul v-show="showList" class="select-list">
          <li
            v-for="item in querySorts"
            :key="item.id"
            :class="{ active: curSort.name === item.name }"
...
export default class ListFormSort extends Vue {
  ...
  onSortsChange(val) {
    this.curSort =
      this.querySorts.length > 0 ? this.querySorts[0] : this.curSort;
  ...
  queryFun() {
    const params: any = {};
    const code = this.curSort ? this.curSort.propertyCode : '';
    if (code) {
