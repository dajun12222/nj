<template>
  <div class="signatrue">
    <div class="signatrue--row clearfix">
      <div class="row__file" @click="showSignatrueOrUseLastAutograph">
      ...
      <div v-if="img" class="img__wrap">
        ...
        <div v-if="!readonly" class="img__delete" @click="deleteImg">
    ...
    <div>
      ...
      >
        <div class="signatrue-panel">
          ...
          <div ref="signatruewrap" class="signatrue-panel__content">
            <h3-signature
              ref="signature"
              :clip="true"
              :maxWidth="1"
    ...
    <template v-if="showSelectType">
      <div class="shadow" @click.self="showSelectType = false"></div>
      <div class="add-type">
        <div class="add-type-item" @click="showSignatrue">
          <span class="add-type-icon"><i class="icon aufontAll h-icon-all-edit-o"></i></span>
        ...
        <div class="add-type-item" @click="copyLastAutograph">
          <span class="add-type-icon"><i class="icon aufontAll h-icon-all-employment-applicati"></i></span>
          <div class="add-type-text">
            <div>复制签名</div>
            <span>复制上一次签名</span>
          </div>
...
<script lang="ts">
import { H3Button, H3Modal, H3Signature } from 'h3-mobile-vue';
...
export default class Signatrue extends Vue {
  ...
  mounted() {
    ...
    this.lastAutograph = this.lastAttachment && this.lastAttachment.refId;
  }
  ...
  lastAutograph: string = '';

  showSelectType: boolean = false;

  showSignatrueOrUseLastAutograph() {
    if (this.lastAutograph) {
  ...
  copyLastAutograph() {
    ...
    this.img = this.getDownloadUrlByRefId(this.lastAutograph) || '';
    this.$emit('useLastAutograph', this.lastAutograph);
  }
