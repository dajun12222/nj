...
import zhToEn from '../../../../list/src/components/pc/locales/zhToEn';
import { FormBuilder, FormControlErrorCodes } from 'cloudpivot-forms';
...
export abstract class FormRendererHelper {
  ...
  static handleDataDefault(
    ...
    properties.forEach((proper: any) => {
      if (controls[proper.code]) {
        ...
        if (
          source &&
          proper.code === source.key &&
          source.options.defaultValue === 'dataItemDefault'
        ) {
          source.options.dataItemDefault = proper.defaultValue;
        }
  ...
  static mergeQuoteAndMappings(source: any, list: Array<any>, sheet?: any) {
    ...
    if (source.options.mappings) {
      let mappingStr = source.options.mappings;
  ...
  static handleSheetDataItem(proper: any, source: any, list: Array<any>) {
    proper.subSchema.properties.forEach((sheetPro: any, index: number) => {
      if (sheetPro.propertyType === schema.DataItemType.Date) {
        const data: any = source.columns.find(
          (s: any) => s.key === sheetPro.code,
        );
        if (data && data.options.defaultValue === 'dataItemDefault') {
          data.options.dataItemDefault = sheetPro.defaultValue;
        }
      } else if (
        sheetPro.propertyType === schema.DataItemType.RelevanceForm &&
        source.columns
      ) {
        const data: any = source.columns.find(
          (s: any) => s.key === sheetPro.code,
  ...
  static convertDesign(
    ...
    if (tabsList.length > 0) {
      const isChinese = localStorage.getItem('locale') !== 'en';
      // flag-
      reverseRelevanceArr.unshift({
        name: '表单详情',
        code: 0,
        isReverseRelevanceTab: true,
      });
      ...
      const formDetail = {
        ...
        tabsName: isChinese ? '表单详情' : 'Form details',
        id: '-1',
      };
      ...
      tabsList.forEach((res) => {
        ...
        const pane = {
          ...
          children: [res],
          tabsName:
            isChinese || !res.options.name_i18n
              ? res.options.name
              : res.options.name_i18n,
          id: res.key,
        };
  ...
  static buildRow(controls: FormControl[]): RendererLayoutControl {
    ...
    // 迭代40布局优化，出现一行没有抵满的情况，需要自动补全
    const spanSum = controls.reduce((sum: any, control: any) => {
      return (
        sum +
        (control.options && control.options.span ? control.options.span : 24)
      );
    }, 0);
    if (spanSum < 24) {
      children.push({
  ...
  static buildCol(control: FormControl) {
    const span =
      (control as any).options && (control as any).options.span
        ? (control as any).options.span
        : (control as any).span
        ? (control as any).span
        : 24;
    ...
    if (
      ...
      // 判断当前是否存在英文，如果英文是中文，做一次映射
      if (
        control &&
        (control as any).name_i18n &&
        (control as any).name_i18n.en
      ) {
        // @ts-ignore
        control.options.name_i18n.en =
          zhToEn[control.options.name] || control.options.name;
      } else {
        // @ts-ignore
        let enName: string =
          zhToEn[control.options.name] || control.options.name;
        if (control.options.name_i18n && control.options.name_i18n.en) {
          enName = control.options.name_i18n.en;
        }
    ...
    if (Array.isArray(children)) {
      ...
      switch (control.nodeName) {
        case 'a-row':
          ...
          list.forEach((l: any) => {
            if (Array.isArray(l)) {
              l.forEach((i: any) => {
                //区分部分控件没有visible 属性
                if (i.controller && i.controller.display) {
                  visible = true;
                }
              });
            } else {
              if (l.controller && l.controller.display) {
                visible = true;
              }
            }
          });
        ...
        case 'a-tabs':
          ...
          if (children.length > 0) {
            if (
              ...
              (control as any).options.tabDefaultValue
            ) {
              opts.props.defaultActiveKey = (
                control as any
              ).options.tabDefaultValue;
            } else {
        ...
        case 'a-tabs-panel':
          ...
          // @ts-ignore
          control.tabsName = control.tabsName.replace('$$777', '');
          // @ts-ignore
          control.tabsName = control.tabsName.replace('$$776', '');

          if (control.show === false) {
            return '';
          }
    ...
    switch (errCode) {
      ...
      case FormControlErrorCodes.MinCount:
        return (window as any)['globalI18n'].t(
          ...
          {
            label,
            num: control.controller
              ? (control.controller as any).minCount
              : control.options.minCount,
          },
        );
      case FormControlErrorCodes.MaxCount:
        return (window as any)['globalI18n'].t(
          'cloudpivot.form.renderer.errorCode.maxCount',
          {
            label,
            num: control.controller
              ? (control.controller as any).maxCount
              : control.options.maxCount,
          },
        );
      default:
        break;
...
function getDateVerifyError(control: any) {
  ...
  const format = control.options.format1;
  let verifyObj: any = {};
  if (isJSONString(verifyFormula)) {
    verifyObj = JSON.parse(verifyFormula);
  } else {
