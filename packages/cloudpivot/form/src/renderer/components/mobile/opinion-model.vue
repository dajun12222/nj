...
<template>
  <div class="popupMotail" :class="[isCancel ? 'popupMotail-active' : '']">
    <div
      class="popupMotail-mask"
      :class="[isCancel ? 'popupMotail-mask-active' : '']"
      @click="onChange"
    ></div>
    <div
      class="popupMotail-contentBox"
      :class="{
        'popupMotail-contentBox-active': isCancel,
        'popup-viewer-ios-wx': isIOSWX,
      }"
    >
      <div class="popupMptail-title">
        <span>{{ $t('cloudpivot.form.renderer.oftenOption') }}</span>
        <span v-if="realTotle > 10" class="popupMptail-title-sum">总共<span class="sum-number">{{ realTotle }}</span>条</span>
      </div>
      <div v-if="realTotle > 10" class="popupMotail-search">
        ...
        <div v-show="keywords.length > 0" class="popupMotail-result">
          搜索结果：共 <span>{{ total }}</span> 条
        </div>
      </div>
      <ul v-if="realTotle > 0" class="popupMotail-list">
        <li
        ...
        >
          ...
          <div class="popupMotail-list-right">
            <i
              class="icon aufontAll h-icon-all-pen-o"
              @click.stop="addOpinion(item, index)"
            ></i>
            <i
              class="icon aufontAll h-icon-all-delete-o"
              @click.stop="deleteOpinion(item.id)"
            ></i>
          </div>
        </li>
      </ul>
      <div v-else class="popupMotail-nodata">
        <img src="../images/empty@2x.png" alt="暂无内容" />
        <p>{{ $t('cloudpivot.form.renderer.noData') }}</p>
      </div>
      <div
        v-show="realTotle < 10"
        class="popupMotail-add"
        @click="addOpinion({ id: '' })"
      >
  ...
  onKeyChange(e) {
    ...
    if (!e) {
      ...
      this.commentList = this.commentList.filter((item) => {
        return item.content.includes(str);
      });
    }
  }
