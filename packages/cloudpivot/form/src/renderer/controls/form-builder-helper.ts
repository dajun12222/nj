...
const isEmpty = (v: any) =>
  v === undefined ||
  v === null ||
  (Object.prototype.toString.call(v) === '[object Array]' && v.length === 0); // || v === '';
...
export abstract class FormBuilderHelper {
  ...
  static resetOpValue(control: any) {
    if (!isEmpty(control.op) && !isEmpty(control.defaultOp)) {
      control.op = control.defaultOp;
    }
  }
  ...
  static defaultValueOf(control: RendererFormControl) {
    ...
    if (isEmpty(val)) {
      if (
        ...
        if (
          dvt === typings.StaffSelectorValueType.Originator &&
          ...
          dvt === typings.StaffSelectorValueType.OriginatorDept &&
          StaffSelectorControl.service
        ) {
    ...
    if (control.type === FormControlType.Sheet) {
      ...
      if (sheet.options.visible === false) {
        ...
        if (sheet.options.displayFormula) {
          sheet.options.displayFormula = sheet.options.displayFormula.replace(
            /大于等于/g,
            '>=',
          );
          sheet.options.displayFormula = sheet.options.displayFormula.replace(
  ...
  static buildFormControlOptionsOf(control: any) {
    const value: any = FormBuilderHelper.getControlValue(control);
    ...
    if (control.options.visible === false) {
      ...
      if (control.options.displayFormula) {
        function isHasEmpty(type: string) {
          ...
          control.options.displayFormula
            .split('&&')
            .forEach((displayFormula: string) => {
              const regExp = new RegExp(`${type}.+`);
              const text = displayFormula.match(regExp);
        ...
        control.options.displayFormula = control.options.displayFormula.replace(
          /大于等于/g,
          '>=',
        );
        control.options.displayFormula = control.options.displayFormula.replace(
    ...
    if (control.required) {
      ...
      if (['true', true].includes(control.options.requiredFormula)) {
        options.required = true;
      } else {
        function isHasEmpty(type: string) {
          // 删除配置的显示条件中的  xx 包含、拥有、位于、属于  空值的显示条件
          ...
          control.options.requiredFormula
            .split('&&')
            .forEach((requiredFormula: string) => {
              const regExp = new RegExp(`${type}.+`);
              const text = requiredFormula.match(regExp);
              if (text && text[0].trim().length !== 2) {
                requiredFormula = requiredFormula.replace(
                  /\s+拥有\s+/g,
                  ' 拥有 ',
                );
                requiredFormula = requiredFormula.replace(
        ...
        control.options.requiredFormula =
          control.options.requiredFormula.replace(/大于等于/g, '>=');
        control.options.requiredFormula =
          control.options.requiredFormula.replace(/小于等于/g, '<=');
        control.options.requiredFormula =
          control.options.requiredFormula.replace(/不等于/g, '!=');
        control.options.requiredFormula =
          control.options.requiredFormula.replace(/等于/g, '==');
        control.options.requiredFormula =
          control.options.requiredFormula.replace(/大于/g, '>');
        control.options.requiredFormula =
          control.options.requiredFormula.replace(/小于/g, '<');
    ...
    //只读规则条件
    if (control.options.readonlyCondition) {
      options.disabled = control.options.readonlyCondition;
    }
    ...
    if (control.options.heads) {
      const source: any = control.options.heads.find(
        (h: any) => h.changeFormula,
      );
      if (source) {
