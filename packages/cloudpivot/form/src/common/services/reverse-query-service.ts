...
import { FormStatusEn, FormStatusZh } from '../../schema';
...
export abstract class ReverseQueryService {
  ...
  static getParams(
    ...
    let formatConditionStr: string = '';
    try {
      const condObj: any = JSON.parse(conditions);
      if (condObj) {
        formatConditionStr = condObj.queryText;
      }
    } catch (err) {
      console.log(err);
    }

    if (!formatConditionStr) {
      return [];
    }

    const params = formatConditionStr
      .split(';')
      .map((arr) => {
                    ...
                    if (key === 'sequenceStatus') {
                      ...
                      if (
                        (col.type === typings.FormControlType.RelevanceForm ||
                          col.type ===
                            typings.FormControlType.RelevanceFormEx) &&
