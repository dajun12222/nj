<template>
  <div :style="customStyle ? customStyle.bgStyle : ''">
    <a-row type="flex" class="header">
      <a-col class="col" :span="span">
        {{ source.title }}
      </a-col>
      <a-col class="col" :span="6"> 函数 </a-col>
      <a-col class="col" :span="span">
        {{ target.title }}
      </a-col>
      <a-col v-if="editable" class="col" :span="2" />
    </a-row>
...
<script lang="ts">
import { Col, Row, Select, Icon, Button } from '@h3/antd-vue';
...
export default class DataitemMapping extends Vue {
  ...
  getTargetOptions(item: DataItem) {
    const res = this.targetOptions.filter((p) => {
  ...
  onValueChange(val: DataitemMappingValueItem[]) {
    if (val) {
      setTimeout(() => {
        ...
        if (Array.isArray(this.sourceTargetOptionsMap.id)) {
          this.sourceTargetOptionsMap.id.forEach((item) => {
  ...
  findSourceOptionAndDisabled(code: string, disabled: boolean) {
    ...
    this.sourceOptions = this.sourceOptions.map((item) => {
      if (item.value === code) {
        ...
        item.disabled =
          this.selected.findIndex((item2) => item2.source.code === item.value) >
          -1;
      }
      return item;
    });
    ...
    // this.sourceOptions=this.sourceOptions.map(item=>{item.disabled=false; return item})
    // if (so) {
  ...
  findTargetOptionAndDisabled(code: string, disabled: boolean) {
    ...
    //逻辑优化
    console.log('targetOptions======>', this.targetOptions);
  ...
  onTargetItemChange(selectedCode: string, item: DataitemMappingValueItem) {
    ...
    if (typeof item.target !== 'string') {
      if (item.target.code !== selectedCode) {
        ...
        const selectedData = this.targetOptions.find(
          (res) => res.value === selectedCode,
        );
