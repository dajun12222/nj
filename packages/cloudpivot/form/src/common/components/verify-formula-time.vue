<template>
  ...
  >
    ...
    <!--如果是动态值-->
    <div v-show="!isQuantity" class="verify-item" style="margin-bottom: 20px">
      <div class="label"></div>
      <div class="content">
        <!--非介于-->
        <a-select
    ...
    <div
      class="verify-item"
      style="margin-bottom: 20px; height: 84px; align-items: flex-start"
    >
...
import { dateFormatter } from '../../renderer/utils/date-formatter';
import {
  ConfigProvider,
  message,
  Modal,
...
export default class VerifyFormulaTime extends Vue {
  ...
  verifyQuantity(
    ...
    if (!this.isRangeRule) {
      // 非介于!~ 规则
      if (time) {
        const t = moment(time).format(this.timeFormat);
  ...
  verifyPrompt(
    ...
    if (promptText.length === 0) {
      ...
      if (partterValue === 1) {
        ...
        let v = '';
        if (this.isRangeRule) {
          const l: string = moment(this.rangeTime[0]).format(this.timeFormat);
          const r: string = moment(this.rangeTime[1]).format(this.timeFormat);
          v = `${this.currControlOptions.name as any}必须${
            VerifyRuleEnum[verifyRuleValue]
          }${l}~${r}`;
        } else {
          const t = moment(this.time).format(this.timeFormat);
          v = `${this.currControlOptions.name as any}必须${
            VerifyRuleEnum[verifyRuleValue]
          }${t}`;
        }
        ...
        if (this.isRangeRule) {
          // 介于 规则
          const [cname1] = this.timeControlOptions.filter(
            (val) => val.code === this.rangeSelectDataItem1,
          );
          const [cname2] = this.timeControlOptions.filter(
          ...
          // 非介于 规则
          const [cname] = this.timeControlOptions.filter(
      ...
      for (const key of Object.keys(obj)) {
        ...
        switch (key) {
          ...
          case 'input':
            ...
            break;
          case 'select':
            st = this.selectListDataItems.filter(
  ...
  created() {
    ...
    for (const key of Object.keys(allControls)) {
      const ctl = allControls[key];
