  ...
  >
    <a-form class="app-form">
      <a-form-item
        ...
        class="form-label"
      >
        <a-switch v-model="syncFlag" @change="onsyncChange" />
        <p class="message">
          即与数据模型{{
            modalData.attributes[0].dataItem.name
          }}默认格式保持一致
        </p>
      </a-form-item>
      <a-form-item
      ...
      >
        <a-select
          v-model="showFormate"
          style="width: 180px"
          @change="changeShowFormate"
        >
          <a-select-option
            v-for="item in formteList"
            :key="item.value"
            :value="item.value"
          >
            {{ item.label }}
...
import { Modal, Form, Select, Switch } from '@h3/antd-vue';
...
export default class DictionaySetting extends Vue {
  ...
  onsyncChange(flag: boolean) {
    if (flag) {
      ...
      // 关闭开关时若不存在显示格式则取默认格式
      this.showFormate =
        this.control.options.format1 || this.control.options.format;
    }
  }
  ...
  onModalDataChange(modalData: any) {
    ...
    const items: any = modalData.items;
    // 判断是否为历史数据
    const code: string = this.control.key;
    // let target:any = fromList[code]
    const target: any = (items && items[code]) || null;
