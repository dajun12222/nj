...
import { Button, Modal, Select } from '@h3/antd-vue';
...
export default class CalculateRule extends Vue {
  ...
  created() {
    ...
    const arr = [];
    const allCtrlKeys = Object.keys(allControls);
    for (const key of allCtrlKeys) {
      const item = allControls[key];
        ...
        while (loop.length) {
          ...
          for (const lt of loopItem.panels) {
            ...
            for (const pnCtrlKey of pnCtrlKeys) {
              ...
              if (
                pnCtrl.type === FormControlType.Number &&
                pnCtrl.key !== code
              ) {
                this.dataItems.push({
  ...
  testRuleString(str: string) {
    ...
    if (~str.indexOf('round')) {
      ...
      while ((execRes = execReg.exec(str)) != null) {
        if (!this.isInSheet && execRes[0].indexOf('.') > -1) {
          return {
            done: false,
            msg: '计算表达式不符合格式要求',
          };
