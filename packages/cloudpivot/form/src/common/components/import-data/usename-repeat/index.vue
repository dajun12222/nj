  ...
  >
    ...
    <div v-if="showBacthUpdate">
          ...
          >
            ...
            <a-col :span="7">
              <div class="batch-update-new-data">
                <div>
                  <template v-for="(user, key) in row.value">
                    <span
                      v-if="key === row.value.length - 1"
                      :key="key"
                      class="row-inner-user"
                ...
                <SelectUser v-model="row.value" :row="row.dataItem" />
              </div>
            </a-col>
            <a-col :span="1" style="text-align: center; padding-top: 4px">
              <span
      ...
      <div class="flex" style="flex-direction: row-reverse; margin-right: 20px">
        <a-button type="primary" size="small" @click="handleOnBatchConfirm">
    ...
    <sheet v-if="showModal" :columns="columns" :dataSource="dataSource" />
  ...
  initColumns() {
    ...
    const _columns = this.importData.headColumns
      .map((col: any, idx: number) => {
        ...
        if (
          col.propertyType === DataItemType.StaffSingle ||
          col.propertyType === DataItemType.StaffMulti ||
          col.propertyType === DataItemType.DeptSingle ||
          col.propertyType === DataItemType.DeptMulti ||
          col.propertyType === DataItemType.StaffDeptMix
        ) {
          dataItem.push({
            title: col.name,
            dataIndex: col.code,
