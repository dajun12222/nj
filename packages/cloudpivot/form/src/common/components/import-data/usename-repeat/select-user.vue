<template>
  <div class="selector-user" style="min-width: 75px">
    <div class="selector" @click="setUser">
      ...
      ></i
      >{{ $t('cloudpivot.list.pc.chooseUser') }}
    </div>
    <staff-selector
...
export default class SelectUser extends Vue {
  ...
  setUser(selector: any) {
    if (this.batch) {
      //批量修改时判断是否有选择数据
      const resouce: any = this.value.find((v: any) => v.checked);
      if (!resouce) {
      ...
      if (this.isSheet) {
        ...
        const sheetResouce = sheetArr.find((s: any) => s.checked);
        if (!sheetResouce) {
          this.$message.error(
            this.$t('cloudpivot.list.pc.chooseSheetData') as string,
          );
          return;
        }
      }
    ...
    switch (this.row.propertyType) {
      case DataItemType.StaffSingle:
        this.taffSelectorOpts.selectOrg = false;
        this.taffSelectorOpts.selectUser = true;
        this.taffSelectorOpts.mulpitle = false;
        break;
      case DataItemType.StaffMulti:
        this.taffSelectorOpts.selectOrg = false;
        this.taffSelectorOpts.selectUser = true;
        this.taffSelectorOpts.mulpitle = true;
        break;
      case DataItemType.DeptSingle:
        this.taffSelectorOpts.selectOrg = true;
        this.taffSelectorOpts.selectUser = false;
        this.taffSelectorOpts.mulpitle = false;
        break;
