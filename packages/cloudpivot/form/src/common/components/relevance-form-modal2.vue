...
<script lang="ts">
import { RelevanceFormControl } from '../controls/relevance-form-control';
...
export default class RelevanceFormModal2 extends Vue {
  ...
  fetch(
    ...
    const params =
      queryData && queryData[0] && queryData[0].length > 0
        ? queryData[0].map((item, index) => {
            if (index === 0) {
              item = item.filter((el) => el.propertyValue);
            }
            return item;
          })
        : [[], []];
