...
import { filterTableMater } from '../../../../../common/src/utils/utils';
...
export default class bizModelsSelector extends Vue {
  ...
  getNodeChildren(code: string, node: any) {
    if (node.dataRef && node.dataRef.type === 'group') {
      this.getAppListByGroupCode(code).then((res: any) => {
        node.children = [...new Array(...res, ...node.children)];
      });
    } else {
  ...
  async getAppListByGroupCode(code: string) {
    const res: any = await listApi.getAppgroupList({
      groupCode: code,
      fromRecycle: false,
    });
  ...
  getAppList(param?: any) {
    listApi.getAllAppGroups(param).then((res: any) => {
      if (Array.isArray(res.data)) {
        ...
        const tree: any[] = gourps.map((item: any) => ({
          title: item.name,
  ...
  onChange(val: string, vNode: any) {
    ...
    if (!val) {
      ...
      this.$emit('change', appsArr[appsArr.length - 1], this.labelName);
  ...
  convert(item: any, parentKey?: string) {
    ...
    const node: any = {
      ...
      disabled:
        (this.onlyPublish && item.publishe === false) ||
        ...
        this.onLoadData(treeNode, appList).then((res: any) => {
          if (res) {
            ...
            if (this.selectModel !== '') {
              ...
              if (theApp) {
                const theKey = this.getDefaultValue(
                  this.value,
                  theApp.children,
                );
                if (theKey) {
