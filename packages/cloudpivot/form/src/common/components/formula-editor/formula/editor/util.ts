...
export function parseFormulaFromEditorText(
  editorText: string,
  editorMark: string,
) {
  const markRegEep = new RegExp(
    `(?<match><${editorMark}[^>]*>(?<id>.*?)<\\/${editorMark}>)`,
    'g',
  );
  let exec: RegExpExecArray | null;
  let sourcePos = 0;
...
export function insertIdentifier(
  cm: FixCodeMirrorEditor,
  tokenInfo: SharedFormulaCompiler.Formula.TokenInfo,
  replaceOptions?: ReplaceOptions,
) {
  const isFunc =
    tokenInfo.type === SharedFormulaCompiler.Formula.IdentifierType.FUNC;
  const isParametricFunc =
    isFunc &&
    !(tokenInfo as SharedFormulaCompiler.Formula.IdentifierTokenInfo)
      .parameterless;
...
export function insertEditorText(
  ...
  // @ts-ignore
  isOverlay
    ? (cm as any).setValue(texts.join('\n'))
    : (cm as any).replaceSelection(texts.join('\n'), 'end');
...
//
export function markFieldMarker(
  cm: FixCodeMirrorEditor,
  markField: MarkField,
): void {
  ...
  nameSpan.innerHTML = `<em class="cm-field-symbol"> </em>${label}<em class="cm-field-symbol"> </em>`;
  // @ts-ignore
...
export function getSelectionText(cm: FixCodeMirrorEditor, editorMark: string) {
  ...
  for (let line = startLine; line <= endLine; line++) {
    ...
    const markedSpans: MarkedSpan[] = (lineHandle.markedSpans || [])
      // 过滤出字段marker
      .filter(
        (markedSpan) => !!markedSpan.marker && markedSpan.marker.__IS_FIELD__,
      );
    ...
    for (; index < end; index++) {
      if (markedStartIndex.includes(index)) {
        const markedSpan: any = markedSpans.find(
          (_markedSpan) => _markedSpan.from === index,
        );
        const markField = markedSpan.marker.__MARK_FIELD__;
        selectText =
          selectText + `<${editorMark}>${markField.field.id}</${editorMark}>`;
...
export function paste(
  ...
  try {
    ...
    if (_editorMark !== editorMark) {
      const markRegEep = new RegExp(
        `(?<match><${_editorMark}[^>]*>(?<id>.*?)<\\/${_editorMark}>)`,
        'g',
      );
...
export function getActiveFuncName(cm: FixCodeMirrorEditor) {
  ...
  const _lineToken = lineToken.filter(
    (token) =>
  ...
  for (let lineNum = 0; lineNum < lineCount; lineNum++) {
    ...
    for (; start < end; start++) {
      if (markedStartIndex.includes(start)) {
        const markedSpan: any = markedSpans.find(
          (_markedSpan) => _markedSpan.from === start,
        );
        const markField = markedSpan.marker.__MARK_FIELD__;
        text =
          text +
          (!isLabel
            ? `<${editorMark}>${markField.field.id}</${editorMark}>`
            : `{${markField.field.name}}`);
