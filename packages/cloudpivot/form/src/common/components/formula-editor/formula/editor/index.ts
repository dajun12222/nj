...
export class FormulaEditor {
  ...
  constructor(element: HTMLElement, options: FormulaEditorOptions = {}) {
    ...
    this.compiler = new SharedFormulaCompiler.FormulaCompiler({
      disabledFuncs,
    });
  ...
  readOnlyToggle(readOnly: boolean) {
    ...
    this.editor.setOption(
      'readOnly',
      this.readOnly ? 'nocursor' : this.readOnly,
    );
  }

  _registerTokenHintList(disabledFuncs: string[]) {
    const _disabledFuncs = disabledFuncs.map((disableFunc) =>
      disableFunc.toLocaleUpperCase(),
    );
    const symbolRegExp = /[+\-*/=<>]+/;
    ...
    this.formulaTokenHintList = tokenInfos.filter(
      (token) =>
        !token.name.match(symbolRegExp) && !_disabledFuncs.includes(token.name),
    );
    this.formulaTokenFuncs = Object.keys(this.compiler.funcs).filter(
      (func) => !_disabledFuncs.includes(func),
    );
  }
  ...
  registerFieldHintList() {
    const tokenInfo: SharedFormulaCompiler.Formula.FieldTokenInfo[] =
  ...
  setEditorMark(editorMark: string): void {
    this.editorMark =
      editorMark && editorMark.length
        ? editorMark
        : this.editorMark || `field-${randomKey(8)}`;
  }

  // 预检查
  preValidate(editorText: string): SharedFormulaCompiler.ValidateResult {
    let result = true;
    let err: SharedFormulaCompiler.ParserError =
      new SharedFormulaCompiler.ParserError('', '', '');
    // 空值
    ...
    if (strs) {
      const markRegEep = new RegExp(
        `(?<match><${this.editorMark}[^>]*>(?<id>.*?)<\\/${this.editorMark}>)`,
        'g',
      );
      for (const str of strs) {
        if (markRegEep.test(str)) {
          err = SharedFormulaCompiler.generateError(
            '在字符串""中不应插入字段或者""没有成对输入',
          );
          result = false;
          break;
        }
  ...
  validate(
    resultType?: SharedFormulaCompiler.Formula.ParamType,
    fieldId?: string,
  ): SharedFormulaCompiler.ValidateResult {
    ...
    return !perResult.result
      ...
      : this.compiler.validate(
          ...
          Object.values(this.fields).map((item) => {
            const { id, name, paramType, fieldType, formula, formulaFormat } =
              item;
  ...
  calculate(
    fields?: SharedFormulaCompiler.Formula.Field[],
  ): SharedFormulaCompiler.CalculateResult {
    const editorText = this.getValue();
    const perResult = this.preValidate(editorText);
    return !perResult.result
      ? { result: 'N/A', err: perResult.err }
