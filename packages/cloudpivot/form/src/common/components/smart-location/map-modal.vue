  ...
  >
    <div class="h3-location-map-content">
      ...
      <el-amap
        ...
        @click="onClickMap"
      >
        <el-amap-marker :position="marker" :events="events" />
      </el-amap>
      <div v-if="showTip && openTip" class="h3-location-map-content-tip">
...
<script lang="ts">
import { Button, Icon, Input, Modal } from '@h3/antd-vue';
  ...
  initGeolocation() {
    ...
    this.geolocation.getCityInfo((status: any, result: any) => {
      if (status === 'complete' && result.info === 'SUCCESS') {
        ...
        self.transformLoaction(
          self.value.lng || self.lng,
          self.value.lat || self.lat,
        );
        self.marker = [self.value.lng || self.lng, self.value.lat || self.lat];
        self.mapCenter = self.marker;
      }
    });
