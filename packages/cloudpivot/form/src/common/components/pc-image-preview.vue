<template>
  <div class="bk-preview">
    ...
    <span class="btn-cirle btn-close" @click="onClose">
      <a-icon type="close" />
    </span>
    <div class="ctl-resize">
      <a-icon
        type="minus-circle"
        :class="['btn-resise', { 'btn-disabled': resize === 10 }]"
        @click="onImgResize(false)"
      />
      <span class="txt-resize">{{ resize }}%</span>
      <a-icon
        type="plus-circle"
        :class="['btn-resise', { 'btn-disabled': resize === 800 }]"
        @click="onImgResize(true)"
      />
      <a-icon type="redo" class="btn-resise marginL20" @click="rotateImg" />
      <a-icon
        type="cloud-download"
        class="btn-resise marginL20"
        @click="downloadImg"
      />
    </div>
    <a-spin :spinning="waiting" size="large" />
  </div>
</template>
...
export default class PCImagePreview extends Vue {
  ...
  downloadImg() {
    const url = this.imgList[this.imgIndex].url;
  ...
  onImgLoad(e) {
    ...
    if (
      img.naturalHeight > this.windowInfo.height * 0.8 ||
      img.naturalWidth > this.windowInfo.width * 0.8
    ) {
      // 确定依据横向or纵向缩放
      const isResizeByHeight =
        img.naturalHeight / this.windowInfo.height >
        img.naturalWidth / this.windowInfo.width;
      // 计算缩放比例
      const resizeRate =
        Math.floor(
          (isResizeByHeight
            ? (this.windowInfo.height * 0.8) / img.naturalHeight
            : (this.windowInfo.width * 0.8) / img.naturalWidth) * 10,
        ) * 10;
      this.resize = resizeRate < 10 ? 10 : resizeRate;
    }
    img.style.width = Math.floor(img.naturalWidth * (this.resize / 100)) + 'px';
    // 保留图片原始尺寸
    this.imgNaturalList[this.imgIndex] = {
  ...
  onImgChange(toPrev) {
    ...
    (this as any).$refs['image']['style'].transform = '';
    this.resize = 100;
  }
  ...
  onImgResize(isAdd) {
    ...
    let style = (this as any).$refs['image']['style'].transform;
    // 获取旋转角度
    ...
    if (translateInfo && translateInfo.length) {
      (this as any).$refs['image']['style'].transform = `translate(${Math.floor(
        (translateInfo[0] / oldResize) * this.resize,
      )}px, ${Math.floor(
        (translateInfo[1] / oldResize) * this.resize,
      )}px) ${rotate}`;
    }
    (this as any).$refs['image']['style'].width =
      Math.floor(
        this.imgNaturalList[this.imgIndex].width * (this.resize / 100),
      ) + 'px';
  }
    ...
    img.onmousemove = (ev) => {
      ...
      if (translateInfo && translateInfo.length) {
        img.style.transform = `translate(${
          Number(translateInfo[0]) + ev.x - mouseInfo.x
        }px, ${Number(translateInfo[1]) + ev.y - mouseInfo.y}px) ${rotate}`;
      } else {
        img.style.transform = `translate(${ev.x - mouseInfo.x}px, ${
          ev.y - mouseInfo.y
        }px) ${rotate}`;
      }
