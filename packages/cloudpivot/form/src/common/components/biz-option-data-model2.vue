<template>
  <div class="unset-height">
      ...
      <div class="custom-info">
        ...
        <div>
          <ext-batch-add-radio-modal
            ...
            :propStyle="propStyle"
            @batchAddOver="handleOk"
          />
          <em class="icon aufontAll h-icon-all-plus" @click="addRow"></em>
        </div>
      </div>
      <template
        v-if="
      ...
      >
        ...
        >
          <draggable
            ...
            @end="handleOk"
          >
            <template v-for="(modulem, index) in options">
              <div :key="index" class="row-item">
                <a-radio v-if="!showModel" class="radio-item" :value="index" />
    ...
    <a-row v-else-if="optionsType === 2">
      ...
      <div class="list-item">
        <div class="isSetDataDictionaryData" @click="showModal('business')">
          <span>{{ isSetDataBusinessModalData }}</span>
          <em class="aufontAll h-icon-all-ellipsis-o"></em>
        </div>
      </div>
      ...
      <a-modal
        ...
        :maskClosable="false"
        :keyboard="false"
        @cancel="businessModalVisible = false"
      >
        ...
        <a-row v-if="orderByFields" class="rowStyle">
          ...
          <a-col :span="12" class="radio-col">
            <a-radio-group v-model="orderType" name="radioGroup">
            ...
            >
                ...
                <config-provider :locale="locale">
                  ...
                  >
                    <a-select-option
                      ...
                      :value="item.code"
                      :title="
                        item.name.concat('[').concat(item.code).concat(']')
                      "
                    >
                      {{ `${item.name}[${item.code}]` }}
                    </a-select-option>
              ...
              <a-col :span="4">
                <a-select v-model="condition.operator.val">
                  <template v-for="(item, key) in condition.operator.listOpt">
                    <a-select-option :key="key" :value="item.type">
                      {{ item.val }}
                    </a-select-option>
                  </template>
                </a-select>
        ...
        >
          ...
          <a-col class="opt-setting" @click="handleShowOpt">
            {{ selecteds && selecteds.length > 0 ? '已设置' : '未设置' }}
          </a-col>
        </a-row>
        <a-row
          v-if="['radioOption', 'checkboxOption'].includes(modalData.type)"
    ...
    <div v-else>
      ...
      <div class="list-item">
        <div class="isSetDataDictionaryData" @click="showModal('dictionaries')">
          <span>{{ isSetDataDictionaryData }}</span>
          <em class="aufontAll h-icon-all-ellipsis-o"></em>
        </div>
      </div>
      ...
      <a-modal
        title="字典设置"
        width="640px"
        :visible="dictionariesVisible"
        class="modal"
        :maskClosable="false"
        :keyboard="false"
        @cancel="dictionariesVisible = false"
      >
        <data-dictionaries
          ref="DataDictionariesRef"
          :modalData="dataDictionaryData"
          :isRadioCheck="
            ['radioOption', 'checkboxOption'].includes(modalData.type)
          "
        />
        <template slot="footer">
          <a-button key="cancel" @click="cancel">
          ...
          <a-button
            v-if="
              ['dropdownOption', 'dropdownMultiOption'].includes(modalData.type)
            "
            @click="clearDictionaryData"
          >
            删除字典
          </a-button>
          <a-button
    ...
    >
      <div class="select-tips">
        当前数据项<span class="total">{{ totalElements }}</span>条 您已选择<span class="selected">{{
          selecteds && selecteds.length
        }}</span>条<span
          v-if="totalElements > 200 && selecteds && selecteds.length < 200"
          >,还可以选择<span class="total">{{
            200 - selecteds && selecteds.length
          }}</span>条</span>
      </div>
      <a-table
      ...
      <div v-show="totalElements > 0" class="pagination">
        <a-pagination
...
export default class BizOptionDataModel2 extends Vue {
  pageSizeOptions: Array<string> = ['10', '20', '50', '100', '200'];
  ...
  queryModalVisible: boolean = false;

  dictionariesVisible: boolean = false;

  businessModalVisible: boolean = false;

  showModal(type: 'business' | 'dictionaries') {
    if (type === 'business') {
  ...
  cancel() {
    ...
    this.dictionariesVisible = false;
  }
  ...
  get isRadioOrCheckBox() {
    return ['radioOption', 'checkboxOption'].includes(this.modalData.type);
  }
  ...
  onTreeChange(modelCode, modelName) {
    this.bizModel = modelCode;
    this.bizModelName = modelName;
    this.sheetDataItem = '';
    this.orderByFields = '';
  ...
  get isSetDataDictionaryData() {
    const value: any =
      this.dataDictionaryData.value &&
      JSON.parse(this.dataDictionaryData.value);
    return (
      (value && value.valueText) ||
      (value && value.checkedDictionary && '已设置') ||
      '未设置'
    );
  }

  get isSetDataBusinessModalData() {
    return (
      (this.appCode && this.bizModel && (this.bizModelName || '已设置')) ||
      '未设置'
    );
  }

  delDataDictionaryData() {
    ...
    const DataDictionariesRef: any = this.$refs.DataDictionariesRef as any;
    DataDictionariesRef && DataDictionariesRef.init();
  }
  ...
  onSelectAll(selected: boolean, selectedRows: any[], changeRows: any[]) {
    ...
    if (selected) {
      ...
      changeRowsKey.forEach((item, index) => {
        if (arrkeys.includes(item)) {
    ...
    row.current.listOpt = cur
      ...
      .filter((item) => {
        ...
        if (
          ...
          return (
            [0, 12, 14].includes(item.type) || item.type === tar.propertyType
          );
  ...
  getcomName(row: any) {
    const targetVal = row.target.listOpt.filter(
      (item) => item.code === row.target.val,
    );
    let comName = '';
    if (row.operator.val === '===' && targetVal.length) {
  ...
  addCondition(val?: any) {
    ...
    const item: any = {
      ...
      operator: {
        val: val ? val.operator : this.isRadioOrCheckBox ? '===' : '==',
    ...
    if (val && val.operator === '===') {
      const propertyType = item.target.listOpt.find(
    ...
    if (data.value) {
      try {
        if (typeof data.value === 'string') {
          if (data.value.indexOf('checkedDictionary') > -1) {
            ...
            this.optionsType = 3;
          } else if (data.value.indexOf('schemaCode') > -1) {
  ...
  setBizmodel(data: any) {
    ...
    this.bizModelName = obj.modelName;
  ...
  getExistItem() {
    const params: any = {
      schemaCode: this.bizModel,
      ids: this.selecteds.map((item) => {
        return item.data.id;
      }),
    };
    listApi.getExistItem(params).then((res) => {
      if (res.errcode === 0) {
        const existList = res.data.true || [];
        const selectedsList: any = [];
        this.selecteds.forEach((item) => {
          const node = existList.find((ele) => item.data.id === ele);
          if (node) {
      ...
      // @ts-ignore
      this.rowSelection.selectedRowKeys =
        Array.isArray(this.selecteds) &&
        this.selecteds.map((item) => item.data.id);
    });
  }
  ...
  getData() {
    ...
    if (this.optionsType === 1) {
      ...
      if (['radioOption', 'checkboxOption'].includes(this.modalData.type)) {
        backData = (this.$refs.DataDictionariesRef as any).bizmodeBack(true);
      } else {
        backData = (this.$refs.DataDictionariesRef as any).bizmodeBack(false);
      }
      this.dataDictionaryData = backData;
  ...
  customBack() {
    ...
    //单选复选选项不能为空
    if (
      ['radioOption', 'checkboxOption'].includes(this.modalData.type) &&
      backData.value === ''
    ) {
  ...
  bizmodeBack() {
    ...
    const condition = this.conditionList
      ...
      .join('&&');
    const string = `schemaCode:${this.bizModel} sheetDataItem:${this.sheetDataItem} condition:${condition}`;
    const obj = {
      schemaCode: this.bizModel,
      modelName: this.bizModelName,
    ...
    const newVal: any = {};
    if (['radioOption', 'checkboxOption'].includes(this.modalData.type)) {
      obj.labels = this.selecteds
        .map((item) => {
          if (typeof item.data[this.sheetDataItem] !== 'string') {
            item.data[this.sheetDataItem] = JSON.stringify(
              item.data[this.sheetDataItem],
            );
          }
          return item;
        })
        .reduce((pre, crr) => {
          ...
          return value.includes(crr.data[this.sheetDataItem])
            ? pre
            : [...pre, crr];
        }, []);
