  ...
  <div>
    ...
    <div class="verify-item" style="margin-top: 16px; margin-bottom: 16px">
      ...
      <div class="content">
        ...
        <div
          v-show="isQuantity"
          style="margin-left: 8px; flex: 1; min-width: 100px"
        >
          <!--非介于-->
          <a-input-number
            v-show="!isRangeRule"
    ...
    if (promptText.length === 0) {
      ...
      if (partterValue === 1) {
        ...
        if (this.isRangeRule) {
          ...
          }${this.verifyFormula.numInput1}~${this.verifyFormula.numInput2}`;
        } else {
          // 非介于 规则
          v = `${this.verifyFormula.currControlOptions.name as any}必须${
            VerifyRuleEnum[verifyRuleValue]
        ...
        if (this.isRangeRule) {
          // 介于 规则
          const [cname1] = this.selectListDataItemsNumber.filter(
            (val) => val.code === this.verifyFormula.rangeSelectDataItem1,
          );
          const [cname2] = this.selectListDataItemsNumber.filter(
          ...
          // 非介于 规则
          const [cname] = this.selectListDataItemsNumber.filter(
