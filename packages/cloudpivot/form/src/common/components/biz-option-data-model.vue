<template>
  <div>
    <div class="option-wrap">
      <a-row class="radio-row">
        ...
        <a-col :span="20" class="radio-col">
          <a-radio-group v-model="optionsType" name="radioGroup">
    ...
    <div v-if="optionsType === 1" class="add-option-wrap">
      <a-row>
        ...
        <a-col :span="20">
          <div class="option-back">
            <div
              v-if="
                ['bizRadioOption', 'dropdownOption'].includes(modalData.type)
              "
              ref="condionwrap"
              class="radio-group"
            >
                    ...
                    >
                      <a-col :span="2">
                        <div class="radio-wrap">
                          <a-radio class="radio-item" :value="index" />
                        </div>
                      </a-col>
                      <a-col :span="20">
            ...
            <template
              v-else-if="
                ['checkboxOption', 'dropdownMutiOption'].includes(
                  modalData.type,
                )
              "
            >
            ...
            <div class="add-div">
              ...
              <ext-batch-add-radio-modal :options.sync="options" />
            </div>
          </div>
        </a-col>
      </a-row>
    ...
    <div v-else-if="optionsType === 2">
      ...
      <a-row class="search-infos">
        ...
        <a-col :span="20" class="condition-col">
          <a-row v-show="conditionList.length > 0">
            <a-col :span="9" class="title"> 联动模型数据项 </a-col>
            <a-col :span="5" class="title" />
            <a-col :span="9" class="title"> 当前模型数据项 </a-col>
          ...
          >
            <a-row>
              ...
              <a-col :span="5">
                <!-- 操作符号 -->
                <a-select
                  v-model="condition.operator.val"
                  style="padding: 0 8px"
                >
                  <a-select-option
                    v-for="(item, ids) in condition.operator.listOpt"
                    :key="ids"
                    :value="item.type"
                  >
                    {{ item.val }}
      ...
      <a-row v-if="isRadioOrCheckbox" type="flex" class="rowStyle">
        <a-col :span="4">
          选项设置
          <a-tooltip>
        ...
        <a-col class="opt-setting" @click="handleShowOpt">
          {{ selecteds.length > 0 ? '已设置' : '未设置' }}
        </a-col>
      </a-row>
      <a-row v-if="isRadioOrCheckbox" class="tips">
    ...
    <div v-else>
      <data-dictionaries
        ...
        :isRadioCheck="
          ['bizRadioOption', 'checkboxOption'].includes(modalData.type)
        "
      />
    </div>
    <a-modal
    ...
    >
      <div class="select-tips">
        当前数据项
        <span class="total">{{ totalElements }}</span>条 您已选择<span class="selected">{{ selecteds.length }}</span>条
        <span v-if="totalElements > 200 && selecteds.length < 200">
          ,还可以选择
          <span class="total">{{ 200 - selecteds.length }}</span>条
        </span>
      </div>
      <a-table
...
export default class BizOptionDataModel extends Vue {
  ...
  get isRadioOrCheckbox() {
    return [DataItemType.Radio, DataItemType.Checkbox].includes(this.dataType);
  }
  ...
  staffSelectorOpts = {
    ...
    selectUser: true,
    showModel: true,
    mulpitle: ['bizRadioOption', 'dropdownOption'].includes(this.modalData.type)
      ? false
      : true,
    showSelect: true,
    placeholder: '请选择',
  ...
  getcomName(row: any) {
    ...
    let comName = '';
    if (row.operator.val === '===' && targetVal[0]) {
  ...
  options: Array<any> = [
    {
      default: ['checkboxOption', 'dropdownMutiOption'].includes(
        this.modalData.type,
      )
        ? false
        : true,
