<template>
  ...
  >
    ...
    <template v-if="isOrgRoot">
      <div class="row">
        <a-radio :checked="isRef" @click="setValueType('ref')">
          表单字段
        </a-radio>
      ...
      <div class="row">
        <a-radio :checked="isCreator" @click="setValueType('originatorDept')">
          创建人部门
        </a-radio>
      </div>
    </template>
    ...
    <template v-else>
      <div class="row">
        <a-radio :checked="isCreator" @click="setValueType(null)">
          创建人相关
        </a-radio>
        ...
        >
          <a-select-option v-if="selectUser" value="originator">
            创建人
          </a-select-option>
          <a-select-option v-if="selectOrg" value="originatorDept">
            创建人部门
          </a-select-option>
        </a-select>
      </div>
...
import { Button, Modal, Radio, Select } from '@h3/antd-vue';
...
export default class UserSelectValueSetting extends Vue {
  ...
  onModalDataChange(modalData: any) {
    ...
    if (this.isOrgRoot) {
      ...
      this.staff =
        this.valueType === schema.StaffSelectorValueType.None &&
        typeof this.opts.defaultValue !== 'string'
  ...
  async getOperateAble() {
    ...
    staff.forEach((el: any) => {
      if (el.type === 1) {
        targetIds.push(el.id);
      } else if (el.type === 3) {
        userIds.push(el.id);
      } else {
        //Else Empty block statement
    ...
    if (targetIds.length && userIds.length) {
      sourceType = 'DEPT_USER';
    } else if (targetIds.length && userIds.length === 0) {
      sourceType = 'DEPT';
    } else if (targetIds.length === 0 && userIds.length) {
      sourceType = 'USER';
    } else {
      //Else Empty block statement
    }
