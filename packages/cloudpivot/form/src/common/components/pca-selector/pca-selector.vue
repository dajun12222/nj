<template>
  <div class="pca-selector">
    ...
    >
      ...
      <label v-if="pcaDate.length === 0" class="placeholder">{{
        $t('cloudpivot.form.renderer.pleaseSelect')
      }}</label>
      <a-icon v-if="!isFocus" class="down" type="down" />
      <a-icon v-else class="down" type="up" />
      <a-icon
        v-if="isHeight && pcaDate.length > 0"
        class="delete"
        type="close-circle"
    ...
    <div
      ...
      :style="{
        top: `${inReport ? '' : position.top + 'px'}`,
        left: `${inReport ? '' : position.left + 'px'}`,
      }"
      @click.stop="isFocus = true"
    >
      <a-button
        v-if="prevConfirm"
        class="pca-selector-confirm"
        type="primary"
        @click.stop="confirmpca"
      >
        {{ $t('cloudpivot.form.renderer.ok') }}
      </a-button>
      <a-tabs v-model="activeTab" @change="setPosition">
        <a-tab-pane
          v-if="tab[0].isShow"
          key="1"
          :tab="$t('cloudpivot.form.renderer.province')"
...
<script lang="ts">
import { Icon, Tabs, Button } from '@h3/antd-vue';
...
export default class PcaSelector extends Vue {
  ...
  emptyClick(type: string) {
    switch (type) {
      ...
      default:
        this.pcaDate = this.pcaDate.filter(
          (res: any) => res.level === 'province' || res.level === 'city',
        );
        this.clearDistrictsSelect();
        break;
    }
  ...
  change() {
    ...
    if (adData.districtAdcode) {
      adData.adcode = adData.districtAdcode;
    } else {
      if (adData.cityAdcode) {
        adData.adcode = adData.cityAdcode;
      } else {
        adData.adcode = adData.provinceAdcode;
      }
    }
  ...
  getPcaBaseData() {
    if (!this.pvcs || this.pvcs.length === 0) {
      ...
      // this.pvcs = pca.default;

      return import(
        /* webpackChunkName: "pca" */ '../../../../../common/src/pca/pca.min'
      ).then((mod) => {
        this.pvcs = mod.default;
        return mod.default;
      });
    }
  }
  ...
  ondefaultValChange(val: any) {
    this.setConfig();
    this.pcaDate = [];
    if (val && (val.adcode || val.districtAdcode || val.provinceAdcode)) {
      // this.pcaDate = this.setValue(val);
      this.setValue(val).then((data) => (this.pcaDate = data));
      return;
    }
    if (!val.pca || val.pca.length === 0) {
      return;
    }
    this.pcaDate = [...val];
  }
  ...
  onformatChange(val: any) {
    console.log('formatChange===>', val);
    const formatArr = val.split('-');
    switch (formatArr.length) {
      case 1:
        this.tab[0].isShow = true;
        this.tab[1].isShow = false;
        this.tab[2].isShow = false;
        break;
      case 2:
        this.tab[0].isShow = true;
        this.tab[1].isShow = true;
        this.tab[2].isShow = false;
        break;
      case 3:
        this.tab[0].isShow = true;
        this.tab[1].isShow = true;
        this.tab[2].isShow = true;
        break;
...
@import './../../../../../application/src/components/pc/style/custom-themes.less';
.pca-selector {
  width: 100%;
  text-align: left;
  display: block;
