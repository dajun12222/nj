...
const IFS: Formula.Identifier<Formula.IdentifierType.FUNC> = {
  ...
  validate(identifier, params) {
    ...
    if (params.length < 4) {
      throw generateError(
        `函数 ${identifier.name}() 至少需要4个参数，但传入了${params.length}个参数`,
      );
    }
    if (params.length % 2 === 1) {
      throw generateError(
        `函数 ${identifier.name}() 要求[条件, 返参]2个参数为一组，存在不成组情况`,
      );
    }
    ...
    if (
      ...
      throw generateError(
        `函数 ${identifier.name}() TRUE为必要条件，表示不满足前者所有条件下需要返回的绝对参数`,
      );
    }
    ...
    if (!pass) {
      throw generateError(
        `函数 ${
          identifier.name
        }() 要求[条件, 返参]2个参数为一组，要求条件为逻辑类型，要求返参类型一致，现在传入为函数 ${
          identifier.name
        }(${params
          .map(
            (param, index) =>
              `${index % 2 === 0 ? '条件类型' : '返参类型'}：${getFieldName(
                param,
              )}${getParamTypeValue(param.paramType)}`,
          )
          .join(', ')})`,
      );
    }
