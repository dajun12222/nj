...
const DATE: Formula.Identifier<Formula.IdentifierType.FUNC> = {
  ...
  validate(identifier, params, compiler) {
    ...
    if (
      ...
      !isNaN(_month) &&
      !isNaN(_day) &&
      !checkDate(_year, _month, _day)
    ) {
      throw generateError(
        `函数 ${identifier.name} (${year.value}, ${month.value}, ${day.value})的日期不是合法日期`,
      );
    }
  ...
  calculate: (identifier, params, compiler) => {
    ...
    if (!checkDate(_year, _month, _day)) {
      throw generateError(
        `函数 ${identifier.name} (${_year}, ${_month}, ${_day})的日期不是合法日期`,
      );
    }

    const date = getDateInstance(
      `${Math.floor(_year)}-${Math.floor(_month)}-${Math.floor(_day)}`,
    );
    if (!date) {
      throw generateError(
        `函数 ${identifier.name} (${_year}, ${_month}, ${_day})时间参数转成时间时出现错误`,
      );
    }
