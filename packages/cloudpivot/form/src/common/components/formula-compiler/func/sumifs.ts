...
const SUMIFS: Formula.Identifier<Formula.IdentifierType.FUNC> = {
  ...
  validate(identifier, params) {
    ...
    if (params.length < 2) {
      throw generateError(
        `函数 ${identifier.name}() 至少需要2个参数，但传入了${params.length}个参数`,
      );
    }
    ...
    if (
      _params.some(
        (param) =>
          ...
          (param.sourceType === Formula.SourceType.FIELD &&
            (param as Formula.FieldParam).fieldType !==
              Formula.FieldType.SUBLIST),
      )
    ) {
      throw generateError(
        `函数 ${identifier.name}() 第二参数开始需要是明细表中的数字类型字段`,
      );
    }

    if (!validateSubFromSame(params as Formula.FieldParam[])) {
      throw generateError(
        `函数 ${identifier.name}() 参数不支持不同明细表的字段`,
      );
    }
