...
const ROLLUP: Formula.Identifier<Formula.IdentifierType.FUNC> = {
  ...
  validate(identifier, params) {
    ...
    if (inputType.length !== params.length) {
      throw generateError(
        `函数 ${identifier.name}() 需要${inputType.length}个参数，但传入了${params.length}个参数`,
      );
    }
    const [relevance, summarize] = params;
    if (
      relevance.sourceType !== Formula.SourceType.FIELD ||
      (relevance as Formula.FieldParam).fieldType !==
        Formula.FieldType.RELEVANCE
    ) {
      throw generateError(
        `函数 ${
          identifier.name
        }() 第一个参数只支持关联数据中的字段，第一个参数${getFieldName(
          relevance,
        )}并不是关联数据中的字段`,
      );
    }
    const summarizes = [1, 2, 3, 4, 5, 6, 7];
    const _summarize = !isNaN(summarize.value)
      ? toNumber(summarize.value) * (summarize.unary || 1)
      : summarize.value;
    if (!summarizes.includes(_summarize)) {
      throw generateError(
        `函数 ${identifier.name}() 第二个参数只允许是 1 ~ 7 的整数`,
      );
    }
    ...
    switch (relevance.paramType) {
      ...
      case Formula.ParamType.BOOL:
        if ([4, 5, 6, 7].includes(_summarize)) {
          throw generateError(
            `函数 ${identifier.name}() 第一个参数是${getParamTypeValue(
              relevance.paramType,
  ...
  calculate: (identifier, params) => {
    ...
    const paramType =
      relevance.paramType === Formula.ParamType.DATE &&
      [4, 5].includes(_summarize)
        ? Formula.ParamType.DATE
        : Formula.ParamType.NUMBER;
    let value = paramDefaultValue(paramType);
    if (!validateSubParamNull(relevance)) {
      const relevanceValue = relevance.value;
      const values = relevanceValue
        .filter(
          (_value) =>
            !validateParamNull({
              ...relevance,
      ...
      switch (_summarize) {
        ...
        case 5: // 最小值
          ...
          const _values =
            relevance.paramType === Formula.ParamType.DATE
              ? values.map((_value) => {
                  ...
                  if (!date) {
                    throw generateError(
                      `函数 ${identifier.name}() 中第一个参数${getFieldName(
                        relevance,
                      )}内转成时间时出现错误`,
                    );
                  }
          ...
          value =
            relevance.paramType === Formula.ParamType.DATE
              ? dateFormat(getDateInstance(compare(..._values)))
              : compare(..._values);
          break;
        case 6: // 平均值
          if (!values.length) {
