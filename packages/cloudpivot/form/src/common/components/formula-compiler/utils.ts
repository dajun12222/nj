...
import toStringFunc from 'lodash/toString';
...
export function checkParamType(
  claimType: Formula.ParamType,
  incomingType: Formula.ParamType,
) {
...
export function checkSourceType(
  claimType: Formula.SourceType,
  incomingType: Formula.SourceType,
) {
...
export function checkParam(claim: Formula.Param, incoming: Formula.Param) {
  const paramType = checkParamType(
    claim.paramType as Formula.ParamType,
  ...
  const sourceTypeInfo = sourceType ? ['', ''] : ['来源：字段', '来源：非字段'];
...
export function checkContainsParam(
  claim: Formula.Param,
  incoming: Formula.Param,
) {
  const paramType =
    ['text', 'radio', 'checkbox', 'dropdown', 'dropdownmulti'].indexOf(
      incoming.paramType,
    ) > -1;
  const sourceType = checkSourceType(claim.sourceType, incoming.sourceType);
...
export function checkFieldParamFromRelevance(
  identifier: Formula.Identifier,
  params: Formula.Param[],
) {
  const _params = params.filter(
    (param) =>
  ...
  if (_params.length > 0 && !extraRelevanceFuncs.includes(identifier.name)) {
    throw generateError(
      `关联数据字段${_params
        .map((_param) => getFieldName(_param))
        .join(', ')}只允许在函数 ${extraRelevanceFuncs
        .map((func) => `${func}()`)
        .join('，')}中使用，不允许在${
        identifier.type === Formula.IdentifierType.FUNC
          ? `函数 ${identifier.name}() 中`
          : `操作符 "${identifier.name}" `
      }使用`,
    );
  }
...
export function checkFieldParamFromSublist(
  identifier: Formula.Identifier,
  params: Formula.Param[],
) {
  const _params = params.filter(
...
export function toText(param: Formula.Param) {
  ...
  switch (paramType) {
    ...
    case Formula.ParamType.NUMBER:
      _value =
        value instanceof Decimal ? value.toString() : toStringFunc(value);
      break;
...
export function formatParamValue(param: Formula.Param) {
  ...
  switch (paramType) {
    ...
    case Formula.ParamType.NUMBER:
      ...
      break;
    case Formula.ParamType.TEXT:
      _value = toStringFunc(value).trim();
      break;
    default:
      break;
  }
...
export function getDateInstance(date: any) {
  ...
  return target instanceof Date && target.toUTCString() !== 'Invalid Date'
    ? target
    : null;
}
...
export function parseFormulaFieldIds(formula: string) {
  ...
  while ((match = strRegExp.exec(formula))) {
    ...
    if (
      strInfo.every((info) => index < info.start || index > info.end) &&
...
export function getFieldName(field: Formula.Param, defaultName: string = '') {
  const fieldName =
    field && (field as Formula.FieldParam).name
      ? (field as Formula.FieldParam).name
      : defaultName ||
        (field.sourceType === Formula.SourceType.FIELD ? '暂无名称' : '');
...
export function validateFormulaNested(
  ...
  for (const _fieldId of fieldIds) {
    ...
    if (_formulaIds.includes(_fieldId)) {
      ...
      err = generateError(
        `公式中存在公式字段${subFormulaIds
          .map((id) => `${getFieldName(fields[id])},`)
          .join(' => ')}嵌套了公式字段${name}，不允许公式循环嵌套`,
      );
      if (err) {
        break;
      }
