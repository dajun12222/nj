...
const GTE: Formula.Identifier<Formula.IdentifierType.OPERATOR> = {
  name: '>=',
  ...
  validate(identifier, params) {
    ...
    if (
      ...
      throw generateError(
        `操作符 "${identifier.name}" 要求参数类型是${getParamTypeValue(
          Formula.ParamType.NUMBER,
        )}或${getParamTypeValue(
          Formula.ParamType.DATE,
        )}，现在传入为 参数类型：${getParamTypeValue(prev.paramType)} ${
          identifier.name
        } 参数类型：${getParamTypeValue(next.paramType)}`,
      );
    }
  ...
  calculate: (identifier, params, compiler) => {
    ...
    if (prev.paramType === Formula.ParamType.NUMBER) {
      ...
      value = prev.value.gte(next.value);
