...
export class FormulaCompiler {
  ...
  operators: Record<
    string,
    Formula.Identifier<Formula.IdentifierType.OPERATOR>
  > = {};
  ...
  constructor(options: GenerateParserOptions = {}) {
    ...
    const _disabledFunc = disabledFuncs.map((disabledFunc) =>
      disabledFunc.toLocaleUpperCase(),
    );
    this.parser.yy = {
      callNumber: (str) => {
        if (!validDigitCheck(str)) {
          throw generateError(
            `数字最多支持15位整数及8位小数，${str}不符合要求。`,
          );
        }
      ...
      callField: (str) => {
        ...
        if (!field) {
          throw generateError('存在无效字段', {
            extraMessage: `{${id}}`,
            errorType: FORMULA_ERROR_NOT_AVAILABLE,
          });
        }
        if (this._mode === 'calculate' && field.formula) {
          ...
          param = {
            ...field,
            value: field.formulaFormat
              ? field.formulaFormat(result, resultType)
              : result,
      ...
      evaluateByFunction: (functionKey, params = []) => {
        ...
        if (
          !func ||
          (this._mode !== 'calculate' && _disabledFunc.includes(name))
        ) {
    ...
    // 移除NULL关键字
    this.setKeywords([
      ...keywordBuiltIn.filter((keyword) => keyword.name !== 'NULL'),
      ...keywords,
    ]);
    ...
    try {
      ...
      if (preErr) {
        ...
        if (_err) {
          ...
          const { paramType, sourceType, fieldType, name } =
            this.parser.parse(formula);
          if (
            sourceType === Formula.SourceType.FIELD &&
            fieldType &&
            fieldType !== Formula.FieldType.BASIC
          ) {
            result = false;
            err = generateError(
              `${getFieldTypeValue(
                fieldType,
              )}字段{${name}}需要结合函数${(fieldType ===
              Formula.FieldType.SUBLIST
                ? extraSublistFuncs
                : extraRelevanceFuncs
              )
                .map((item) => `${item}()`)
            ...
            if (resultType) {
              ...
              if (!result) {
                err = generateError(
                  `公式结果要求返回数据类型为${getParamTypeValue(
                    resultType,
  ...
  _operatorValidate(
    ...
    if (inputType.length !== params.length) {
      throw generateError(
        `操作符 "${name}" 需要${inputType.length}个参数，但传入了${params.length}个参数`,
      );
    }
    const result: any[] = [];
    inputType.forEach((claim, index) => {
      ...
      const { paramType, paramTypeInfo, sourceType, sourceTypeInfo } = func;
      if (!paramType || !sourceType) {
        result.push(
          `第${index + 1}个参数要求是【${[paramTypeInfo[0], sourceTypeInfo[0]]
            .filter((str) => str)
            .join('，')}】，传入的是${getFieldName(incoming)}【${[
            paramTypeInfo[1],
            sourceTypeInfo[1],
          ]
            .filter((str) => str)
  ...
  _funcValidate(
    ...
    if (inputType.length !== params.length) {
      throw generateError(
        `函数 ${name}() 需要${inputType.length}个参数，但传入了${params.length}个参数`,
      );
    }
  ...
  _setOperators(
    params: Array<Formula.Identifier<Formula.IdentifierType.OPERATOR>> = [],
  ) {
    params.forEach((param) => {
      this._setOperator(param);
    });
  }
  ...
  setFuncs(
    params: Array<Formula.Identifier<Formula.IdentifierType.FUNC>> = [],
  ) {
    params.forEach((param) => {
      this.setFunc(param);
    });
  ...
  getTokenInfo(): Formula.TokenInfo[] {
    const keywordInfo: Formula.TokenInfo[] = Object.values(this.keywords).map(
      (keyword) => ({
        name: keyword.name,
        type: Formula.TokenType.KEYWORD,
      }),
    );
    const operatorsInfo: Formula.TokenInfo[] = Object.values(
      this.operators,
    ).map((operator) => ({
      name: operator.name,
      type: operator.type,
    }));
    const funcInfo: Formula.IdentifierTokenInfo[] = Object.values(
      this.funcs,
    ).map((func) => ({
      name: func.name,
      type: func.type,
    ...
    return [...keywordInfo, ...operatorsInfo, ...funcInfo];
  }
}
