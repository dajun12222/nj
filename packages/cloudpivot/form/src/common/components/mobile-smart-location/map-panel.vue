<template>
  <div class="map-panel">
    <div class="map-panel--input">
      ...
      <template v-else>
        <div v-if="address" class="map-panel--input__detail">
          <span>{{ address }}</span>
        </div>
      </template>
    </div>
    ...
    >
      <el-amap
        ...
        @click="onClickMap"
      >
        <el-amap-marker :position="mapCenter" :events="events" />
      </el-amap>
      <div
        v-if="!readonly"
    ...
    <div v-if="!readonly" ref="searchResult" class="map-panel--search">
      <template v-if="showSearchPanel">
        <p>搜索结果：共{{ areaList.length }} 条</p>
        <ul v-if="areaList.length > 0" class="search-list">
          <li
          ...
          >
            <span
              ...
              class="res-name"
            ></span>
            <span class="res-district">{{
              area.district || area.address
            }}</span>
          </li>
        </ul>
...
export default class MapPanel extends Vue {
  ...
  // @Watch('mobileIng')
  // handleChangeMobileIng(prev: any, next: any) {
  //   if (this.mobileIng && this.mobileLat) {
  //     this.mapCenter = [this.mobileIng, this.mobileLat];
  //     this.transformLoaction(this.mobileIng, this.mobileLat);
  //   }
  // }
    ...
    if (this.isWechat) {
      ...
      this.geolocation.getCityInfo((status: any, result: any) => {
        if (status === 'complete' && result.info === 'SUCCESS') {
          ...
          vm.marker = vm.mapCenter;
          vm.address = result.province + result.city;
          vm.lng = result.center[0];
          vm.lat = result.center[1];
          vm.adcode = result.adcode;
          vm.transformLoaction(vm.value.lng || vm.lng, vm.value.lat || vm.lat);
          vm.marker = [vm.value.lng || vm.lng, vm.value.lat || vm.lat];
          vm.mapCenter = vm.marker;
        }
      });
