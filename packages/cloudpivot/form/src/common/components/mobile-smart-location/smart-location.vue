<template>
  <h3-field
    ...
    class="smart-location"
    style="padding: 0"
  >
    <div v-if="showLabel" class="smart-location--row clearfix">
      <div class="row__file" @click="getFocus">
        ...
        <div class="field__content">
          <template v-if="!readonly && !address && !readonlyFormula">
            {{ $t('cloudpivot.form.renderer.addition') }}
          </template>
          <template v-else>
            <span>{{ address }}</span>
          </template>
          <template v-if="!readonly && !readonlyFormula">
            <span v-if="!address" class="icon aufontAll">&#xe968;</span>
            <span v-else class="icon aufontAll" @click.stop="clear">&#xe994;</span>
          </template>
        </div>
      </div>
    </div>
...
import { H3Modal, H3Signature, H3SearchBar, H3Field } from 'h3-mobile-vue';
...
export default class SmartLocation extends Vue {
  ...
  getFocus() {
    ...
    if (this.address && !this.readonly && !this.readonlyFormula) {
      return;
    }
