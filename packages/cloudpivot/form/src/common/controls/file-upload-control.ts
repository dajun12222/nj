...
export abstract class FileUploadControl extends UploadControl<typings.ImageUploadOptions> {
  uploadFunc?: Function;
  ...
  get displayTemplate() {
    // console.log(this.controlOpts, 'controlOpts');
    return this.controlOpts.displayTemplate
      ? this.controlOpts.displayTemplate
      : false;
  }

  get hasTemplates() {
    const files: any = this.controlOpts.templateFile
  ...
  emitUpload(files: any[]) {
    ...
    if (!this.uploadFunc) {
      this.uploadFunc = new Function('value', 'oldValue', 'form', this.controlOpts.onUpload);
    }
    this.uploadFunc.call(this.getThisProxy(), files, null, this.getThisProxy());
  }
  ...
  emitDelete(files: any[]) {
    ...
    if (!this.deleteFunc) {
      this.deleteFunc = new Function('value', 'oldValue', 'form', this.controlOpts.onDelete);
    }
    this.deleteFunc.call(this.getThisProxy(), files, null, this.getThisProxy());
  }
  ...
  removedFile(file: any) {
    ...
    if (value.length) {
      ...
      const delValue = value.filter((s: any) => s.uid === file.uid);
      this.recordDelFile(delValue[0]);
