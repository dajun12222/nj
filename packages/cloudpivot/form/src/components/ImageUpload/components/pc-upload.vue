  ...
  >
    ...
    <a-upload
      ...
      :class="{
        ...
        'has-no-parent': !control.parentKey && canUpload,
...
import { FileUploadControl } from '../../../../../form/src/common/controls/file-upload-control';
import { UploadStatus } from '../../../../../form/src/common/controls/upload-control';
...
export default class PcUpload extends FileUploadControl {
  ...
  @Watch('uploadFileList', { deep: true })
  uploadFileListChange(arr = []) {
    this.uploadFileList.forEach((f) => {
      if (f.status !== 'error' && f.hasOwnProperty('response')) {
        if (
          !f.response ||
          (f.response &&
            f.response.errcode !== undefined &&
            f.response.errcode !== 0)
        ) {
          f.status = 'error';
        }
      }
  ...
  beforeUpload(file: any) {
    ...
    if (this.controlOpts.compressible) {
      ...
      return new Promise((resolve, reject) => {
        ...
        this.compressFile(file, quality)
          .then((res: any) => {
            res.uid = file.uid;
            resolve(res);
          })
  ...
  onChange(info: { file: any; fileList: any[]; event: any }): void {
    ...
    const files = this.uploadFileList.map((f) => {
  ...
  transformFile(file: any, imageQuality: any) {
    ...
    return new Promise((resolve: any) => {
      ...
      reader.onload = () => {
        ...
        img.onload = () => {
          const base64 = that.compress(img, file['type'], imageQuality);
  ...
  compressFile(file: any, imageQuality: number) {
    return new Promise((resolve: any) => {
      const temp = new Compressor(file, {
  ...
  compress(img: any, type: string, imageQuality: number) {
    ...
    //进行最小压缩
    const MinData: any = canvas.toDataURL(type, imageQuality / 100);
  ...
  handlePreview(file: any) {
    ...
    this.previewIndex = this.uploadFileList.findIndex(
