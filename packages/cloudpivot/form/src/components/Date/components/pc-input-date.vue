...
import { DateInputControl } from '../../../../../form/src/common/controls/input-controls/date-input-control';
import { dateFormatter } from '../../../../../form/utils/date-formatter';
...
export default class InputDate extends DateInputControl {
  ...
  setControl() {
    ...
    if (_ctrl.value) {
      ...
      // @ts-ignore
      this.text = this.format.includes('APM')
        ? DateHandle.dateFormatApm(_ctrl.value, this.format)
        : dateFormatter(_ctrl.value, this.format);
    } else {
      this.val = this.text = '';
    }
    ...
    this.subscription =
      _ctrl.propertyChange &&
      _ctrl.propertyChange.subscribe((change: ControlPropertyChange) => {
        switch (change.name) {
          case 'value':
            if (change.value) {
              ...
              this.text = this.format.includes('APM')
                ? DateHandle.dateFormatApm(_ctrl.value, this.format)
                : dateFormatter(change.value, this.format);
  ...
  onChange(date: moment.Moment, dateString: any) {
    ...
    if (this.timePicker) {
      dateString = dateString
        ? `${moment(new Date()).format('YYYY/MM/DD')} ${dateString}`
        : null;
    }
