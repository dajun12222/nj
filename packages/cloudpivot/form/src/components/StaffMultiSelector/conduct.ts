        ...
        formatter: (
          ...
          switch (valueType) {
            ...
            case StaffSelectorValueType.Ref:
              ...
              if (controls) {
                const key =
                  (orgRoot && orgRoot.slice(1, orgRoot.length - 1)) || '';
                if (key) {
                  const sheetCode = key.split('.')[0];
                  const controlCode = key.split('.')[1];
                  const sheetControl = controls[sheetCode];
                  if (sheetControl && sheetControl.columns) {
                    const control = sheetControl.columns.find((item: any) => {
                      return item.key === controlCode;
                    });
                    if (control) {
