<template>
  <div class="address">
    <template>
      ...
      <div
        v-if="controlOpts.addressDetail === 'true'"
        class="address-textarea"
        :class="{ addressVertical: layoutType }"
      >
      ...
      <template v-if="!!readonly">
        <div class="field">
          <!-- deewewre -->
          <div class="field__label top">
            {{ label }}
          </div>
          <div class="field__control">
            <mutil-text :text="addressString" />
          </div>
        </div>
      </template>
    </template>
...
import { FromAddressValueService } from '../../../common/services';

import { FormLocationControl } from '../../../common/controls/form-location-control';

import MutilText from '../../../common/components/mutil-text.vue';

@Component({
  name: 'input-text',
  components: {
    PcaSelector: () =>
      import('../../../common/components/mobile-pca-selector/pca-selector.vue'),
    H3Textarea,
    MutilText,
    SmartLocation: () =>
      import(
        '../../../common/components/mobile-smart-location/smart-location.vue'
      ),
  },
})
  ...
  initLocation() {
    ...
    geolocation.getCurrentPosition((status: any, result: any) => {
      if (status === 'complete' && result.info === 'SUCCESS') {
        ...
        vm.mobileIng = result.position.lng;
        vm.mobileLat = result.position.lat;
        const backData = {
