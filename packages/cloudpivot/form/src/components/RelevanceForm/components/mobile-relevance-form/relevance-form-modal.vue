<template>
  <div v-show="visible">
    <search-panel
      ...
      :fetch="superFetch"
      :isRelevanceEntrance="true"
      :showStyle="control.options.showStyle"
      :relevanceVm="vm"
      :getDisplayField="getDisplayField"
      :relevanceId="control.controller.id"
    ...
    <div
      v-show="canAdd"
      v-if="control.key !== 'parentId'"
      class="home-add"
      @click="openRelevanced"
    >
      <img src="./add.png" />
    </div>
  </div>
...
import { Component, Model, Prop, Watch, Inject } from 'vue-property-decorator';
...
export default class RelevanceFormModal extends RelevanceFormControl {
  ...
  superFetch(
    ...
    return this.fetch(page, pageSize, value, query, true, view);
  }
  ...
  get canAdd() {
    const addFlag = this.queryActions.some((v) => v.actionCode === 'add');
    return addFlag;
  }
