<template>
  ...
  >
    <div class="field__control" @click="onClick">
      ...
      <span
        v-else-if="isEditState && !readonlyFormula"
        class="placeholder scan"
        style="position: relative"
      >
        <!-- 下面的i标签虽然不可见，但是不能删除：iphone 8 Plus 在扫码输入的情况下会丢失 ‘扫一扫添加’ 文字中的'添加'二字 -->
        <i
          style="
            pointer-events: none;
            position: absolute;
            ...
            overflow: hidden;
          "
          >{{ inputPlaceholder }}</i>{{ inputPlaceholder }}</span>
    </div>
    <span
      v-if="!readonly && !readonlyFormula && ctrl.value"
...
import { isiOS } from '../../../../../../common/src/utils/common';
...
import { deepCopy, parseUrlParam } from '../../../../../../form/utils';
...
export default class MobileRelevanceForm extends RelevanceFormControl {
  ...
  scan() {
    ...
    //this.setVal('http://47.107.160.206/api/public/short_code/resource/1OJz84p316g');
  }
