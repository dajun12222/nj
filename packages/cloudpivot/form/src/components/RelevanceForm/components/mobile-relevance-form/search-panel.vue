<template>
  <div class="search-panel">
      ...
      <div class="search-bar">
        ...
        <div class="form-states">
          <div
            ...
            :key="index"
            :class="{
              'form-states-inner': true,
              selectActive: searchItem.type === currentWay,
            }"
      ...
      <div v-show="searchMode">
        <div v-show="fetching" class="message">
          <template v-show="searchMode">
            {{ $t('cloudpivot.form.renderer.searching') }}
          </template>
        </div>

        <div v-show="searchMode && total" class="message">
          {{ $t('cloudpivot.form.renderer.searchResultTip', { total }) }}
        </div>
      </div>

      <div v-show="fetched && total < 1" class="no-result">
        <div v-show="searchMode && value">
          {{
            $t('cloudpivot.form.renderer.searchNoResultTip', { text: value })
          }}
        </div>
    ...
    <!-- 单据状态查询 -->
    <div v-show="showState" class="search-content">
...
import { Segment } from '../../../../../../form/src/common/controls';
import H3Scroll from '../../../../../../form/src/renderer/components/layout/h3-mescroll.vue';
...
export default class SearchPanel extends Vue {
  ...
  @Prop({ default: false })
  isRelevanceEntrance?: boolean;

  @Prop() showStyle?: string;

  @Prop() relevanceVm?: any;
  ...
  @Prop() getDisplayField?: string;

  @Prop() relevanceId?: string;
  ...
  async doFetch() {
    ...
    this.fetched = false;
    const view: any = {
      type: this.showStyle === 'tree' ? 'TREE' : 'LIST',
      depth: this.showStyle === 'tree' ? 0 : undefined,
      parentRef: this.showStyle === 'tree' ? '#' : undefined,
    };
    try {
      const seg = await this.fetch(
        this.page,
        this.pageSize,
        this.value,
        this.queryFormState,
  ...
  get treeList() {
    let arr: any = [];
    arr = this.dataToTreeList(this.data);
    return arr;
  }

  dataToTreeList(data: any) {
    if (!data) {
      return [];
    }
    const indexType: string = this.getDisplayField as string;
    const arr: any = [];
    for (let i = 0; i < data.length; i++) {
      if (data[i].data[indexType]) {
        arr.push({
          value: data[i].data.id || '',
          key: data[i].data.id,
          title: indexType ? data[i].data[indexType] : '',
          children: this.dataToTreeList(data[i].data.t__children_),
        });
      }
    }
    return arr;
  ...
  beforeMount() {
    ...
    this.searchWays =
      this.showStyle === 'tree' && this.relevanceId === 'parentId'
        ? [
            {
              name: '单据状态',
              type: 'status',
