        ...
        >
          ...
          >
            <template>
              <p>最近搜索</p>
              <span
                v-for="(item, key) in historySeachList"
                :key="item + key"
                class="history-item"
                :class="'history-' + key"
                @click="onSearch(item)"
...
import { Icon, Radio, Select, Spin, Tooltip, TreeSelect } from '@h3/antd-vue';
...
export default class RelevanceFormDropdown extends RelevanceFormControl {
  ...
  setHighlightText(value) {
    if (!this.currentSeachValue) {
      // 没有搜索返回原值
      return value;
    }
    const first = this.preHighlightText(value, this.currentSeachValue);
    const last = this.nextHighlightText(value, this.currentSeachValue);
    if (this.currentSeachValue === value) {
      return `<span class="font-ele">${this.currentSeachValue}</span>`;
    }
    if (!first && !last) {
      return value;
    }
    return `${first}<span class="font-ele">${this.currentSeachValue}</span>${last}`;
  }
  ...
  get drowdownList() {
    if (this.list.length) {
      const arr: any[] = [];
      for (const val of this.list) {
        if (val[this.getDisplayField] || val[this.getDisplayField] === 0) {
  ...
  // 保存历史最近10个查询记录
  saveHistorySeach() {
    const value = this.currentSeachValue;
    if (value === undefined || value === '') {
      return;
    }
    const index: number = this.historySeachList.indexOf(value);
    if (index !== -1) {
      this.historySeachList.splice(index, 1);
    } else {
      if (this.historySeachList.length >= 10) {
        // 历史查询记录大于10个，删除一个老数据
    ...
    localStorage.setItem(
      this.ctrl.id as string,
      JSON.stringify(this.historySeachList),
    );
  }

  // 删除历史记录
  delHistoryItem(item, obj) {
    const index: number = this.historySeachList.indexOf(item);
    if (index !== -1) {
      this.historySeachList.splice(index, 1);
      localStorage.setItem(
        this.ctrl.id as string,
  ...
  treeViewSend() {
    if (!this.treeView) {
      return;
    } //不是树形视图直接返回，不用发送数据给tree-query-form组件
    if (this.$route.path !== this.myRouter) {
      return;
    } //缓存的组件任然会触发delete或close事件从而触发该函数，如果触发的时候路由不相同，则不发送数据
    this.search('').then(() => {
      this.sentEventBus();
    });
  }
  ...
  sentEventBus() {
    ...
    Bus.EventBus.$emit('treedata', {
      ...
      setTreeView: false,
      getDisplayField: this.getDisplayField,
  ...
  setOffWidth() {
    const offWidth =
      (document.querySelector(`#${this.refId}`) as HTMLElement) &&
      (document.querySelector(`#${this.refId}`) as HTMLElement).offsetWidth;
    if (offWidth) {
