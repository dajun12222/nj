<template>
  <div v-show="visible">
    <search-panel
      ...
      @notifyRes="notifyRes"
    >
      <template v-if="data.length > 0" slot-scope="{ data }">
...
import { H3Avatar, H3Button, H3Checkbox } from 'h3-mobile-vue';
...
export default class RelevanceFormModal extends RelevanceFormControl {
  ...
  change(item: any, isChecked: boolean) {
    ...
    if (Array.isArray(item)) {
      item.forEach((x) => {
        const curItem = this.queryResult.find((z) => {
          return z.id === x;
        });
  ...
  async confirm() {
    this.$emit('back');
    const item = await this.convertForMappings(
      this.multiValue.map((item) => item.data),
    );
