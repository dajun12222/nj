...
import H3Textarea from '../../../../../form/src/common/components/h3-textarea.vue';
import MutilText from '../../../../../form/src/common/components/mutil-text.vue';
...
export default class PcInputText extends TextInputControl {
  ...
  get isShowSearch() {
    if (this.$refs['inputWrapper' + this.inputId]) {
      this.searchResLeft =
        this.getElementPosition(this.$refs['inputWrapper' + this.inputId]).x +
        'px';
      this.searchResTop =
        this.getElementPosition(this.$refs['inputWrapper' + this.inputId]).y +
        32 +
        'px';

      // @ts-ignore
      this.searchResWidth =
        (this.$refs['inputWrapper' + this.inputId] as any).clientWidth + 'px';
    }
    return (
  ...
  created() {
    try {
      this.control.options.dictionaryData = JSON.parse(
    ...
    if (
      ...
      try {
        let options: any = {};
        if (
          typeof this.control.options.dictionaryData.options.value === 'string'
        ) {
          options = JSON.parse(
  ...
  mounted() {
    const inputWrapper: any = this.$refs['inputWrapper' + this.inputId];
    if (inputWrapper) {
      this.searchResWidth = inputWrapper.clientWidth + 'px';
    }
  }
