<template>
  <div class="h3-upload" :class="isImage ? 'h3-upload-phone' : ''">
    ...
    >
      ...
      <div class="close-bar" @click="hideSignsture">
...
import { UploadFile } from '../../../common/controls/upload-control';
...
export default class Upload extends FileUploadControl {
  ...
  beforeUpload(files: File[]) {
    if (!this.checkFileNumber(files, this.showError)) {
      return false;
    }
  ...
  checkFileNumber(files: File[], showError?: (msg: string) => void) {
    let msg = '';
    const that = this;
    const maxNumber = this.control.options.fileNumber;
    const limitNumber = this.control.options.limitNumber;
    const number = this.fileList.length;
    if (limitNumber === 'custom' && number + files.length > maxNumber) {
      if (showError) {
