<template>
  <div>
    <ul class="alledit-ul">
      ...
      <li>
        ·
        逻辑、地址、人员单选、人员多选、部门单选、部门多选、混选、关联单选、关联多选不支持批量编辑
      </li>
    </ul>
    ...
    <template v-for="(item, i) in formData">
      <a-row :key="i" :gutter="20" class="row">
        <a-col :span="10">
          <a-select v-model="item.key" @change="onHandleData">
            <a-select-option
              ...
              :options="s.options"
            >
              {{ $i18n.locale === 'zh' ? s.options.name : s.options.name_i18n }}
            </a-select-option>
          </a-select>
        </a-col>
        <a-col :span="11">
          ...
          <template v-else-if="item.type === 5">
            ...
            >
              <a-select-option
                v-for="s in item.options.options"
                :key="s"
                :title="s"
                :value="s"
              >
          ...
          <template v-else-if="item.type === 7 || item.type === 19">
            <!-- 下拉 -->
            <a-select
              v-model="item.value"
              :getPopupContainer="getPopupContainer"
    ...
    <div v-if="formData.length < GetList.length" class="add">
      <span>
...
import { Row, Col, Button, Icon, Select, DatePicker } from '@h3/antd-vue';
...
export default class AllEdit extends Vue {
  ...
  get GetList() {
    const type = [1, 2, 3, 4, 5, 6, 7, 19];
    console.log(this.data, 'this.data');
    const arr = this.data.filter((d: any) => {
      if (type.includes(d.type) && d.key !== 'rowStatus' && d.writable) {
        const f = d.type === 1 && d.options.noRepeat;
        if (!f) {
          if (
  ...
  addRows() {
    const flag = this.formData.every((d) => {
  ...
  onHandleData(value: any, obj: any) {
    console.log(value, obj.data.attrs, 'obj.data.attrs');
