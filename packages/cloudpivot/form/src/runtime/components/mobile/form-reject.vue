<template>
  <h3-list>
    ...
    <h3-list-item :hasExtra="true">
      ...
      <h3-switch slot="extra" v-model="back" color="#2970ff" />
    </h3-list-item>
  </h3-list>
</template>
...
export default class FormReject extends Vue implements FormActionComponent {
  ...
  mounted() {
    this.isShow = Boolean(
      JSON.parse(sessionStorage.getItem('commonCommentSwitch') || 'false'),
    );
  ...
  valid() {
    const checkedData = this.checkboxOptions.filter((res) =>
      this.val.includes(res.value),
    );
    const checkType: any[] = [];
  ...
  submit() {
    if (this.val.length === 0) {
      (this.$parent.$parent as any).$message.error(
        `${this.$t('cloudpivot.form.runtime.modal.rejectTips')} `,
      );
      return;
    }
    return {
