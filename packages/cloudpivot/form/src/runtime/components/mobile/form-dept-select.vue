<template>
  <div>
    ...
    <template v-if="!isTrust">
      <approve-opinion
        v-if="options.approval"
        :lastAttachment="lastAttachment"
        :control="options.approval"
        :showCode="showCode"
      />
    ...
    <template v-else>
      ...
      <template v-if="trustorSelected.value === '1'">
        ...
        <div class="trust-tips">
          <span>{{
            $t('cloudpivot.form.runtime.modal.trustTips', {
              name: name,
...
<script lang="ts">
import { H3List, H3ListItem, H3Radio, H3RadioList } from 'h3-mobile-vue';
...
@Component({
  ...
  components: {
    ...
    H3RadioList,
    H3Radio,
    ApproveOpinion: components.ApproveOpinion1,
  },
})
export default class FormDeptSelect extends Vue implements FormActionComponent {
  ...
  get name() {
    if (this.options && Array.isArray(this.options.trusts)) {
      const item = this.options.trusts.find(
        (t: any) => t.id === this.trusteeSelected.value,
      );
  ...
  get trustList() {
    if (this.options && Array.isArray(this.options.trusts)) {
      const filterTrust = this.options.trusts
        .filter((tl: any) => tl.isTrustor)
        .map((d: any) => ({
          value: d.id,
          label: d.name,
        }));
  ...
  get trusteeDepts() {
    if (this.options && Array.isArray(this.options.trusts)) {
      const index = this.options.trusts.findIndex(
        (t: any) => t.id === this.trusteeSelected.value,
      );
      if (
        this.options.trusts[index] &&
        this.options.trusts[index].departments
      ) {
        const deptTrust = this.options.trusts[index].departments.map(
          (d: any) => ({
            value: d.id,
            label: d.name,
          }),
  ...
  setOptions() {
    if (this.options && this.options.data.length > 1) {
      const index = this.options.data.findIndex((opt: any) => opt.isMain);
      this.value = this.options.data[index < 0 ? 0 : index].name;
      ...
      this.selected = this.depts[index < 0 ? 0 : index] as any;

      // 委托人处理
      if (this.options.trusts) {
        const filterTrust = this.options.trusts.filter(
          (tl: any) => tl.isTrustor,
        );
        if (this.options.trusts.length > filterTrust.length) {
          this.isTrustor = this.$t(
            'cloudpivot.form.runtime.modal.ownIdentity',
          ).toString();
          ...
          this.chooseTrustor = [
            ...
            },
            {
              value: '1',
              label: this.$t(
                'cloudpivot.form.runtime.modal.clientIdentity',
              ).toString(),
            },
          ];
        } else {
        ...
        this.trusteeSelected = filterTrust
          ? filterTrust.map((d: any) => ({
              value: d.id,
              label: d.name,
            }))[0]
          : {};
    ...
    if (this.options && this.options.code) {
      this.showCode = this.options.code;
    }
  }
  ...
  setTrustDept() {
    ...
    if (curTrust && curTrust.departments) {
      this.trustDept = curTrust.departments.length
        ? curTrust.departments[0].name
        : '';
      this.trustDeptSelected = curTrust.departments.length
        ? curTrust.departments.map((d: any) => ({
            value: d.id,
            label: d.name,
          }))[0]
        : {};
  ...
  get lastAttachment() {
    ...
    return (
      (this.options &&
        ...
        this.options.approval.controller &&
        (this.options.approval.controller as any)._value &&
        (this.options.approval.controller as any)._value.lastAttachment) ||
      ''
    );
  }
  ...
  submit() {
    ...
    if (_approval) {
      const ctrl = _approval.controller as any;
      const content = window.sessionStorage.getItem('$approval');
      // 用户没有输入审批意见时，填充按钮文本
      if (!ctrl.value) {
        ctrl.value = {
          content,
        ...
        !ctrl.value.content &&
        (!ctrl.value.resources || !ctrl.value.resources.length)
      ) {
        // 如果有附件或签名则不设置默认值
