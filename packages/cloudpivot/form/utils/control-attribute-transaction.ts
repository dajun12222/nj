 ...
 * @FilePath: /frontend/modules/../utils/control-attribute-transaction.ts
 */
import { ModalAttributeType } from '../src/typings/form-attribute';
...
export const formatterValueToSetStatus = (val: string | '') => {
  try {
    ...
    if (
      Object.keys(options).includes('$queryData') &&
      Object.keys(options).includes('queryText')
    ) {
  ...
  if (typeof val === 'string' ? val : Array.isArray(val) ? (val as any[]).length : val) {
    return setConst.set;
  }
  return setConst.notSet;
};
...
export const formatterLocations = (val: any, ctrls: any) => {
  if (val) {
    return `${val.provinceName}${val.cityName}${val.districtName}${
      val.address ? val.address : '未设置'
    }`;
  }
  return '未设置';
};
...
export const formatterAddress = (val: any, ctrls: any) => {
  ...
  if (target && val !== '') {
    ...
    const addressDetail = ctrls.find(
      (item: any) => item.field === 'addressDetail',
    );
    const showDetail = addressDetail.value === 'true';
    return `${p ? (val.provinceName ? val.provinceName : '') : ''}${
      c ? (val.cityName ? val.cityName : '') : ''
    }${a ? (val.districtName ? val.districtName : '') : ''}${
      showDetail ? (val.address ? val.address : '') : ''
    }`;
  }
  return '';
};
