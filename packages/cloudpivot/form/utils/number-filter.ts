...
export default function (control: RendererFormControl) {
  ...
  // 满足科学计数法时将其转化为字符串显示
  const strVal = (val - 0).toLocaleString();
  const formatStrVal = strVal.replace(/\,/g, '');
  let dotIndex: number = 0;
  dotIndex = formatStrVal.indexOf('.');
  if (dotIndex === -1 && formatStrVal.length > 21) {
    return formatStrVal;
  }
  if (dotIndex > 20) {
    return formatStrVal;
  ...
  // if (formatStrVal.substr(dotIndex + 1).length > 7) {
  //   return formatStrVal;
  // }
  const ft =
    (control.options as NumberOptions).format1 ||
    (control.options as NumberOptions).format;
  switch (ft) {
    default:
