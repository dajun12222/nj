  ...
  <div class="activity-box">
    <div
      ...
      :class="[
        ...
        ['START', 'END'].includes(instance.activityType) &&
          setStartOrEndStatus(instance),
      ]"
      :style="{
        left: instance.x + 'px',
        top: instance.y + 'px',
        width: Number(instance.width) + 'px',
        height: instance.height + 'px',
      }"
      :title="lang === 'zh' ? instance.activityName : instance.name_i18n[lang]"
      @mousedown="mouseDown"
    ...
    >
      <div class="activity-content">
        <div class="content-wrap">
          ...
          <span>{{
            lang === 'zh' ? instance.activityName : instance.name_i18n[lang]
          }}</span>
        </div>
      </div>
    </div>
...
export default class ActivityInstance extends Vue {
  ...
  // async loadLogs() {
  //   const RouteParam: any = (this as any).$route;
  //   const res = await workflowApi.getWorkflowLogs({
  //     workflowInstanceId: RouteParam.params.workflowInstanceId,
  //   });
  ...
  mouseMove(e) {
    e = e || window.event;
    const { mouseX, mouseY, mouseThresold } = this;
    if (
