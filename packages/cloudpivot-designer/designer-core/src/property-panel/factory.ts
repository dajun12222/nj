...
export function build(
  ...
  for (const key of keys) {
    ...
    propInfo = propInfo as PropertyInfo;
    const required =
      schemaInfo.required && schemaInfo.required.indexOf(key) > -1
        ? true
        : false;
        ...
        if ($ref) {
          ...
          switch (propInfo.type) {
            case DataType.String:
              ...
              value = '';
              if (
                typeof (propInfo as StringPropertyInfo).minLength === 'number'
              ) {
                opt.min = (propInfo as StringPropertyInfo).minLength;
              }
              if (
                typeof (propInfo as StringPropertyInfo).maxLength === 'number'
              ) {
            ...
            case DataType.Number:
              ...
              value = 0;
              if (
                typeof (propInfo as NumberPropertyInfo).minimum === 'number'
              ) {
                opt.min = (propInfo as NumberPropertyInfo).minimum;
              }
              if (
                typeof (propInfo as NumberPropertyInfo).maximum === 'number'
              ) {
            ...
            case DataType.Array:
              ...
              if (
                typeof (propInfo as ArrayPropertyInfo).minItems === 'number'
              ) {
                opt.minCount = (propInfo as ArrayPropertyInfo).minItems;
              }
              if (
                typeof (propInfo as ArrayPropertyInfo).maxItems === 'number'
              ) {
