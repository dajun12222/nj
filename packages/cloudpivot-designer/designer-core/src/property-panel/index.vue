<template>
  <div class="property-panel" :class="`property-panel-${css}`">
    <collapse
    ...
    >
      <template v-slot="{ group }">
        ...
        >
          <slot name="title" :property="{ key, item: controls[key] }"></slot>
        </property-item>
        <!--        <div-->
        <!--            :key="key"-->
    ...
    <template v-else-if="schema.properties">
      <template v-for="(item, key, index) in controls">
        <property-item
          ...
          @change="(val) => onItemValueChange(key, val)"
        >
          <slot name="title" :property="{ key, item }"></slot>
        </property-item>
        <div
...
export default class PropertyPanel extends Vue {
  ...
  getDisplaySettings(key: string) {
    return this.properties && this.properties.display
      ? this.properties.display[key]
      : null;
  }
  ...
  beforeCreate() {
    (this.$options as any).components.Collapse =
      propRegister.components.Collapse || register.components.Collapse;
  }
}
</script>
