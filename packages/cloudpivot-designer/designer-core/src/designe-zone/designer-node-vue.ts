...
export default class DesignerNodeVue extends Vue {
  ...
  onChildIdsChange() {
    this.children = this.childIds.map((cid) =>
      (register.store as any).get(cid),
    );
  }
  ...
  calcDragDirection(evt: DragEvent) {
    let { offsetX, offsetY, srcElement } = evt;
  ...
  onDrop(evt: DragEvent) {
    ...
    const dropIn =
      this.isRoot ||
      (this.asset.info.container && direction === Direction.Center);
